...
**Except what CI cannot run.** Tests skip there for want of local data or mounts — `data/ohlc-full`, the engine-journal mount, the gitignored refdata snapshot and universe JSON, the panel and trade-archive mounts, `data/ohlc-15m`, the ops journal mirror. **Run the data-gated tests locally before PR whenever the diff can reach them**, and never assume a skip is coverage — a full-suite run from a scratch worktree skips them silently, since `data/` lives only in the main checkout: a local full-suite claim names whether `data/` was present, and a worktree run symlinks it in first (`agent-ops.md`'s worktree clause): `tests/test_costmin_drift.py` is COSTMIN's only guard against venue-side drift, sits on the live trade path, and skips in CI.
