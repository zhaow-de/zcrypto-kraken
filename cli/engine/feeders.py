...
def replay_stages(record: CycleRecord, reader: Reader, *, config: CrossfreqSystemConfig | None = None) -> CycleStages:
    """Rebuild one journaled cycle and return its forming-row book at every pipeline stage, BASE-keyed
    whatever the record's schema, since `accumulation_payload` looks its floors up by base. Both schemas
    rebuild here (spec 00094 D2/D3): a schema-2 record's full symbols reach the builder only through
    `select_model_inputs`, since raw they raise a `PortfolioError` no per-record catch would hold."""
