...
def seed() -> None:
    ...
    if absent:
        ...
        try:
            canonical_dir = resolve_canonical_root(resolve_data_dir(app_config))
        except (ConfigError, DataSyncError) as exc:
            # Neither resolver's remedy names the seed; the seed says whose refusal this is first.
...
def flatten(
    ...
    Without `--execute` it reads the account, prints the plan and stops. With `--execute` it needs the engine's kill file already in place, asks for a typed confirmation on the terminal, then cancels every resting order, closes every margin position reduce-only, and sells every non-EUR balance -- all at market, all journaled. Exit 0 the account reads flat, though an open-order or balance row the adapter could not parse drops out of its read without failing it, so confirm on Kraken's own pages; 1 refused with nothing sent; 2 something is still open; 3 the venue could not be read before anything was sent. The cancel is account-wide. An order or position a read cannot resolve does not stop the cancel, and the run then ends at 2, never 0."""
