...
def _sidecar_by_dataset() -> dict[str, dict[str, str]]:
    ...
    silently inert. This file is where those sets' hashes live instead.
...
def _manifest_sha256s(node: object) -> set[str]:
    """Every ``sha256`` string anywhere in a legacy manifest, whatever its shape. A manifest-level
    ``manifest_sha256`` is NOT collected -- the key must be exactly ``sha256``, so a set that exposes
    no per-parquet hashes yields the empty set."""
    found: set[str] = set()
