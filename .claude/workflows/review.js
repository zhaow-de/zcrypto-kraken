...
Ask of every surface the range adds or keeps whether it should exist at all, before asking whether it is true: a surface that should not exist outranks every truth finding about it, and three reviewed into correctness on one 2026-09 branch were then deleted. Read \`git diff ${range}\` first, then each commit message. Write a Markdown report to ${reportDir}/${lens.name}.md with \`## Verdict\`, \`## Findings\` (one \`### [Severity] path:line — claim\` heading per finding with evidence and a \`Consequence:\` line) and \`## Executed\`, then return the structured output with the same findings; the report and the structure must agree.`
