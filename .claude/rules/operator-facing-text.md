...
- `WP<N>` is banned from every git-tracked file, not just these, and from PR titles and bodies, which the test cannot see — `zcrypto-grooming`'s `references/memo-protocol.md`, which also records the one historical exception; don't add more, and don't "fix" the recorded exception away. The ban is enforced repo-wide by the same test (two-way allowlist).
