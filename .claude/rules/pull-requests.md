...
- **WHEN to open is `branch-workflow.md`'s PR gate** — one completed, nameable component, on the word `branch-workflow.md` names — `zcrypto-main`'s by delegation in the multi-session setup, the user's for a single attended session — or at `/zcrypto-auto-exec` item completion (unattended).
