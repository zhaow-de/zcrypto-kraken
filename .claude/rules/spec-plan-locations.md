...
**A spec or decisions-log entry that rules on how operations must be performed lands the imperative on the operator-facing surface — the owning rule, runbook, or skill — in the same change**: a ruling recorded only in a spec is invisible at execution time.
