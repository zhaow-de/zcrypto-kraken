...
**Rank the review by blast radius, and say so in the dispatch.** Name the two or three things whose being wrong would cost something — what ships to production, what a paged operator acts on, what a number would change — and tell the reviewer that everything else is Minor by construction. An open-ended "sweep for every stale claim" returns a prose backlog whose fixes are themselves prose the next round finds, so the loop converges on wording while the substance was settled in round one. **Two rounds on the same subject is the ceiling**: if round two's findings are all Minor or all prose, the branch is done — record what was consciously left rather than opening round three.
