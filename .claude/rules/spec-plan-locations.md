...
- **Substantive iteration** (a feature, a non-trivial fix, anything multi-file or with design choices): run the full flow — `superpowers:brainstorming` → committed spec, `superpowers:writing-plans` → committed plan, subagent-driven execution, and a `docs/iterations-history-phase<N>.md` closeout entry.
