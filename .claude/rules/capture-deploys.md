...
- **An engine re-pin whose revision also touches `roles/capture/files/config.alloy` never converges `--tags engine` alone** — the keep-list and its drift assert live in the *capture* role, so the new families publish unadmitted and silently; run `--tags capture,engine` with `-e capture_image_digest=<currently-running> -e capture_alloy_digest=<currently-running>`.
