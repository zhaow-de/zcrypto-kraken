...
- Converges run `site.yml`; `bootstrap.yml` is first-provision only — both refuse mechanically: `site.yml` refuses an un-tagged run on the live primary (a bare run pulls in the engine play and can restart the LIVE trade engine), `bootstrap.yml` refuses an already-provisioned host without `-e rebootstrap=true`.
...
- **Stop the timer BEFORE the converge, and converge with `-e ops_panel_timer_hold=true`** — the flag makes the role leave the panel timer stopped instead of silently re-arming it. Verify the hold (`disabled`, `inactive`, absent from `systemctl list-timers`) **before the next tick is due** — read that deadline from `systemctl list-timers`, never assume it is far off. Then run the regeneration inside the unit, so a stray trigger is a no-op rather than a second writer.
