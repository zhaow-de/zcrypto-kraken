...
- **A `config.alloy` change needs BOTH `-e capture_alloy_digest=<currently-running>` AND a container recreate** — the Alloy block is gated `when: <tier>_alloy_digest is defined`, so an ordinary converge skips the config copy; and the compose bind-mounts the config as a **single file**, so Ansible's atomic write replaces the inode the running container still holds. Passing the digest alone updates the host and changes nothing in the process. Finish with `docker compose up -d --force-recreate` in the Alloy project dir — `restart` does not re-resolve the mount — and verify by comparing the host and in-container `sha256sum`, never the host file alone. Config-only, so no bake owed.
...
- **Omit `ops_alloy_digest` unless Alloy is the subject — but a `config.alloy` edit MAKES it the subject**: the Alloy block is gated `when: ops_alloy_digest is defined`, so an ordinary converge skips the config copy and exits 0. Pass the currently-running digest, then **recreate the container** — see the capture bullet above for why the digest alone is not enough.
