...
- **type** — one of `feat`, `fix`, `docs`, `refactor`, `perf`, `test`, `build`, `ci`, `chore`, `revert`, or **`claude`**. Use **`claude`** — not `docs` — for any change to `CLAUDE.md` or `.claude/`.
