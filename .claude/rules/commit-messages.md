...
Stage by explicit path, one commit-type's file kind per commit — never `git add -A`/`-u`: blanket adds have repeatedly swept another kind's files into a typed commit; when a change spans kinds, plan the split before staging anything.
...
- Every Claude-authored commit ends `Co-Authored-By: <the ACTUAL authoring model> <noreply@anthropic.com>` — last line, blank-separated from the body; a subagent credits its **own** model (verify against the dispatched model; dispatch prompts never hardcode a version), and the main session's system prompt goes stale after a mid-session `/model` switch — the transcript's per-message `model` field is ground truth.
