...
Mechanics: **copy-then-truncate, never `mv`** — the gitignored running log must survive, so copy verbatim into the committed sibling, `git add`, then truncate it to empty. **Each continuation file opens with a one-paragraph memo** (what it continues, which close-out era drained it, the `[iter-<NNN>]` range), then the verbatim entries. **Keep every `-decisions.md` and `-cont-decisions-<K>.md` OFF the mdformat allowlist** — mdformat would reflow their option lists and `(Decision: N)` markers (the close-out *reports* may be on it; their decisions siblings never are).
