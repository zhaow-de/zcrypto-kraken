...
The **per-phase** running logs `docs/research/decisions-running-phase<N>.md` (git-tracked, spec 00056 D7) record **subject-matter research decisions** — one paragraph per decision, prefixed `[iter-<NNN>]` (the iteration number tracked in `docs/iterations-history.md`); `<N>` is the master-plan-§12 phase whose subject matter the decision concerns (see *Phase persistence*). Applies in **both** interactive and unattended modes.
...
So a close-out **fans out**: at the Phase-6 close-out, `decisions-running-phase6.md` → `<serial>.phase6-decisions.md`, `decisions-running-phase4.md` → `10.phase4-cont-decisions-0.md`, `decisions-running-phase5.md` → `13.phase5-cont-decisions-0.md`; later Phase-4 backlog drains at the Phase-7 close-out → `10.phase4-cont-decisions-1.md`.

Mechanics: **copy-then-truncate, never `mv`** — even though the running log is now git-tracked, it must survive the close-out as the still-live file the next iteration appends to, so copy verbatim into the committed sibling, `git add`, then truncate it to empty (both the copy and the truncation are their own tracked changes, in the same close-out commit). **Each continuation file opens with a one-paragraph memo** (what it continues, which close-out era drained it, the `[iter-<NNN>]` range), then the verbatim entries. **Decisions logs are verbatim** — never let a formatter restructure their option lists or `(Decision: N)` markers.
