...
- **Never converge or reboot inside a published Kraken maintenance window.** They are listed at `https://status.kraken.com/api/v2/scheduled-maintenances.json`; the ones that matter carry `WebSocket` or `REST` in `components` (the recurring one is "Kraken Website and API Maintenance"). The entry appears only **2–6 days ahead**: check at planning time *and* again immediately before — an empty feed a week out proves nothing.
...
- **Prune images at pins-update time, strictly AFTER that host's new row is written — every tier, not just capture** — `uv run python infra/scripts/prune-host-images.py <host>`, then `--apply`. Run before the row lands and it takes the digest that row is about. Nothing else removes images, and a capture host stops appending at 1 GiB free.
...
- **Read a running image's digest from the container, never from the compose file** — `docker inspect --format '{{.Config.Image}}' <name>`: a compose file can pin the wrong image while the container runs the right one — the drift assert does not run when `ops_alloy_digest` is passed — and the mistake fires at the next `docker compose up`.
...
- **A fingerprint-invalidating pin (any change inside the transitive `cli.*` replay closure that `gate_cache.py` digests — e.g. `journal.py`) replays the whole gate-export cold** — the better part of an hour, and it grows with the journal: size the window from the current figure in `docs/reference/fleet-pins.md`'s standing constraint and re-measure it; never reuse a remembered or older figure.
