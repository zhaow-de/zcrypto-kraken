...
- **Mutation probes run through `infra/scripts/mutate-probe.sh`, never a hand-rolled mutate-and-restore loop** — it refuses every observed failure mode (dirty tree, pytest-in-sandbox, stale `.pyc`, missing failing control) instead of relying on you to remember them.
