...
- **State decisions and invariants, never statuses or schedules.** "Deliberately absent (T0045 dropped it)" survives; "planned in T0065" / "acted on by nothing yet" rot the moment the work moves. Status lives in the topic — point at it.
- **Every citation resolves from the repo alone**: a symbol, a test name, `T<NNNN>`, `spec NNNNN`, a path. A plan-task number carries its 5-digit serial on the same line (enforced by `tests/test_code_prose_citations.py`); a hash is copied from git output, never from memory; no blanket provenance stamp over independent facts.
...
- **A closed citation is RE-TENSED, never deleted** — "(T0105, resolved, records why)". Internal tokens are fine in code prose; the ban is operator-visible surfaces only (`operator-facing-text.md`).
