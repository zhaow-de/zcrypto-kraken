...
- **Never call a diff prose-only without `uv run python infra/scripts/prove-inert.py` returning 0 for every file in it** — the claim that makes a review cheap is the one never made by reading.
...
- **Pruning has four dispositions**: cut (false or stale, or true and not acted on), condense (only where the content is load-bearing — a condensed sentence is a NEW sentence and therefore a new claim, and claims written to fit have been false here), keep (load-bearing, or unverifiable but valuable), relocate (true but about another symbol — check the target first; it is often already there). Ask *is this about this symbol?* before *is this true?* A preceding block describes what follows it; only a same-line trailing comment binds to its line. Findings agreed per file before editing, false-or-stale first; a config file's non-comment lines extracted before and after, byte-identical. A test licenses a cut only by an assertion it ALREADY makes, never by one the cut would need. New prose verifies its own referents before it lands. A relocation ruling opens the target and quotes the line that carries the claim — a home that carries nothing yet is a registration, not a pointer; and a comment can be load-bearing as TEXT, when a guard greps or parses the source: ask *does anything read these bytes?* before cutting it.
