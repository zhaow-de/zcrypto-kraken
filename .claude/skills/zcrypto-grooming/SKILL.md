...
3. If the queue holds narrative status prose rather than discrete items, the same rule applies at block level: a block describing only finished work is **deleted**; a mixed block keeps only its open remainder in the queue. **After grooming, the milestone contains only open work.** Then match against every `WORK-ITEMS QUEUE` item:
   - **fully resolved** → **delete** the item — read back with the grep the protocol's *done* procedure prescribes;
   - **partially resolved** → **delete the finished sub-items**;
