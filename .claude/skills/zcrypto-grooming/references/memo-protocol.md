...
- **An item is its bold line AND every bullet indented under it** (no count command: the memo is unversioned, and a touch leaves no record in the tree). A rewrite that reads only the head line rewrites what it read and leaves what it did not, and each later scan re-reads that head line and confirms the same mistake; so a condensation reads the whole block, and what it leaves standing it has read.
...
- **done** (`T9999 is done`) → find the `WORK-ITEMS QUEUE` item(s) referencing that topic and **delete** them; the evidence is the merged PR and its commits, never the memo. (no count command: the memo is unversioned, and a touch leaves no record in the tree) Move an item to `DONE ITEMS` instead — one line: the subject, the PR, the date — only when a still-queued item's `DependsOn:` names it; on the read-back, grep the topic id across the whole memo and confirm it stands nowhere in `WORK-ITEMS QUEUE`.
- **partially done** (`T9999 is partially done`) → **delete the finished sub-items**; never annotate them in place, and never write what was implemented, measured or found — that is the commit message's. (no count command: the memo is unversioned, and a touch leaves no record in the tree) When the remainder changed *shape* — scope shrank, effort resized, prerequisites moved — rewrite the item's **subject**, **Size**, and **DependsOn** to describe the remainder alone, then **re-order the milestone list** so it stays a dependency-true suggested sequence.
