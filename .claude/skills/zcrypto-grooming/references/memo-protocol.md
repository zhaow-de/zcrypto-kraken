...
**Concurrent writers:** the memo is written under the token `docs/reference/multi-agent-protocol.md` defines — request it from `zcrypto-marco`, read before, Edit/Write tools only, read back after, hand back the `sha256 · lines · bytes` chain.
