...
- **The trade.** The ambient set grows only for a stated reason: the commit message carries one line, `Ambient grows by N bytes: <reason>`, with N the exact growth the guard measures from the staged files; a deletion of equal size in the same commit needs no line, and a line on a commit that does not grow the set is refused. The guard measures the commit against `HEAD`, or against the amended commit's parent when the subject is unchanged — every file the whole commit changes, so such an amend states the whole commit's growth and a message-only amend must keep its line right; a reworded amend is measured against `HEAD` and states the increment. A commit that changes no ambient file is not judged. What no commit-msg hook sees — a reworded amend, a squash or fixup in a rebase — `merge-pr`'s gate catches at merge: `infra/scripts/guidance-guard.py --range <base>..<head>` judges every commit of the branch against its parent, so a line a rewrite lost or doubled is refused there. A growth with no reason worth one line is a line that does not belong.
