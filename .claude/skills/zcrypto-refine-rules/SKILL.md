...
The candidate set is the inbox records themselves: every `.local/agent-lessons/<session>.jsonl` in the main checkout (swept with `infra/scripts/sweep.sh --control '^{"ts": "[^"]*", "session": "' -e '^{"ts":'` — a rotated round leaves the inboxes empty, so the sweep's ordinary answer is a clean, and the control is what makes that clean evidence: anchored, it is a lesson record's own opening, which no tracked file carries, while any single key of it is prose in tracked files, this one included. rc 2 then says no record is under `.local/agent-lessons/` at all, live inbox or harvested archive; whether `.local/` was in the list is the separate door, the `no <path>/.local` line on stderr), `infra/scripts/check-agent-lessons.py` on each first, a malformed line being a finding, not a skip. Read per session and grouped by class into the round's table, id `<session>:<ts>`. A harvested inbox rotates to `.local/agent-lessons/harvested/<round-date>/` in the same step — that rotation is the archive — so the next round starts from empty inboxes and no record is harvested twice.
...
- **A non-zero count is a finding**, except where zero is not the healthy value: the four entries `tests/test_count_list.py` holds as `NOT_NAMED` — the corpus names none of them — `prose-chars`, a number to watch, `pinned-leaves-the-edge-renders`, the client PEMs the edge renders, healthy at 1, a value that reads `N passed`, a passing suite, and `operator-term-surfaces`' second value, which counts every commit whose diff touches `_WP_CARRIERS` — prose that merely names the allowlist moves it as surely as an edit to it, so read those diffs before calling a rise a widening. Resolve a finding in the round: fix the practice, narrow the rule, or delete the rule.
