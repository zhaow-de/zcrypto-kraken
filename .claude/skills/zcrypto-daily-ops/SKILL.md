...
- **a date** — compare with today, unless the trigger's own text says the date is a handle for a check this
  ...
  unevaluated.
- **another topic's resolution** — run the check the trigger carries, against the topic's own definitions: a change-index cell is a parse of a PR body, not evidence that a spec exists, and a grep that matches a trigger's words is not its arm — open the topic and the spec listing before reading it as fired.
