...
- The `process_*` families present for the host (the capture, ops and NAS keep-lists admit six, a cache node's one, resident memory; `tests/test_infra_alloy_series.py` is the authoritative per-host series checklist — read `NAS_REQUIRED` / `OPS_REQUIRED` / `CAPTURE_REQUIRED` there rather than trusting any list copied here).
