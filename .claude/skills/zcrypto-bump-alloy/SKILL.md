...
- `docs/reference/fleet-pins.md`: the alloy row's digest and version from the deploy-log line, its `since` from the container's `.State.StartedAt`, and the operand — re-trued, not re-typed.
