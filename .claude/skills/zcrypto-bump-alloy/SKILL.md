...
3. Record the **current baseline** before changing anything — the deployed digests live only on the hosts for 3 of 4 (per-converge extra-vars, no repo default): `docker inspect grafana-alloy --format '{{.Config.Image}}'` on `hp`, `nas`, `red`, `zcrypto` — into `docs/reference/fleet-pins.md` (the durable record the *next* bump diffs against, and the rollback reference for this one) — `.Config.Image`, never `.Image` (the block).
