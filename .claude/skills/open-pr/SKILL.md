...
**Before the first push, run `uv run pytest -m data` on the data-bearing checkout.** CI runs the suite un-marked on a machine with no `data/` and no NAS mount, so every gate in that family skips there whatever else is green — and so does this run from a worktree, which carries no `data/` either. Read the skip count: an all-skipped green is not a pass.
