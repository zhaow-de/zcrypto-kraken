...
**Before the first push, run `uv run pytest -m data`.** CI runs the suite un-marked and its machine has no `data/` and no NAS mount, so every gate in that family skips there whatever else is green; the marked run is the only one before a merge that executes them, and `tests/test_data_gated_marker.py` is what keeps the family's files marked. The rest of the suite is CI's, on every push to the pull request.
