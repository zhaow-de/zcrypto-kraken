...
**Where the branch reaches the data-gated family, run `uv run pytest -m data` before the first push, on the checkout holding the datasets.** The family is `git grep -lE '^pytestmark = .*pytest\.mark\.data' -- tests`, the list form included; the branch reaches it when it changes one of those files or a path one of them imports or invokes. CI runs the suite un-marked but skips the family, as does any worktree the datasets were not linked into, so read the skip count: an all-skipped green is not a pass.
...
3. `Read before push by: Claude Opus at <sha>` (or `Claude Fable`; the gate's floor matches the `Claude` prefix, and a body that drops it is refused) — one line naming the agent that read the whole branch before push, a different agent from the author, and the tip it read — the `review` workflow run over `develop..HEAD` before the push, `re-review` over the fix range before the push that carries the fixes; `merge-pr`'s gate refuses a body whose sha is not the PR head, so a commit pushed after the read takes a read of the delta and an updated line — except the heads the gate admits past it: above the tip, at most one change-index row commit of Step 4 and at most one refine round's closing commit — one that changes no file and carries the `Refine-Round-Closed:` trailer, whose message the refine skill sends to a pre-review — in either order, on the base the tip was read on or on one that moved under both, each carrying a message this line does not cover; a head with the tip's tree and the read's messages from the base, a re-dated or re-signed commit (a reworded message, at the tip or below it, takes `pre-review` over the amended commits and `re-review` over `<read>..<head>`); and the tip rebased onto a moved base, or that base merged into it, with every patch, message and rendered-file resolution kept, judged by `git merge-tree`. When the read is Opus and the PR touches a `--fable-paths` path, a second line goes directly below this one — `Fable floor substituted by Opus: <reason>` — whose reason the gate refuses when it is the placeholder, a filler token, or too little to be a reason, and which it reads as a reader sees it — a line inside a comment, a fenced block, a `<details>` block or a quote does not count, and an unterminated comment or fence hides everything after it.
