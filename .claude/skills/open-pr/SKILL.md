...
**Where the branch reaches the data-gated family, run `uv run pytest -m data` before the first push, on the checkout holding the datasets.** It reaches it when a file the branch changes carries `pytestmark = pytest.mark.data`, or when one of those files consumes what the branch changed — `grep -l '^pytestmark = pytest.mark.data' $(git diff --name-only develop...HEAD -- 'tests/*')` answers the first, and the consumer grep the second. CI runs the suite un-marked and the merge gate requires it, so a branch reaching nothing in the family owes no local run at all; `-m data` selects the part CI structurally cannot reach, it does not add a gate. The gates skip where the datasets are absent — CI, and any worktree they were not linked into — so read the skip count: an all-skipped green is not a pass.
...
3. `Read before push by: Claude Opus at <sha>` (or `Claude Fable`; the gate's floor matches the `Claude` prefix, and a body that drops it is refused) — one line naming the agent that read the whole branch before push, a different agent from the author, and the tip it read — the `review` workflow run over `develop..HEAD` before the push, `re-review` over the fix range before the push that carries the fixes; `merge-pr`'s gate refuses a body whose sha is not the PR head, so a commit pushed after the read takes a read of the delta and an updated line — except the four heads the gate admits past it: the change-index row commit of Step 4 when it is the tip's only child, on the base the tip was read on or on one that moved under both; a single commit above the tip that changes no file, a refine round's closing commit, admitted on the same terms and by the same arm — one, never a run of them, each carrying a message this line does not cover; a head with the tip's tree and the read's messages from the base, a re-dated or re-signed commit (a reworded message, at the tip or below it, takes `pre-review` over the amended commits and `re-review` over `<read>..<head>`); and the tip rebased onto a moved base, or that base merged into it, with every patch, message and rendered-file resolution kept, judged by `git merge-tree`. When the read is Opus and the PR touches a `--fable-paths` path, a second line goes directly below this one — `Fable floor substituted by Opus: <reason>` — whose reason the gate refuses when it is the placeholder, a filler token, or too little to be a reason, and which it reads as a reader sees it — a line inside a comment, a fenced block, a `<details>` block or a quote does not count, and an unterminated comment or fence hides everything after it.
