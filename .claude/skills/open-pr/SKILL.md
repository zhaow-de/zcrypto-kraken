...
Iterations are zero-padded to three digits (`iter-007`), several keys of one kind are comma-separated, and a kind with no key is an em dash `—`. Rows stay sorted by PR number ascending, and **no cell may hold a file path** — a spec is its bare serial `00034`, never `docs/specs/00034-…`. No key of any kind in the branch name, the title or `## Spec / Plan` ⇒ no row — but a key in the BRANCH NAME always takes a row, even when the PR delivered none of it and every cell is an em dash: `tests/test_change_index.py::test_every_keyed_merge_on_develop_has_a_row` reads the branch name, and the row it wants cannot be written by this PR's own CI, which is green before the merge exists. The registration-only PR is exactly this case. Then re-read the file and confirm the row is there before reporting the PR open. The row commit is one of the two heads `merge-pr`'s gate admits past the tip the read line names — a single commit touching the index alone (the other is a message amend of that tip) — so it takes no delta read and the line stays as written.
