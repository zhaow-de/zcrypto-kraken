...
**Before the first push, run `uv run pytest -m data`.** CI runs the suite un-marked on a machine with no `data/` and no NAS mount, so every gate in that family skips there whatever else is green.
