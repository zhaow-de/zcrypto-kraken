...
5. **No failing CI** — any `statusCheckRollup` entry in a failing state stops the merge. This repo's `coverage.yml` now runs the test suite on **`pull_request` into `develop`/`main`** (as well as on push), so a PR **does** report a "Test coverage" check — wait for it to finish and be green before merging (the gate above flags a *failing* check; it does not yet block on a *still-running* one, so don't merge while it is in progress). The Coveralls upload stays push-only, so the PR check is purely the pytest pass/fail gate.
