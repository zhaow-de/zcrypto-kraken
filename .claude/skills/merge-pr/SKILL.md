...
8. **The `ops-journal` month PR is exempt from gate 7** — its README declares the month PR outside the pre-push read; a head branch named `ops-journal` skips the read-line arm, and every other arm still applies.
