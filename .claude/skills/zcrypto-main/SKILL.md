...
> Hourly tick. (1) `ListAgents`; a session missing for a second consecutive tick is flagged. (2) For every payload session that is idle with an open assignment in `docs/coordination.local.md`, send one line: what it last declared, and continue. (3) Read `git branch -r`, `git worktree list`, `gh pr list --state open`, and the memo's work-package markers. (4) Post one report to the owner: per session — branch / topic / spec, and whether the branch moved since the last tick; the backlog's next three items; anything flagged. Update the table's `last tick` and each session's `last report` line. Do not assign, merge, or write the memo inside the tick.
