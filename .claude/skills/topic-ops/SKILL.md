...
If only some sub-items are done the topic is `partial`, not resolved (see *Partially completing a topic*). If none are, it stays `open`. The split above is a condition on closing, never a remedy for a live arm.
