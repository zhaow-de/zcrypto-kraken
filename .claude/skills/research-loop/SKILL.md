...
**The loop ends exactly two ways:** the **time-gate** (default 08:00 Berlin — `TZ=Europe/Berlin date`; the invocation may override it), or a **genuinely unrecoverable blocker** (jot a free-form stop-note — not a gated decision entry — at the end of the active `docs/research/<serial>.phase<N>-decisions.md`). Nothing else is a stop: an empty backlog (manufacture a package — see Constraints), a failed or negative iteration (record the verdict, continue), a mid-execution error (fix it, continue), a reversible ambiguity (decide it), an idea containing an irreversible step (park that step, continue).
...
7. **Closeout** — instrument checks first (below), then the **verdict** — hypothesis, spec hash, dataset hash, result, adopt/reject/park — into the decisions log **and** the append-only trial registry (never an unlogged "it seemed better"). Suggest the next step; write the iterations-history entry. Iteration closeouts *append* to the relevant `docs/research/<serial>.phase<N>-decisions.md` (routing in `.claude/rules/decisions-log.md`) and commit the append as part of the iteration's closing commit.
...
- **Drain the open topics toward human-only.** Draining means *solving*, never merely archiving — a topic filed away with its issue unsolved is a permanently lost problem, not a shortened list. Fold them into brainstorms; keep each topic's `status` in sync per sub-item (a checked-off item under `status: open` is a drift bug); pursue the researchable sub-items of human-gated topics per the decomposition rule. By hand-back, everything still open is human-gated or deliberately deferred — an autonomously-resolvable item, or a fired `ripe_when:` trigger neither picked up nor consciously re-deferred, left parked is a miss.
...
- **Phase close-out**: sweep the topics first (a current-phase topic gets a dedicated iteration before the phase closes; future-phase topics stay parked), then write the close-out report; the per-phase decision log `docs/research/<serial>.phase<N>-decisions.md` is already the persistent record (appended live per iteration — no drain), so a closed phase's later backlog just appends under a `**Continuation —**` divider (routing in `.claude/rules/decisions-log.md`). **Never cross a phase boundary without the close-out report.**
...
| Read "resolve topic X" as "flip its status and archive it" | "Resolve" means SOLVE the issue, then archive. Archiving an unsolved topic destroys it. Out of scope? Leave it open and say so. |
