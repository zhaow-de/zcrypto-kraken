...
- **Each node's replication link up before the next**: on the node just converged, `vk INFO replication` reads `role:slave` and `master_link_status:up`, and on the primary its `slaveK:` line reads `state=online` with `lag=0` or `lag=1` — seconds after the converge, not a bake.
