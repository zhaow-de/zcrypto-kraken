...
- Converges via `infra/ansible/scripts/converge.sh`: the invariants are `.claude/rules/fleet-deploys.md`'s, the mechanics this file's *Converge mechanics*.
...
2. **≥3 full segment-rotation hours** — the first hour's every book `<HH>.parquet` beginning where the other host's does (identical first rows) proves the boundary write; three full hours separate a real trend from the rotation sawtooth. **FULL hours only — hours that BEGIN after the re-pin; the converge's own partial hour never counts** (re-pin at 14:07 → hours 15/16/17 → gate 18:00Z). Counting boundaries instead of full hours reads the gate an hour early. The gate is the same `<HH+4>:00Z` wherever in hour `<HH>` the re-pin lands, so the bake occupies `4 h − MM` and the secondary is re-pinned **late in the hour**: 14:07 → 18:00Z holds the window 3 h 53 m, 14:59 → 18:00Z holds it 3 h 01 m.
...
**Memory is not a gate event.** `zcrypto-fleet-memory-headroom`, `zcrypto-fleet-memory-leak` and `zcrypto-fleet-daemon-restarted` watch every daemon continuously and fleet-wide — before, during and after any rollout — so a bake owes no RSS read at T+anything. During the bake, the headroom rule firing on the just-converged host is an abort signal (table below). A leak page days later names the image through `docs/reference/fleet-pins.md`'s `since` column and `docs/reference/deploy-log.jsonl`, and the rollback operand is in the same row.
...
Before the window, run the preview EARLY — check mode skips the engine's window assert (`when: not ansible_check_mode`), so a `--check --diff` from the rollout tree hours before the gap costs nothing and catches a wrong command shape with time to spare. Then, on the user's word, the same-day default shape, BOTH digests: `converge.sh site.yml --limit zcrypto -e converge_primary=true -e capture_image_digest=sha256:<candidate> -e engine_image_digest=sha256:<candidate> --tags capture,engine` — the engine role pins from its own variable and fails fast on an empty one AFTER the capture play has already converged, so a command carrying only the capture digest leaves the two tiers on different digests. The capture tag discipline per `fleet-deploys.md`.
...
The outcome is read on the FIRST FULL HOUR after the restart, on the **pulled** copy (the hosts have no parquet reader): the restart hour itself is partial and excluded — a restart's own gap runs from the old container's last row to the new one's first, ~1 min on liquid pairs and longer on thin ones — and the row waits for the pull that carries the full hour. The checks, the row (with any standing constraint the converge produced) and the prune follow the *Converge mechanics* block; the bake form (`deleted=N`) joins the evidence in the row's commit message. **Commit the deploy-log line in the same change as the row it feeds** — until it is committed, the only record of a production converge lives in a working tree that `git stash`, `git checkout --` and `git clean` all discard, and `mutate-probe.sh` refuses to run beside. When the row is committed before its first full hour can be read, the read is owed back onto that commit the moment it exists — a message-only amend with the tree frozen, or a second commit — never a note in `.local/`, which is gitignored and outside `git log --follow`.
...
**One PR per rollout, two recording commits** — the secondary's row, then the primary's and the engine's; review folds ride the same PR; a row's digest cell is the digest and the revision it was built from, and what the image carries is that revision's log (`infra/docker/Dockerfile`'s `COPY` lines: `cli/` and the project files) — a role, alert or controller-side change rides no image — merged before any other branch can touch `fleet-pins.md`, `deploy-log.jsonl` or `fleet.md` — same day is the default, not a gate: the converge-and-bake window's minimality is the invariant, the calendar day is not — and **never branch other work from it**: a recording branch that lives for days collects everyone else's commits. **The rollout record is not complete while the two capture hosts differ**: either the primary's leg is done, or the hold and its bound — a UTC clock time, never a date, which the pins file refuses outside `since` — are in the primary's row. A gate passed with no primary leg and no bounded hold is an open rollout that reads as finished.
...
- **Adding a capture pair** (`capture_pairs` in `group_vars/capture_host/vars.yml`): the capture role reads the primary's deployed pairs and refuses a secondary-first add, fail-closed when the primary is unreachable — secondary-first floods the append-only ledger with false heals, silently on the trades half. Pass the currently-running digest; no bake owed — but the two `--limit` runs, primary then secondary, are ≥ 1 h apart: `fleet-pins.md`'s standing constraint on restarting both capture hosts close together, and `count-list.sh capture-hosts-converged-within-an-hour` books a closer pair.
...
- Converge only from a tree whose rendered config matches the fleet: read the `--check --diff` preview and stop on any diff you did not intend — a tree behind an unmerged branch's deploy silently reverts it.
...
- A `daemon.json` diff refuses to apply without `-e daemon_json_ack=true` — the docker role shows the diff first; its handler restarts dockerd, every container on the host with it, live capture included; the role is shared, so the ack gates every host.
- **`fleet-pins.md` is a STATE record.** `converge.sh` appends every real pass to `docs/reference/deploy-log.jsonl`; re-true the row from that line — never from memory — commit the line with the row, and put the converge's evidence (every check read, the values quoted) in the commit message, never in the file — a row is its cells, each under the cap `tests/test_fleet_contracts.py` holds, and a date sits in the `since` column alone. `git log --follow` on the file is the deploy chronicle.
...
- Verify by outcome, never by exit code. Ops: `infra/scripts/ops-postverify.sh`, every check in one command. Capture: on the pulled copy, every book stream's `<HH>.parquet` begins where the other host's does (identical first rows), not at a fixed `:00:00.0x`; the NAS pull's next cycle logs `pull complete … failed=0` for every verified channel (that IS the manifest verification — fresh segments are hash-verified on arrival, and whole-archive coverage completes within 24 pull cycles under the incremental hash scope); `infra/scripts/continuity.py` on a pulled copy, never the live dir, shows no new truncated hours. Engine: the next `cycle-HH.json` lands with `completed_at` inside `[B, B+30 min]`; the restart marker is the container's `.State.StartedAt`, never the converge command's return time.
...
- **An engine re-pin whose revision also touches `roles/capture/files/config.alloy` never converges `--tags engine` alone** — the keep-list and its drift assert live in the *capture* role, so the new families publish unadmitted and silently; run `--tags capture,engine` with `-e capture_image_digest=<currently-running> -e capture_alloy_digest=<currently-running>`, then verify the new family by VALUE at the next scrape (`uv run python infra/scripts/grafana-query.py '<family>{host="zcrypto"}'`).
...
- **Each node's replication link up before the next**: on the node just converged, `vk INFO replication` reads `role:slave` and `master_link_status:up`, and on the primary its `slaveK:` line reads `state=online` with `lag=0` or `lag=1`, the reads of `cache-rejoin-node`'s step 3, with `cache-manual-failover`'s step 2 for the lag — seconds after the converge, not a bake.
