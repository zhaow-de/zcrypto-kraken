...
- **Compare the two hosts at COMMON WALL-CLOCK times, never at each host's own T+.** Both stepping in one wall-clock window while their process ages differ is environmental — an age-driven cause cannot synchronise across daemons of different ages. This outranks the amplitude rule below; run it first.
- **A flat stretch alone discharges nothing** — the shape does. Two steps of decaying amplitude, each with a flat trough, is a converging allocation (T0136: +4.17 then +1.38, ratio 0.33); equal-or-growing steps are a real leak. One step and a quiet stretch cannot tell them apart, so hold the topic open rather than closing on it. **Amplitudes are the fallback when no control exists — never escalate on them while a wall-clock comparison is still available**, since the affected host reads equal-or-growing in exactly the case the control exculpates.
- **Sample the secondary THROUGH its band at 2 h resolution, and read the PRIMARY in its own band too**, not only at the secondary's anniversary — converges far apart leave the primary's band unopened at gate-close, which is how a bake ends with no control.
