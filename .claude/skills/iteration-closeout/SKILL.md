...
2. **Find the phase's decisions log.** A phase has exactly ONE: `ls docs/research/*.phase<N>-decisions.md` names it — append there. A phase's research docs may span several serials (Phase 4 runs `05`–`10`), so never derive the log's serial from the phase's other docs. The glob matches nothing → create the log at the next-free serial: the highest serial BELOW 90 across `docs/research/` + 1 — the 90-series is the meta band (protocols, assessments) and never mints a phase log's serial.
