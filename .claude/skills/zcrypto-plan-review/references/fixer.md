...
- Did you introduce a sentence asserting a fact about a file other than the one it sits in? (It goes false on the next change to that file, and nothing checks it but a second reader — test docstrings included.)
...
- Did you measure anything over the tree — a formatter shift, a line count, a coordinate? The measurement is taken from the repo root, in the tree as the task order leaves it, with every file an earlier task creates present; two coordinate families on one pair came from a scratch root and from a tree missing one created file.
