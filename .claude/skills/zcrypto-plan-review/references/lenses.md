...
- Every deferral names a registered `T<NNNN>` topic whose file exists, a trigger arm on an existing topic, or an explicit drop with its reason — "later", "follow-up", "out of scope" and "known" are not registration.
