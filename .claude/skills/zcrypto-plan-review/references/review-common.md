...
- Origin: `in-original` — present before any fix round; `last-fix` — introduced by the commit named in the prior fix report; `earlier-fix` — introduced by an older round's fix. With no prior fix report, everything is `in-original`.
