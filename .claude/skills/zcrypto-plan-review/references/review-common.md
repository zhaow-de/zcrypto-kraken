...
- The origin bracket holds exactly one of the three words and nothing else — a hash, a note or a second word makes the heading unparseable, and the finding vanishes from the count while the count line still prints; the severity bracket is read forgivingly, the origin bracket is not.
