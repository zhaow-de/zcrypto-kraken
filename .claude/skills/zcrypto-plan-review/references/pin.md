...
A claim about a fleet host — a unit, a mount, a running container, a host's config — cannot be checked from here: ssh is blocked in a dispatched agent. Write it as `HOST · <the claim, quoted> · <the exact read-only command that would check it, scoped per CLAUDE.md's secrets rule — a docker inspect names one field, never .Config or .Config.Env, never docker exec … env>`; do not run it and do not grade it — the orchestrator runs those lines itself.
