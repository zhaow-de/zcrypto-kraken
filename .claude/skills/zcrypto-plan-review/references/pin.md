...
A claim about a fleet host — a unit, a mount, a running container, a host's config — cannot be checked from here: ssh is blocked in a dispatched agent. **This is about where the COMMAND runs, not what it reports on**: a repo script you can run here that queries a monitoring API is yours to run and grade, however much its subject is a host; only a command that must execute ON the host is a `HOST` line. Write it as `HOST · <the claim, quoted> · <the exact read-only command that would check it, scoped per CLAUDE.md's secrets rule — a docker inspect names one field, never .Config or .Config.Env, never docker exec … env>`; do not run it and do not grade it — the orchestrator runs those lines itself.
