---
status: open
ripe_when: one `min_over_time(process_resident_memory_bytes{host="zcrypto-red",job="capture_app"}[6h])` read taken after **2026-08-08 18:20Z** — i.e. after the window in which the next daily step is due. The two observed steps landed in the same clock band (15:20–18:20Z) exactly 24 h apart, so a reading before that window cannot distinguish a plateau from the trough between steps. One Grafana query against a series already being collected, via `infra/scripts/grafana-query.py`
---
...
- **After 2026-08-08 18:20Z**, take one `min_over_time(...[6h])` reading on `zcrypto-red` and compare against the 141.20 MiB floor. Interpret as: **no third step** → the asymptote is confirmed, discharge the residual honestly and record it; **a third step of ~1.5 MiB** → still converging, re-read after the next daily window; **a third step of ≥3 MiB** → the residual is **not** dischargeable, and the leak question is live against `ccedc9dd6bf4` on the capture path.
