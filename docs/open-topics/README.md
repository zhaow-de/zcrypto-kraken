...
- [T0034 — grafana-push can silently mis-point every alert](T0034-grafana-push-safety.md) — the push takes datasource UIDs as unvalidated env vars (a Grafana Cloud stack has 2 Prometheus and 3 Loki datasources, so "first of each type" silently repointed all 7 rules at the wrong data, still `health=ok`), and it **never prunes** — a rule deleted from `alerts.yaml` keeps evaluating and emailing forever (ripe when: live now — both were hit for real on 2026-07-13).
