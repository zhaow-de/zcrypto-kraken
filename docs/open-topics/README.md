...
- [T0020 — Grafana Cloud observability](T0020-grafana-cloud-observability.md) — the canonical dashboard, alert rules, push script, and creds path landed via iter-094's NAS build (spec 00049); remaining = the VPS `obs` role + app `/metrics` exporters (spec 00043's original scope) and running `infra/scripts/grafana-push.sh` to provision the dashboard/alerts on the live instance (ripe when: an observability build+deploy session; capture exporter flip additionally after the ≥7-day clock ≈ 2026-07-15).
