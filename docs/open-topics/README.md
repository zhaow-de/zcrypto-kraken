...
- [T0036 — a restart silently truncates the hour it lands in](T0036-segment-writer-restart-clobber.md) — **fix complete + dry-run-validated** (committed-final invariant, atomic parts, validated recovery, cross-stream rotation quorum per T0037); remaining: run the validated migration at the image rollout and verify post-deploy, then resolve (ripe when: the deploy window).
