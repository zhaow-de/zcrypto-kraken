...
- [T0037 — hour rotation trusts an untrusted timestamp](T0037-rotation-trusts-an-untrusted-timestamp.md) — `append()` rotates the hour from Kraken's own `timestamp` field, so one event stamped up to `MAX_TS_AHEAD` (1 h) ahead publishes the live hour as a **committed, complete** segment and every genuine row for the rest of that hour is then dropped as late — ~54 min × 20 streams, and (unlike pre-T0036) it now survives a restart, because the premature final is what "this hour is closed" means; no window size and no wall-clock veto can fix it without blacking the stream out in the other direction, so the fix is rotation **corroboration** (two events must agree on a new hour) (ripe when: live now — one bad `timestamp` from Kraken truncates the hour it lands in).
