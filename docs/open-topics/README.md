...
- [T0056 — the NAS runs whatever was last hand-copied to it](T0056-nas-hand-deploy-drift.md) — the drift class is structurally closed on `feat/ops5-offload`: the `nas` role converges the stack files + rendered envs (option (a)), the `${CAPTURE_IMAGE:?}` pin kills the `:latest` placeholder trap, and the operating quirks are recorded in `infra/nas/README.md`; remainder = the first attended converge (TZ flip to UTC + apply + verify), then resolve (ripe when: the first attended NAS converge).
