...
- [T0039 — the reconciler's `--min-gap-seconds` needed cross-host validation](T0039-min-gap-seconds-needs-cross-host-validation.md) — **Task 12 done 2026-07-17**: 66 h of two-host soak (217 windows, max 12.08 s) VALIDATES the deployed 30 s — and the phantom-splice risk is now measured rather than feared (a 12.08 s AVAX artifact, with 9 sibling pairs flowing on a demonstrably healthy primary, would have spliced at a 10 s threshold). Partial: the `--mint` flip remains and wants Task 13's drills first — this soak contained no real outage, so the catches-a-real-gap leg is untested.
