---
status: partial
ripe_when: the alert-rule leg and the ledger correction are ATTENDED (a Grafana push and a write to the NAS ledger); ripe at the next attended session. The intersection-vs-own-window booking is a DECISION and is ripe NOW — every input is measured and on disk; its cross-hour rider folds into whatever lands with it
---
...
**What the corrected counters would book for the blackout hour**, per the code as landed: 82.955463 s healed, and 41.553798 s of per-pair residual on top of the fleet record's existing 2,385.847992 s — 2,427.401790 s of the measured 2,437.147792 s permanent loss. The 9.746002 s shortfall is ADA/EUR's: it has no *minted* record to carry its own remainder, so only its 198.820666 s share of the fleet window is booked. It now at least leaves an `unwitnessed` record naming the window (above); closing the 9.7 s itself is the intersection-vs-own-window decision under *Suggested next steps*. Two adjacent classes stay unrecorded by the command rather than by the detectors: a pair-hour with a primary file and **no secondary file** is skipped before either finder runs, and an hour minted in an *earlier* cycle is skipped by `already_minted`, so at deploy the already-minted hours inside the 48 h window get no retrospective `unwitnessed` record.
...
- *(decision)* **The fleet intersection under-books every stream except the binding one.** Re-measured 2026-07-28 from the raw mirrors, hours 06 and 07 of 2026-07-13, all 10 pairs. **The event predates the secondary** — `capture-segments-red` has no 2026/07/13 day directory — so every figure below is primary-only, and "both mirrors" does not apply to it:
  ...
  | sum of each stream's OWN silence, clamped to the hour | **2,696.031909 s** |
  | cross-hour straddle, per stream | 0.002794 s (DOT) … 0.308901 s (ETH); **fleet total 1.203668 s** |

  **2,696.031909 + 1.203668 = 2,697.235577 exactly** — so this file's original figure was RIGHT, and reproduces as *own window + own straddle head*. Only its **attribution** was wrong: it blamed `fleet_dark_windows`'s clamp to `hour_start` for the whole ~35 s, and the straddle is 1.203668 s of it. A 2026-07-28 revision then over-corrected — calling the original figure unreproducible and quoting the straddle as 0.002794 s *per stream*, which is DOT's alone, the fleet-wide latest stamp misapplied to all ten. Both errors are recorded because each was a durable claim someone would otherwise act on.

  The remaining **34.243169 s (1.27%)** is the real defect: `both_streams_silent` books the **intersection** — the window in which every stream was silent — times the stream count. LTC/EUR's own window equals the intersection exactly because it is the stream that returned FIRST; every stream's window starts at the clamped 07:00:00, so only the return edge binds. The other nine stayed dark longer and that surplus is booked nowhere.

  **The decision, where the wording matters more than the direction.** Keep the intersection as the *discriminator* separating an outage from a thin market — its whole reason for existing. But "then book each stream's own dark window" is unsafe as stated: a stream's own >30 s windows over an hour include quiescence unrelated to the outage, so a 31 s fleet blip plus a thin pair's unrelated 10-minute quiet would book those 10 minutes as permanent loss — the thin-market false positive moved from *detection* to *quantification*, where it is worse, because it lands in a monotonic counter that drives the CRITICAL page. The safe form is **per-intersection-window**: for each fleet-dark window `W`, book per stream the silence window that CONTAINS `W`, never every dark window that stream had in the hour. On this event the two coincide — each pair had exactly one window, containing the intersection — which is precisely why the measurement cannot distinguish them.

  Settle before implementing: (a) `_booked_dark` reads the fleet record's single `windows` list to subtract fleet-booked seconds from a pair's own residual, so if booking becomes per-stream that field and its lookup must become per-stream too, or the "attributed exactly once" invariant breaks in both directions; (b) the per-stream window must be computed from the **union of both mirrors'** stamps, or a window the secondary witnessed is booked as loss and double-counted against `find_book_gaps`.

- *(autonomous, a rider)* **Cross-hour silence.** `fleet_dark_windows` clamps to `hour_start`, so a straddling head is measured from the boundary rather than from that stream's last stamp in the previous hour. Measured at 1.203668 s across the 2026-07-13 event, and bounded above by `min_gap_seconds` per stream — a straddle wider than the threshold is already booked by the previous hour's own tail window. Not worth its own change; fold it into whatever lands above.
