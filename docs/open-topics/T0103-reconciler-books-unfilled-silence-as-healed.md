...
- *(autonomous)* **Cross-hour silence: `fleet_dark_windows` clamps to `hour_start`.** The 2026-07-13 event's true per-pair total is **2,697.235577 s** against a booked 2,661.788740 s, because the silence began at 06:59:59.69–06:59:59.99 — inside the *previous* hour. The detector has no cross-hour context, so the straddling head is invisible. (The `>=` / `>` half of this item is done — see *Done so far*.)
