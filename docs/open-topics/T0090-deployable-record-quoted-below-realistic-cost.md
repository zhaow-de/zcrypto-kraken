...
- *(the carried §12 decision is DISCHARGED — ruled and amended 2026-08-03, moved out of the open list)* The band collision this topic surfaced is settled: §12 now states the two uncertainties as **separate terms** rather than one merged interval — the statistical CI, and the cost basis — with the note that only the second resolves (to a point, once rung 1 measures the realized blend). The amendment also splits the failure diagnosis, which the single-band form could not express: a breach attributable to the estimate means the backtest is wrong ⇒ Phase 5, while one attributable to the cost basis is an execution finding that re-prices the basis without invalidating the backtest. Widening one band to hold both was rejected — a band wide enough to contain both passes execution it should fail, which is the same failure §12 already warns about for a stale band.
