...
- **(human-action, executable)** Provision the dashboard + alert rules on the live Grafana Cloud instance: export `GRAFANA_URL` (the stack's `<stack>.grafana.net` base URL — the only piece not yet vaulted, since only the hosted Prometheus/Loki push URLs are), `GRAFANA_SA_TOKEN` (vaulted service-account token with dashboards + alerting-provisioning write scope), `GRAFANA_PROM_DS_UID` / `GRAFANA_LOKI_DS_UID` (the Prometheus/Loki datasource UIDs, read off the live instance's datasource list), and `GRAFANA_ALERT_FOLDER_UID` (the alert folder UID to provision into); in Grafana, create the `email` contact point (Alerting → Contact points → New contact point, name it exactly `email`, integration `Email`, enter the destination address(es)) since every rule in `alerts.yaml` routes there by name; then run `infra/scripts/grafana-push.sh` with those five vars exported — idempotent, safe to re-run after any `infra/grafana/` commit (full steps: `infra/nas/README.md` § "Grafana dashboard + alerts").
