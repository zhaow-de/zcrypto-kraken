...
3. **Bookkeeping / reproducibility.** Catalog stale + incomplete (OPS-6 owns the rewrite); the deployable's identity is mis-recorded in the closeout/runbook ([[T0063]]); some dataset-hash *recipes* live only in gitignored scratchpad drivers (record 1's `ba47e37e` is **not** reproducible from committed code + manifest alone); there is **no committed backtest command** — research runs via gitignored scratchpad scripts with literal paths; the compiled datasets are single-copy on the workstation (OPS-6 replication fixes).

**What OPS-6 closes, and what it doesn't.** OPS-6 (spec 00056) closes **durability** (replication) and **bookkeeping** (catalog rewrite + data-model reclassification + dead-config cleanup). It does **not** close **reach** (trades→bars + Q2/Q3 dumps), **validation** ([[T0064]]), or **execution-reproducibility** (a committed backtest command + committed hash recipes).
...
- **Already tracked — do NOT duplicate here:** durability + catalog rewrite → **OPS-6 / spec 00056**; deployable doc drift → **[[T0063]]**; deployable out-of-sample validation → **[[T0064]]**.
