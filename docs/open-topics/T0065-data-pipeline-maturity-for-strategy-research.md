...
- **REST reach cadence**: a round seams only inside the interval's reach-back of the canonical tail (catalog `ohlc-reach`); if `tape-bars` retires the intraday reach, say so there in one clause.
