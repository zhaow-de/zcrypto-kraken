---
status: partial
ripe_when: "Kraken's asset host serves the 2026 Q3 OHLCVT incremental — `curl -sIL -o /dev/null -w '%{http_code}' https://assets.kraken.com/marketing/institutions/Kraken_OHLCVT_2026Q3.zip` prints `200` (it printed `404` on 2026-09-20); the URL pattern is the one the support article's Incremental Updates section lists"
---
...
- **The Q2 2026 OHLCVT ingest landed 2026-09-20 (branch `t0065/q2-ohlcvt-ingest`) — the frozen canonicals reach 2026-06-30, as siblings.** `zcrypto data rebuild ohlc-full ohlc-15m --no-push` read `Kraken_OHLCVT_Q2_2026.zip` off the NAS custody dir and minted `data/ohlc-full-20260920` (36 series) and `data/ohlc-15m-20260920` (12 series). `ohlc-full` and `ohlc-15m` themselves are untouched, so every registry pin, the record-44 extent guards and `soak-check`'s canonical stay what they were. The reach round now joins the newest stamped `ohlc-full-<stamp>` sibling (`resolve_canonical_root`, `cli/data/rebuild.py`), and `data/ohlc-reach-20260920` over the Q2 canonical is 24 continuous and 12 detached at 1h. All three siblings are on the hub, and so is `ohlc-reach-20260813`, which had never been pushed although `universe-20260813` names it. **The distribution changed under the trigger**: since the article's 2026-09-16 revision the dumps are direct URLs on `assets.kraken.com` (the full archive in five ~2 GB parts with a SHA256SUMS file; incrementals at `…/Kraken_OHLCVT_<YYYY>Q<N>.zip`), no Google Drive listing. A download is renamed to the reader's spelling — `cli/backfill/read.py` globs `Kraken_OHLCVT_Q*_*.zip` — before the rebuild reads it. One oddity, not acted on: the host's `Kraken_OHLCVT_2026Q1.zip` is 523,665,624 bytes against the NAS's 545,431,093-byte Q1 from the Drive era; the reader keeps quarterly rows only past the base archive's last stamp and refuses a conflicting row at a shared stamp, and the Q2 rebuild raised none.
...
- **Fine-cadence reach round — the Q3 2026 ingest, the last dump this topic waits on.** Only the dump can close 1h 2026-07-01 → 08-20 and 15m 2026-07-01 → 07-08 in the canonical: the tape starts 2026-07-08, and REST holds most of the 1h span only as detached reach segments — 2026-08-13 → 08-21 in none of them — which become a REST-vs-dump cross-check once it lands. **The check** is the trigger's `curl` HEAD; the same URL pattern serves the file. Then: download it to the NAS custody dir as `Kraken_OHLCVT_Q3_2026.zip` (the reader's glob), `zcrypto data rebuild ohlc-full ohlc-15m --no-push`, verify the siblings as the Q2 round did (prefix identity against the newest sibling, tails at the last Q3 stamp), `zcrypto data rebuild ohlc-reach --no-push` — 1h should come back continuous — push all three, and resolve this topic if nothing else is left open here.
