...
**The era split is an instrument artifact, and that makes the span a mechanical fix rather than a judgement call.** The null's asset universe GROWS across its span: BTC from 2013-09-10 through AVAX on 2021-12-21, with all ten legs present only from bar 17971 of 27337. In the first decile, 88.2% of active bars hold exactly one name, where concentration is 1.0 by definition whatever the strategy does, and 98.8% of the first-decile-against-late gap is explained by asset count alone. The earlier framing here — that the null pools two strategies — was wrong in its reason; a residual strategy difference does survive the correction, the early book being more concentrated at the counts where the eras overlap, but it is a fraction of the composition effect. The consequence is that restricting the null has a derivable start date, the bar at which the basket is complete, instead of a research argument about regimes.

**The span moves the band further than the aggregation does, in the opposite direction.** Restricting the null to the complete-basket era moves the concentration band's 95th percentile from 0.6634 to 0.4006, a fall of 39.6%; the best aggregation candidate moves the same figure from 0.6634 to 0.9214, a rise of 38.9%. On the era-matched null that candidate's objectionable widening shrinks to 0.4006 against 0.4740, a rise of 18.3% — so settling the span first does not merely reorder the work, it changes which aggregation is defensible.

**The question is not soak-specific.** The same builder over the same full span is the reference for the deployable's committed validation basis in `cli/portfolio/record44_legs.py`, so anything scoring a new sleeve over that span inherits the same composition effect. No committed code forces a future family through that path, so the inheritance is a property of the shared instrument rather than an enforced one.

## Suggested next steps

The measurement that fed the decision is done and recorded above; what remains is the ruling and the fix.

- **Rule on the null's span before ruling on the aggregation** (the owner's). Every candidate above was scored against a null whose first two-thirds cannot express the metric, because the basket is still filling; restricting it to the complete-basket era is a mechanical change with a derivable start bar, and it moves the band further than any aggregation candidate does. Until it is settled, an aggregation ruling is being made against a reference the live engine does not correspond to.
