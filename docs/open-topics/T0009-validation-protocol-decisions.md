...
- **Deployable bar / frozen benchmark (added iter-055)**: choose between **gated-B1** (net-of-cost Sharpe 1.047 full / 1.074 post-warm-up; zero-fee maxDD 12.3 %) and **B3+vt-dynamic** (the basket's own 200-day equity gate × vol-target: 1.245 / 1.278; zero-fee Sharpe 1.419; maxDD 21.9 %). The Sharpe edge is **not significant** (head-to-head SPA p ≈ 0.27, seed-stable; the K=4 overlay-family p = 0.0085 is return-driven by raw B3) — this is a point-Sharpe-vs-drawdown judgment, the same tradeoff Phase 3 weighed when it froze gated-B1. Whichever is chosen becomes the kill bar's **frozen benchmark** (the SPA leg) — a pre-registered-protocol change. Evidence: `docs/benchmark-b2-dynamic-report.md` §"B3 and B4".
