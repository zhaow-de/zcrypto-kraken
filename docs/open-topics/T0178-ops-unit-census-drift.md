...
- `docs/reference/fleet.md:34` — the row's trailing `.prom outputs under …` clause spans `grafana-watchdog`, which publishes no textfile at all. **Wrong before the keep-alive branch** — `git show develop:docs/reference/fleet.md | sed -n 34p` on the pre-branch tree shows it.
