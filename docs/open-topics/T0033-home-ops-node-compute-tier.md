...
- **(from the wave-2 review, for OPS-4/5 observability)** `verify-replay`/`engine replay` exit 0 on an EMPTY tree/journal ("no data found"), so once the ops node is seeded, a silently-broken pull would leave both replay dead-men pinging green forever. When OPS-4/5 wire the ops-node metrics into Grafana, add a staleness alert over `ops_*_last_run_timestamp` + the pull-lag metric so data disappearance pages.
