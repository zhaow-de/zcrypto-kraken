...
- Execute the remaining increments: **OPS-5 Offload** (reconciler off the Atom; dissolves [[T0044]]'s growth concern; wire the ops-node scraper + the replay-staleness alert; scope the verified-replay timer with `--date` once cadence matters), **OPS-6 Loop** — which now also owns (owner directive 2026-07-15): **migrate the workstation's `./data` (~260 MB of compiled research datasets) to the ops node** — moved, or rebuilt there from the NAS-held sources — **and synced to the NAS**, so the research loop's inputs are local to where it runs and the loop can never resume blind to data that lives elsewhere (the data-locality half of the catalog-sync discipline).
