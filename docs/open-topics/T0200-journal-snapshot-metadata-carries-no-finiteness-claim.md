...
**The helper is shared well beyond the engine**, with nine call sites across seven modules — three in `cli/engine/store.py`, one in `cli/engine/cycle.py`, and one each in `cli/ohlc/ingest.py`, `cli/ohlc/reach.py`, `cli/backfill/backfill.py`, `cli/derivatives/oi.py` and `cli/derivatives/funding.py` — and two of those callers write frames carrying no close column at all, so a claim stated at the write cannot assume the frame has one. Carrying it on `SnapshotEntry` instead keeps the change inside the journal's own record and avoids that reach, which is the trade this topic's next steps weigh.
