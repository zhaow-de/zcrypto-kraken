...
**The helper is shared well beyond the engine**, with nine call sites across seven modules beyond `cli/engine/store.py` and this file's own `cli/engine/cycle.py`, reaching the OHLC, backfill and derivatives paths too — and two of those callers write frames carrying no close column at all, so a claim stated at the write cannot assume the frame has one. Carrying it on `SnapshotEntry` instead keeps the change inside the journal's own record and avoids that reach, which is the trade this topic's next steps weigh.
