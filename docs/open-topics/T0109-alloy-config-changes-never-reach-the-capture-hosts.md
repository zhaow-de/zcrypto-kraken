---
status: partial
ripe_when: the repo-side work is DONE — first layer (the digest gate), second layer (the directory mount, the reload handler and a CI guard), and the rule line. What remains is ATTENDED and is **one window per tier**: a converge that passes `<tier>_alloy_digest`, immediately followed by `docker compose up -d` in the Alloy project dir, because the roles never run compose and the new mount does not apply until the container is recreated. On ops that window must not straddle the 02:25 UTC auto-reboot. Ops keep-regex is already delivered; capture and the NAS are untouched
---
