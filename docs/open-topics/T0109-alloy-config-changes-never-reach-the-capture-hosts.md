...
- **FIXED IN CODE (this branch), and it needs one attended step per host to take effect.** Both roles now bind-mount the config **directory** (`./conf:/etc/alloy:ro`) instead of the file, so a later edit is visible through the path, and a `reload alloy` handler POSTs `127.0.0.1:12345/-/reload` so Alloy actually parses it. Deliberately a reload, not `--force-recreate`: neither role has ever owned Alloy's lifecycle, and the directory mount makes the heavier option unnecessary. The handler is tolerant of Alloy not running, since it is started attended.
  - **The one-time transition, per host**: the converge writes `conf/config.alloy` and the new compose, but **the roles never run `docker compose up`**, so the running container keeps the old single-file mount until an attended `docker compose up -d` in the Alloy project dir recreates it. Until that runs, the tier is exactly as stale as before.
  - **Do the converge and that `up -d` in the same window.** The converge also deletes the superseded top-level `config.alloy`; the running container holds it by inode so it is unaffected, but a bare `docker restart` in the gap would find the old mount source missing. `docker compose up -d` is safe throughout because it reads the new compose.
  - **Verify by comparing host and in-container `sha256sum`**, not the host file alone — every other check reported success while the container read a stale inode.
  - **Ops's keep-regex is DONE**: converged and recreated 2026-07-28, `node_textfile_mtime_seconds{instance=zcrypto-ops}` now reports 6. **Capture is pending** this transition; the NAS shares the pattern and is manual (Container Manager), so it wants the same treatment separately.
