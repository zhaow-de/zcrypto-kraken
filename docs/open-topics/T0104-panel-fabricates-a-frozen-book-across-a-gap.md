...
- **(ATTENDED, in the window) Delete the BTC-quoted subtree on BOTH copies** — owner's decision 2026-07-28. `SOL/BTC` and `ETH/BTC` hold 224 hours at the old generation, written by the pre-T0092 ops image; the sweep is `PANEL_QUOTE`-scoped so they are never regenerated and never written again. **Keeping them is no longer an option the code allows** — the guard refuses a tree holding out-of-scope hours, because no re-run can repair that state. Nothing is lost: the panel is `f(raw)` and the NAS retains the raw book indefinitely (171 `ETH/BTC` + 133 `SOL/BTC` hourly finals from 2026-07-23, covering the panel's full span), so [[T0092]]'s calibration iteration rebuilds them with the quote-aware ladder it needs anyway. `rm -rf` the two dirs on `zcrypto-ops` **and** on the NAS — the pull is `rsync -a` with no `--delete`, so one side alone comes straight back.
- **The already-materialized 2026-07-27 hour 07 is the whole corrective payload** — the only hour in the panel with a frozen run over 10 s. **All 12 pairs** are affected (198 rows inside the ledger's window; longest frozen runs 198–207 s), but the regeneration reaches only the **10 EUR-quoted** ones: `ETH/BTC` and `SOL/BTC` are outside the sweep's scope and get deleted rather than rebuilt. The manifest worry is narrower than it looked but not absent: no external manifest *pins* these digests, yet the NAS archive pull **hash-verifies the panel channel hourly**, and `write_hour` publishes the sidecar before the parquet — so an in-place rewrite has a new-sidecar/old-final window the concurrent pull will hit, logging `panel pull failed … continuing`. It fails soft (the panel is convenience-durability, not custody), but that is a FOURTH signal during the window, not counted in the three below. Subsumed by the regeneration, which rewrites them anyway.
