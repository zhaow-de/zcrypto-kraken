...
- **Re-derivation trigger for the band, carried here because this is where the gate is written**: the floor above is derived from **shadow** targets — what the live book would have traded. If rung 1 or rung 2 execution changes the target series materially, the band wants re-deriving (`zcrypto engine accum-replay` over the then-current journal) **before** rung 3's gate reads it. Re-check the minimums stamp in the same pass: the figures rest on `kraken-refdata-20260707T032900Z.json`, `fetched_at 2026-07-07T03:29:00+00:00`, and Kraken moves those without notice ([[T0113]]).
