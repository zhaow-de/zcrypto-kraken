...
3. **Store-seeding deadline (node iteration)** — the engine's live price store seeds from the canonical dataset (ends 2026-03-31) plus a REST gap-fill; **Kraken's 720-bar OHLC window covers the 4h gap from 2026-04-01 only until ≈ 2026-07-25**. The node iteration (iter-083) must seed the store before then, or fall back to the quarterly OHLCVT dump for the gap.
...
- **Shadow engine node (iter-083, next attended session)**: the TradingNode, scheduler, price store + REST append (constraint 3's ≈ Jul 25 window), journal writing, `zcrypto engine` CLI — on iter-082's builder + concordance core. Interface caveat from the iter-082 final review: `evaluate_gate` requires `cycle_ts` and `now` to share tz-awareness (repo convention is naive-UTC).
