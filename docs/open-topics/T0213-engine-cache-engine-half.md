---
status: open
ripe_when: 'a milestone: Rung 1 has its verdict — `grep -c "\[rung-1\]" docs/research/14.phase6-decisions.md` prints at least 1 — after which Rung 2''s design point opens the engine-side spec and plan of 00118'
---
...
Spec `00118` decides a persistent cache for the engine — three Valkey nodes under Sentinel on a WireGuard mesh — and its plan `docs/plans/00118-engine-cache-infra.md` delivers the infrastructure half only: the nodes, the mesh, Valkey and Sentinel, their telemetry, and a compatibility probe of the pinned library's client. The engine-side half is a second spec and plan, written at Rung 2's design point on the owner's word of 2026-09-24: the `cache_proxy` HAProxy service in the engine's compose project and its `cache-proxy` tag, the engine host's `:9104` scrape, the Cache board's proxy row and its two rules, D10's `[zcrypto.engine.cache]` wiring of the library's Redis backing to the proxy, D11's startup pass against the cache, D15's two-process harness with its CI Valkey service, and D1's live proof — the restart with a margin position open whose boot reads the position at Kraken's entry price from the cache.
...
- Spec `00118`'s decisions D1, D4, D10, D11 and D15 are the engine half's; its plan's *Resolution* names this topic as the half's home, and the plan's Global Constraints keep every task off `cli/`.
...
- The trigger keys on the `[rung-1]` tag the verdict entry in `docs/research/14.phase6-decisions.md` will carry, on that log's own tag convention (`[iter-118]` and the like); the runbook's probe-window procedure records the verdict there at its step 9.

## Suggested next steps

- **(design, after the trigger)** Write the engine-side spec and plan under `00118`'s decisions: read `docs/specs/00118-engine-cache-design.md` D1, D4, D10, D11 and D15 and the infrastructure plan's *Resolution*, then brainstorm the second pair with the owner — the proxy's compose service, caps and health checks; the library's `RedisCacheConfig` fields against the proxy's address; the startup pass; the harness and its CI service; the live proof's window under the operating rule — and take it through `zcrypto-plan-review` before Task 1.
