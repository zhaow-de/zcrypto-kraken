...
- **(design, with the spec)** Re-gate the engine host's cache-link converge behind the engine window once the engine routes the cache through zcache0 — the role's handler restarts wg-quick@zcache0 on any conf change — and amend the infrastructure spec's D14 sentence "restarts nothing" in the second spec, since the first is pinned.
