...
- The message has to name the canonical path and a recovery that works: a rebuilt set, never a recast in place,
  which invalidates the canonical's `dataset_hash` — `data rebuild ohlc-full --no-push`, verified, then promoted.
