---
status: partial
ripe_when: **2026-07-29 03:17 UTC (secondary)** — verify the secondary's FIRST real prune deletion pass (its oldest final is 2026-07-14 19:16, so its cutoff first crosses on 07-29; Persistent=true makes this a floor). The PRIMARY's pass was verified 2026-07-23 (deleted=560, sandboxed -delete proven, ring buffer confirmed — see Done so far), and the measurable-probe fix 3e03aac is now deployed fleet-wide by spec 00069's rollout, so both earlier clauses are discharged
---
...
- **Retention: designed, config-implemented, deployed AND proven on the primary (2026-07-23).** Spec `00050` D8's `zcrypto-capture-prune` timer is **live on both hosts** (14-day retention, finals + `.sha256` only, never `.part`/`.held`/`.corrupt`). The primary's first real deletion pass is **verified** (deleted=560, sandboxed `-delete` proven, ring buffer confirmed — see Done so far), so the `≈2026-11-23` disk-fill deadline is dissolved there. **The one remaining sub-item: verify the SECONDARY's first real deletion pass, ~2026-07-29** (its oldest final is later) — same read-only check: the expected oldest day emptied, `.part` survives, `-delete` succeeds. **Recorded stance (explicit, 2026-07-19): the NAS custody mirror is deliberately never pruned** — it is the keep-forever archive (spec `00048` Role A); only the capture-host working copies ring-buffer.
