---
status: partial
ripe_when: "spec 00101 is implemented and converged — the next attended, canary-gated engine converge, before the first armed probe pass"
---
...
**Remainder — implement spec `00101` (A, hardened).** `ws_idle_timeout_ms=0` as a literal in `cli/engine/node.py::_data_client_config`, pinned by tests proven to bite against `None`, with the runbook paragraph landing in the same change (spec D6). It reaches the fleet on the next attended, canary-gated engine converge, before the first armed probe pass. Both options stay written up below so B remains available on its own evidence.
