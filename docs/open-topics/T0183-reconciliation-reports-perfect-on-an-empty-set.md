...
Three members are held, each for a reason of its own, and none for want of a repo state to wait on — which is why no `ripe_when:` is declared.

- **`cli/capture/gap_monitor.py`'s `is_healthy`** returns `not any(...)` over `pairs`, which is `True` over an empty list. It gates the healthchecks.io ping at `cli/capture/command.py:301`, where `client.connected` is independent of `pairs` and `_default_pairs` refuses a missing or unparseable universe file but returns `[]` when the file names no `/EUR` symbol. With no pairs the daemon captures nothing and keeps pinging the dead-man switch green — a false all-clear on the unbackfillable capture path, which is the failure the switch exists to catch. Held because the capture path carries a different review floor from the rest of this topic, not because it is smaller.
- **`analyze_soak`'s `hhi` aggregate** should arguably exclude a flat bar's out-of-range sentinel rather than averaging it (above). It moves that metric's window and `effective_n`, and `hhi` is a gating verdict, so it is the owner's call rather than a defect to fix quietly.
- **`_net_live_from_result`'s `reconcile_ok`** at `cli/engine/soak.py:299` is `all(...)` over `assets`, which is `True` with no assets — the same unguarded-vacuous shape as `all_match`, in the file the HHI decision already covers, so it rides that ruling rather than surfacing after it.
