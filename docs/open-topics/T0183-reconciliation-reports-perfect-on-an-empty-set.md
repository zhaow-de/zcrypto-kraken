...
The members do not share one blast radius, and the sweep found that out in stages. Most sit on data QA, which is why the topic was registered rather than escalated. `cli/engine/soak.py` is a tier up: `soak_report` is reached only by the offline `soak-check` command, the shadow node's `run()` never references it and the file places no order, so its floor is ordinary — but it is the instrument the go-live decision reads, and a fabricated number there cannot place a trade while it can still steer whether trades are placed at all. And `cli/capture/gap_monitor.py` is on the unbackfillable capture path, which is a different floor again and is why that member is held rather than fixed here.
...
Three items are held, awaiting the owner's decision rather than any repo state — which is why no `ripe_when:` is declared. Two are members; the third is not, and the difference is the point of the entry.

- **`cli/capture/gap_monitor.py`'s `is_healthy`** is a member: it returns `not any(...)` over `pairs`, `True` over an empty list, and it gates the healthchecks.io ping at `cli/capture/command.py:301`, where `client.connected` is independent of `pairs`. With no pairs the daemon captures nothing and keeps pinging the dead-man switch green — a false all-clear on the unbackfillable capture path, which is the failure the switch exists to catch. **The reachable blast radius is narrower than that, and a floor decision needs the real one**: `_default_pairs` refuses a missing or unparseable universe file, and a `selected` list that is non-empty but names no `/EUR` symbol logs an ERROR that `alerts.yaml`'s `Capture · daemon ERROR logs` rule selects (`cli/capture/command.py:113-120`). Only a fully empty `selected` reaches an empty `pairs` silently. Held because the capture path carries its own review floor, not because it is smaller.
- **`_net_live_from_result`'s `reconcile_ok`** at `cli/engine/soak.py:299` is a member too: `all(...)` over `assets` is `True` with no assets, the same unguarded-vacuous shape as `all_match`. It rides the HHI ruling because it is in the same parked file.
- **`analyze_soak`'s `hhi` aggregate is NOT a member, and the row would be wrong if it read as one.** `bar_hhi`'s `0.0` is specified at `docs/plans/00058-soak-check-oos-report.md:52` and lies outside the metric's range, whose lower bound `docs/specs/00059-soak-check-realized-governor-cap-design.md:49` gives as `1/n > 0`. The defect is that `analyze_soak` averages that sentinel as though it were data. Excluding the flat bar moves that metric's window and `effective_n`, and `hhi` is a gating verdict, so it is the owner's call rather than a defect to fix quietly.
