...
- **`infra/scripts/continuity.py:250` is UNREACHABLE with `covered == 0`, and `:301` cannot raise** — one invariant forecloses both, `span_hours >= 1`, held by `report()`'s own `if not segs: continue` and its sort rather than by the arithmetic. Two independent readers established it by construction. The site moves to *Excluded* with that proof, as `_match_stats`'s arm did, and the sentence claiming `:301` would raise is rewritten rather than annotated. Superseded, originally: The line — `pct = 100.0 * gap / covered if covered else 0.0` — has the family's shape, but the file already refuses at the gate: `unmeasured` is a separate list (`:205`, `:241`) that forces `*** FAIL ***` at `:283-285` with no totals and at `:307-308` with them, both before `worst` decides anything, and the D6 comment at `:305-306` states this topic's principle outright. So the aggregate cannot lie, and the item is pure reachability: a pair reaching `:250` has passed the `measured` guard (`n >= MIN_POOL` samples), so the question is whether such a pair can have `covered == 0`. Establish it by construction. Reachable → the per-row `0.0` is a member, and `:301`'s `100.0 * tg / tc` raises `ZeroDivisionError` in the all-uncovered case, a crash rather than a false green — fix with `None` and a test; unreachable → *Excluded* with the proof, as `_match_stats`'s arm was.
- **Three census members in `cli/engine/`, found by an independent sweep and confirmed by a second.** None is named anywhere in this file. Two escaped the earlier sweeps for a structural reason worth recording, because it is what the instrument must cover next time: the value is a STRING verdict rather than a number, or the vacuous branch is a loop that falls through with no emptiness test to grep for at all.
  - `realized_internals`' `identity_ok` (`cli/engine/soak.py`), which was `True` on an unmade comparison.
  - `reconcile_ledger`'s `status: "ok"`, whose success value is a string, so no numeric sweep reaches it. Its fix is a third status value, which a string verdict can carry where the boolean `all_match` could not.
  - `_chain_consistent`'s `chain_ok`.

  Each takes this topic's own convention — `bool | None`, `None` on an unmade comparison — and each needs arm (b)'s pair. One carries a test that asserts the defect (`tests/test_engine_tracking.py:703`), so that test must FLIP rather than gain a sibling; this file's own Findings record that trap.
- **`reconcile_series`'s `volume_rel_diff_max` has no populated control**, the one arm-(b) breach among the thirteen rows already done. Replacing its measured value with `None` unconditionally leaves the whole of `tests/test_backfill_reconcile.py` green. One assertion on a populated call closes it, and the proof it is needed and the proof it works are the same mutation.
- **Re-check the `cli/engine/tracking.py` exclusion.** It is granted on the reason "it contains no `nan`", which the module's own comment at `:206` contradicts. Either the reason is wrong and the site is a member, or the reason needs restating from what the code actually does.
- **`infra/grafana/alerts.yaml`'s `zcrypto-engine-dark-with-exposure` needs a ruling rather than a silent exclusion** — it carries the family's shape at the site and sits on the live trade path.
