...
- While there: `tests/test_engine_venuestate.py`'s `_XBT_LEG_ATTRS` gives the two `/BTC` legs distinct `ordermin`/`lot_step`/`tick_size` against a stated intent ("a bug that reused the EUR fixture values for these two symbols would go undetected otherwise"), but no test reads those values back — one assertion makes the comment true.
