...
- The engine host's store repair is a carrier of the same delete, and the recovery text covers it or says why not: the comment above the store delivery in `infra/ansible/roles/engine/tasks/main.yml` removes the store dir and lets the converge re-deliver the workstation's copy, naming neither a copy aside nor a loss.
