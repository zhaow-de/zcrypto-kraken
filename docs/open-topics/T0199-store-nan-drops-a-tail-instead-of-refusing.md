...
- The interior off-grid stamp on the LIVE path is a case the door's spec holds: `select_model_inputs` in `cli/engine/cycle.py` admits it into the model calendar from any EUR leg, and only the /BTC case is pinned (`tests/test_engine_cycle.py::test_a_btc_only_stamp_never_enters_the_model_calendar`); the soak's realized leg refuses it since [[T0201]], the cycle does not.
