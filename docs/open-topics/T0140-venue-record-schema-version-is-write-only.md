...
The gap is not hypothetical. A fixture in `tests/test_engine_metrics.py` describes itself as matching `write_venue_record`'s schema while writing `schema_version: 1`; that drifted the moment the constant moved to 2, and **no test objected**, because there is nothing to object with. That is the same failure shape the cycle journal's validator exists to prevent.
...
- Either way, fix `tests/test_engine_metrics.py`'s venue fixture docstring so it describes what the fixture actually is (deliberate v1-reader coverage) rather than claiming a schema match it no longer has.
