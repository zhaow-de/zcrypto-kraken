---
status: partial
ripe_when: an attended ops-host maintenance window for the zcrypto-panel-regenerate drill (converge.sh drilled 2026-08-04)
---
...
- **`zcrypto-panel-regenerate` is the last open sub-item** — drill it against the real tree at the next `SCHEMA_VERSION` bump or as a deliberate exercise on the ops host, in a window sized per the script's own ETA guard.
