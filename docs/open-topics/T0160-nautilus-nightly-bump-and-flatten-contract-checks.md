---
status: open
ripe_when: 'per sub-item. Bump: engine converged onto 00106 + rest-hold, AND `zcrypto engine flatten` observed to succeed once on the pin in `pyproject.toml`. Cancel sweep: the `nautilus_trader` pinned in `pyproject.toml` answers an order-report instrument-cache miss with a diagnostic rather than a dropped row, OR an attended engine start on the live account is planned with an order resting at the venue that the starting process did not place, on one of BTC/EUR, ETH/EUR, XRP/EUR, LTC/EUR, ETH/BTC'
---
...
**It now holds a second subject, which the bump does not own.** The same adapter's order-report read drops an order whose pair its instrument cache cannot resolve — silently, with a successful empty return — and that blinds the engine's kill-switch cancel sweep on five basket legs. The repair is parked pending the v2 release rather than absent, and it is registered as a sub-item below because a deferral whose only home is a PR body or an unmerged branch's prose is untracked. What the defect makes *false* is already corrected on every surface that claimed otherwise — the commit subject is `fix(engine): say where the flat verdict and the trip's cancel sweep are blind` — and what remains here is the repair itself.
