...
So the branch went red at `6321d192`, cleared it at `6e86d5b6` by re-recording the baseline, outgrew the recorded keep again at `8b6d698e`, and was never re-recorded. Post-#450 `develop` at `7e5bbe17` is clean, exit 0.
...
- **A CI job running the tripwire on `pull_request` into `develop`.** Closes it structurally, needs no per-clone state, and is visible in the merge gate. Cost: another check that `merge-pr`'s evaluator will block on when it fails.
- **A `pre-push` hook stage.** Cheaper to run, but it is per-clone state: a fresh worktree or a new session's clone silently has no such hook, so the guard is absent exactly where it is most needed.
