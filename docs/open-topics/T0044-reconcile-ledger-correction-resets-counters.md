...
- **The runbook's backup is now its own topic — see [[T0057]].** `reconcile-ledger.jsonl.bak-20260714-220209`, which this topic's runbook (`b22d3e2`) writes *in place*, lives inside the replicated overlay: it broke the new ops→NAS channel on a permission mismatch, and it is the only surviving record of what the LINK/EUR `total_loss` correction removed. The runbook should stop writing backups into the canonical tree.
