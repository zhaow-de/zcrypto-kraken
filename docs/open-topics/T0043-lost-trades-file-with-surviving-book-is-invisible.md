...
The landed half sits on the pushed branch **`feat/t0101-remediation`** (no PR). The export item below is NOT on it — no `infra/` file is touched by that branch — so its autonomous half is still unwritten.
