...
- **The `infra/` docs and runbook pages are a second assignment on a second branch**, cut after `cleanup/prose-infra` merges: fifteen pages, worklist at `.local/dispatch/2026-09-05-t0164-infra-docs-worklist.txt`, 163 offenders measured at that branch's tip and unchanged from its merge-base — all table-row and section, none comment-block, so it is a docs pass and not a code one. One of the fifteen, `infra/runbooks/order-semantics-verification.md`, was already edited on `cleanup/prose-infra`, so that assignment reads it against the tip rather than against the worklist's own base.
