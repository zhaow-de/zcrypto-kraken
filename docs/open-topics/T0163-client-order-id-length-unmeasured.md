---
status: open
ripe_when: 'docs/reference/adapter-verification/ carries a row quoting a client order id as echoed by the venue: grep -l "echoed" docs/reference/adapter-verification/*.md is non-empty'
---
...
`infra/scripts/kraken-order-semantics-probe.py` mints client order ids of the shape `O-<YYYYMMDD>-<HHMMSS>-901-P6V-<seq>` — 27 characters at a one-digit sequence — and the `1.230.0`, `1.231.0` and `2.0.0rc4.dev20260825` rows in `docs/reference/adapter-verification/` record those runs passing. A comment in the same file says the probe's recovery read matches on the `-901-P6V-` infix because that infix "survives Kraken's 18-char client-order-id truncation". The infix sits at characters 23–25, so an 18-character cut ends before it begins: the comment's two halves cannot both be true. The wider question is unmeasured rather than merely inconsistent — acceptance at submit does not establish what the venue STORES, and no run in this repository has ever read a client order id back from the venue.
...
- `docs/reference/adapter-verification/2.0.0rc4.dev20260825.md` and the two `1.23x` rows: probe runs passing with 27-character ids. Both `--probes 6` rows found zero open orders, so neither exercised the recovery match against a real leftover.
...
- At the fixture-minter's first attended `--execute`, record in that version's `docs/reference/adapter-verification/<version>.md` row what the venue did with each 32-, 31- and 29-character id: accepted unchanged, refused, or echoed back shortened. Read the ids from Kraken's own open-orders page as well as from the adapter, and quote the echoed string verbatim — the point is the venue's spelling, not ours.
