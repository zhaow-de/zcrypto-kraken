...
- **Runbook fragments scattered:** ledger correction (`infra/nas/README.md`, [[T0044]]), capture-deploy canary + maintenance windows (`.claude/rules/fleet-deploys.md`), Alloy tailer restart ([[T0048]]), reboot slots (spec `00050`).
...
The day-2 runbook this topic plans **now exists** — but under `infra/runbooks/`, **not** the `docs/reference/day2-operations-runbook.md` path proposed below. It sits beside `infra/grafana/alerts.yaml`, which is what points at it, and `infra/` already carries operational docs. Extend that tree; do not create a second one. Since the 2026-08-18 split ([[T0141]]) the procedures live in per-subsystem files (`capture.md`, `zaccess.md`, `ops.md`, `engine.md`, `reference-data.md`) and `README.md` is a **pure index** — a new section joins the subsystem file that already covers its signal, or mints a new `infra/runbooks/<subsystem>.md` plus an index row; nothing goes in the README.

The pattern it established, arrived at by asking what an operator (human or model) actually does:

- **Two entry points, one destination.** An alert lands in Slack, or a guard in the code says "read this" — either way the operator arrives already holding the anchor. Alert summaries carry `infra/runbooks/<subsystem>.md#<uid>`; code comments carry `#<slug>`. The citation names the file as well as the anchor, so a section that moves takes every citation with it in the same change. Nothing has to be searched for.
...
- **Split when** a subsystem file exceeds ~12 sections or gains a second subsystem's worth: move to a new `infra/runbooks/<subsystem>.md`, anchors byte-identical. Done once already, 2026-08-18.
...
- Run each scenario in an attended maintenance window, per `fleet-deploys.md` discipline (never induce a failure on live capture outside a window); record per scenario: time-to-alert, channel(s) that fired, and the operator action taken.
- Extend `infra/runbooks/` (created 2026-07-29; split by subsystem 2026-08-18) with one section per alert/dead-man in the established four-part shape, folding in the scattered fragments — ledger correction, Alloy restart, canary rule — so it becomes the single entry point. **Do not create `docs/reference/day2-operations-runbook.md`**: that path was this topic's original proposal and would now be a second, competing home.
