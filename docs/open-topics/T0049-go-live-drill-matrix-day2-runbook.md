...
- **The ⏱ snapshot-register re-confirmation is a MONTHLY routine of this runbook, not a one-off go-live step** *(added 2026-07-30; the topic is [[T0113]])*. The master plan marks fees/fee tiers, borrow-rollover rates, tradeable pair lists, MiCA status, tax rules and market-data pricing as externally owned and requires re-confirmation "at Phase 0 and again at go-live" — but each is a third-party fact that can move any month, and a stale one is silent. So the runbook carries it as a recurring section, not a checklist line: re-fetch, update `docs/reference/kraken-snapshot-register.md`, and **bump its `Fetched at:` stamp even when nothing changed** — otherwise the next reader cannot distinguish "re-confirmed, identical" from "never re-run". Where a delta touches fees or borrow rates, name the downstream figures it invalidates ([[T0090]]'s cost basis, the deployable's quoted Sharpe) rather than leaving that inference to the reader. *Retire when:* the sweep is automated and its staleness is alertable from the register's own stamp.
