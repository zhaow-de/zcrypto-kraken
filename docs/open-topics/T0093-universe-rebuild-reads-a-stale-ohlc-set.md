...
- **The wiring decision was made and landed 2026-08-11** (branch `feat/t0093-universe-volume-source`, spec `00093`, six commits, all Fable-reviewed; the owner's ruling: extend reach to the BTC-quoted legs rather than dropping them or waiting on tape-derived bars). `_refresh_universe` now reads through `resolve_ohlc_source`, which resolves the newest `ohlc-reach-<stamp>` sibling over the fixed `ohlc-full` anchor, and refuses when the resolved source lacks any `CANDIDATE_SYMBOLS` member — naming the missing legs in a `DataSyncError` instead of an untyped `FileNotFoundError` from polars. The reach round (`cli/ohlc/reach.py`) is now quote-aware — `_canonical_symbols` discovers full `BASE/QUOTE` symbols from the tree instead of globbing `*/EUR/` — so a fresh round can mint all twelve legs, including `ETH/BTC` and `SOL/BTC`. `PAIR_KEYS` (`cli/ohlc/fetch.py`) is re-keyed by full symbol to express two quotes per base; the engine (`cli/engine/store.py`) keeps a **derived** EUR-only view so its live ten-leg basket is provably unchanged, pinned by test. The canonical universe artifact now publishes as immutable `universe-<stamp>/` sets resolved newest-wins by `resolve_universe_path`, which loudly ERROR-logs a newer stamped directory missing its artifact rather than silently falling back to an older set; the legacy `universe/` set stays as the fallback, unretired. **The DATA has not moved yet** — no fresh reach round has been minted with the BTC legs, and no `universe-<stamp>/` set has been published; both need the attended sitting below, sequenced after merge as one sitting so the canonical universe is rebuilt once, not twice. Until that sitting runs, a rebuild is now *capable* of succeeding but nothing has yet asked it to.
- **The "real options" bullet this topic used to carry is now decided (D1, spec `00093`).** Of the three options listed — (a) a REST volume pull in the rebuild path, (b) [[T0065]]'s live-trades→bars materializer, (c) widen the staleness budget — the owner took a variant of (a): extend the REST reach round itself to be quote-aware, rather than a separate top-level pull. (b) was rejected as too immature (the `/BTC` tape was 20 days deep against a 30-row median, and `derive_bars` has no production caller); (c) was rejected as previously recorded, unchanged. The quarterly-dump finding below stands as the reason (a)/(b) were the only live options.
- **(Dump ingestion does NOT discharge the data gap — background, not an action)** The OHLCVT dumps are **quarterly** (`Kraken_OHLCVT_Q<N>_<YYYY>.zip`; newest on the NAS is Q1 2026, with Q2 still absent 22 days after the quarter closed). A dump-derived frontier is therefore always a quarter boundary: ingesting Q2 today gives 2026-06-30, still 22 days stale against a 7-day budget, so the guard fires anyway. **A universe rebuild needs a live-tailed volume source, not a fresher dump** — which is what the wiring above now supplies, once the attended sitting runs it.

## Suggested next steps

- **(The attended sitting — discharges the DATA gap)** Run, post-merge, in order, one sitting (splitting it rebuilds the canonical universe more than once): `zcrypto data rebuild ohlc-reach` to mint all twelve legs into a local `ohlc-reach-<stamp>` sibling — verify `ETH/BTC` and `SOL/BTC` come back `status: continuous` with non-zero `overlap_bars`, a `detached` status on either being a failure, not a caveat; `zcrypto data rebuild universe` to re-measure selection on the fresh window and state every median; confirm the hub carries the published `universe-<stamp>/` set and the legacy `universe/` is untouched; regenerate `docs/universe/point-in-time-universe.md` by hand with `as_of` set to the published stamp, and run `uv run pytest tests/test_universe_provenance.py`. The wiring landing above only makes this possible — it does not run it.
