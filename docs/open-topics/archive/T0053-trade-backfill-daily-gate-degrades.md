...
**All three steps this topic listed are discharged, and the split one with them.** The **retry policy** was decided as option (a) above and is what the deployed script runs; the step has since moved off the NAS to the ops node (spec `00054` D2/OPS-5, recorded at `infra/nas/pull-entrypoint.sh:204`), where `infra/ansible/roles/ops/templates/archive-pull.sh.j2:147-151` writes the day stamp *before* the `docker run` and states the reason in its own comment — the stamp is written unconditionally, success or failure, so a permanent error costs one attempt per day, not one per cycle. The **`KRAKEN_ALTNAME` tie** was answered by [[T0055]], resolved 2026-07-16 by *deleting* the map instead of guarding it: `cli/trades/rest.py:75` derives the altname through `cli/backfill/read.py::dump_pair_name`, and the name `KRAKEN_ALTNAME` no longer occurs anywhere under `cli/`, `tests/` or `infra/`, so an 11th capture pair derives with no code change. The **visibility this topic declared itself dependent on** arrived in the same iteration: [[T0052]] is resolved and `infra/grafana/alerts.yaml` carries both `zcrypto-trade-backfill-stale` (uid at line 1583, `noDataState: Alerting`) and `zcrypto-trade-backfill-exit-nonzero` (line 1629), so the failure this fix routes to the metric is the one an operator now sees.

**The steps this topic carried at its close, kept verbatim with what answered each:**

- **(autonomous, design)** Decide the retry policy. Options: (a) stamp the day regardless of outcome and let [[T0052]]'s alert carry the failure — simplest, restores the daily cost bound, but a transient then waits 24h; (b) bounded retries per day (e.g. stamp after N consecutive failures); (c) distinguish transient (transport/5xx) from permanent (unknown pair, structural residual) and only retry the former. (c) is the most correct and the most code; (a) is one line and defensible given the metric is the real signal. — **ANSWERED:** option (a), the owner's decision, is what the deployed script runs. `infra/ansible/roles/ops/templates/archive-pull.sh.j2:150` writes `echo "$backfill_today" > "$backfill_stamp"` *before* the `docker run … archive backfill-trades` at `:152`, and the comment at `:143-146` states the rule in its own words — the stamp is written unconditionally, success or failure, so a permanent error costs one attempt per day, not one per cycle.
- **(autonomous, cheap, independent of the above)** Tie `KRAKEN_ALTNAME` to the capture universe so an 11th pair cannot silently trigger this — either derive it, or add an assert/test that the map covers every pair in `capture_pairs` and fails loudly at build time rather than nightly at runtime. — **ANSWERED:** split to [[T0055]] at this topic's close and answered there by *deleting* the map rather than guarding it. [[T0055]] is `status: resolved` (2026-07-16, iter-100); `KRAKEN_ALTNAME` returns no hits at all under `cli/`, `tests/` or `infra/`, and `cli/trades/rest.py:75` derives the altname through `dump_pair_name` from `cli/backfill/read.py`, so an 11th capture pair needs no code change.
- Depends on [[T0052]] for visibility: until the metrics are alerted, this failure mode is invisible by construction. — **ANSWERED:** [[T0052]] is `status: resolved` in the archive, closed 2026-07-16 in the same iteration and deliberately first, because stamping the day unconditionally removes the retry and leaves the metric as the only signal. Both rules are in `infra/grafana/alerts.yaml`: `zcrypto-trade-backfill-stale` at `:1583` (the dead-man, `noDataState: Alerting`) and `zcrypto-trade-backfill-exit-nonzero` at `:1629`.
