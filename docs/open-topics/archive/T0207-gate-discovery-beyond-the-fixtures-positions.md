...
Both halves hold as equality, the mark half included, which this topic expected to fail. Over the guard's own fixture the walks differ in the cases `_FLAT_WALK_UNSEEN` lists, each a skip not written under its own name. So what remains unheld is narrower than it was and is written in the module docstring: a skip written any other way AND in a position `_guards_of` does not walk.
