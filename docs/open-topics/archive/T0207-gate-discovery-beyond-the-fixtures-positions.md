...
The assertion runs one way on purpose. A skip nothing decides — a helper's own body, a `mark.skip` declared on a test or on one `pytest.param`, a bare `pytest.skip(...)` — is no gate for either walk, and a name is read as pytest's only through the module's `import` lines, so another object's `skip` method is no skip. The other direction, a gate the second walk does not see, is a binding it does not read, and `_FLAT_WALK_UNSEEN` holds those fixture cases as a list. So what remains unheld is narrower than it was and is written in the module docstring: a skip bound any other way, where it sits in a position `_guards_of` does not walk or is reached by a binding `_pytest_bindings` does not follow.
