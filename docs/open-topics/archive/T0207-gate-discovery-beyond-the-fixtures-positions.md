...
Delivered by PR #569, in `tests/test_live_venue_opt_in.py`. `_flat_sites` is the second implementation: it reads each node alone — a call or a raise that is a skip under its own name, a `skipif` / `skipIf` / `skipUnless` call — resolves no import and knows nothing of what encloses a node, so it has no position to miss. `test_a_second_walk_of_the_tree_finds_exactly_the_gates_the_walker_finds` holds the two walks equal over `tests/` by file and line, and a third test holds that `_flat_sites` names no function `_gates` reaches or is reached from, a set read off the module's own call graph.

Both halves hold as equality, the mark half included, which this topic expected to fail. Over the guard's own fixture the walks differ in the cases `_FLAT_WALK_UNSEEN` lists, each a skip not written under its own name. So what remains unheld is narrower than it was and is written in the module docstring: a skip written any other way, where it sits in a position `_guards_of` does not walk or is reached by a binding `_pytest_bindings` does not follow.

Five positions remain unwalked — a boolean short-circuit, a conditional expression, a `for`'s `else`, a comprehension, a nested function. The walker gains no arm for them: none is in the tree, and one that arrives fails the suite with its file and line and the instruction to add the arm and a fixture case, which is the reader this topic said did not exist.
