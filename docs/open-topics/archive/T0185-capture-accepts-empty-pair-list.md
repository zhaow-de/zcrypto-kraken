...
- A `CaptureError` raised at startup is logged with a level before the process dies: `cli/__main__.py`'s `run()`, the console-script entry point, wraps `app()` and logs an unhandled fault, so a refusal pages through the ERROR rule and, since nothing pings, through the dead-man — two channels, both truthful, immediately.
