...
- **(Decide first — this is the whole topic)** Choose between two shapes, then implement: — **ANSWERED:** the choice below went to **option 2**, ruled and shipped — `fe1632728` *feat(cli): derive replay-fingerprint coverage from the import closure*, `bb7290138` *fix(cli): hash ancestor packages — the closure shipped the same hole one level up*, and `8a77014bc`. The closure is live as `gate_cache._replay_code_paths()` (`cli/engine/gate_cache.py:119`) and pinned by `tests/test_engine_gate_cache.py:986`, the superset test named above; option 1 was never needed as a stopgap.
...
- **(Either way)** Pin whichever is chosen against the exploit above: rebinding the fast builder in `cli/portfolio/__init__.py` must change `replay_fingerprint`. That test is the guarantee; the module list is just today's implementation of it. — **ANSWERED:** `tests/test_engine_gate_cache.py:500` `test_rebinding_the_fast_builder_in_the_portfolio_init_moves_the_fingerprint` clones the tree, appends `build_crossfreq_system_fast = build_crossfreq_system` to `cli/portfolio/__init__.py` and asserts `replay_fingerprint() != fp_before`; its preconditions force a re-aim rather than a quiet relaxation if `concordance.py` or the package stops binding through it.
- **(Either way)** Correct spec `00060` D3's wording, or the next reader inherits the same false invariant that produced all three rounds of this. Say what the list actually covers, or make it cover what it says. — **ANSWERED:** amended at its root by `8a77014bc`. `docs/specs/00060-*.md` line 23 now reads "**The wording was the defect, not just the list.** … Coverage is now *derived* … and, more importantly, *measured*: `test_closure_covers_every_module_the_replay_roots_actually_execute` … **That test is the invariant; this paragraph only describes it.** If the two ever disagree, believe the test."
- **(Cheap, do it with the change)** If option 1: the `_REPLAY_CODE_PATHS` comment block already states the invariant does not hold and names this topic — update it in the same commit so the code and the topic never disagree. — **OVERTAKEN:** option 2 removed the thing to be commented. `grep -rn '_REPLAY_CODE_PATHS' cli/ tests/` finds no hit, the coverage being computed by `gate_cache._replay_code_paths()` instead; the name survives as history in the specs and plans that record the change (`00064`/`00065`), in `docs/research/14.phase6-gate-guarantee-mutation-audits.md`, and in [[T0075]].
