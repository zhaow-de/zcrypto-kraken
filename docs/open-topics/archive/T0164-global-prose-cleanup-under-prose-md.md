...
### The docs remainder — `README.md` and `docs/reference/`

Branch `cleanup/prose-docs-remainder`, 2026-09-06, `zcrypto-bravo`: `README.md` and the `docs/reference` pages a coverage audit found carrying offenders. Read-only drafters in disjoint groups, one writer, two disjoint read-only reviewers. A different shape of work from every pass before it: **every offender was a table row or a section — no comment blocks, no file-prose** — so the proof is every heading and `<a name>` anchor byte-identical, every fenced block byte-identical, every table row's rendered cell count unchanged, and every link target still resolving, not an AST. `docs/reference/` is outside mdformat's reach, so nothing normalises those pages and a split row would ship in silence.

Two findings outrank the byte count. **Two options the README never documented**, which `readme-usage.md` requires it to: `zcrypto archive pull`'s `--verify/--no-verify`, which the sidecar-less engine-journal pull uses in production, and `[zcrypto.engine].exec_armed` — absent while `exec_enabled` sat in the same table, so the table read complete while omitting the config half of the two arming keys. And **a document telling readers a guard does not exist**: `captured-spread-calibration.md` said "nothing in the test suite checks span, so that regression would have shipped green", where `tests/test_costs_spread.py` asserts `span_days >= 14.0` and its docstring says a shortened restamp fails there rather than un-discharging the gate silently.

The correction that took two passes is worth recording as one fault. `docs/reference/sleeve-composition-ledger.md` and `infra/runbooks/engine.md` — the runbook in scope by the coordinator's extension, so the two surfaces cannot disagree — both said the exposure governor "halves the result". `cli/risk/governor.py` computes `mult = min(ladder_mult, daily_mult)`: `ladder_mult` starts each bar at 1.0 and falls only as drawdown crosses its rungs, while any bar losing more than the daily limit halves the book for the cooldown with `ladder_mult` untouched. The first pass fixed "a sixth" to "a third"; the review found the second path still unnamed. Both errors were the same one — reading a single arm of a `min()` as the whole mechanism — and an operator seeing a halved book after a 3 % day is who pays for it.

Point-in-time records took no length fixes: the drill log two re-tensings, the adapter-verification records none, and two of their sections GREW because the correct runbook name is longer. `kraken-snapshot-register.md`'s re-confirmation log was checked with the parser that reads it rather than by eye — `ops_daily.py`'s `last_sweep_date` returns the same date on both revisions, and a reviewer showed the gate is real by making one extra space move it back a month.

### The `infra/` docs and runbook pages

Branch `cleanup/prose-infra-docs` (PR #411, 2026-09-06): the fifteen pages, 23 commits, five per-page Opus rounds and one whole-branch Opus read. Measured over the fifteen at the merge-base and the tip (`uv run python infra/scripts/prose-tripwire.py <the fifteen>`): 163 → 101 offenders, and 559,268 → 515,023 bytes (−7.9 %). Table rows over the 200-char bar 72 → 21 and sections over the 2,048-byte bar 91 → 80, with no file regressed; the 101 offenders at the tip are the per-page REGISTERED RESIDUALS blocks in the branch's commit messages. What remains at the tip is registered, not missed: every one of the 101 is named in a page's REGISTERED RESIDUALS block with its reason and, where a row cannot reach 200 characters, a computed floor — the whole-branch read spot-checked twelve of those floors arithmetically and found all twelve arithmetic.
...
### The status-annotation sweep and the ratchet

Branch `cleanup/status-annotation-sweep` (PR #419, 2026-09-06, `zcrypto-bravo`): every status annotation on a topic id in `cli/`, `infra/`, `docs/reference` and the README under the owner's closed-citation rule — the sentence re-tensed to its outcome or the bare id kept where a reader looks the history up, the two vault files untouched, one verb citation left; prose-only by the same proofs as the batches before it.

Branch `feat/t0164-prose-tripwire-hook` (the closing PR, `zcrypto-main`): `infra/scripts/prose-tripwire.py` runs at commit as a ratchet over `infra/scripts/prose-tripwire-baseline.txt` — a new offender, or a recorded one that grew, fails the commit; the tool skips vault content by content, applies the file-prose bar only above a code-line floor, counts a string literal as prose only in docstring position, exempts `docs/open-topics/` from the section bar, and reads every README under `infra/`; `prose.md` names the ratchet and its remedy and states the closed-citation rule.

### The batches' registrations, where each landed

- **The owner's closeout decisions the batches collected, one clause each, landed as follows** (the owner's "agreed" of 2026-09-06): `fleet.md`'s ten operator procedures moved to the runbook pages owning their subsystems and phase 0's two minors — the registry append after a hand-edited unterminated last line, `fetch_public`'s result-less body — are T0173's; phase 2 iter-013's CPCV purge/embargo windows are recorded in `docs/research/03.phase2-decisions.md` as proven sufficient and consciously not tightened; iter-168's unmeasured in-flight REST query is a T0160 next step; `notification_settings.repeat_interval` surviving a `grafana-push.sh` upsert is asserted through the push path (T0168 wave 2); phase 3's regime-gate EMA variant and the §12 hand-back confirming B4 as the fallback deployable are recorded in `docs/research/04.phase3-decisions.md`. Batch 2's check-mode coarseness and converge-guard claims were T0168's ops-role bullet, asserted there.
- **Owed by `zcrypto-main`, landed with the closing PR**: the rollout skill's NAS section carries the two clauses cut from the pins row (every rollout pins a `develop` build; the digest extra var is silently unused); archived T0162 is re-tensed now that the `alerts.yaml` comment is a pointer; the one closeout entry is in `docs/iterations-history-phase6.md`; the tripwire is a pre-commit ratchet over a committed baseline — its residuals (the 2026-09-05 T0170 entry, `fleet.md`'s rows and sections, the agentboard pins row and Standing constraints) are recorded keeps the baseline holds from growing, and `docs/open-topics/` is exempt from the section bar because a topic's registry sections grow with every registration.

### From the `cli/` batch — the 407 candidates, dispositioned
...
- **130 of the 407 needed no main-tree action** — the `draft-disposition` and `home-verified` classes, a drafter's own choice and an audit record that a cut lost nothing.
- Of the 53 `tree-defect` and `registration-owed` items, the ones live in landed files are fixed in the closing PR's `docs(cli)` commit or verified true of the tree (the checkpoint docstring's refusal reason holds under polars' naive-to-UTC localisation; the capture command's restart policy had already been corrected), with two dead imports named and left, since `general.md` says to mention and not delete them and the `ruff` `select = ["F"]` proposal is the coordinator's; the ones owed by whoever finished one of the 33 REVISE files are dropped with the owner's decision that the 33 stay.
- The 22 `ruling-owed` items were ruled on 2026-09-06 and applied on the branches that carried them (two routed to T0169, one to T0168 wave 2, one restoring a comment that names an operator-surface id).
...
### From the `cli/` batch — the follow-up pass this pass's rule froze, landed

`zcrypto-main`'s ruling (2026-09-05) widened that pass's proof to AST-with-docstrings-stripped identical plus the trailing-comment inventory, since the AST proof already guarantees the code and a same-line comment is prose. Its worklist landed: `cli/derivatives/oi.py`'s status comment now names the non-5xx branch it sits on, `cli/engine/probeplan.py` cites `Master-plan §10` as its `cli/risk/` siblings do, and the three `cold-review` citations are gone (the closing PR's `docs(cli)` commit); `cli/engine/soak.py`'s bare numbers and `cli/panel/primitives.py`'s plan-local step were cited by serial on T0169's branch; `cli/capture/ws_client.py`'s `T0035` stays as a bare id, a pointer a reader looks up.

### The 33 `cli/` files left REVISE — kept at their original prose by the owner's decision (2026-09-06)

Untouched in the tree, by decision: every Critical and Important was against an uncommitted draft — a cut that lost an invariant, a rewrite that claimed more than the code — and the originals hold those invariants; their offenders are keeps in the tripwire's baseline, which the ratchet holds from growing. The roster and each draft's leading finding stay as the record of what was on the table.
