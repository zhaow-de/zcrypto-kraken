...
- **(Autonomous)** Pin the four original modules the same way the six added ones are pinned — an explicit membership assertion over the full `_REPLAY_CODE_PATHS` list, so the assertion fails if *any* entry is removed rather than enumerating the ones someone once forgot. — **ANSWERED:** `52a99bf33` *test(cli): pin the full replay-code module list* — a membership assertion over the whole `_REPLAY_CODE_PATHS` tuple, so dropping *any* entry fails.
- **(Autonomous)** Extend the evidence-key tests to cover `first_ts`, `last_ts` and `path`, using the realistic tamper the audit constructed (a stale hit against a mutated entry) rather than a field-presence check. — **ANSWERED:** `10a32832b` *test(cli): pin each evidence-key field by a stale cache hit* — `test_evidence_fingerprint_pins_first_ts_via_stale_hit` and its `last_ts` and `path` siblings.
- **(Autonomous)** Add the fail-open case that no existing test covers: a structurally-valid cache file with one malformed row inside `entries[]` must invalidate wholesale, not serve its good rows. — **ANSWERED:** `78aa58f2c` *test(cli): pin the cache's fail-open and corrupt-input guarantees*, with the corrupt-journal keystone asserted at journal level.
- ~~**(Autonomous)** Correct the LATENT/LIVE docstring labels in `_REPLAY_CODE_PATHS` to match the module-scope imports at `crossfreq_system.py:53-56`.~~ **DONE, and this bullet was WRONG as written** — following it would have restored the very label the fix removed. Corrected in PR (commit `1acb852`): only `a1.py` is live on the fast route (`build_crossfreq_system_fast` calls `_asset_returns` at `crossfreq_system.py:617`, plus `:475/:505/:537`); `a2.py`/`builder.py`/`strategies.py` are reached only from `build_crossfreq_system` at `:215-276`, the *verified* route. Module-scope import does NOT make a module verdict-determining -- it binds `def`s; the justification for covering those three is D3's over-invalidation rationale. — **ANSWERED:** `1acb8527b` *fix(cli): correct the LIVE/LATENT module labels — and the spec that got them wrong*; the correction this bullet carries is the record of what was actually right.
- **(Autonomous research, then a human ruling)** Make the case for or against hashing `cli/engine/command.py` and `cli/ohlc/dataset.py`. The research is autonomous — trace whether a change in either can alter a replay verdict without altering any currently-hashed byte — but adopting it edits ratified spec `00060` D3, so present the finding and the recommendation rather than changing the spec unilaterally. — **ANSWERED:** researched, ruled by the owner and implemented rather than presented and deferred — `4827e5be3` hashed both modules, then spec `00065` replaced enumeration with the import closure (`fe1632728`, `4e212b6f1`, `bb7290138`), which is why `test_closure_covers_every_module_the_replay_roots_actually_execute` enumerates nothing.
- ~~**(Autonomous, do this FIRST)** Re-run the audit itself with bytecode control (`PYTHONDONTWRITEBYTECODE=1`, purge `__pycache__`, assert baseline-green) in an isolated worktree, since the 44 "detected" verdicts may include false ones — see the caveat above. The 8 gaps already found stand regardless.~~ **DONE** 2026-07-20 — Audit C above. Zero false detections; the scope grew to 19 via triage error instead. — **ANSWERED:** the `## Findings so far` section above carries the result — re-audited 2026-07-20 under bytecode control, committed as Audit C in `docs/research/14.phase6-gate-guarantee-mutation-audits.md`.
- **(Method, applies beyond this topic)** When pinning any of the above, pin the *general* guarantee, not the instance that prompted it — that is the defect this whole topic describes. And bracket constants from both sides rather than sampling far from either edge; the companion audit found every mutation loosening a threshold survived while every one tightening it was caught. — **ANSWERED:** its durable home is that committed audit — `docs/research/14.phase6-gate-guarantee-mutation-audits.md:529` states the bracket-the-constant half as its own unifying prescription, `:868` names the general-guarantee half as this topic's, and `:617` demonstrates the bracketing on `_ROTATION_SLICES` (24→23, 24→12 and 24→1 all caught).
