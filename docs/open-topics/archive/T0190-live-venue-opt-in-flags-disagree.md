...
surviving occurrence is a record of what they WERE, and there are four of them, measured at this
resolution with `git grep -l`: this file; `docs/plans/00106-engine-flatten.md:4104`, a committed plan;
...
tests/ cli/ infra/ .claude/ CLAUDE.md` returns ONE hit, the guard's own docstring. `d4b34a260`'s
message states all of this as "nothing outside `tests/` referenced either old name", which is broader
than the command under it and wrong in both halves -- the sweep it quotes is not empty, and the files
outside `tests/` are three rather than none. `docs/plans/00106-engine-flatten.md:4104` is a committed plan and
...
- **The census the guard walks**, re-derived at this resolution by classifying each gate on what its guards REACH rather than on how they read: 61 skip gates in `tests/`, of which 6 read the environment and all 6 name `ZCRYPTO_LIVE_VENUE_TESTS`. The other 55 split 32 testing a path on disk, 7 a binary on `PATH`, 4 the effective uid, 3 whether a git ref resolves, and 9 something else again, 7 of those asking whether a lookup found anything. Two earlier wordings here were wrong about that tail: "an absent dataset, a uid or a missing binary" for all 55, and then a breakdown giving 1 binary and 15 others. The binaries were undercounted eightfold because six of them read as `bash is None` or `sed is None` — the RESULT of a `shutil.which` two lines up — and a classifier keyed on the guard's surface text cannot see what a name holds. Two reachability-keyed `pytest.fail` gates exist, both in `test_engine_node.py`. So the degeneracy control lives in the tree rather than only in a fixture, and a fourth test requires both counter-shapes to be present — a non-environment gate, and a reachability-keyed `pytest.fail` — so no assertion can pass vacuously on an empty set.
