...
- ~~The upstream trackers~~ — **dropped (owner ruling 2026-07-23; see the Retirement finding)**: no Alloy-bump check on grafana/alloy#6716, no `prometheus/prometheus` filing. Recorded here so the deferral exits explicitly rather than evaporating. — **DROPPED:** struck through in the bullet itself on the owner's 2026-07-23 ruling (the [[T0042]] tecnativa precedent) and recorded in the Retirement finding above; the tree carries no Alloy-bump check — `grep -rn 'prometheus_sd' infra/ tests/` finds only retirement comments in three Alloy configs plus the absence pins in `tests/test_infra_alloy_series.py` — and no `prometheus/prometheus` report was ever filed.
