...
- **(Autonomous, PARTLY DONE — and the first attempt did not work, which is worth recording)** Ship the SD counters and alert on the refresh counter going flat. Landed 2026-07-20: all three keep-lists admit `prometheus_sd_refresh_duration_seconds_count` and `..._failures_total`, `zcrypto-alloy-docker-sd-wedged` fires under one refresh in 15 min, and `tests/test_infra_alloy_series.py` pins both series on nas/ops/capture so a keep-list edit cannot silently disarm the alert.
  **The first version of this was inert and nobody would have noticed.** The keep-list was assumed to be why the counters were invisible in Grafana Cloud. It is not the only reason: **nothing scraped Alloy's own `/metrics` endpoint at all** — the single `prometheus.scrape` targeted only `prometheus.exporter.unix`, and each config says so in its own comment ("nothing scraped above is Alloy's own /metrics endpoint, so this is normally a no-op"). So the counters never entered the pipeline, the alert received no data forever, and `noDataState: OK` rendered it permanently, silently green — the exact failure shape the alert exists to catch, reproduced in the alert itself. Caught at review, not in production. Fixed by adding `prometheus.exporter.self` to all three configs and folding it into the existing scrape with `array.concat` (plain `concat` is deprecated in v1.17.1 — the config was validated against the real Alloy binary before deploy).
  Verify a claim like this end-to-end, not at its endpoints: the metric existing on `:12345` and the keep-list admitting it were both true while the path between them did not exist.
  **Proven live 2026-07-20 after the fix**, by querying Grafana rather than inferring: all four hosts report `prometheus_sd_refresh_duration_seconds_count{mechanism="docker"}` and `..._failures_total` (the latter 0 everywhere, so no erroring refreshes), and evaluating the rule's own expression returns 8.05 / 4.37 / 2.73 / 2.44 against a `< 1` threshold — four series, none firing, none in no-data.
- **(Autonomous, optional — recorded so it is not an unregistered deferral)** `prometheus_sd_refresh_failures_total` now ships but **no rule alerts on it**. It is the documented disambiguator: a climbing failures counter means a persistently erroring refresh rather than a hang — identical symptom, different cause, and the erroring case is deliberate upstream behaviour (the refresh loop withholds updates on error so a transient blip does not drop every target). A second alert may be unnecessary, because `zcrypto-alloy-docker-sd-wedged`'s annotation already directs the responder to read both counters before restarting; the case for one is that the erroring variant currently produces **no** alert at all, since a still-incrementing duration counter keeps the wedge rule green. Decide, rather than leave it implicit.
...
## Resolution (2026-07-26, spec `00069` rollout, Step 8)

**The mechanism this topic worried about no longer exists anywhere in the fleet.** Spec `00068` retired `discovery.docker` and `loki.source.docker` from every Alloy config — verified directly (2026-07-26): grepping the live `config.alloy` on all four hosts (NAS, ops, `zcrypto`, `zcrypto-red`) finds no `discovery.docker`/`loki.source.docker` component anywhere, only comments recording the retirement (`// T0048 defect 1's SD-wedge detection RETIRED with discovery.docker (00068 D6/D8)` on three hosts, `// socket mount + discovery.docker retired with the docker log path, spec 00068 D6` on ops). None of this topic's three defects (discovery-freeze, dead-tailer-retry, label-staleness-on-recreate) can recur, because the component they all lived in is gone, not merely patched — logs now reach Grafana Cloud either via each daemon's own direct-ship (`--ship-logs`) or via `loki.source.journal` keyed on stable systemd-unit names, neither of which depends on Docker's container-ID churn across a recreate.

**This topic's own archive gate fired**: the `zcrypto-alloy-docker-sd-wedged` rule — built 2026-07-20 specifically to detect defect 1 — was deleted at rollout Step 8 (`GRAFANA_PRUNE=1`, 2026-07-26 14:17 UTC: `DELETED orphaned rule zcrypto-alloy-docker-sd-wedged`; a re-run confirmed "no orphaned rules"). Verified in the repo too: `infra/grafana/alerts.yaml` carries no `zcrypto-alloy-docker-sd-wedged` or `prometheus_sd_refresh` rule reference any longer.

**Every remaining Suggested-next-step above is moot, not deferred** — each one either instruments or investigates the retired `discovery.docker` path: the SD-refresh counters/alert (built 2026-07-20, now watching a mechanism that no longer runs), the wedged-in-the-act diagnostic read, the upstream Alloy/Prometheus fix-tracking (`grafana/alloy#6716`, the Prometheus docker-client timeout), and codifying the restart-after-recreate on render-only hosts — the restart existed to make Alloy's docker-based discovery adopt a recreated container, and there is no docker-based discovery left to adopt anything into. None of these needs an owner decision or further work; they are dropped as moot with the reason recorded here.
