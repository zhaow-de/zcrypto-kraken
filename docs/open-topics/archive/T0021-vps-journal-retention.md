...
**Resolved 2026-07-26** by spec `00070` + commit `02a223f` (branch `feat/t0021-journal-retention`), deployed and verified the same evening. All three next-steps are answered:

- **The safety semantics** are spec `00070` D1–D6. "Prune only after the NAS's verified replay" resolves to **margin plus monitoring**, not a mechanical handshake: the architecture is pull-only (`rrsync -ro`, spec `00051` D10), so a NAS→VPS acknowledgement would mean opening a write-capable path *into the trade-key host* — disk hygiene is not worth that trade. A day instead survives ~336 hourly pull opportunities before becoming eligible, under a 6 h lag alert (T0069). Verified concretely at deploy rather than argued: the day due for first deletion (`2026-07-11`) was **127 files / 12,553,096 bytes on both the VPS and the NAS — byte-identical**.
...
**Deploy verified by outcome** (2026-07-26 20:39 UTC): timer armed for 01:23, service `static` (timer-only, so no prune on boot), the engine container **not restarted** (`StartedAt` identical across the converge, `RestartCount` 0), and a `--dry-run` on the host reporting exactly the predicted `deleted=1 kept=15 cutoff="2026-07-12 UTC"` — the single eligible day, with the day sitting exactly *on* the cutoff correctly spared.
