...
**Retention is 60 days, not the 14 originally ruled** — raised by the owner during implementation, before deploy. The design is value-independent (both the cutoff and the floor read the same parameter), so only numbers moved: steady state ~780 MB, ~1,440 pull opportunities of margin.
