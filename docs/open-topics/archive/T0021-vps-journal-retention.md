...
- **The implementation** is a role task, not a gate-export extension — `roles/engine/` installs `zcrypto-engine-journal-prune` + a 01:23 UTC timer, `ProtectSystem=strict` with `ReadWritePaths` limited to the journal dir and the textfile dir it publishes into.
