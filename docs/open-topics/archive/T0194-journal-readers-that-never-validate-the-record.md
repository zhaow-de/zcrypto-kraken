...
- **`journal.py`'s two comments were re-read against the enumeration** (`dd1e0b7e8`), the third next step: the plural was wrong once the set existed, so each comment keeps only the reason a reader would act on.
