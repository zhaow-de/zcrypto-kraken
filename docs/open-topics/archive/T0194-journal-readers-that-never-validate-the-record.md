...
- **One residue the enumeration does not close**, named here rather than registered (the freeze holds): `_evaluate_journal` is adjudicated `replays`, which holds on a gate-cache MISS. On a hit it scores from the stored verdict, and `evidence_fingerprint` does not cover the three fields `validate_record` alone checks, so a record whose `closes` gained a zero, a negative or a symbol key keeps its fingerprint and carries a cached PASS. Bounded by the cache's own re-evaluation, and stated in the table beside the row.
