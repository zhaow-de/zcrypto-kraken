...
**What had landed before the close:** the branch's own record of the fix, as it stood when it was handed to C7's converge.
...
So the trigger is the first natural spill after deploy. A deliberately single-stream window would widen it to 5 minutes, but that means stopping a live stream to make a metric readable, which is not worth inducing a capture fault for. — **DROPPED:** the Resolution refuses it in terms, with the measurement — `max_over_time` and `increase` over 30 days read 0 on both hosts across ~20,000 scrapes each, no `.held` file exists or has existed, and constructing one would mean repeatedly restarting a capture daemon to hit a millisecond window. What watches it instead is the standing `zcrypto-capture-rows-quarantined` rule this Resolution names.
