...
- **Topic ids carry no status annotation** in the files this branch touched — five annotations across `cli/panel/primitives.py`, the ops role's defaults and its archive-pull template. The same form elsewhere in the tree was the sweep, landed since — the first bullet under `### The remainder, closed 2026-09-06`.

### The remainder, closed 2026-09-06

- **The status annotations on topic ids** are gone from `cli/`, `infra/`, `docs/reference` and the README (PR #419, `cleanup/status-annotation-sweep`): each sentence re-tensed to its outcome or the bare id kept where a reader looks the history up; the `engine_host/` and `observed/` vault files carry theirs untouched; the one verb citation left.
- **The tripwire's string-literal exclusion** rides T0164's closing PR (`feat/t0164-prose-tripwire-hook`, the commit `fix(tooling): a literal is prose only where the AST puts a docstring`): a triple-quoted string assigned or passed is a value, counted nowhere; its fixture trips on a real docstring and passes on a probe source.
- **The logrotate task is deleted** (this PR): `/etc/logrotate.d/zcrypto-capture-docker` measured absent on zcrypto and zcrypto-red on 2026-09-06; ops's copy was purged by hand on 2026-07-21 (archived T0089) and every converge since re-asserted the absence, and the bridgehead's base role post-dates the install task — so the `state: absent` guard had nothing left to remove anywhere the role runs.
- **The admin accounts carry no old key**, measured rather than assumed: on zcrypto and zcrypto-red the deploy account's `authorized_keys` holds its own key alone, root's holds the bootstrap master key alone, and `zcrypto-data`'s holds only the role-managed forced-command pull keys (capture, and on the primary the engine journal's); on the NAS the deploy account holds its own key alone and `zcrypto-data` holds the hot-push key under its rrsync forced command. No old copy remained anywhere.
