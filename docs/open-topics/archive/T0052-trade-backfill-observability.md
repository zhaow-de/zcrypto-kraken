...
**Correction (2026-09-12), on the sweep's read of the archive:** "**Not yet pushed to Grafana.**", and the verification it defers to the next NAS deploy, are both contradicted by the tree. The rules reached Cloud and one of them was measured there by value in drill K (2026-08-31) — `grep -o 'trade-backfill-stale[^—]*— \*\*[^*]*\*\*' docs/reference/drill-log.md` → ``trade-backfill-stale` 08:00:50Z — **10 min 01 s**``, the interval from induction to that rule's own `Alerting` transition. And no NAS step is left to verify at: `grep -rn 'TRADE_BACKFILL_TEXTFILE' infra/ | wc -l` → `0`, because spec `00054` D2 moved the writer to the ops node (`a67ddb616` *feat(config): move the overlay writer to the ops node (spec 00054 D2)*), where `infra/ansible/roles/ops/templates/archive-pull.sh.j2:198-200` emits all three series and the [[T0048]] Alloy-restart caveat went with it. The two paragraphs below record both facts already; this sentence is the point-in-time one they overtook.
