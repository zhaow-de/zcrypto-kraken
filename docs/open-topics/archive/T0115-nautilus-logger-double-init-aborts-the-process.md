...
**Correction (2026-09-12), on the sweep's read of the archive:** the bullet above names "the committed script's own docstring" as the durable home of a per-bump routine; both the script and the obligation are gone — `ls infra/scripts | grep -ci naut` → `0`, and `grep -n 'per-bump obligation' docs/open-topics/T0085-final-pre-golive-steps.md` → `53:- **The logger-guard sub-item is closed by retirement, and the per-bump obligation it carried is dropped.**`, a bullet that closes *"Nothing is owed at the next bump on this account."* `infra/scripts/nautilus-logger-guard-probe.py` was deleted on the v2 wheel because every premise it rested on had gone with `system/kernel.py` (`docs/plans/00100-nautilus-v2-migration.md:442`); what covers the property now is structural — one node per process in `zcrypto engine run`, one child interpreter per node in `tests/test_engine_node.py`, and `tests/test_nautilus_interface_pin.py` reporting what moved on a bump. The neighbouring "the abort is fixed in the version the repo now pins" is left alone deliberately, because it still holds: the pin is `nautilus-trader===2.0.0rc4.dev20260825` (`pyproject.toml:8`), and T0085 records measuring a second `init_logging(...)` there succeeding outright rather than aborting.
