...
- The falsified "proportional to the count" claim has been corrected in place on the runbook and in the alert rule's comment (this branch). The remaining carriers of the same prediction are point-in-time records and are deliberately left, named so nobody re-sweeps them: spec `00084`'s design line, and the archived [T0116](T0116-stage6b-subphasing-amendment.md) and [T0124](T0124-deployable-runs-as-a-one-sleeve-book.md), whose text is the record of what was believed when they were written.
...
The registered column reproduces T0116's recorded curve to within a decimal on every cell — T0116 has 147.0/209.2, 51.2/115.7, 11.6/30.9, 6.2/14.2, 4.3/8.9 over 136 cycles against these 138, and the first-decimal medians move exactly as two extra cycles should. That is what qualifies the instrument.
...
*The comparison is not fully like-for-like.* `accumulation_payload` initialises held quantity to zero at its first cycle, so a window opening in a low-gross stretch carries an additive offset that never decays. A standalone run of 08-15 → 08-22 reads **95.0 / 168.1** — the flat-start artifact — against **53.9 / 148.1** for the identical 46 cycles carried from 07-11 (the curve above runs to 47, which moves the carried median to 54.3 and leaves the p95 untouched), and the carried figures are the honest ones because a live book is never flat. But **the registered window is itself the start of the chain and carries the identical artifact**: drop its first two days and it reads 49.7 / **91.0**. De-transiented on both sides the widening is ≈**×1.63**, not ×1.28. The direction survives either way — 07-11 is the journal's first day, so no warm start exists for it, and that is exactly why the like-for-like number is a range rather than a point.
...
The transition cycle placed nothing, and its nearest miss was **BTC at 4.7911331e-05 against a 5e-05 floor, 95.8 %** — striking to look at, but **not a record**: the journal-wide maximum is **ETH/EUR at 749.2 % of its floor on 2026-08-19 16:00**, three days earlier and with one sleeve live. That cycle placed five of its eight. The step's €16.35 is the **fourth** largest of the 28 cycles from 08-18 to 08-22, not an outlier, and the two cycles after it (€9.13, €7.19) stayed well above the €0.27–1.74 the first draft of this section called "normal" — that baseline was the two smallest cycles of one day.
...
No change is proposed here, and **the owner has not been asked** — the question is put to them in this branch's PR rather than settled by me. It is reserved, not closed. The weights are part of what registry record 47 ratified, so altering them is a re-ratification rather than a tuning, and nothing measured here argues for one: the sleeves agreed in sign (cancellation ratio 1.0), so the combination is behaving exactly as specified. The measurements above are what an owner would need if they ever chose to revisit it.
