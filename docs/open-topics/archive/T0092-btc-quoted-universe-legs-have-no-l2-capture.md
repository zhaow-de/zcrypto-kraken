...
**Nothing was left as a next step.** The capture half landed 2026-07-23 and the ladder / regeneration / calibration / re-key remainder 2026-08-08, with the docs rewrite alongside them — commit `8e776c75`, *docs(universe): retire the superseded "no L2 capture" claims*, which took the stale claims out of `cli/universe/rules.py`, `cli/universe/build.py` and `cli/data/rebuild.py` and re-pinned the two tests that asserted them. The one thing this topic proposed and did not build is **consciously dropped**: the cross-spread sanity bound it floated while the legs were blind was superseded by having real capture on both of them, and nothing in the tree carries such a bound — `grep -rn -i 'cross-spread' cli/ tests/ docs/specs/ docs/plans/` returns nothing, because with twelve calibrated symbols in `SPREAD_CALIBRATION` (`cli/costs/spread.py:22`, `ETH/BTC` and `SOL/BTC` among them) there is no longer a leg for a proxy bound to stand in for.
