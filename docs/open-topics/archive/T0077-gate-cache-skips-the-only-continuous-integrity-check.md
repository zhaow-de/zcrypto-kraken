...
**Correction (2026-09-12), on the sweep's read of the archive:** "remains an attended deploy" is overtaken — the flag is deployed and is now the documented steady state, shown by `grep -n -o -- "--cache [^ ]*" infra/nas/pull-entrypoint.sh` → `123:--cache /tmp/gate-cache.json` and by `grep -n "no longer passes" infra/runbooks/gate.md` → `175:` … `infra/nas/pull-entrypoint.sh no longer passes --cache and --slice to zcrypto engine gate-export`, which reads the flag's *absence* as the `zcrypto-gate-cache-reverify-stalled` rule's retirement condition rather than its precondition. The same reversal leaves the Findings bullet below — "`--cache` is not enabled in any deployment" — a record of what was true when this topic was filed.

**The three steps this topic left are answered above, each by an act rather than an intention.** **Pushing the alert rule live** was done 2026-07-20 through `infra/scripts/grafana-push.sh`, whose read-back asserted every rule's datasource with the prune step run dry-run; the rule is still committed at `infra/grafana/alerts.yaml` under uid `zcrypto-gate-cache-reverify-stalled`. **Siting the cache file** was decided rather than deferred — spec `00062` D9 makes it container-ephemeral and never `/archive` or any share both hosts reach, because two hosts sharing one file would compute *identical* `replay_fingerprint`s over different compiled polars runtimes (`POLARS_RUNTIME=compat` on the NAS, the default on ops, neither digested) and each could serve the other's entries as its own; nothing is owed at deploy time beyond *not* pointing `--cache` at the share. **The owner's threat-model judgement** was given the same day — keep daily — so `_ROTATION_SLICES = 24` stands as built with no change required (`cli/engine/gate_cache.py:30`, with the assert at `:36` holding the ceiling). That last one is written down because the alternative was to leave it an omission rather than a decision.

**What had landed before the close**, kept in full because it carries the measurements, the counters that prove the treatment engaged, and the two traps closed on the way:
...
- ~~Push the alert rule live~~ — **done 2026-07-20.** Pushed via `infra/scripts/grafana-push.sh`; the script's read-back asserted every rule's datasource, and the prune step ran dry-run. The metric now has its witness. — **ANSWERED:** the rule is still committed — `infra/grafana/alerts.yaml:207`, uid `zcrypto-gate-cache-reverify-stalled`, whose summary names the failure it catches — and `infra/runbooks/gate.md#zcrypto-gate-cache-reverify-stalled` documents it, with its absence from `alerts.yaml` made a tripwire at `:175`.
- ~~Site the cache file container-ephemeral~~ — **decided and recorded as spec `00062` D9.** Ephemeral, never on `/archive` or any share both hosts reach: the NAS builds `POLARS_RUNTIME=compat` (no AVX) while ops uses the default runtime, and `replay_fingerprint` digests neither, so a shared file would let two hosts compute identical fingerprints over different numeric runtimes and serve each other's entries. Nothing to do at deploy time beyond *not* pointing `--cache` at the share. — **ANSWERED:** `docs/specs/00062-gate-cache-rotating-reverification-design.md:38` carries D9 with that mechanism, and the deployed invocation obeys it — `infra/nas/pull-entrypoint.sh:123` passes `--cache /tmp/gate-cache.json`, a container path, never the share.
- ~~Owner's threat-model judgement~~ — **answered 2026-07-20: yes, keep daily.** In-place alteration is judged worth defending against (bit-rot on the RAID, partial or interrupted writes, and anything holding group-write on the deliberately `0775`/`0664` share), and ~1.5% of the pull interval is an acceptable premium. `_ROTATION_SLICES = 24` stands as built — no change required. Recorded here because the alternative was to leave it an omission rather than a decision. — **ANSWERED:** as built and unchanged — `cli/engine/gate_cache.py:30` still reads `_ROTATION_SLICES = 24`, with the ceiling assert at `:36` holding it against the entrypoint's `cycle % 24` index.
