...
**What this ruling treated as permanently lost turned out not to be.** On 2026-08-21 the scratchpad runners behind rows 43 and 44 were recovered verbatim from the iter-080/081 session transcript — read four minutes before the tooling's retention prune destroyed it — and the stage-1 driver reproduces row 43's registered figures exactly, on two independent machines ([[T0148]], `docs/reference/trial43-recovered-runners/`). The 43-vs-44 re-read, including the maker–taker reversal point, is answerable again and registered there. **The re-grounding above stands on its own merits and is not re-coupled to it**: the gate reads the benchmark-relative basis, exactly as ruled.
