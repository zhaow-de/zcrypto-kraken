...
- **A lost ack degrades, never strands.** The waiter times out at 5 s and subscribes anyway, falling back to exactly the old ordering, and counts `resubscribe_ack_timeouts_total` so the degradation is visible. Stranding a pair is the failure [[T0008]]'s ladder exists to remove; trading it for a closed race would have been strictly worse.
