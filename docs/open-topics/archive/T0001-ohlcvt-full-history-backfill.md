...
**Correction (2026-09-12), on the sweep's read of the archive:** the deferral above of "finer cadences (1m/5m/15m)" is contradicted at its 15-minute leg, which was built rather than left open, shown by `git log --oneline --reverse -- cli/backfill/substrate15m.py | head -1` → `dd4bc608f feat(backfill): 15m substrate driver + QA/reconciliation (spec 00044 task 2)` and by `grep -n "15-minute cadence" docs/reference/data-catalog-full.md` → `66:12 pairs, full dump history (BTC from 2013-09) at 15-minute cadence, ~3.12M bars, derived from the 1-minute OHLCVT dumps via cli/backfill/substrate15m.py` — a dataset of its own, `data/ohlc-15m`, basket `0fed24a6…`, cataloged and consumed by trials 45–46. It landed 2026-07-11, four days after this Resolution's date. Four more of the legs are in the tree and were missed by the sentence above. Empty-interval reconstruction is `fill_gaps` at `cli/ohlc/reconstruct.py:6` (`6966dc1c0`, 2026-07-08), exported from `cli.ohlc` and with no production caller, so it is available rather than applied. The symbol & corporate-action ledger is `docs/reference/symbol-corporate-action-ledger.md`, dated 2026-07-07. The Binance-Vision cross-check was DELIVERED at iter-012: `docs/research/02.phase1-data-foundation-closeout.md:17` books it **Met** against `docs/research/02.phase1-binance-crosscheck-report.md`, and `cli/xcheck/binance.py` carries `crosscheck_series` (`:63`) and `crosscheck_dataset` (`:96`) — what is still deferred is narrower and is recorded in that report rather than on this leg, its line 24 reading "full-overlap cross-check via startTime pagination is a deferred follow-up" beside the recent-window check that was run. And pointing the universe/backtests at the new dataset hash is delivered too: `cli/engine/command.py:60` reads `CANONICAL_DIR = Path("data/ohlc-full")`, `cli/data/rebuild.py:118` is `_require_ohlc_full`, and `grep -c dataset_hash docs/reference/trial-registry.jsonl` → 47. **The two legs the tree does not contradict are 1m/5m and full-exchange breadth**, each with the command that shows it: `grep -niE '1-?minute|5-?minute' docs/reference/data-catalog-full.md` returns six lines and no Kraken dataset at either cadence — three are the 1-minute OHLCVT dumps the 1h/4h/1d and 15m datasets are reconstructed FROM (`:11`, `:12`, `:66`), three are Binance's own series (`:87` and `:91`, 5-minute open interest; `:143`, 1-minute liquidation buckets) — and `grep -rn -i 'full-exchange breadth' docs/` returns deferral prose alone.
