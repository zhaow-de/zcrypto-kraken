...
- **Surfaces the rule named but nothing checked**: Prometheus metric HELP text, Grafana alert summaries and panel descriptions, compose `${VAR:?message}` errors, and `infra/scripts/` — the instruments `fleet-deploys.md` tells an operator to run.
