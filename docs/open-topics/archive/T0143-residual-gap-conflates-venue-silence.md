...
- **The input was decided: cross-host agreement, not venue status.** `ts` is Kraken's own payload timestamp (`cli/capture/command.py`), never local receipt time, so two independent hosts that receive the same message record byte-identical values by construction and a host that was not receiving cannot manufacture one. Measured against both known venue events, cross-host classifies 2026-08-20 and reads 2026-08-06 `undetermined`. **A complementarity claim recorded here at closing was corrected the same day and is withdrawn**: it read the status PAGE's silence on 08-20 as venue silence, but Kraken's WebSocket `status` channel announced **both** events on both hosts. Venue status therefore dominates cross-host rather than complementing it — there is no uncovered half. What the reconciler still cannot reach is a *retroactively readable* form of it: the public endpoint reports current state only, and the WS channel is counted into `zcrypto_capture_venue_status_total` but written nowhere the reconciler reads. [[T0144]] was opened for that and resolved as a measured refutation; see its `## Resolution`.
