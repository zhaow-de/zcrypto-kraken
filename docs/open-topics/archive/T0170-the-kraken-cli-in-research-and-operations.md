...
- **Nothing was left open at the close** — every item above is done or has its recurring home, which is why this file's next-steps list read `(none)` when it was archived. The two recurring homes are named where they run: `infra/runbooks/engine-procedures.md`'s pre-probe step 8 and `docs/reference/captured-spread-calibration.md`'s Recalibrating section. The allowlist is still sixteen `Bash(kraken <cmd>:*)` entries in `.claude/settings.json`, and CLAUDE.md's `## Secrets` carries the workstation-only boundary as a counted rule, so it is no longer ruled only in a report.

**The step list this topic carried at its close, kept verbatim — it held no step:**

_(none — every item above is done or has its recurring home)_ — **ANSWERED:** both recurring homes are named where they run, `infra/runbooks/engine-procedures.md`'s pre-probe step 8 and `docs/reference/captured-spread-calibration.md`'s Recalibrating section, so nothing was owed at the close.
