...
**The ordering trap this hit is now in `capture-deploys.md`**: the converge re-enables the panel timer, so a pre-converge disable is silently undone — and leaving it armed *across* the converge is its own hazard. Stop it before and again after, and verify the second stop before the next tick is due.
