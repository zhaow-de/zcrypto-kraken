...
**What had landed before the regeneration** — the marker itself, which is what the run above had to write:
...
**Correction (2026-09-12), on the sweep's read of the archive:** "hours for a pair outside the `PANEL_QUOTE` scope are refused, because the sweep will never revisit them" is contradicted by the tree: `PANEL_QUOTE` no longer exists in the code — `grep -rl PANEL_QUOTE cli/ tests/` → no match (exit 1); the identifier survives in `docs/specs/00085-quote-aware-notional-ladder-design.md`, `docs/plans/00085-quote-aware-notional-ladder.md`, this file and `docs/open-topics/archive/T0092-btc-quoted-universe-legs-have-no-l2-capture.md` — and the sweep now materializes every quote that carries a notional ladder, EUR and BTC both. The refusal itself survives, re-keyed to ladder membership: `cli/panel/command.py:74-85` aborts on an hour whose quote is absent from `NOTIONALS_BY_QUOTE`. The widening landed 2026-08-06 on `bc12475c0` *feat(panel): the sweep materializes every quote that has a ladder* and `070ad4d5d` *feat(panel): generation manifest carries the per-quote ladder; scope guards lift*.
