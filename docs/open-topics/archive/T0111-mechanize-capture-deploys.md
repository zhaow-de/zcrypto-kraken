...
**Closed 2026-08-05.** Both waves landed and every mechanized surface has now been exercised against the live fleet rather than only against its tests: `converge.sh` at the 2026-08-04 secondary re-pin (where its typed-confirm gate refused two agent-driven attempts with rc 3, proving the unattended-refusal property on the fleet), and `zcrypto-panel-regenerate` at the 2026-08-04 full panel rebuild (verified by value — the queued tick skipped exactly the 6370 hours the rebuild wrote).

Nothing is deferred out of this topic. The accepted residuals above are recorded as measured, fail-safe, and deliberately unfixed; none is a live deferral, so none needed re-homing.

**What had landed before the close** — both waves, the two live drills and the residuals accepted at review, folded in from this topic's former `## Done so far`:
...
- **The window re-assert** (the 2026-08-03 backgrounded-converge lesson) is in: `site.yml` reads the clock at task execution time and refuses outside a window that ends 10 min before the next boundary and, as wave 1 shipped it, began at a fixed B+30 min — wave 2 replaced that floor with the boundary cycle's journaled `completed_at` + 5 min, keeping B+30 only as the fallback when the journal cannot be read, so the interval is no longer the fixed `[B+30 min, next−10 min]` this bullet originally recorded. `tags: [engine]` so capture-only primary runs are unaffected.
...
- The corresponding `fleet-deploys.md` lines shrank to pointers in the same PR (per-edit owner sign-offs).
...
- **`converge.sh` is drilled on the live fleet — 2026-08-04, the secondary re-pin to `6b36bf23a0ff`** (the websockets 17.0.1 image, PR #262). Both legs recorded in `fleet-pins.md`: the `--check` leg previewed and stopped clean (`ok=201 changed=2 failed=0`, every wave-1 guard exercised), and the real leg **refused two agent-driven attempts with rc 3** — agent-harness commands carry pipes and no controlling terminal in any shape, foreground or backgrounded, so the typed-confirm gate held as designed — landing only when the operator ran it in a plain terminal. The unattended-refusal property is now proven on the fleet, not just in tests.
- **`zcrypto-panel-regenerate` is drilled on the live fleet — 2026-08-04, a full delete-and-rebuild of the ops panel tree.** Ran 08:39:46 → 16:45:31 UTC (8 h 06 m) inside the materialize unit, exactly as designed so a stray timer tick is a no-op: `pairs=10 pairs_out_of_scope=2 hours_written=6370 hours_skipped=0 rows=22827108 errors=0`. The two non-EUR-quoted pairs are skipped by design (the notional ladder is quote-denominated). **The generation was proven by value, not by presence**: the tick the timer had queued behind the rebuild ran immediately after and reported `hours_written=80 hours_skipped=6370 errors=0` — it read the new tree and skipped *precisely* the hours the rebuild wrote, which a mismatched generation would have refused and a corrupt tree would have rewritten. Timer re-armed on success; `hc_check_up{name="zcrypto-panel"}` 1 with a fresh `ops_panel_last_success_timestamp`.
- **The drill's real yield was two render bugs no offline test could have caught, and the guard that now catches them.** The first attempt installed a script that never rendered: bash's string-length expansion `${#var}` opens with a brace-hash pair, which **Jinja reads as a comment tag**, so the override guard silently vanished. The fix for that then shipped a *second* broken script, because Ansible renders with `trim_blocks=True` while a bare `jinja2.Environment()` defaults it False — the test that cleared the fix was lying about the engine. Both are dead: the guard is written as offset expansion (`"${override:8}"`, inert to Jinja entirely) and `tests/test_infra_shell_templates_render.py` now walks **every** `roles/*/templates/*.sh.j2` through Ansible's own Templar and `bash -n`, with a registry that fails on an unregistered template. The lesson generalises past this script: a template test that does not use the renderer the fleet uses proves nothing.
