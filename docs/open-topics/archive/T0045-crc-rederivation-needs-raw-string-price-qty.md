...
- ~~Decide whether byte-exact CRC re-attestation is worth a capture-schema change.~~ Decided 2026-08-23: no. — **ANSWERED:** the ruling above *is* that decision, and the shipped verifier reads the same way rather than awaiting one: `cli/archive/replay.py`'s module header states the limit as standing — the stored `checksum` is capture-time ground truth and never re-derived, because `price`/`qty` are `Float64`.
- ~~If pursued: add `price_str`/`qty_str` to `BOOK_SCHEMA`, thread the raw WS strings through `_handle_book_message` (they arrive as `Decimal` via `parse_float=Decimal`; keep the original string), and extend `cli/archive/replay.py` to re-derive and compare the CRC when the columns are present (older float-only segments keep the structural-only check). Ship on the next gated capture re-pin, never a standalone live restart.~~ This remains the correct build path if the decision is ever revisited under a changed premise — via a new topic, not a revival of this one. — **DROPPED:** never built, and the tree still reads that way — `BOOK_SCHEMA` (`cli/capture/segment_writer.py`) types `price`/`qty` as `pl.Float64` with only `ts`/`symbol`/`type`/`side`/`checksum` beside them, and no `price_str`/`qty_str` is named anywhere under `cli/` or `infra/`. One detail that keeps the path buildable under a changed premise: the exact strings still reach the parse boundary, where `cli/capture/ws_client.py` reads each frame with `parse_float=Decimal`.
