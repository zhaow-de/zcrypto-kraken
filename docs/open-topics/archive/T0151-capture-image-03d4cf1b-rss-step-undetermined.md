...
That is the whole answer. A leak, and equally an age-driven converging allocation, are both functions of PROCESS AGE — neither can make two processes 11.4 h apart in age step simultaneously in wall clock. Synchronisation across hosts at different ages is environmental by construction, which is the same shape that exculpated `636012cc`'s +11.04 MiB step against a +10.07 MiB control.
