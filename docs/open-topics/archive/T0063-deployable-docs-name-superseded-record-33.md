...
**Correction (2026-09-12), on the sweep's read of the archive:** the paragraph above renders the one-lookup line as `dataset_hash 45275ebe…` **=** daily `ba47e37e` ‖ 4h `81dc9b44`; that equality is contradicted by the very line it describes — `sed -n '19p' docs/research/12.phase5-system-spec-runbook.md` now reads *"…which the registry records as composed from the daily (`ba47e37e`) and 4h (`81dc9b44`) inputs — **read that as semantics, not as an executable recipe: it does not reproduce.**"* It was tested 2026-08-08 across ~100 concatenation forms of those two digests, none of which yields `45275ebe`, and the driver that produced it was never committed ([[T0065]]). The same equality in "Findings so far" above is stale the same way. What the paragraph delivered stands — the pointer line exists and every stale site carries its dated supersession marker — and the dataset is still identified, by the registry record and the extent pins in `tests/test_crossfreq_system.py` rather than by a reproducible composition.

**The steps this topic carried at its close, kept verbatim with what answered each:**

- Reconcile the deployable's identity across the three stale docs: add a dated supersession pointer at each (matching the T0003/T0032 inline-correction style) naming **trial 44 / `build_crossfreq_system` / `dataset_hash 45275ebe` / daily+4h** as the current deployable, so a reader landing on the closeout or runbook body is not misled. Keep the registry (record 44) as the single source of truth and point the prose at it. — **ANSWERED:** every site it named carries an inline bracketed pointer today — `docs/research/13.phase5-closeout.md:16` (dated 2026-07-10, naming record 44 and this topic) and `:53` (the `build_combined_system` build-target line, superseded at iter-082), `docs/specs/00039-phase6-kickoff-design.md:13` (decision-5 prose) and `:49` (the non-goals record-33 line), plus the historical-pin note on the runbook body's `ba47e37e` dataset line at `docs/research/12.phase5-system-spec-runbook.md:41`. Each pointer defers to the registry record.
- Do NOT rewrite the historical closeout narrative — mark the superseded claim in place (the Phase-5 closeout was correct as of Phase-5 exit; the supersession is a Phase-6 event). — **ANSWERED:** honoured rather than merely agreed — `docs/research/13.phase5-closeout.md:16` still asserts *The deployable system is **registry record 33***, and the correction sits in a bracketed clause appended beside it. Nothing in the historical sentence was restated.
- Consider a one-line "current deployable" pointer in a durable, re-read location (e.g. the runbook's top) so the answer to "what deploys?" is one lookup, not a registry archaeology. — **ANSWERED:** `docs/research/12.phase5-system-spec-runbook.md:19` — *Current deployable — one lookup (added 2026-07-19, T0063):* registry **record 44** · builder `build_crossfreq_system_fast` · `dataset_hash 45275ebe…`. That line has since been amended by a finding this topic did not anticipate: it now records the `45275ebe` composition as *semantics, not an executable recipe*, because the digest does not reproduce from its two operands (tested 2026-08-08 across ~100 concatenation forms, none of which yields it, and the driver that produced it was never committed — [[T0065]]), leaning instead on the registry record, the builder and the span pins in `tests/test_crossfreq_system.py`.
