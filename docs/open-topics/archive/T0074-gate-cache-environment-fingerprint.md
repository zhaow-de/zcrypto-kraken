...
**The three steps this topic parked are all discharged by the change above.** The one-line fold is `replay_fingerprint` in `cli/engine/gate_cache.py`, which digests `version("numpy")` and `repr(sys.version_info[:2])` beside the ten modules, the effective config and the replay path. And the parquet-decode judgement was taken rather than left to be silently revisited: polars stays out, recorded in the paragraph above and in `gate_cache.py`'s own comment that third-party names are out of the module walk because numpy's version is digested separately per this topic.

**The steps this topic carried at its close, kept verbatim with what answered each:**

- **(Autonomous, one line)** Fold the environment into `replay_fingerprint`: `importlib.metadata.version("numpy")` plus `sys.version_info[:2]` (and `polars` if the decode path is ever hashed). Consistent with spec `00060` D3's stated rationale that over-invalidation is safe and under-invalidation silently corrupts evidence — the cost of a version bump is exactly one full rebuild. — **ANSWERED:** `replay_fingerprint` at `cli/engine/gate_cache.py:165` digests `version("numpy")` — with a `numpy-not-found` sentinel on `PackageNotFoundError` — and `repr(sys.version_info[:2])` at lines 180-185, beside the ten modules, the effective config and the replay path; the docstring at `:168` cites this topic.
- **(Autonomous)** Add a test that a changed environment string moves the fingerprint, mirroring the existing byte-mutation tests for the ten covered modules. — **ANSWERED:** `tests/test_engine_gate_cache.py::test_replay_fingerprint_covers_the_environment` (line 330) cites this topic and asserts the digest moves on a numpy bump (`version` monkeypatched to "99.99.99") and on a Python *minor* bump; the `PackageNotFoundError` sentinel is pinned by the sibling `test_replay_fingerprint_survives_missing_distribution` at line 347, not by this test.
- **(Judgement, cheap)** Decide whether to also hash the parquet decode path. The review's argument for leaving it is that a decode change fails loudly rather than silently — record the decision either way so it does not get silently revisited. — **ANSWERED:** the decision is recorded twice — the Resolution paragraph above (polars deliberately not fingerprinted, the `errors.py` residual likewise judged non-silent, with the `snapshot_content_hash` argument for why a decode change fails loudly) and `cli/engine/gate_cache.py:57` in code, where `_resolve_module`'s docstring reads "D3: third-party is out -- numpy's version is digested separately per T0074".
