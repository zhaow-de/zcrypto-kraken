...
- **Threshold derived, not guessed**: worst *natural* intra-hour book spacing measured fleet-wide is **12.196 s** (ETH/BTC, the thinnest leg; 101-hour p99 **12.299 s**), so 30 s is ~2.4× the binding pair's p99, and it equals the reconciler's `--min-gap-seconds` so the two producers finally measure the same thing.
...
- [[T0104]] — the panel emits a frozen book across a canonical gap; measured 212 rows with 2 distinct `mid` values against 8 in an equal-length quiet control, already materialized.
