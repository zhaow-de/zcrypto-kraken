...
Both guards are **latent by construction**, measured rather than assumed: `reconcile` sources its `now` internally, and `cli/trades/backfill.py` takes its hours from `canonical_segments`, which parses them out of the path with `tzinfo=UTC` (`cli/archive/reader.py:31`), unioned with hours floored from the REST frame's UTC-typed `ts`. No production path reaches either arm today — they are boundary guards against a future caller, not repairs of a live hazard. Every arm was proven by deleting it under `infra/scripts/mutate-probe.sh` — the two offset arms KILLED on `DID NOT RAISE CaptureError`, and `settled_hours`' naive arm, which changes only the message because the offset arm already refuses a naive `now`, KILLED on that old message's `at UTC offset :00:00` rendering. Each failure is the defect itself rather than an incidental error.
