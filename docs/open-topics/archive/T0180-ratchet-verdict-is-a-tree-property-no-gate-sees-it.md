...
Any operation that writes commits without running that hook therefore produces an unmeasured tree. `git rebase` is the routine one; `git merge` and a server-side merge are the same shape. No workflow ran pre-commit or the tripwire when this was found, so no CI check evaluated it and the PR merge gate could not see it. The result is a tip nothing has evaluated: a rebase replays commits without running the hook, so the ratchet never ran against the replayed tips at all. It is not that they passed — it is that nothing measured them, and four of #451's eight are red when measured after the fact.
...
**A `pre-push` stage on the ratchet** — local and earliest, the owner's ruling, and **installed**. It catches a rebased or locally-merged red tip before it leaves the machine. `.git/hooks` holds `pre-commit` and `pre-push`, and `pre-commit run --hook-stage pre-push --all-files` resolves that stage to the ratchet alone. Delivered by `.pre-commit-config.yaml` declaring `stages: [pre-commit, pre-push]` on `prose-tripwire`, plus `default_stages: [pre-commit]` and an explicit `stages: [pre-commit]` on the five upstream hooks whose own manifest declares `pre-push` — without those, installing the hook type puts all eighteen hooks in the push stage, and two of the five rewrite files.

Proven both ways from this checkout against a local bare repository as the remote: a clean tree runs the hook, passes, and the remote ref lands; a tree carrying `infra/scripts/check-agent-lessons.py:1: file-prose 22.2 > 20` is refused with `hook id: prose-tripwire`, the push exits non-zero, and the refused ref is absent from the remote while the ref pushed by the clean arm is still there.

**The install's precondition, and what satisfying it required.** Installing the hook type on a branch lacking `default_stages: [pre-commit]` and the five explicit pins puts the full hook set in the push stage, two of whose hooks rewrite files — so every branch a push can originate from carries them before the install, not after. `develop` carries them. `ops-journal` and `feat/00111-flatten-cache-and-funding-guard` were brought up to date by merging `develop`, `ops-journal` because the daily pass pushes to it on every run. `main` and the Dependabot branch do not carry them and are not pushed from a local checkout: `main` advances only by merging a release branch cut from `develop`, which carries them, and a Dependabot branch is written by GitHub. A push from either runs that branch's own set, and the two are not the same set: `main`'s config predates the ratchet and declares no `prose-tripwire` at all, so fifteen hooks resolve to its push stage and the ratchet is not among them, while the Dependabot branch's eighteen include it. Whether such a push is refused was not constructed and turns on whether the rewriting hooks find anything to rewrite; either way it is recoverable and nothing is owed.
...
**The residual after all three, stated rather than implied**: a red `develop` can go UNDETECTED for about a minute — it persists until a corrective commit lands, which is a different quantity. What the push job bounds is the time before anyone knows, and the harm is bounded by that, since a red `develop` bites only someone who cuts a branch or commits against it while nobody yet knows.
