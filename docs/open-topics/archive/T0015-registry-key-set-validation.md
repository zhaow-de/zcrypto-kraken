...
**Correction (2026-09-12), on the sweep's read of the archive:** "Loader acceptance for both versions is retained", and the three coordinates the paragraph cites, are contradicted by the tree: the loadable set is three versions now, and every cited line has moved — `grep -n '_LOADABLE_SCHEMA_VERSIONS = \|_EXPECTED_STORED_KEYS = \|def validate_stored_record\|    surplus = ' cli/registry/record.py` → `13:_LOADABLE_SCHEMA_VERSIONS = frozenset({2, 3, 4})`, `21:_EXPECTED_STORED_KEYS = {`, `185:def validate_stored_record(rec: dict, where: str) -> None:`, `189:    surplus = sorted(set(rec) - _EXPECTED_STORED_KEYS[version])`. Schema 4 — `_BASE_STORED_KEYS | {"variant", "datasets"}` — was added by `6f2859695` *feat(registry)!: schema 4 — dataset_hash is derived from an observed datasets block*, which is what made "both versions" stop being the whole set. The mechanism this topic delivered is otherwise intact: the exact per-version key set and the `surplus` rejection are still there, keyed for 2, 3 and 4.
