...
Both next steps this topic carried are delivered by that same work. The dynamic B3/B4 build is that report's own instrument QA in §"B3 and B4", which reproduced the committed fixed-window B3/B4 line-for-line (B3 0.2369 / maxDD 48.4 %; B3+vt 0.2696 / 14.8 %; B4 −0.1362 / 83.0 %) and gated-B1's 1.244 zero-fee / 1.047 net-of-cost before any new number was read. The carried-forward "basket-turnover cost model + §9.6 stress ladder" is reconciled in the same report's `## Caveats`: the per-asset turnover + margin-carry model **is applied** net-of-cost to all six variants at the 1.5× Tier-1 maker rung, and the remaining §9.6 rungs (2×, taker-only) are **explicitly dropped** for benchmarks — the drop the bullet allowed for, recorded rather than left implicit.

**The steps this topic carried at its close, kept verbatim with what answered each:**

- Build dynamic B3/B4 mirroring the fixed-window method (gate on the basket's own cumulative-equity 200-day average; B4 short below) over the union calendar, zero-fee + net-of-cost, vs gated-B1. — **ANSWERED:** delivered as that report's own §"B3 and B4" (iter-055), whose instrument QA reproduced the committed fixed-window B3/B4 line-for-line (B3 0.2369 / maxDD 48.4 %; B3+vt 0.2696 / 14.8 %; B4 −0.1362 / 83.0 %) and gated-B1's 1.244 zero-fee / 1.047 net-of-cost before any new number was read.
- While at it, reconcile the Phase-3 carried-forward "basket-turnover cost model + §9.6 stress ladder": largely superseded by the per-asset-turnover cost model of iters 046–048 — apply that model to the benchmark basket or record an explicit drop. — **ANSWERED:** both halves the bullet allowed for are recorded in the same report's `## Caveats` — the per-asset turnover + margin-carry cost model **is now applied** net-of-cost to all six variants at the 1.5× Tier-1 maker rung, and the remaining §9.6 rungs (2×, taker-only) are **explicitly dropped** for benchmarks.
