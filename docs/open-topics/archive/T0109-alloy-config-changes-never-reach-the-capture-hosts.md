...
**What had landed before the close** — the repo-side fixes and the converges themselves:
...
**How the four steps this topic actually held disposed** — recorded because an earlier rewrite of the file had substituted a different list under a claim of being verbatim, and that was corrected before the close. The NAS's *attended, manual* move is **superseded**: the NAS is Ansible-converged after all (`roles/nas/tasks/main.yml` deploys both the compose file and the config), so it converged with the other tiers on 2026-07-29 under `nas_apply_compose=true`. The attended OPS converge for the structural fix is **done** — 2026-07-29, followed by `docker compose up -d`, the mount now `/etc/zcrypto-ops/alloy/conf`. The attended capture converge is **done** with [[T0107]]'s roll on both hosts, the recreate load-bearing on each. And the re-evaluation of [[T0105]]'s trigger is **done**: its venue half is armed as `zcrypto-capture-venue-not-online` and its paging half dated, and T0105 has itself since resolved. The detection gap none of the four covered is [[T0110]], closed on this same branch by a converge-time `stat`/sha256 assert that sits outside the digest gate. The one deferral the record above left — the delete of the pre-`conf/` top-level `config.alloy`, held back while a missing bind source could still be recreated as an empty directory and crash-loop Alloy — has since been taken on all three tiers: each role carries a *remove the pre-conf layout's stale alloy config* task, `state: absent` on the old path, in `infra/ansible/roles/capture/tasks/main.yml`, `roles/ops/tasks/main.yml` and `roles/nas/tasks/main.yml`.
...
- ~~*(ATTENDED, manual — the NAS)* move `config.alloy` into `conf/` and recreate the container by hand~~ — **superseded**: the NAS is Ansible-converged after all (`roles/nas/tasks/main.yml` deploys both compose and config), so it converged like the other tiers on 2026-07-29 with `nas_apply_compose=true`. The "Container-Manager-managed" premise was wrong and had hidden a role/compose mismatch. — **OVERTAKEN:** the role writes the config to `{{ nas_stack_dir }}/conf/config.alloy` in `infra/ansible/roles/nas/tasks/main.yml`, so there was never a by-hand move to make.
- ~~*(ATTENDED, one more OPS converge — for the STRUCTURAL fix)*~~ — done 2026-07-29, followed by `docker compose up -d`; mount is now `/etc/zcrypto-ops/alloy/conf`. — **ANSWERED:** the structural fix is in the tree — `- ./conf:/etc/alloy:ro` in `infra/ansible/roles/ops/templates/alloy-compose.yaml.j2`, the capture template and `infra/nas/compose.yaml`, guarded by `tests/test_infra_compose_templates.py`.
- ~~*(ATTENDED, at the next capture converge)* pass `capture_alloy_digest`, recreate Alloy, confirm the series~~ — done with [[T0107]]'s roll on both hosts; the recreate was load-bearing on each, host `fda087ea…` against container `e89f00d1…` on the primary. — **ANSWERED:** the table above carries both hosts' times (secondary 00:54:10Z, primary 07:36:45Z) and the primary's five undelivered series arriving.
- ~~*(autonomous, after that converge)* re-evaluate [[T0105]]'s trigger~~ — done: its venue half is armed as an alert, its paging half is dated. — **ANSWERED:** T0105 is itself resolved and archived, its venue half armed as `zcrypto-capture-venue-not-online` with the procedure in `infra/runbooks/capture.md`.
