...
**Resolved 2026-08-22 by spec `00098` (iter-143), ahead of its ripeness trigger by owner fiat** — `exec_armed` still renders `false` and no non-terminal row ever crossed a restart; the owner directed the resolution as its own iteration rather than waiting for continuous arming to make the defect live. The topic's dilemma was a false binary: claiming and observing are separate acts, because the engine publishes a reconciled EXTERNAL order's events unconditionally on `events.order.EXTERNAL` — the topic merely had no subscriber. The engine now subscribes it (`cli/engine/node.py`) and a disposition filter (`cli/engine/executor.py::_on_external_event`) matches events against the ledger-vouched `_attached` rows: a matched fill appends to its row, moves the fill/fee counters, and latches the overfill trip exactly as an own order's fill does; terminal acks close their rows; neither path ever removes a row (the no-pop symmetry, forced by the ownTrades/openOrders cross-stream race). An unmatched event — the operator's sanctioned hand settle — is counted (`zcrypto_exec_external_events_total{disposition="unmatched"}`) and logged, and reaches no row, no cancel, and no fill-time trip (the position-reconciliation trip under the settle preconditions reads the venue's position rather than an order event, and is untouched by any of this). The library boundary is pinned two ways: a real `MessageBus` publish on the installed engine's own topic format, and a really-registered strategy whose expected topic is derived from the library's register-time subscriptions.

**The subscription alone was not the whole answer, and the whole-branch review is what established that.** `system/kernel.py` awaits execution reconciliation strictly before `trader.start()` — which is what runs `on_start` and installs the subscription — so a fill that happened while the process was DOWN is replayed onto `events.order.EXTERNAL` before any subscriber exists, and the message bus has no replay for late subscribers. Two neighbouring windows behave the same way: a fill landing before the first adopt tick matches nothing, and an order that fully filled or was canceled while down never reaches `orders_open` at all. None of the three was created by this spec — the adopt pass predates it — but the resolution would have overclaimed without them. Spec D7 closes all three from the one place that holds venue truth for exactly those fills: the reconciled order's own `filled_qty`, resident there because the engine applies an event to the order and publishes it in the same synchronous body. The pass now journals a positive delta as a repair (never as a fill — there is no fee behind it), completes or trips on it by the same arithmetic a live fill uses, trips rather than clamping when the ledger claims MORE than the venue, and writes a closed order's terminal state so its row stops reading open forever.
...
- **The hand-settle visibility measurement** — **instrumented, not answered**, and the difference is written down rather than glossed. Whether Kraken's settle-position act emits an order event on the adapter's streams at all has never been measured; the `unmatched` counter (spec `00098` D3) is the instrument that answers it at the first settle, and the probe window's verify step now queries the family so the reading is actually taken rather than passed over. The position-propagation half (whether a settle reaches the engine's position view) predates this topic and is now a step of its own — `infra/runbooks/engine.md`'s step 4b — which records the verdict with nothing downstream depending on it, rather than the subordinate caveat clause it used to be.
