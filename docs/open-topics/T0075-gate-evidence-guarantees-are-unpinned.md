...
- ~~**(Autonomous)** Correct the LATENT/LIVE docstring labels in `_REPLAY_CODE_PATHS` to match the module-scope imports at `crossfreq_system.py:53-56`.~~ **DONE, and this bullet was WRONG as written** — following it would have restored the very label the fix removed. Corrected in PR (commit `1acb852`): only `a1.py` is live on the fast route (`build_crossfreq_system_fast` calls `_asset_returns` at `crossfreq_system.py:617`, plus `:475/:505/:537`); `a2.py`/`builder.py`/`strategies.py` are reached only from `build_crossfreq_system` at `:215-276`, the *verified* route. Module-scope import does NOT make a module verdict-determining -- it binds `def`s; the justification for covering those three is D3's over-invalidation rationale.
