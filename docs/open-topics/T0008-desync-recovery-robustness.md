...
- **(human, small)** As part of the iter-039 deploy verification, watch a **deliberate** desync self-heal on the live feed (the T0003 incident's verification step) — that is the only real test of the race-condition question above; if it does not cleanly self-heal, this topic jumps in priority.
