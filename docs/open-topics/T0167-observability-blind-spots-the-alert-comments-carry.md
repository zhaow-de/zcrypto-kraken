...
- The gap the widening surfaced — nothing watched whether the sweep RUNS; its gauges hold their last value between runs: **new rule** `zcrypto-ops-verify-replay-stale`, 48 h on `ops_verify_replay_last_run_timestamp`, mirroring the verified-replay sibling, with its `infra/runbooks/ops.md` section and panel bar. The owner's option named the last-success stamp; the rule reads the last-run stamp because the runner freezes `last_success_timestamp` while any known bad hour stands (rc stays 1) — the exit-code page this family retired. KILLED on the expression and on the panel bar.
- Rule health: **the daily pass reads it** — `feat(ops): the daily pass reports every alert rule Grafana could not evaluate`: a rule whose `health` is not `ok`, or whose instance state keeps the `(Error)` reason that `execErrState: OK` leaves, is listed under "Rules not evaluating" with its `lastError`, and the pass exits non-zero. Four TDD tests.
