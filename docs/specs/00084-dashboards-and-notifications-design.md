...
| `severity` | last word of a long title, after `grafana_folder`, `host`, `instance` and `job` | first glyph of the title, `🔴 CRITICAL ·` / `🟠 WARNING ·` | 26 of 58 rules are critical and 32 warning; the attachment colour is fixed red-firing / green-resolved by Grafana and **cannot** vary with severity, so the glyph is the only place the distinction can live |
