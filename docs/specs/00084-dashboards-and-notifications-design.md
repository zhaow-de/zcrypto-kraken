...
**The template alone cannot carry the log line.** Nine of the ten logs rules that existed when this was written aggregated with a bare `sum(count_over_time(…))`, so the alert carried no labels at all — not even `host`; only `zcrypto-capture-error-logs` had `sum by (host)`. The message is hoisted into a label at query time, on the ERROR-log rules covering direct-ship daemons — **three** of the eleven shipped logs rules (`zcrypto-capture-error-logs`, `zcrypto-ops-error-logs`, and `zcrypto-engine-error-logs`, which D11 adds with an identical shape):
