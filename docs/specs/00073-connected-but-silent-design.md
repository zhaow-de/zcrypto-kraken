...
- **Alerting** — the healable-gap-rate threshold is in pair-seconds while its summary claims minutes, and the critical permanent-loss rule self-resolves after 60 minutes. Registered, not re-derived here: the history contains only `0` and `≥2311`, so any re-derivation now would be intuition wearing a table.
