...
**`ordermin` is a base-asset quantity, not EUR** — 20 ADA, 3.9 DOT, 0.001 ETH, 0.00005 BTC (`data/snapshots/kraken-refdata-20260707T032900Z.json`, `fetched_at 2026-07-07`, 1,509 pairs). The EUR floor is therefore **time-varying** and must be priced per cycle from the journaled closes, never from a fixed table. `costmin` is €0.45 across the book.

## Decisions

**D1 — No builder change; the harness recomputes the middle stages and PROVES the recomputation.** `combined` and `capped` are builder-internal, but `combined = ⅓B + ⅓A1 + ⅓A2` from `sleeve_positions` and `capped = apply_position_caps(combined, …)` reproduce them from public parts. The duplication risk (builder changes, harness drifts silently) is closed by an identity check run **per cycle, not once**: `multiplier × capped_recomputed[a] == final_targets[a]` for every asset, exactly, or the cycle raises. That identity is the harness's proof it reconstructed the builder's own intermediate — it is the verification, not an assertion about it.
...
**D6 — The band is reported the way the gate reads it.** The pre-registered band is weekly `|live − sim| ≤ 10 bps of NAV`, so the output aggregates per ISO week and reports **median and p95 weekly drift in bps of NAV** per NAV level — the shape a re-registered band can be written from directly. The per-cycle series is retained for inspection.
...
- **The harness proves itself against the journal**: for all 136 records, the replayed forming-row targets equal the journaled `final_targets` bit-exactly. A record that fails is reported, not skipped — a silently-dropped cycle would bias every aggregate.
- **The D1 identity is the load-bearing check** and is proven by construction: perturb the recomputed `combined` (e.g. weight ½ instead of ⅓) and the identity must fail on the first cycle. Run that mutation, see it fail, restore.
...
- **Held state is quantities, and mark-to-market is implicit** (D4): with `held_qty` fixed and the close doubling between two cycles, the reported drift must move because `target_qty` halved — a test pinning that a price move alone changes drift with no order placed. This is the check that would have caught the EUR-denominated design this spec's self-review rejected.
- **NAV scaling is monotone in the right direction**: relative drift (as % of NAV) must be non-increasing as NAV rises across the D5 sweep — the floor is absolute, so its relative weight shrinks. A violation means the pricing or the accumulation state is wrong.
