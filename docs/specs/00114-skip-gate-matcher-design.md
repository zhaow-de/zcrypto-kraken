...
**D4. The registry is closed-world by test, not by convention.** One test parses `tests/skip_gates.py` and asserts that its imports are within an allowlist (`shutil`, `subprocess`, `pathlib`, `typing`), that every call in it reaches a name one of those imports binds or a builtin, that the one process launch it may make is `git`, and that every public function it defines anywhere in the module — not only at its top level — is a single `return`. The last two are what close the direction that matters: `subprocess` is on the allowlist for `develop_resolves()`, and without them a `subprocess.run(["curl", ...])` under a module-level `if` satisfies every other clause. A registry that could open a socket would be the reducer's leak moved one file over; this test is what makes D3 a declaration rather than a loophole.
