...
**Both new inputs reach the operator's own surfaces, because the rule's DESCRIPTION is now narrower than the rule.** Exit 3's cause is pinned on the venue everywhere an operator meets it (spelled "the venue could not be reached or read", or just "the venue could not be read"), and the whole-listing refusal is not the venue — an operator reading it during an incident goes and checks Kraken's status page while the venue is healthy and our own cache writer is not, with the red button unpressed for the length of that detour. Exit 2's new residual `reason` is likewise absent from the runbook's exhaustive per-`reason` triage list, whose nearest neighbours all end in "run it again" — which against an unchanged cause reproduces the same residual, when the action owed is to hand-check the named pairs this run was blind to. So the exit-3 clause is widened wherever it is written to say the account could not be READ before anything was sent — unreachable, or the reads not preparable — and the triage list gains the new `reason`. **Widening the cause is not enough on its own**: exit 3's one statement of what to DO reads "nothing was sent; the account is as it was", whose implied wait-and-retry is right for an outage and wrong for a cache writer that will refuse identically on the next run, so that statement gains the action the new cause owes — the account was not read at all, so it is checked by hand on the venue's own pages. The rule is unchanged; what changes is that its description stops being false.
