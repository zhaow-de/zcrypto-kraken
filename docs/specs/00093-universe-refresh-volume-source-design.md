...
Source resolution becomes the same rule as the artifact's: **newest `ohlc-reach-<stamp>` sibling wins, falling back to `ohlc-full`** — exact `ohlc-reach-<8 digits>` names only, so a stray hand copy is never a candidate. One concept at both ends, no new mechanism, and D4's guard then applies to the *resolved* root — where a ten-of-twelve reach set is a real possibility rather than a hypothetical. The resolver is consumed by the universe refresh ALONE: the reach builder keeps its dump-derived canonical anchor, because a reach round must never consume a freshly-minted reach sibling — least of all its own, which exists (empty) before the builder runs.
...
- **Resolution is newest-wins**: the reader takes the highest-stamped `universe-<stamp>/point-in-time-universe.json`, admitting exact `universe-<8 digits>` names only — a stray hand copy (`universe-backup`) is never a candidate. The stamp is `%Y%m%d`, so lexicographic order *is* chronological — no date parsing, no tie-break.
...
- **The re-key is proven non-vacuous**: a test that all twelve `CANDIDATE_SYMBOLS` resolve to a pair key, and that the engine's basket is proven unchanged — ten EUR legs, derived from the re-keyed map and pinned by test (D2).
...
- **The operational sitting, in order, and it is one sitting**: fresh `zcrypto data rebuild ohlc-reach` (the minted `ohlc-reach-<stamp>` sibling lands in the local data root, so no fetch step is needed — the hub's promoted unstamped `ohlc-reach/` is frozen at 2026-07-22 and deliberately not a resolution candidate) → `zcrypto data rebuild universe` → publish the stamped set. Splitting it rebuilds the canonical universe more than once, and every downstream selection reads that artifact.
...
- **It does not close [[T0025]].** That topic's owned work is the corporate-action ledger; the universe refresh appears in its file only as a trigger/blocker note — its frontmatter has carried "the universe-refresh leg is BLOCKED by [[T0093]]" since 2026-07-22 — while the refresh work itself is framed in [[T0024]], the index, [[T0093]] and the memo. Unblocking that leg is this spec's job; the ledger is not, and this spec does not inherit it.
