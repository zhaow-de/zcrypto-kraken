...
| --- | --- | --- |
| A | the alert-requires-runbook guard, then the 51 missing sections across seven new subsystem files | no signal can page an operator without a procedure behind it |
...
Read at the branch's merge-base `10477297`, 2026-08-29, from the repo alone:

| reading | value | what it settles |
| --- | --- | --- |
| rules in `infra/grafana/alerts.yaml` | **83**; **32** carry a resolving `infra/runbooks/<file>#<anchor>` link (resolving, not anchor-equals-uid — four rules share anchors), **51** do not | the gap is the majority, not the tail |
| `infra/runbooks/capture.md` | 12 sections — the README's split bar | new capture material needs a new file |
| `infra/runbooks/ops.md` / `engine.md` | 9 / 9 sections; 18 and 4 rules owed across ops, ops-node and reconcile | ops needs a second file; engine crosses the bar by one |
...
`tests/test_infra_alert_rules.py` already resolves a `Runbook: infra/runbooks/<file>#<anchor>` link *when a summary carries one*. D1 makes the link **required** on every rule. The test is landed and seen **red naming exactly the uids that carry no resolving summary link — 51 at the base, the set pinned by the red run's own output, never by this spec —** before any section is written, and green only when the last section lands — the proof that the guard bites on the defect it names, and the permanent bar against a rule shipping without a procedure. A section serving several uids carries one `<a name>` per uid (the `verify-replay` precedent).
...
- **Auth.** The vault-token resolver in `grafana-query.py` moves to an importable sibling, `infra/scripts/grafana_auth.py`, that both scripts use; the docstring's two decrypt footguns move with it and `grafana_auth.py` gets its own test file carrying the two footgun tests (`test_grafana_query.py` today reaches into the module's `subprocess` and `vault_password_file` — those tests follow the code). `grafana-query.py` loads the sibling by `spec_from_file_location`, since its directory is not on `sys.path` under pytest, and keeps its query and render tests.
...
- **Exceptions registered (E).** `branch-workflow.md`: the journal branch is the second standing exception to the attended PR-open gate, beside `/zcrypto-auto-exec`, and the daily pass's fix PR is the third — bounded to fixes off the capture write path, the live trade path, canonical data, and anything a host converges; merged on CI green. `commit-messages.md`: journal commits and the monthly PR are the fourth review exemption. CI stays — `develop`'s protection requires the `Full test suite` check, so a PR without it cannot merge at all.
...
3. Remediate within two tiers. **Autonomous**: everything read-only; runbook steps that touch telemetry only, **on ops, the NAS or zaccess only** — restart Alloy, re-arm a timer (**corrected 2026-08-29**: "clear a stale cache" was listed here, but its only real form is an `rm -f` inside a container on the NAS, which the tier classifier prepares by design — the example was wrong, not the tier) (the capture pair's `grafana-alloy` goes through `zcrypto-bump-alloy`, attended); a code fix taken the normal way — fix branch, tests, the mandatory subagent review, PR — **merged on CI green when the fix is off the protected paths**, the third standing exception registered in E. **Prepared, then the user's word**: any restart or converge of a capture daemon or the engine; anything touching the venue account (the arm file, the kill file, orders); deleting data; a fix landing on the capture write path, the live trade path, canonical data, or anything a host converges — the PR waits; running `grafana-push.sh` after a merged `alerts.yaml` fix, since it changes what pages and pruning keeps `fleet-deploys.md`'s verify-by-value order; and deploying *any* fix to a host is a converge, always attended (`fleet-deploys.md` stands unchanged).
...
| principle | home |
| --- | --- |
| the four parts in fixed order for ALERT, KNOWN LIMITATION and SCHEDULED REMINDER sections — a PROCEDURE carries the four parts or, for a drill, the seven parts spec `00105` D1 names; the four section kinds; drills produce sections, not reports | `infra/runbooks/README.md` scope |
...
- **D1**: the tightened test fails on the branch's base naming exactly the 51 unlinked uids (set equality against the measurement); passes at the end. A mutation probe (`mutate-probe.sh`) removes one link and sees the test bite.
...
- **D6 bounds**: a dry run of the skill against a fixture report carrying a protected-path finding ends in *prepared, waiting* and no action; one carrying a telemetry-only finding on ops ends in the action taken.
