...
- **Workstation (data-bearing):** (a) the runner's **control against history** — the loading path over `data/ohlc-full`'s record-44 slice must reproduce the registered figures the suite already pins (`test_crossfreq_system.py`, `instrument_self_check`) and a frozen block extent; (b) the **disk conformance pass** — every schema-4 record's `files` re-hashed against disk, memoised per `(path, size, mtime)`, verdicts `rederived` / `absent-here` / **finding** — and `absent-here` is legal **only for dataset dirs named in a committed exceptions list** (starts empty; grows only by reviewed PR): on the canonical data host an unlisted citation that does not resolve is a **finding**, so a fabricated citation cannot be laundered as merely-elsewhere; (c) the loader's **full-set expectations** over all three frozen sets (36 files / 1,052,322 rows; 12 / 3,122,044; 10 / 30,032 — measured 2026-08-08). Data-gated tests skip, never pass vacuously.
