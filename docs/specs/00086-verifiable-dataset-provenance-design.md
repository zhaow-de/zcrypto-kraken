...
- `block() -> dict` — the accumulated `datasets` block; refuses an empty accumulation. `read_series` itself refuses a read that windows to zero rows, so a zero-row dataset can never accumulate — a block that says nothing is the failure being replaced, so it cannot exist.
...
- The identity path parses no manifest (the module touches one only for the optional cross-check below). Consequences, each the direct negation of a recorded killer: the four-shape `series` zoo, the stamp-format splits, and the axis-resolution machinery have nothing to attach to; a new dataset backing a trial needs **zero provenance code** — no allowlist entry, no adapter; the holdout's opaque external digest appears nowhere in record 47's identity, so a byte-identical re-pull cannot move it.

**Vouched-hash cross-check (bonus layer, not identity) — at the manifests' own grade.** The frozen manifests' per-series `sha256` are **frame-content hashes** (`cli.ohlc.dataset.dataset_hash` — sha256 over the frame's canonical CSV), NOT file-byte hashes; a byte-grade comparison would refuse every healthy read of `ohlc-full`/`ohlc-15m` (measured: ADA/EUR 1440's vouched `29d21a63…` equals `dataset_hash(read_parquet(...))` and differs from the file's byte sha256). So where a manifest vouches hashes (`sync.py::_manifest_sha256s`'s shape-agnostic recursion), the loader computes `dataset_hash(frame)` on the **full frame, before any windowing**, and refuses when it is absent from a non-empty vouched set — the data changed since the freeze, and fitting on disputed data is exactly what should stop a run. An empty vouched set (the holdout) makes the check inert, and the runner prints that it is inert. The check's **true-positive path is a test obligation**: a synthetic manifest vouching the correct content hash must pass — a mismatch-only test suite cannot catch a wrong-grade comparison, which is precisely how the first draft of this spec shipped one. The block's identity stays the file-byte sha256; the cross-check cannot weaken what it never produces.
...
- The `datasets` keys themselves are checked like the file keys: a non-empty str, no leading `/`, no backslash, no `..` segment — `{"../infra": …}` is shape-valid JSON that would resolve the conformance pass outside `data/` onto a real repo file and re-hash it green, laundering exactly the fabrication bounded claim 1 names.
- `files`: non-empty dict, POSIX relpath under `data/<dataset>/` → 64-char lowercase hex. `canonical_json`'s key sorting orders it; no list-order digest split is possible. The key form is enforced at load like the hex: relative POSIX only — refuse a key that is empty, starts with `/`, contains `\\`, or carries a `..` segment — so a forged block cannot point the conformance pass outside `data/`.
...
```
uv run zcrypto research eval --subject <name> --dataset <dir> [--window A B] [--register --n-trials N [--variant V] ...]
```

- **Subjects are an enum of committed builders** — `cli/research/subjects.py`, initially `record44-crossfreq` (`build_crossfreq_system_fast`) and `record33-combined` (`build_combined_system`), each entry naming its assets and intervals (which resolve to its required series) and a build function returning its committed metrics. `--register` additionally requires the caller fields **including `--n-trials`** — the registry's family floor demands `prior + 1` per family, and A1 already holds 40 trials, so a hardcoded 1 would forbid every second registration in any family — plus optional `--variant`. Adding a subject is adding an enum entry and an import — the per-need growth discipline that made the allowlist reviewable, applied to the axis that actually needed it.
- The command resolves the repo `data/` root (module constant, `record44_legs.DATA_ROOT`-style — no caller-chosen root), instantiates `ObservedReader`, asks the subject for its required series, and **refuses a subject whose required series the dataset lacks, naming the missing files**. `--window` must demonstrably reach the subjects' reads — a windowed production-subject test is a named obligation, because a closure that drops `window` silently fits full history while the block stays self-consistent and nothing reds (the crossfreq deployable on a daily-only holdout is a refusal, not a crash).
...
- **Workstation (data-bearing):** (a) the runner's **control against history** — the loading path over `data/ohlc-full`'s record-44 slice must reproduce the registered figures the suite already pins (`test_crossfreq_system.py`, `instrument_self_check`) and a frozen block extent; (b) the **disk conformance pass** — every schema-4 record's `files` re-hashed against disk, memoised per `(path, size, mtime)`, verdicts `rederived` / `absent-here` / **finding** — and `absent-here` is legal **only for dataset dirs named in a committed exceptions list** (starts empty; grows only by reviewed PR): on the canonical data host an unlisted citation that does not resolve is a **finding**, so a fabricated citation cannot be laundered as merely-elsewhere. The real-file half runs **only where the data root is present** (skip-with-reason when `data/ohlc-full` is absent — the canonical-host marker): without that second gate, the first schema-4 record turns every bare-checkout CI run permanently red, and the only appeasement — listing the dataset in the exceptions list — would neuter the workstation fence for it forever, since the list names dirs, not hosts; (c) the loader's **full-set expectations** over all three frozen sets (36 files / 1,052,322 rows; 12 / 3,122,044; 10 / 30,032 — measured 2026-08-08). Data-gated tests skip, never pass vacuously.
- **Probes:** every load-bearing guard proven by a constructed failure through `infra/scripts/mutate-probe.sh` — the derivation equality, the floor, the files-emptiness clause, the re-homed `dataset_hash` str check, the loader's hash (a flipped byte moves the block), the zero-row refusal, the missing-series refusal, `--register` gating, and the vouched cross-check's **grade** (mutating the content-hash comparison back to a byte-grade one — the first draft's own defect, reconstructed — must be caught by the true-positive test) — each with a control that fails first. The remaining shape clauses (rows/span arity/hex form/key form) are carried by the parametrized forge tests.

## What this does NOT do — bounded claims

1. **A fabricated block needs no hand-rolled writer — the sanctioned `append()` accepts any shape-valid dict.** The floor guarantees a fabricated record is schema-4, shape-valid, derivation-consistent — nothing more. The fences are named, not imagined: the record lands in a reviewed PR diff against a committed file, and the conformance pass requires every schema-4 citation to **resolve on the canonical data host or be named in a committed exceptions list** (D7) — so an invented citation either fails the suite or is itself a visible, reviewed commit. The honest-negligence path that actually produced the 44 losses — a driver hashing whatever it liked — is closed by the paved road being less work than fabrication; the deliberate-liar path cannot be closed mechanically on any design. Recorded here as the reason, not parked as a topic.
