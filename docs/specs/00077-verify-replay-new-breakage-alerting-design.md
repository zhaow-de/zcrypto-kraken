...
**D10 — Deploy order, and the liquidations decision the digest change forces.** Converge before pushing rules (the new series must be flowing before a rule reads them; until then `exit_code` is still the only rule and nothing is worse than today). And because this converge **moves `ops_image_digest`** — unlike the previous one, where the digest was unchanged and the question was moot — `fleet-deploys.md`'s standing hazard is live: the same variable re-pins the liquidations compose, which the role never restarts, so the file moves while the container does not. The converge must end with an explicit decision to roll the liquidations container or to leave it deliberately pinned-but-not-rolled, recorded either way.
...
- **Because `ops_image_digest` moves, the converge re-runs every ops runner on a new image**, so `fleet-deploys.md`'s standard ops outcome checks are load-bearing here rather than routine: `ops_archive_pull_exit_code` / `ops_panel_exit_code` 0, `reconcile.prom` mtime advanced, reconcile counters unchanged, `hc_checks_down_total` 0.
