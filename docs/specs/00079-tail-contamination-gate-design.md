...
| stable legitimate synthetics: bursty-typical, bimodal fast/slow (200 seeds) | 1.00 – 2.18 |
| infinite-mean-adjacent synthetics: pareto α=1.1 / lognormal σ=3 (200 seeds) | median 8.7 / 10.1 — **straddle the cut by construction** |
| contaminated (k=2 @ n=11,389; k=20 @ n=50,000) | **88.8 – 200** |

The load-bearing false-positive evidence is the **real streams**: 1.05–1.96 against a cut of 10 — a 5× margin. Distribution-family synthetics at the infinite-mean boundary (α→1) straddle the cut *by design* — the cut IS the α=1 per-decade ratio — so they are the cut's definition, not evidence against it: pareto α=1.1's theoretical per-decade ratio is 10^(1/1.1) ≈ 8.1, and measured over 200 seeds it brushes and crosses 10 exactly as theory says. Contamination sits at 88.8–200: an order of magnitude above the cut, two above real streams. *(One nuance from re-derivation: polars' nearest rounds half **up**, so T0112's "k=2 needs 15,001" is actually 15,002 — immaterial here since the ratio gate supersedes the n-bound, but recorded so the resolution is exact.)*

## Decisions

**D1 — Two chained tail-steepness ratios, one cut.** A measured stream must satisfy **both** `p99.99/p99.9 < TAIL_RATIO_CUT` and `p99.9/p99 < TAIL_RATIO_CUT`, with `TAIL_RATIO_CUT = 10.0`. The first ratio catches p99.99 landing on an outage (k from 2 up to ~0.001·n, where p99.9 is still bulk); the second extends coverage to p99.9 itself being contaminated (k up to ~0.01·n). Why 10: it is the per-decade quantile ratio of a Pareto tail at α = 1 — the infinite-mean boundary no physical spacing distribution crosses — so real spacing tails sit well inside (measured ≤ 1.96 across all 12 production streams) while a cliff from same-scale outages jumps to ~100. A synthetic family parked *at* that boundary (pareto α ≤ 1.1) straddles the cut by construction — which is the cut working, not failing: it separates physical spacing from cliffs, not "every conceivable distribution" from cliffs. One constant, same value both decades, explainable at 3 a.m.: *the tail must not steepen more than 10× across a decade of quantiles*.
...
**D4 — Refusal is `UNMEASURED` + exit-bar FAIL, exactly like the `MIN_POOL` refusal.** A contaminated threshold's output *is* the false GREEN (0.0000 % beside missing data), so printing it with a caveat would print the lie. The two refusal reasons are distinguishable in the post-table note — the existing `N stream(s) under the <bound>-interval bound` line gains a sibling naming the steepened-tail count in plain operator language, and both print whenever *any* stream was refused rather than only when nothing was measurable, under the prefix `unmeasured: ` (the old `no measurable segments: ` prefix was false as soon as the note could appear beside a TOTAL row) (this is runtime output of an infra script: **in scope** for `operator-facing-text.md`, so no topic/spec tokens).
...
**D6 — The residual, named and accepted rather than deferred.** k ≥ ~0.01·n similar outages contaminates p99 itself; both ratios then read ≈ 1 and the gate is blind. Constructed during the cold review: blindness begins at exactly k=115 for n=11,389 (k=114 is still caught, r2=53.4). Accepted on the absurdity bound alone, stated honestly: that regime requires the window to be **77–97 % outage** (23,000 s of silence against 619–6,764 s of data, depending on spacing shape) — no investigation shape survives that unnoticed, and the on-screen `n` beside the window's span is the eyeball check. **Deliberately NOT claimed as backstops** (the cold review disproved them): the truncated-hours count uses `secs > thresh`, so a contaminated threshold blinds it too; missing-hours needs a geometry these mid-hour outages never produce; `covered_s` is span-derived and cannot disagree with the span. No production stream is within three orders of magnitude of even the k=2 regime. This is a conscious drop recorded here and in the gate's code comment — not a deferral, per the owner's no-new-topics directive.
...
- **No false positives on legitimate shapes**: bursty-typical and bimodal fixtures stay measured as seed-independent properties (measured 1.00–2.18 over 200 seeds). Pareto/lognormal fixtures, if kept, are pinned to named seeds verified < 10 with a comment that these families brush the cut by construction — an unpinned seed there is a lottery that fails healthy code roughly one seed in three.
