...
- **The unsendable node AND strategy** — both `LiveNode` and `Strategy` are pyo3-unsendable: any attribute read from a thread other than the one that built them aborts the process with SIGABRT, uncatchable. Measured, both exit 134. Supervision must hold `node.handle()`, captured on the building thread. Because the strategy is unsendable too, a driver that submits orders cannot run on a second thread while `run()` owns the first — which is what blocks the probe harness.
