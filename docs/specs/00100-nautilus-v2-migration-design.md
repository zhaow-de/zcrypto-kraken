...
- **The always-truthy watchdog** — `Strategy.is_running` changes from property to method, so `if strategy.is_running:` on a bound method is permanently true. `LiveNode.is_running` remains a property, so the trap is real for one object and not the other. The supervision watchdog must be proved to fire, not merely to compile.
- **The unsendable node** — `LiveNode` is pyo3-unsendable: any attribute read from a thread other than the one that built it aborts the process with SIGABRT, uncatchable. Anything supervising the node from a timer or worker thread must hold `node.handle()`, captured on the building thread, and read state through it. Measured: `handle.state` answers off-thread; `node.is_running` exits 134.
