...
- **D10 — ancestor packages are resolved explicitly (ADDED AT REVIEW; the first cut shipped this bug).** Importing `cli.engine.command` *executes* `cli/__init__.py` and `cli/engine/__init__.py` first, unconditionally. The first implementation resolved a dotted name to its leaf file only, so **seven** modules ran on every replay while unhashed (four ancestor `__init__.py` files, plus `cli/ohlc/{ingest,qa,reconstruct}.py`, which became traversable only once `cli/ohlc/__init__.py` entered the closure). *Corrected at final review: this spec first said six — `60 executed − 54 covered`, arithmetic on two totals rather than the set difference, which is 7 because one covered module (`cli/engine/node.py`) is not executed. A count derived instead of measured is the same defect class the rest of this spec retracts.* — and it was exploitable exactly like the hand-list before it: rebinding `build_crossfreq_system_fast` in `cli/engine/__init__.py` made every replay run the verified daily-oracle builder at a **byte-identical** fingerprint with all 64 tests green. `_resolve_module` now emits every ancestor `__init__.py`. Measured after the fix: 61 covered, 60 executed, **0 executed-but-uncovered**.
