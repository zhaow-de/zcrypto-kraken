...
- **Component A falls back to replay for cycles predating the widening**, and says which path it used. It runs on a workstation where 73 s is a cost, not a hazard.
...
- **It reads journaled numbers only** (D9) — that week's cycle artifacts and exec records, all small JSON. No replay, no parquet, no builder, and an explicit exception boundary: any failure to read leaves the trip un-evaluated and logged, never a trip and never a raise onto the trade path.
- **It evaluates on COMPLETE ISO weeks only, at the first boundary after a week closes** — never per cycle. The band is a weekly-mean quantity, and a partial week's mean is not comparable to a full week's; a Tuesday comparison would fire on arithmetic, not divergence. Between closes there is no quantity to test, so nothing trips — the correct behaviour, not a gap.
- **A week with no fills still trips.** The absence of fills inside a week is not missing data once the realized series has started: `held` is non-zero, targets moved, and the drift is fully measured. An engine that stopped placing altogether is the maximal tracking error and the single failure this trip exists for — so "no data" means *the realized series has not started at all*, never *this week was quiet*.
- **Its threshold is the measured band, not a guess.** Until ≥3 complete gate-eligible weeks exist the trip is **configured off** and renders its state, exactly as `00088`'s envelope renders a disarmed gate. `tracking_band_bps` is a parsed, range-refusing config key (`math.isfinite` included — a `nan` band defeats every `>` comparison), not a literal.
- **It is refused entirely while `exec_armed` renders false**, so it cannot fire before there is anything to protect — and the disarmed state is *rendered*, not merely asserted: a gauge in the capture role's keep-regex and a dashboard target, so "disarmed" is confirmable on the board after the converge rather than inferred from an absence.
- **The defect is constructed and seen to trip before it ships** — [[T0018]]'s rule, and the reason the reconciliation family was held out of `00088`: *"a guard whose defect cannot be constructed is unproven by this project's own rule."* Both directions are proven: a constructed over-band week must latch the kill file, and a healthy production-shaped week must not. **The CALL SITE is proven too** — a test drives the executor through a boundary tick and asserts the kill file appears, because a guard that is never called ships green exactly as an always-refusing one does.
- **It ships on the live trade path.** Fable review floor, canary-gated engine converge, and — because it publishes state and widens the cycle record — `--tags capture,engine`, readers before writer.
