...
`zcrypto engine tracking-report`, reading the NAS-pulled engine journal exactly as `accum-replay` does (`--journal-dir`, `--since`, `--until`, `--minimums`, `--gate-from`, `--ledger-export`, `--json`, and a SINGLE-valued `--nav` defaulting to `shadow_nav_eur` — not repeatable like the sibling's, because this command issues one verdict and the size a human bands at must be the size the engine trips at). No timer, no unit, no ops converge, no keep-regex, no alert-rule question, **no canary bake owed** — it ships when it merges. The cadence is weekly and attended, and the consumer is a human reading a decision.
...
- **Readers converge before the writer** (`fleet-deploys.md`): old code meeting the first widened record must not misread it. The reader tolerates the key's absence by construction, which is also what makes the 258 existing artifacts still readable.
