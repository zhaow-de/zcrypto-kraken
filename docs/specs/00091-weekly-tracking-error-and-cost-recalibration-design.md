...
**The engine never computes the floor, and a review round was spent discovering why it cannot.** The floor half needs per-asset venue minimums, which `load_minimums` reads from a **refdata snapshot file** — and the engine host carries no refdata snapshot (only its state dir and config file are mounted); its venue floors come from the nautilus Cache and the committed `COSTMIN`. Routing the trip through `weekly_tracking` → `accumulation_payload` therefore cannot work on the trade path at all, independently of cost: `accumulation_payload` raises on the first asset it has no minimum for.

So the trip is defined against a **configured band**, not a computed floor:

- **The comparison is one weekly mean against one number.** `tracking_band_bps` is set by a human from component A's output, where the floor IS computed, on a workstation that has the snapshot. The engine compares that week's realized mean drift against it. This is what "the measured band" always meant; the earlier draft merely put the measuring in the wrong process.
- **Both components share ONE drift function** taking plain dicts — `final` weights, `closes`, `held`, `nav` — with no `CycleStages`, no minimums, and no `accumulation_payload`. Component A calls it per cycle from replayed stages; component C calls it per cycle from journaled artifacts. One implementation, two callers, so the number the human bands cannot drift from the number the engine trips on.
- **NAV comes from `EngineConfig.shadow_nav_eur`**, which already exists with a parsed, positive-only arm. It is the same NAV the engine sizes against, so the trip measures the portfolio it actually runs.
- **The journaled `final_targets` is pair-keyed twelve**, including the two `/BTC` legs at zero weight; the trip contracts it to the ten EUR bases at its own edge, the same contraction the fill reader performs. One key space per computation, established once.
- **It evaluates COMPLETE ISO weeks only**, at the first boundary after a week closes. A partial week's mean is not comparable to a full week's band, so a mid-week comparison would fire on arithmetic rather than divergence. Between closes there is no quantity to test.
- **A week with no fills still trips** once the realized series has started: `held` is non-zero and targets moved, so drift is fully measured. An engine that stopped placing is the maximal tracking error and the single failure this trip exists for. "No data" means the series never started, never "this week was quiet".
- **Any read failure leaves the trip un-evaluated and logged** — never a trip, never a raise onto the trade path. A week whose cycles lack journaled closes is refused rather than guessed.
- **It ships disarmed** (`tracking_band_bps` unset) and **renders that state** — a gauge in the capture role's keep-regex and a dashboard target, so "disarmed" is confirmable on the board rather than inferred from an absence. It is refused entirely while `exec_armed` is false.
- **The defect is constructed and seen to trip, in both directions, AND the call site is proven** — a guard nothing calls ships green exactly as an always-refusing one does. [[T0018]]: *"a guard whose defect cannot be constructed is unproven by this project's own rule."*
- **It ships on the live trade path.** Fable review floor; canary-gated converge; `--tags capture,engine`, readers before writer. The branch also changes `command.py`, `cycle.py` and `journal.py`, which are inside the NAS gate-export's replay closure — so the NAS leg replays the whole export at its measured **2490 s** cold cost, and that window is sized against 2490, never the smaller figure.
