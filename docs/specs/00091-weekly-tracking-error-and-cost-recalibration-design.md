...
- **Base-keyed, never symbol-keyed.** `replay_stages` "stays BASE-keyed whatever the schema", so `target_qty` is keyed `BTC`; a fill's `intent["symbol"]` is a basket symbol (`BTC/EUR`, and `ETH/BTC`/`SOL/BTC`). The realized side contracts symbols to base keys through the same `select_model_inputs` contraction the cycle uses, or the two sides overlap on nothing and silently report zero drift.
- **Post-decision attribution.** The floor measures drift *after* the placement decision — *"a placing asset contributes exactly 0"*. The realized side attributes a fill to the cycle whose decision produced it, so a placing cycle is compared like with like rather than carrying the full traded delta the floor zeroes.
...
- **Component C's surfaces**, which A and B do not have: the trip's state in the capture keep-regex, a dashboard target, an alert-rule decision (a latched trip already pages via `zcrypto-engine-exec-kill-tripped`, so a second rule is probably wrong), and a runbook section — a trip an operator can meet at 3am owes one, unlike the report.
- The decisions-log entry (phase 6) records D2's, D4's and D9's choices.
...
- **It reconciles against the ledger, and refuses on disagreement.** Every export row it consumes must match a journaled fill by `trade_id`, or be a rollover row for a position the journal carries. An export row matching nothing is an abort, not a warning: it means the account did something the engine's record does not know about, which is the one thing this whole component exists to detect.
...
### D9 — the tracking-error kill-switch trip (component C, live trade path)

[[T0018]]: *"Drawdown trips stay with `00092` and **tracking-error trips with `00091`**, where their inputs are built."* This is an automatic trip in the executor, not a report: it latches the kill file, cancels resting orders, refuses every further intent, and pages.

- **It trips on realized tracking error exceeding the band, evaluated at a cycle boundary** — the same quantity D3 defines, computed in-process, never a number a report handed it.
- **It evaluates on COMPLETE ISO weeks only, at the first boundary after a week closes** — never per cycle. The band is a weekly-mean quantity, and this spec's own rule is that a partial week's mean is not comparable to a complete week's; a trip that compared a Tuesday's running mean against a full-week band would fire on arithmetic, not on divergence. Between closes there is no quantity to test, so nothing trips on tracking error — the correct behaviour, not a gap.
- **Its threshold is the measured band, not a guess.** Until ≥3 complete gate-eligible ISO weeks exist the trip is **configured off** and renders its state, exactly as `00088`'s envelope renders a disarmed gate. Shipping it armed against an unmeasured threshold would page on the first noisy week and teach the operator to clear it.
- **The defect is constructed and seen to trip before it ships** — [[T0018]]'s own rule, and the reason the reconciliation family was held out of `00088`: *"a guard whose defect cannot be constructed is unproven by this project's own rule."* A constructed cycle whose realized drift exceeds the configured band must latch the kill file, and a healthy production-shaped cycle must not.
- **It ships on the live trade path.** Fable review floor, canary-gated engine converge, and — because it publishes its state — the capture role's keep-regex, so its converge runs `--tags capture,engine`. None of that applies to A or B, which is why the three are tiered rather than merged.
- **The trip is disarmed by the same two keys as everything else** and is refused entirely while `exec_armed` renders false, so it cannot fire before there is anything to protect.
