...
- **`cli/archive/` is outside the replay-fingerprint closure.** `gate_cache._replay_code_paths()` resolves to 79 files, none under `cli/archive/`; `cli/config.py` and `cli/__init__.py` **are** inside it. That keeps the FINGERPRINT stable, which is worth doing — but it does not avoid the cold gate replay, because every converge that RECREATES the container wipes the ephemeral `/tmp` cache and replays anyway (measured on both legs; see the deploy section).
...
Both legs are `--limit nas`, outside the canary regime (no secondary, no bake owed). **Both DO pay the cold gate-export replay** — measured, after this was written: the cache is `--cache /tmp/gate-cache.json` and `/tmp` is on none of the container's mounts, so every `-e nas_apply_compose=true` recreate takes it, whatever the replay fingerprint says. Leg A's cycle went quiet for 3534 s and leg B's for 3565 s, against the 3505 s recorded for a cold export. Keeping `cli/archive/` outside the fingerprint closure was still correct; it simply buys nothing when the converge recreates the container anyway.
