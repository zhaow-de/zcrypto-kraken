...
**Per submission, not per cycle.** This is the correction that motivates the design. A post-only order that rests and later crosses is a *second* submission decision taken minutes after cycle entry, when the arm file, the kill switch and the venue status may all have changed. A gate evaluated once at cycle entry would be bypassed by exactly the repricing path it exists to guard. So `evaluate()` is specified to be cheap (two `Path.exists()` calls and a cached venue read) and free of side effects, and 00090 calls it immediately before **every** submission.
...
- **The streak invariant, tested directly**: a synthetic day whose every execution artifact records a refusal still scores clean under `evaluate_gate`.
...
- **Live drill on the host, by outcome**: with the engine converged, create the kill file and watch `zcrypto_exec_kill_tripped` flip 0→1→0 in Grafana Cloud, the same end-to-end shape as the reboot-detector proof; and confirm the resting verdict reads `none` — `zcrypto_exec_gate_level` at 0 — with `reasons` naming both the absent arm file and the restart hold, which is what a correctly disarmed engine looks like.
