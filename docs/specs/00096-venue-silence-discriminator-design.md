...
**D2b — the interior is the inter-window GAPS, and the silence must dominate the episode.** Two constructed defects closed, both of which promoted `venue_silent` on evidence D3 was written to refuse:

- *Disjoint blips.* Defining the interior as the whole span from the first window's end to the last one's start puts the dense healthy traffic between two far-apart windows inside it. Constructed: two 40 s fleet-dark windows 2,660 s apart — the both-host crash-loop shape, since the canary rule puts the same image on both hosts — read `venue_silent` on 1,330 healthy updates, while each blip alone is exactly the single-window shape D3 keeps `undetermined`. Fixed twice over: the interior is the union of the inter-window gaps, and `venue_silent` additionally requires `interior_seconds < dark_seconds` — the silence must dominate, or these are separate events with healthy traffic between rather than one episode. Measured against the real event, that bound is not tight: 2026-08-20 has 98.497572 s of interior against 420.524664 s of dark, a 4.3× margin.
- *Unlanded mirrors.* A pair with only one readable mirror was silently skipped, so a single agreeing pair could carry the verdict. Single-mirror hours exist **by construction** (a pair is added primary-first), and `_decided` writes this record once — so a mirror landing a cycle later that would have shown divergence could never demote it. Any skipped pair now caps the verdict at `undetermined`, and `pairs_skipped` is recorded.
