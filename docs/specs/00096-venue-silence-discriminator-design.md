...
| --- | --- | --- |
| Kraken WS `status` channel (received by capture, archived nowhere) | announced the ladder | announced the ladder |
...
Costs measured rather than assumed: `fleet-deploys.md`'s **readers-before-writer** converge ordering is **not owed** — the only reader of `reconcile-ledger.jsonl` is `cli/archive/command.py` itself, the NAS transports it without parsing, and every read is `record.get(...)` with a default, a pattern `_booked_dark` already uses to tolerate records written before an earlier widening. No **Alloy keep-list edit** is owed either: the ops keep-list admits `zcrypto_reconcile_.*` as a prefix family. `gate_cache.py`'s replay fingerprint is untouched — `cli/archive/command.py` is measured absent from the 74-file transitive `cli.*` replay closure — so no NAS converge pays a FINGERPRINT-driven cold gate-export replay. It does not follow that no converge replays: one run with `-e nas_apply_compose=true` recreates the container and wipes the ephemeral `/tmp` gate cache, which replays cold regardless (measured 2026-08-29; see `docs/reference/fleet-pins.md`'s standing constraint).
...
**The triage line names the signal this rule cannot reach, for the `undetermined` case**: `zcrypto_capture_venue_status_total` counts venue status messages by reported system state, and a series for anything other than `online` is itself the signal. That is what identified 2026-08-06, and 2026-08-20 as well: the `status` channel announced both, so it is the one signal with a clean record on every episode since it began being counted. This design's own rule reaches only the second. An operator reading `undetermined` at 3am must be sent there rather than left to derive it.
...
- It does not see a pair whose hour is absent from **both** mirrors. Such a pair never enters the classifier's input at all, so it neither agrees, diverges, nor counts as `pairs_skipped` — the cap that exists precisely because a mirror landing next cycle could have shown divergence. The asymmetry is accepted rather than closed: a per-pair both-mirrors-absent hour beside healthy siblings is `total_loss` territory, which pages on its own, and the exposure here is triage-only.
- It does not close the interior entirely. Up to `INTERIOR_MAX_SECONDS` of genuinely healthy traffic can still sit between two large dark windows and read `venue_silent` — two independent incidents 299 s apart is the shape. The exposure is BOUNDED (it was unbounded before D2b) and is **accepted, not deferred** — no topic tracks it, deliberately. A wrong verdict here changes the booking by nothing, the page by nothing, and the seconds by nothing; it costs a triage hint. The named remedy, should it ever matter, is the density test D2b rejected on complexity — recorded here so the reasoning is not re-derived from scratch, not as work owed.
...
- **Retroactive venue-status input to the reconciler.** The public endpoint is **current-state only**, so a reconciler running hours later cannot ask it about a past hour; and the WS `status` channel — which capture *does* receive, and which announced both known events — is counted into `zcrypto_capture_venue_status_total` but written nowhere the reconciler can read. Archiving it was weighed and **dropped rather than deferred**: 2026-08-06's two dark windows are *adjacent* (the first ends exactly where the second begins), so its interior span is a zero-length instant and no status row could fall inside it; and with `effective_time` observed only as `None` and the frame carrying no venue timestamp of its own, an archived row would hold **local receipt time**, inheriting none of the cross-host verifiability that makes this discriminator sound. What the operator needs already exists: the venue alert, the capture log's ladder, and D6's hygiene-map row.
