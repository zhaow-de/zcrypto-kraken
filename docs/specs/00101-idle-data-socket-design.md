...
**Config surface, measured.** `ws_idle_timeout_ms=0` genuinely disables the timer — 190 s keyless, 0 timeouts, 0 reconnects, through three of Kraken's ~60 s inactivity windows, the heartbeat keeping the socket alive. `None` silently falls back to 10000. `subscribe_instruments(KRAKEN)` is **not** a sentinel — the timer fired 10.0 s after "Subscribed KRAKEN instruments". `transport_backend=TUNGSTENITE` changes nothing (4/4 timeouts on both backends).
...
- **The guard's true positive:** a real reconnect must still log. Provoked keylessly by killing the socket from the client side once and reading `Reconnecting` → `Reconnect succeeded` exactly once.

## Deploy

A is an engine re-pin and follows `capture-deploys.md` in full: attended, inside the 4-hourly inter-cycle gap, canary-gated on the secondary's capture bake, `--tags capture,engine` with both `-e` digests at their currently-running values, verified at the next boundary. It ships **before** the first armed probe pass, never during it — the change is disarmed-only in effect, and the arming record (00100 D11) is reconciled against the moving pin.
