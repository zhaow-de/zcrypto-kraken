...
- **Keyless, local:** a data-only `LiveNode` (`exec_enabled=False`) at `ws_idle_timeout_ms=0` runs ≥ 180 s with 0 `Read idle timeout` and 0 `Reconnecting` lines — the existing 190 s run is the reference (0 timeouts, 0 reconnects, 1 `Connected: client_id=KRAKEN`).
