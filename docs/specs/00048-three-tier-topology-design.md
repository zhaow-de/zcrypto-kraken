...
### NAS share ACL (one-time prerequisite)

The share's inheritable **default ACL** on `/volume1/ZhaoCrypto` must yield new **dirs `0775`** and **files `0664`** for user `zcrypto`, group `zcrypto`, and group `administrators`, so the containers (`1000:1000`), the workstation mount, and admins can all read/write the archive. **The current default ACL is wrong**: its three inheritable entries (`user:zcrypto`, `group:zcrypto`, `group:administrators`, all `rwxpdDaARWc--:fd--`) grant `rwx` **with the execute bit** and file-inherit it, with **no other-read** entry — so a file `touch`-ed from the workstation lands `-rwxrwx---+` (mode 770, executable) on the NAS view instead of `0664`. The fix (via `synoacltool`, applied when the NAS work begins — the `ssh nas` identity `zcrypto-run` is an `administrators` member): set **file-inherit** entries *without* execute (→ `0664`) and **dir-inherit** entries *with* execute (→ `0775`) for the three identities, plus the other read/execute bits. **Acceptance test**: a file `touch`-ed from the workstation reads `-rw-rw-r--` (`0664`) on **both** the workstation and NAS views, and a new subdirectory reads `drwxrwxr-x` (`0775`).
