...
What changes is the record, and **it has to be written, because nothing carries it today**: `fleet-pins.md`'s capture rows still name the 2026-08-28 digests and hold no row for the 2026-09-01 re-pin, so a paged operator has no provenance for the `1` anywhere. The durable home is the runbook entry a page links, as a dated note beside the corrected baseline — it is read at exactly the moment the question is asked. The pins map is re-trued by whoever performs the re-pin, from host reads; this spec does not author that row.
...
- **A control that must stay silent**: measured, the selector returns exactly ONE series — the primary publishes neither counter, because it does not yet carry the image. So the control is **the sibling counter on the same host** (`hour_finalized_early_total`, 0 under the same absolute-value form), and the two-host control is re-taken once the primary is re-pinned. An absent series and a series reading 0 are different states, and `noDataState` decides which one pages.
