...
| **`zcrypto-data`** | m2m: runs the data containers, owns the data, does/serves the m2m pulls; **no sudo, no tty, nologin** | ✔ *(capture ctr)* | ✔ | ✔ *(poller/reconciler/panel)* | ✔ *(= renamed `zcrypto`, uid/gid 1000)* | — |
...
- **D2 — `zcrypto-data` is the single m2m identity.** It runs every data-path container (`--user`), owns every data tree, and both *initiates* pulls (the NAS's archive-pull) and *serves* them (the forced-command exports on the producer hosts). No sudo, no tty, `nologin`. It needs no docker-group/rootless access (root/systemd invokes docker; the container's identity is `--user zcrypto-data`).
