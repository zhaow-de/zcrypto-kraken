...
- **Healthcheck ping** (reusing the engine's `_ping_healthcheck` pattern, `cli/engine/cycle.py`; `--healthcheck-url` sourced from a `GATE_HEALTHCHECK_URL` compose env → a **new, dedicated** healthchecks.io check, distinct from the engine's own): ping `<url>` when `clean` — the gate is healthy (no fresh break in the most recent complete day, any reason: missing cycle, late cycle, mismatch, validation failure, or sidecar) **and** the journal is fresh (`lag ≤ lag-fail-seconds`) — ping `<url>/fail` otherwise. `evaluate_gate`'s `streak`/`last_failure` already carry this: `streak > 0` means the last complete day was clean (at any streak length, including a gate that broke earlier and has since recovered); `streak == 0` with no `last_failure` means no complete day is evaluable yet (early phase, liveness only); `streak == 0` with a `last_failure` means the most recent complete day broke. `--lag-fail-seconds` defaults to **`18000` (5 h)** — one 4 h cycle plus the hourly-pull margin. This is the **independent** paging path (see §Alerting).
