...
- **D8 — emit `zcrypto_gate_export_duration_seconds`** (wall). [[T0069]]'s entire budget table is a linear extrapolation from one datapoint on the Atom; the step's real cost had never been measured in production when this was decided (the first one-off production measurement — 649.85 s, 2026-07-20, see D9's correction — arrived with the cache deploy; the continuous metric this decision adds is what a one-off cannot provide). This closes that, and makes the effect of enabling `--cache` observable rather than inferred. Both new metric names match the existing Alloy keep-regex `zcrypto_gate_.*`, so no infra change is required.

- **D9 — the cache file is container-ephemeral, and never on `/archive` or any share both hosts can reach.** The NAS image builds with `POLARS_RUNTIME=compat` (`polars-runtime-compat`, no AVX — the Atom Goldmont has none) while ops runs the default AVX runtime; same polars *version*, different compiled runtime (`infra/docker/Dockerfile`, [[T0029]]). `replay_fingerprint` digests neither the runtime nor polars, deliberately ([[T0074]]'s closure argued the decode path fails loudly via the content hash — which is true for a *replay*, and precisely what a cache hit skips). Two hosts sharing one cache file would therefore compute **identical** replay fingerprints while potentially producing different numeric results, so each could serve the other's entries as its own. Ephemeral siting makes that unreachable by construction rather than by a fingerprint field nobody has added. The cost is one cold rebuild per container restart — the same cost as a no-cache run on that host.

  **CORRECTED 2026-07-20 (measured at the first post-re-pin pull):** the cold rebuild on the NAS is **~650 s** (measured 649.85 s over 60 cycles, 10.8 s/cycle), not the "~94 s" this decision first stated. The ~94 s was the **ops i7** figure (T0069's 1.603 s/cycle × the then-current ~59 cycles ≈ 94.6 s) used in a decision about the **NAS** container — whose own rate this topic's evidence had already measured at ~10.9 s/cycle. The number was derived on the wrong host, not mismeasured; the decision itself is unaffected (the equality claim — cold-with-cache ≈ no-cache — was verified on the i7 at 62.77 vs 63.11 s and holds on the NAS at 649.85 s vs the ~630 s no-cache history), and the 3600 s pull interval absorbs 650 s with the dead-man window at 3 h. What does not survive is quoting this paragraph for the cost of a NAS container restart: use ~650 s.
