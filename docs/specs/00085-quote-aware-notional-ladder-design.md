...
**This was a real fork, resolved deliberately.** A shorter window ending 2026-08-06 06:00Z would exclude the maintenance entirely and is uncontaminated — but it spans 13.67 days and drops Phase 2's ≥2-week exit-bar row. The trade was taken toward the longer window: a 17-minute frozen-book interval is **disclosable** (~0.0013 % of the window's 1,314,000 seconds, an order of magnitude smaller than the 0.602 % the superseded table already carried), whereas a regressed exit-bar criterion is invisible and unchecked. Whoever revisits this should not "clean up" the caveat by shortening the window back.
