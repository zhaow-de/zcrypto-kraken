...
single-asset BTC?* The fixed-window B2 (`docs/research/04.phase3-benchmark-b2-basket-report.md`) said the basket
...
lines use a 10%/yr target (`vol_target(..., target_vol=0.10 / 365**0.5, lookback=30, max_leverage=1.0)`).
...
1. **All four CIs are strictly positive** (lower bounds 0.40–0.44). Over the full history every
   ...
   where all six CIs straddled zero (`docs/research/04.phase3-benchmark-b2-basket-report.md`). The full 2013–2026
...
- **Even the 4-major ~9-year proxy's "residual structural drag" (Sharpe 0.798 < BTC 0.929,
  `docs/research/04.phase3-benchmark-b2-basket-report.md`) does not persist** in the full 2 → 10 dynamic basket. The
...
These match `docs/research/04.phase3-benchmark-b2-basket-report.md` line-for-line — the cross-check that the dynamic
