...
| ops timers | zcrypto-ops | `zcrypto-{archive-pull,panel-materialize,tape-bars,verify-replay,verified-replay}.service` — ephemeral `docker run` per tick (+ grafana-watchdog, grafana-keepalive: plain scripts); `.prom` outputs under `/var/lib/zcrypto-ops/textfile` | scraped by the ops Alloy textfile collector |
