...
| ops timers | zcrypto-ops | `zcrypto-{archive-pull,panel-materialize,tape-bars,verify-replay,verified-replay}.service` (+ grafana-watchdog, grafana-keepalive) — ephemeral `docker run` per tick; `.prom` outputs under `/var/lib/zcrypto-ops/textfile` | scraped by the ops Alloy textfile collector |
