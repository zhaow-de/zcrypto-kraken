...
- The bridgehead's sshd allows two authentication tries, so `infra/ansible/scripts/run.sh` offers its deploy key first when `--limit zaccess` names it, and it converges through `converge.sh` like the other hosts. Its Alloy is a native deb followed from apt — no digest, no pins row, no bake: `zaccess-alloy-converge` on the same page. The mTLS client pins are PEMs under `roles/access/files/pinned-leaves/`, globbed into the Caddyfile at converge; revoking one is `zaccess-revoke-client-cert`.
