...
- **The bridgehead's Alloy is a native deb followed from apt, not a pinned image** — no digest operand, no pins row, no bake: `infra/runbooks/observability.md`'s `zcrypto-alloy-dark` carries what converging it takes.
...
**Inducing a fault on a throwaway subject, and what a drill's latency does and does not prove**, are `infra/runbooks/drills-telemetry.md`'s standing rules.
