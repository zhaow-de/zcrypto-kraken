...
- Route53: `zaccess.zhaow.me` is its own hosted zone, delegated from `zhaow.me`; the apex `A`/`AAAA`, the `4.` `A` (the ops-side WireGuard `Endpoint`, so no `AAAA`), the `tmux.` and `nas.` records and the `CAA` live in that child zone, created by hand (spec `00075` D18) — an apex record in the parent beside the `NS` delegation is shadowed.
