...
A live spot-check before a full recalibration, from the workstation: `kraken orderbook <pair> --count 500 -o json` walked to each rung by `fill_bps`'s own rule (VWAP against the mid, bps per side) beside the mean of `kraken spreads <pair> -o json`'s half-spreads. One snapshot is one sample of a fifteen-day mean, so it flags an order-of-magnitude break and never restamps the table.
