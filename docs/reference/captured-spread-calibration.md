...
| **both** | **12,304** | **1,025.3** | **0.0780 %** |
  ...
  The sweep was exhaustive — all 4,380 hour-files present, and **exactly two** of the window's 365 hours carry any frozen row at all, so there is no third interval to find. The figure previously quoted here, "~1,020 s is 0.078 %", was attributed to 2026-08-06 alone: it is almost exactly right for **both together** and overstates that one interval by about a fifth, because it used the venue's 17-minute announced span rather than the frozen rows the panel actually holds. Same shared-gap class as the superseded window's 0.602 %, and far smaller. A third episode, 2026-07-13, predates this window's start by ten days and cannot touch these means. It is all disclosed rather than trimmed because the alternative — ending the window before 2026-08-06 — costs the ≥2-week exit bar (spec `00085` D4). Do not "clean this up" by shortening the window.
