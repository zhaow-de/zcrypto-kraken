...
- **TWO frozen-book intervals are inside this window and neither is excluded from the means.** Both are Kraken venue maintenances beginning within seconds of 07:01 UTC and published days ahead ([[T0145]]): **2026-07-27** and **2026-08-06**. In each the venue emitted nothing, neither capture daemon restarted, and no archive gap was booked — so the panel's 1 s grid carries those seconds computed off a **frozen** book rather than showing a hole. Measured from the panel itself rather than from the venue's reported status-ladder span, by counting rows with `stale_seconds > 30` — the [[T0104]] column that separates a genuinely quiet second from a fabricated one — across every hour of the window and all twelve pairs:
  ...
  The sweep was exhaustive — all 4,380 hour-files present, and **exactly two** of the window's 365 hours carry any frozen row at all, so there is no third interval to find. The figure previously quoted here, "~1,020 s is 0.078 %", was attributed to 2026-08-06 alone: it is almost exactly right for **both together** and overstates that one interval by about a fifth, because it used the venue's 17-minute **reported status-ladder** span (`maintenance` 07:01:02 → `online` 07:18:18) rather than the frozen rows the panel actually holds — and note that is not the *announced* window either, which was 900 s ([[T0145]]). Same shared-gap class as the superseded window's 0.602 %, and far smaller. A third episode, 2026-07-13, predates this window's start by ten days and cannot touch these means. It is all disclosed rather than trimmed because the alternative — ending the window before 2026-08-06 — costs the ≥2-week exit bar (spec `00085` D4). Do not "clean this up" by shortening the window.
