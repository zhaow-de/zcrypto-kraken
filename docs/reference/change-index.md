...
**What this file does not record, decided rather than left open.** Because the row is parsed at create time from the branch name, the title and the `## Spec / Plan` section, a pull request that resolves or archives a topic without naming it in one of those three records no key for it: the index captures what a pull request was called, not everything it did. The owner's decision of 2026-09-22 is that the rule binds from here and the history stands: the alternatives were a mechanical completion behind a test reading git rather than the parse, or a topic, and neither buys back a record of what was already delivered. A reader wanting every topic a pull request touched reads `git log`, and the topic index is what records a registration.
...
| #607 | 2026-09-24 | docs(specs): 00118 the engine's cache, spec and the infrastructure plan | — | 00118 | — |
