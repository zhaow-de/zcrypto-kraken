...
One row per merged pull request whose BRANCH NAME spells a key, and one per pull request that delivered one; a row is written when the pull request is created, so the file also holds rows for keyed pull requests still open. The cells carry only what the pull request delivered — for a row written before the pull request completes, what it delivers so far, re-derived by `open-pr` when its title or keys change — so a pull request whose branch spells a topic it merely registered keeps its row with every cell an em dash — `test_change_index.py::test_every_keyed_merge_on_develop_has_a_row` reads the branch name, not the cells, and #133 and #161 are that shape; a registration spelled only in the title takes no row at all. A key is an `iter-NNN` iteration serial, a 5-digit spec serial, or a `T<NNNN>` topic id, an iteration serial read from the pull request's own title and branch name, a spec serial or topic id from those and its `## Spec / Plan` section — never from a sentence citing an earlier iteration as a precedent, and never a topic the pull request only registers — both are cross-references, not deliveries, and a registration is what the topic index records — a topic key case-insensitively, since a branch spells it `t0189`, and written with an upper-case T; a cell with no key of that kind is an em dash. The backfilled rows also drew on two sources a pull request does not hold — the iterations-history entry git keeps for the row's iteration, and the trial registry's `spec_hash` resolved through the digest of the spec file it pins — so a cell can name a spec or topic the title does not spell. No file path appears in this file (`test_change_index` asserts it), so a rename sweep has nothing to edit here — a path-shaped title has its `/` written as `-`, a bare `long/flat` keeps it, and titles are truncated to 72 characters. `date` is the date the pull request was created, UTC — which is the day the row is written when the row is written at create time, and is not when it is written later. Rows are sorted by pull-request number ascending. The row is written by the `open-pr` skill when the pull request is created, which is also where an iteration's serial comes from: this file's highest `iter` plus one. `test_change_index` guards its shape and its completeness against git.

**What this file does not record, decided rather than left open.** Because the row is parsed at create time from the branch name, the title and the `## Spec / Plan` section, a pull request that resolves or archives a topic without naming it in one of those three records no key for it: the index captures what a pull request was called, not everything it did. Measured on 2026-09-14 over every first-parent merge on `develop`: 432 delivery pairs in git against 192 here, of the 240 missing 76 in 33 existing rows and 164 in 19 pull requests with no row at all — the bulk archiving rounds, merged under keyless names — and 27 of 191 not-open topics carry no row. The owner's decision of 2026-09-22 is that the rule binds from here and the history stands: the alternatives were a mechanical completion, 33 rows edited and 19 created behind a test reading git rather than the parse, or a topic to carry it, and neither buys back a record of what was already delivered. A reader wanting every topic a pull request touched reads `git log`, and the topic index is what records a registration.
...
| #387 | 2026-09-04 | [WIP] feat(engine): 00111 — the adapter's blind reads; parked pending na | — | 00111 | — |
...
| #587 | 2026-09-21 | fix(engine): the store's shortfall hint routes by the fetch's reach | — | 00117 | — |
