...
- The engine must not be re-pinned to the digest the capture pair now runs — capture carries it on both hosts; the engine does not. From `2.0.0rc6.dev20260915` the Kraken adapter resolves fees through an authenticated `POST /0/private/TradeVolume` at each credentialed instrument load, and a failure of that call aborts the whole listing: the execution client fails to connect, the account is absent from the Cache, and no boundary cycle completes. **The denial is INTERMITTENT, and that is measured rather than reasoned** — probing this account gave one `EGeneral:Permission denied` in ten identical well-formed requests, with the larger request that followed it succeeding; request encoding, payload size and asset class were each refuted in the same run. So a re-pin may appear to succeed and fail at a later restart, which is why this is a standing refusal rather than a retry. It lifts when upstream makes that fee resolution non-fatal — the public-fee path it would fall back to already exists for uncredentialed clients. The capture tier makes no such call and is unaffected.
...
- `7ccd97eb7e3c` = `sha256:7ccd97eb7e3c134bd51b09caf9182dc64642fbf7c76b795fe1c9d6c79c51679e` — revision `8d00ead9`; capture on both hosts. Refused by the ENGINE: its exec client fails to connect on it
