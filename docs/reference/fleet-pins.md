...
- **A fingerprint-invalidating NAS pin replays the whole gate-export** — anything touching `journal.py` changes `gate_cache.py`'s fingerprint, and the cold export measured **3363 s** over 280 cycles (2026-08-26), not the incremental ~627 s: size converge windows against the newest figure, which grows with the journal.
