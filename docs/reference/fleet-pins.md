...
- The engine must not be re-pinned to the digest the capture pair now runs. From `2.0.0rc6.dev20260915` the Kraken adapter resolves fees through an authenticated `POST /0/private/TradeVolume` at each credentialed load, and a failure of that call aborts the whole listing: the exec client fails to connect and no boundary cycle completes. **The denial is INTERMITTENT, measured** — one `EGeneral:Permission denied` in ten identical requests, the larger one after it succeeding; encoding, payload size and asset class each refuted. A re-pin may look fine and fail at a later restart. It lifts with `nautechsystems/nautilus_trader#5005`, which falls back to public fees. Capture makes no such call.
