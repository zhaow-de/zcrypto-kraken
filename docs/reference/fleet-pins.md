...
- **ops current**: `sha256:0b2fdcfff6e873b71661fb50cc42c83858394827edc69ca813f5d121deab3d19` (revision `50fc4979`) — the timers converged 2026-07-28 15:59, and the liquidations poller was rolled to match in the same window rather than left armed.
- **ops prior, and the rollback operand**: `sha256:e5a44e1cb138bcc0d6291fc8be76d00375ce69512e63da532389d3c4821bd8b7` — read from the host 2026-07-28 because **this file had no ops row at all until then**, which by its own opening rule meant the operand existed nowhere in the repo. `ops_image_digest` has no repo default by design, so this row IS the record. **One variable drives two things**: the four timer runners *and* the liquidations compose file — which the role renders but never restarts, so after a converge the file points at the new digest while the container still runs the old one, until some later `docker compose up` silently rolls an unbackfillable stream.
