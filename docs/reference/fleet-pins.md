...
- The engine must not be re-pinned to the digest the capture pair now runs. From `2.0.0rc6.dev20260915` the Kraken adapter resolves fees through an authenticated `POST /0/private/TradeVolume` at each credentialed instrument load, and a failure of that call aborts the whole listing: the execution client fails to connect and no boundary cycle completes. **The denial is INTERMITTENT, measured** — one `EGeneral:Permission denied` in ten identical requests, the larger request after it succeeding; encoding, payload size and asset class each refuted. So a re-pin may look fine and fail at a later restart. It lifts when upstream makes that fee resolution non-fatal. Capture makes no such call.
