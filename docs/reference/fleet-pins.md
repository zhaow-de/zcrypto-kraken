...
- The engine must not be re-pinned to the digest the capture pair now runs — capture carries it on both hosts; the engine does not. From `2.0.0rc6.dev20260915` the Kraken adapter resolves fees through an authenticated `POST /0/private/TradeVolume` at each credentialed instrument load, sent as JSON where the other private calls are form-encoded, and asking for all 1,627 pairs unbatched; this account answers `EGeneral:Permission denied`, so the listing aborts, the exec client fails to connect, and no boundary cycle completes. Measured at the re-pin and undone by the rollback that followed it on the same key, which connects. The capture tier makes no such call and is unaffected.
