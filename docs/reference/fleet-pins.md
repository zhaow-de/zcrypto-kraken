...
- `infra/ansible/scripts/converge.sh` appends one line per real pass to `deploy-log.jsonl` beside this file (`tests/test_converge_sh.py`); a row's digest is re-trued from that line, not from memory, and its `since` is read off the container.
