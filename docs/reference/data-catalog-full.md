...
Reconstructed from Kraken OHLCVT 1-minute dumps (base + quarterly) at the NAS mount's `kraken-ohlcvt-updates/` (`resolve_ohlcvt_source_dir`, `cli/config.py`); generated 2026-07-07T21:17:30.846941+00:00 (⏱).
Cadences 1h/4h/1d reconstructed from 1-minute bars (vwap = Σ(close·vol)/Σvol, a proxy — the dumps carry no vwap).
The Q2 2026 extension is the sibling `ohlc-full-20260920`, the same reconstruction through 2026-06-30; this unstamped set stays the frozen canonical every trial pins.
...
12 pairs, full dump history (BTC from 2013-09) at 15-minute cadence, ~3.12M bars, derived from the 1-minute OHLCVT dumps via `cli/backfill/substrate15m.py` (iter-085, spec/plan `00044`). Dataset root `data/ohlc-15m/`; `basket_sha256` `0fed24a6…`. Tick-reconciled bit-exact against Q1-2026 windows across all 12 pairs (100,759 comparisons); the 15m→1h seam is bit-identical on prices, ≤1 ULP on volume, zero Int64 count-leg mismatches over 25,909 hours. Consumer: the B1 seasonality-conditioning family, trials 45–46 ([[T0022]]). The Q2 2026 extension is the sibling `ohlc-15m-20260920`, through 2026-06-30.

### `ohlc-reach`

**A mixed set by construction — read the manifest before treating any file as continuous.** Produced by `zcrypto data rebuild ohlc-reach` (`cli/ohlc/reach.py`, T0065), which carries the newest frozen `ohlc-full` sibling (`resolve_canonical_root`, `cli/data/rebuild.py`) forward from Kraken's public REST OHLC window rather than from a dump. It exists because the dump timetable does not fit the data's expiry: REST serves only the most recent ~720 bars per interval, so a bar not captured before the window recedes past it is unavailable until the next quarterly dump.

The window's reach depends on the interval (~720 d daily, ~120 d at 4h, ~30 d at 1h, ~7.5 d at 15m), so a round meant to keep an interval continuous runs within that reach of the canonical tail, and each series lands one of two ways — the **filename is the contract**:
...
Dataset root `data/ohlc-reach-<stamp>/` — every `data rebuild ohlc-reach` mints a dated sibling, and the manifest's hashes carry the identity, so a backtest pins the hash, never the path; the unstamped `hot/ohlc-reach/` the hub carries is the first round, promoted under that name once and never refreshed since. First promoted run (identity digest `356826a172dd33ca…` over the **continuous** series, `subset_sha256["detached"]` `dc8beef0e01bfb40…` — the two are deliberately separate hashes, so the set's identity never absorbs a segment the round refused to join; per-series `sha256`/`rows`/`first_ts`/`last_ts` accompany each row, and the manifest now declares which digest identifies it rather than leaving that to a consumer's per-set knowledge, per spec `00099`). A record pinning the pre-2026-08-24 continuous digest `8a826898241a5f1e…` names the same bytes: the manifest preserves it under `provenance.legacy` and the converter refuses the whole set unless every per-series hash matches what the legacy manifest attested. The detached value is unchanged. 30 series, **20 continuous** (daily → 2026-07-22, 4h → 2026-07-23T12:00, closing the 2026-03-31 → 07-08 hole at both grids with zero irregular steps after the seam) and **10 detached** (every 1h series, gap ≈ 83.7 d, a standalone 720-bar segment 2026-06-23 → 2026-07-23). Consumers: the intraday families via the daily/4h tail, and the universe rebuild — wired 2026-08-11 (iter-136, spec `00093`, [[T0093]]): `_refresh_universe` resolves its source through `resolve_ohlc_source`, which reads the newest stamped `ohlc-reach-<stamp>` sibling directly and ignores this unstamped canonical name (the hub's promoted copy is frozen by the additive-only transport), falling back to `ohlc-full` only when no stamped sibling exists. A fresh reach round is therefore a precondition of each universe refresh — the per-symbol 7-day staleness budget refuses anything older, `ohlc-full` included. The newest round is `ohlc-reach-20260920`, over `ohlc-full-20260920`: daily and 4h continuous, 1h detached.
...
**Null semantics.** `sum_open_interest`/`_value` are **complete (zero nulls)**. The four ancillary ratio columns carry genuine Binance gaps, absent in two forms (bare-empty and quoted-empty `""`) that parse to **null**, so a missing auxiliary ratio never discards a valid OI reading; a consumer must treat the ratio columns as nullable and the OI columns as dense. **Dense is about nulls, not about zeros**: `sum_open_interest` is exactly `0.0` on **2,329** rows and `sum_open_interest_value` on **2,430**, every symbol represented in both. The two sets **nest rather than coincide** — every `sum_open_interest` zero carries a zero notional on the same stamp, and **101 further rows** read a zero notional against a healthy positive `sum_open_interest` (one incident, 2023-04-10 08:25→09:40 UTC, all ten symbols at once) — so a count taken on one OI column says nothing about the other. What wrote them is not established, and the account-ratio columns do **not** mark them: of the 2,329, **none** has all four ratio columns null and only **338** have the three account ratios null, so nothing may detect them by ratio absence — the predicate is the zero itself. They arrive in 647 runs, 497 of them a single 5-minute bar and the longest 116 consecutive bars identical across all ten symbols; a perp with zero open interest is not a market. A consumer maps them to null before treating them as a level whatever wrote them, because a zero level puts `log(0)` and a division by zero on real rows. The opposite holds for `sum_taker_long_short_vol_ratio`, whose **45** exact zeros all sit on healthy-OI bars and are real all-sell readings.

**The gaps are ONE YEAR, not an early-history artifact — measured 2026-09-02.** BTCUSDT's `count_toptrader_long_short_ratio` is **87.3 % null across 2022** (13,387 of 105,120 rows present, none before 2022-01-30) and ~0 % in every other year. The four ratios are not interchangeable either: whole-series on BTCUSDT, `count_long_short_ratio` 0.9 % null, `sum_taker_long_short_vol_ratio` 6.0 %, and both toptrader ratios 14.9 %. **Inside 2022 the four diverge by an order of magnitude** — measured panel-wide over that year's 1,051,199 rows, both toptrader ratios **87.24 %** null, `sum_taker_long_short_vol_ratio` **35.03 %**, `count_long_short_ratio` **5.09 %** — so the 87 % figure belongs to the toptrader pair alone, and `count_long_short_ratio` is 94.9 % *present* through 2022. **A consumer of either toptrader ratio is blind through 2022** — the bear market and the FTX collapse — so a backtest reading a statistic off that column computes it on a sample missing that regime, and a cross-validation fold covering 2022 is near-empty. Check coverage per year, never in aggregate: 14.9 % overall reads as a nuisance and hides a missing year.
