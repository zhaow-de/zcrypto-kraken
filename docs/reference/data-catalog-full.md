...
**The gaps are ONE YEAR, not an early-history artifact — measured 2026-09-02, and this line previously said otherwise.** BTCUSDT's `count_toptrader_long_short_ratio` is **87.3 % null across 2022** (13,387 of 105,120 rows present, none before 2022-01-30) and ~0 % in every other year. The four ratios are not interchangeable either: `count_long_short_ratio` 0.9 % null, `sum_taker_long_short_vol_ratio` 6.0 %, and both toptrader ratios 14.9 %. **A consumer of either toptrader ratio is blind through 2022** — the bear market and the FTX collapse — so a backtest reading a statistic off that column computes it on a sample missing that regime, and a cross-validation fold covering 2022 is near-empty. Check coverage per year, never in aggregate: 14.9 % overall reads as a nuisance and hides a missing year.
