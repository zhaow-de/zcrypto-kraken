...
Dataset root `data/ohlc-reach/` — the canonical, **unstamped** name, like every other live set. `data rebuild ohlc-reach` mints a dated sibling (`ohlc-reach-<stamp>`) which is promoted into the canonical name once verified; the path is stable and the manifest's hashes carry the identity, so a backtest pins the hash, never the path. First promoted run (`basket_sha256` `8a826898241a5f1e…` over the **continuous** series, `detached_sha256` `dc8beef0e01bfb40…` — the two are deliberately separate hashes, so the basket hash never absorbs a segment the round refused to join; per-series `sha256`/`rows`/`first_ts`/`last_ts` accompany each row, matching `backfill_basket`'s manifest shape): 30 series, **20 continuous** (daily → 2026-07-22, 4h → 2026-07-23T12:00, closing the 2026-03-31 → 07-08 hole at both grids with zero irregular steps after the seam) and **10 detached** (every 1h series, gap ≈ 83.7 d, a standalone 720-bar segment 2026-06-23 → 2026-07-23). Consumers: the intraday families via the daily/4h tail, and — once wired, an owner decision under [[T0025]] — the universe rebuild's freshness budget, which this set satisfies at 1 day against the 7-day limit where `ohlc-full` fails closed at 114 ([[T0093]]).
...
- **Scope: `EUR`-quoted pairs only** (`PANEL_QUOTE`). The `fill_bps_*` ladder walks `price × qty` in the pair's QUOTE currency, so on a BTC-quoted pair the 100/1k/10k rungs ask BTC notionals — orders of magnitude past those books' depth — and all six columns return null. BTC-quoted hours written before the scope existed are deleted rather than carried; they are `f(raw)` and the raw is retained, so [[T0092]]'s calibration iteration rebuilds them once the ladder is quote-aware. The materializer **refuses** a tree holding out-of-scope hours, since a scoped sweep can never bring them to the current generation.
