...
The Q2 2026 extension is the sibling `ohlc-full-20260920` (minted 2026-09-20, the same reconstruction through 2026-06-30, every prefix byte-identical to this set); this unstamped set stays the frozen canonical every trial pins.
...
The window's reach depends on the interval (~720 d daily, ~120 d at 4h, ~30 d at 1h, ~7.5 d at 15m), and a round joins a series only while the canonical tail still lies inside it — a round meant to keep an interval continuous runs within that reach of the tail — so each series lands one of two ways, and the **filename is the contract**:
