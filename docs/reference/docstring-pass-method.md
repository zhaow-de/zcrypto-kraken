...
**Inert means no observable effect, not an unchanged shape.** `replay_fingerprint` digests whole files across the replay roots' import closure, so a docstring edited in one of them changes the gate's cache key and the next run pays a cold replay — an effect no arm can see. `prove-inert.py` refuses those files rather than certifying them, and needs the package importable: run it through `uv run` or it refuses everything. **The closure is not one directory** but the transitive `cli.*` import closure of the replay roots — size a `cli/` batch from `_replay_code_paths()`, never by guessing. `tests/` is outside it, which is why this pass's test-file batches are inert.
