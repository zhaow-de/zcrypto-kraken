...
**Each verdict costs something different, so read which one fired rather than whether the run was green.** A 1 withdraws the prose-only claim for that file, and with it the licence that made the batch cheap to review. A 3 says a comment's position moved, which costs whichever guard reads that position — `tests/test_config_selectors_are_parsed.py` reads `# config-selector-ok:` markers on the line above the check or anywhere inside a multi-line one, so a marker that slides off the check silently stops exempting anything. A 4 is not a failed run: it says the claim cannot be made from here, because a docstring that is program output changed, or a path could not be read at the base revision.
