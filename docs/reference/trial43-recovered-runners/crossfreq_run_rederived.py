...
# RECOVERY VARIANT: iter-074 cache is gone. The arms are recomputed from COMMITTED
# a2_book_returns below; only the elementwise cross-check against that cache is dropped.
# The independent assert against each arm's REGISTERED Sharpe is kept untouched.
arm_positions = {}
for (lookbacks, tvol), recorded in A2_GRIDS.items():
    ...
    # (cache cross-check dropped — see RECOVERY VARIANT note above)
    ...
    print(f"  arm {(lookbacks, tvol)} QA PASS (Sharpe {s_arm:.4f}, vs REGISTERED; cache check dropped)", flush=True)
...
# RECOVERY VARIANT 2: the iter-074 cache also held bench_noc as a genuine INPUT.
# Sourced instead from the COMMITTED re-derivation module, which builds the same frozen 4h
# benchmark. The pre-existing assert below validates it against the REGISTERED figures
# (1.2128/1.2447) — i.e. the substitution proves itself by this script's own criterion.
from cli.portfolio.record44_legs import benchmark_4h_net_of_cost

bench4h = benchmark_4h_net_of_cost(h_prices, cost_per_side=FEE)
