...
The consequences were measured at the transition rather than assumed — see [T0149](../open-topics/archive/T0149-sleeve-reversal-invalidates-the-one-sleeve-derivations.md), which carries the re-derived drift band, the re-measured placeability (still 0 of 10, with `ordermin` and not `costmin` the binding floor), and the re-measured expected-gross parameter.
