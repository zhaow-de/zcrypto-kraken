...
- **DOT 1:100 redenomination (Aug 2020)** — **IN our price history, in the first bar's `open` and `high`, and invisible to the close-based audit.** Corrected 2026-08-28 (T0025); the previous claim here was "not in our price history", reasoning that Polkadot redenominated before Kraken listed DOT/EUR. The dump says otherwise. `DOT/EUR`'s first daily bar is **2020-08-18** and reads `open=29.0000 high=45.0000 low=2.5100 close=2.6095` — an ~11x intraday collapse from pre-redenomination (old DOT) prints to the post-redenomination book. At 60 m the whole transition sits inside the **first hourly bar** (`17:00 open=29.000 high=45.000 low=2.510 close=2.750`); every subsequent hour trades 2.5–3.4.
  - **Consequence, and it is narrow but real**: any series reading DOT/EUR's `open` or `high` on 2020-08-18 reads a price in a different unit. Closes are unaffected at every resolution — the first close is already post-redenomination — so a close-only strategy is untouched.
