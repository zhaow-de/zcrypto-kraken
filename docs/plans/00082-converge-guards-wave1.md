...
- [ ] **Step 1: Failing test** — `daemon.json — refuse an unacknowledged change (its handler bounces dockerd)`: `{"docker_daemon_json_diff": {"rc": 1}, "daemon_json_ack": False}` → refuse; `{"docker_daemon_json_diff": {"rc": 0}, "daemon_json_ack": False}` → pass; `rc: 1` + ack `True` → pass.
Include the rc==2 fixture (I5): `{"docker_daemon_json_diff": {"rc": 2}, "daemon_json_ack": False}` → refuse (absent deployed file counts as differing).

- [ ] **Step 2: FAIL.** **Step 3: Implement**: template `daemon.json.j2` to `/run/zcrypto-daemon-json.probe` (`changed_when: false`, `check_mode: false` — **a stated exception to the read-only probe idiom: it writes to /run even under `--check`; tmpfs, no secrets, no service touched**, spec D3-8), `command: diff /run/zcrypto-daemon-json.probe /etc/docker/daemon.json` → `docker_daemon_json_diff` (probe idiom; rc != 0 counts as differing), then **a `debug: var=daemon_json_diff.stdout_lines` task gated `when: docker_daemon_json_diff.rc != 0`** — I5: a failed_when-false command prints nothing under the default callback, so without this the operator acks unseen content and the guard's boolean rationale is void — then assert `docker_daemon_json_diff.rc == 0 or daemon_json_ack | default(false) | bool` with fail_msg: *"daemon.json would change and its handler restarts dockerd — under live capture that is a data gap. Review the diff above, then re-run with -e daemon_json_ack=true."*
