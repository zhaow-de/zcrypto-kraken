...
- [ ] **Step 3: wire into site.yml ops play** — `- role: access_ops` + `tags: [access]`; extend ops keep-regex + admission test with `zaccess_.*`. **The ops `config.alloy` edit makes Alloy the subject** — the converge must pass `-e ops_alloy_digest=<currently-running>` (rule in `fleet-deploys.md`).
