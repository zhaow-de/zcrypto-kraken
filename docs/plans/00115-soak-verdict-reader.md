...
- The reduction order is fixed (spec D4), and Task 2 Step 4's docstring states it beside the arms it orders: change one and both move.
...
Expected: PASS, every test. The runbook-corpus tests assert a ratio (`>= 0.70` autonomous) and name no command, so check the four engine reads the runbooks spell — `exec-status`, and `report`, `accum-replay` and `tracking-report` with `--journal-dir` — against the table by hand.
