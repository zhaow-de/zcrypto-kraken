...
- **No code change.** Throwaway scratchpad script (session scratchpad dir, not committed); the committed artifact is the extended `docs/research/04.phase3-benchmark-b2-basket-report.md`. No CLI/README change.
...
**Files:** Modify `docs/research/04.phase3-benchmark-b2-basket-report.md`.
...
- [ ] **Step 2: Add a `## B3 and B4: gating and shorting the basket` section** to `docs/research/04.phase3-benchmark-b2-basket-report.md`, placed **after `## Interpretation (honest)`** and **before `## Distrust-the-instrument note`**. It must contain:
...
- [ ] **Step 4: Full gate** — `git add docs/research/04.phase3-benchmark-b2-basket-report.md` (report only; NOT the scratchpad); `uv run pre-commit run -a` clean; `uv run pytest -q` green (no code change ⇒ unchanged **469 passed**).
