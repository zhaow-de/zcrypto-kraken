...
- [ ] **Step 4 — site play.** Append a third `site.yml` play: `- name: converge the ops node — compute tier (spec 00051)` / `hosts: ops_host` / `become: true` / roles `[base, hardening, firewall, fail2ban, chrony, docker, ops]` (tagged like the capture play). Add a `pre_tasks` debug (`tags: [always]`) stating the charter: *no trade key, never engine_host, pull-only transport (D10)*. Do **not** add the `converge_primary` guard here (that guard keys on `engine_host`; the ops play never touches it).
...
**Files:** none (operational). Verifies Task 1's artifacts against the real host.

- [ ] **Step 1 — pre-flight.** `ssh root@z-home-zcrypto.zhaow.pro` (master key `zhaow-master-2018`). Confirm hardware: `nproc` ≥ 20, `grep -m1 avx2 /proc/cpuinfo` non-empty, `df -h /` ≈ 3+ TB free, and nothing zcrypto runs yet (`docker ps -a` empty or non-zcrypto).
