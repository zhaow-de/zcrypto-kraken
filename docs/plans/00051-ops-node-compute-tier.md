...
- Produces: `replay_segment(path: Path, symbol: str, depth: int) -> ReplayResult` and `verify_replay(primary_root, reconciled_root, *, pair=None, since=None, depth) -> list[ReplayResult]`; `ReplayResult(pair, hour, rows, messages, snapshot_anchored: bool, ts_ordered: bool, checksum_present: bool, replay_ok: bool, error: str | None)`.

**Scope (finalized 2026-07-15 per the owner's OPS-3 call + the [[T0045]] precision finding — READ THIS):** the archive stores `price`/`qty` as **Float64**, so `OrderBook.checksum()` is **not** byte-exact re-derivable and MUST NOT be compared to the stored `checksum` column (it mismatches on every zero-trailing level — a guaranteed false alarm). The **"structural desync" idea is dropped**: for a depth-bounded book, a legitimate update to an out-of-window level is indistinguishable from corruption without the CRC, so that heuristic is unreliable and MUST NOT be implemented. This driver proves only what is derivable *without* the CRC — that the canonical archive **replays as a coherent book**: (1) `snapshot_anchored` — each hour opens with a `type=="snapshot"` row; (2) `ts_ordered` — rows are non-decreasing in `ts`; (3) `checksum_present` — every message has a non-null `checksum` (capture-time attestation exists); (4) `replay_ok` — rows regroup by `(ts, symbol, type, checksum)` into WS-shaped `data` and feed `ingest_snapshot`/`ingest_update` in row order **without a structural throw** (ignore `OrderBook`'s CRC-based `validate()`/`desynced` result — do not pass judgement on the returned bool). Its one genuinely new payoff over manifests + `continuity.py`: because it reads the **reconciled-first canonical** stream, it confirms the reconciler's **spliced output stays snapshot-anchored and coherent across splice boundaries** (Role C keeps snapshot rows at the boundary for exactly this). The stored `checksum` is trusted as capture-time ground truth. The richer byte-exact CRC replay is deferred to [[T0045]] (the raw-string schema change unblocks it).
...
- [ ] **Step 2 — `replay_segment` happy-path test.** A synthetic one-hour book segment (snapshot then N coherent updates) → `ReplayResult(snapshot_anchored=True, ts_ordered=True, checksum_present=True, replay_ok=True, error=None)`.
- [ ] **Step 3 — anomaly tests.** (a) an hour with no leading snapshot → `snapshot_anchored=False`; (b) an unreadable/short parquet → `error` set, not raised; (c) rows out of ts-order → `ts_ordered=False`; (d) a message row with a null/absent `checksum` → `checksum_present=False`; (e) a row that makes `ingest_*` throw → `replay_ok=False`, `error` set. One bad hour must not abort the sweep.
- [ ] **Step 4 — implement `replay.py`** (`replay_segment` + `verify_replay` over `canonical_segments`, per-hour try/except isolating failures into `ReplayResult.error`, same isolation pattern as `infra/scripts/gap_distribution.py::observe_gaps`). Do NOT compare a re-derived CRC to the stored `checksum`.
- [ ] **Step 5 — `verify-replay` command + a report** (per-hour: anchored / ordered / checksum-present / replay-ok; exit non-zero if any hour has `error` or fails any of the four booleans, mirroring `engine replay`'s non-zero-on-drift contract). Add to `README.md` §Usage.
