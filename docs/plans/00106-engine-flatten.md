...
- Produces, for every later task:
  - `ACCOUNT_ID: str = "KRAKEN-001"`, `TAKER_RATE: float = 0.0080`, `CONFIRM_WORD: str = "FLATTEN"`, `CLIENT_ORDER_ID_PREFIX: str = "FLT-"`, `BOOK_DEPTH: int = 1`, `QUOTE_CURRENCY: str = "ZEUR"`
...
class FlattenRefused(Exception):
    """Refused before the venue was touched -- exit 1."""
...
class Recorder:
    ...
    serialization, and a reader mid-incident needs what came back, not our summary of it.
    """
...
- Consumes from Tasks 1–2: `PairConstraints`, `Leg`, `FlattenUnreachable`, `step_precision`.
- Produces:
...
def test_a_quantity_that_clears_ordermin_only_before_flooring_is_dust():
    ...
    assert flatten.classify_balance(1.9, coarse, 100.0) == "residual"
    assert flatten.classify_balance(0.9, coarse, 100.0) == "dust"
...
def build_plan(client: Any, rec: Recorder, snapshot: Snapshot, listing: dict[str, Any]) -> Plan:
    ...
    prices = {symbol: read_book_price(client, rec, constraints[symbol], side) for symbol, side in wanted.items()}
...
- Produces:
  - `@dataclass(frozen=True) class LegOutcome: kind: str; symbol: str; side: str; qty: float; pass_name: str; client_order_id: str | None; sent: bool; reason: str | None; answer: str | None; error: str | None`
...
def test_a_rejected_margin_leg_is_journaled_and_the_sweep_continues_to_the_spot_pass():
    """A rejection is never retried and never stops the rest of the account being flattened."""
    client = _sweep_client(
        orders=[[]],
        positions=[[_Position("BTC/EUR", "LONG", 0.5)], [_Position("BTC/EUR", "LONG", 0.5)], [], []],
        balances=[[_Balance("ADA", 1200.0)], [_Balance("ADA", 1200.0)], [], []],
...
def submit_leg(client: Any, rec: Recorder, sized: SizedLeg, constraints: PairConstraints, client_order_id: str) -> Any:
    ...
    params = {
        ...
        "order_side": sized.leg.side,
        "order_type": "MARKET",
        "quantity": float(quantity),
        "time_in_force": "IOC",
        "reduce_only": margin,
        "account_type": sized.leg.account_type,
    }
    kwargs: dict = {"reduce_only": margin, "account_type": AccountType.MARGIN if margin else AccountType.CASH}
    if margin:
        params["leverage"] = MARGIN_LEVERAGE
    ...
    return rec.call(
        ...
        lambda: client.submit_order(
            ...
            OrderSide.SELL if sized.leg.side == "SELL" else OrderSide.BUY,
            OrderType.MARKET,
            quantity,
            TimeInForce.IOC,
            **kwargs,
...
def _send(client, rec, sized: SizedLeg, constraints: PairConstraints, stamp, index: int, pass_name: str) -> LegOutcome:
    ...
    """
    base = dict(kind=sized.leg.kind, symbol=sized.leg.symbol, side=sized.leg.side, qty=sized.qty, pass_name=pass_name)
    if not sized.send:
...
def test_a_venue_that_is_not_online_exits_three_with_nothing_sent(tmp_path):
    _armed(tmp_path)
    client = _flat_client()
    assert _run(client, tmp_path, venue=_offline) == 3
    assert client.calls == []


def test_a_missing_field_on_a_pre_write_read_exits_three_and_the_cancel_never_goes_out(tmp_path):
    """The first write is the cancel. Before it, a shape the venue changed aborts with the account
    untouched -- that is the whole difference between exit 3 and exit 2."""
    _armed(tmp_path)
    broken = _Position("BTC/EUR", "LONG", 0.5)
    del broken.position_side
...
def test_a_sub_ordermin_margin_row_is_sent_and_its_rejection_still_exits_two(tmp_path):
    ...
    venue refuses."""
    _armed(tmp_path)
    tiny = [_Position("BTC/EUR", "LONG", 0.00002)]
...
def run_flatten(
    ...
    *,
    ...
    0 flat (or the default dry run completed) · 1 refused before the venue was touched · 2 partial:
...
def test_absent_credentials_refuse_with_exit_one_and_never_construct_a_client(monkeypatch, tmp_path):
    """Exit 1 is 'refused before the venue was touched', and a client built without a key is one
    request away from touching it."""
...
| **1** | refused before the venue was touched — no kill file, no terminal, the word did not match, or no credentials in the container | nothing was sent; fix what it named and run it again |
...
# A LONG or SHORT row in the final snapshot really is a residual.
infra/scripts/mutate-probe.sh --file cli/engine/flatten.py \
  --control 's/^def judge_final/def judge_final_DISABLED/' \
  --mutation 's/if row.side in ("LONG", "SHORT"):/if row.side in ():/' \
...
# The kill file is really written before the unit is stopped.
infra/scripts/mutate-probe.sh --file infra/ansible/roles/engine/templates/zcrypto-flatten.sh.j2 \
  --control 's/^  systemctl stop "\$UNIT"/  systemctl stop_DISABLED "$UNIT"/' \
  --mutation 's|^  printf .%s flatten.n. "\$(date -u .*$|  : # the latch, moved|' \
...
Run: `git status --porcelain && uv run pytest tests/test_engine_flatten.py tests/test_engine_flatten_wrapper.py tests/test_engine_executor.py -q`
...
- Keep the three human-gated bullets (the attended converge, the live read-only dry-run through the wrapper, drill B) verbatim. No `ripe_when` is added: these wait on a person's decision, not on a measurable precondition, and a `ripe_when` that cannot be evaluated from data is worse than none.
