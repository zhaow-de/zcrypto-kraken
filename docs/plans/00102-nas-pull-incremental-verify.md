...
- [ ] **Step 4:** On the NAS, pull that digest and prove two things by RUNNING the image, never by reading a label or a tag: `runtime=compat` (import polars and print its version — an AVX build is a silent `Illegal instruction` on the Atom), and that the CLI is the new one (`zcrypto archive pull --help` contains `--hash-scope`). Confirm the rollback operand `9f814848fce1` is still resident (`docker image ls --digests`). Then run the fleet's replay-fingerprint pre-check (the one `infra/ansible/host_vars/nas/vars.yml` already documents, its wording) inside **both** the rollback operand `9f814848fce1` and the interim digest and compare: `docker run --rm --entrypoint python <image> -c 'from cli.engine.gate_cache import replay_fingerprint; print(replay_fingerprint())'`. Equal → leg A's Step 3 wait is sized as written (one pull cycle). Differing → say so explicitly, and size leg A's Step 3 wait for a re-measured cold replay (`fleet-deploys.md`: ~3500 s and growing) instead of one pull cycle — the NAS loop runs gate-export between the capture pulls and the liquidations/panel/reconciled pulls, so a cold replay would otherwise delay the last three channels past a one-cycle wait and read as a false FAIL on a partial channel set.
