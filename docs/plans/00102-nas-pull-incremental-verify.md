...
Then the final whole-branch review at the **Fable** floor, trailers, and the branch waits for the rollout to close, rebases onto `develop`, and continues at Task 6; the PR opens at **Task 9 Step 5** on the user's word.
...
**Why there is no third converge.** This repo merges with `--merge`, so the branch's commits keep their SHAs and become reachable from `develop`; the interim digest's `revision` label stays resolvable permanently. `infra/docker/Dockerfile:29-30` copies all of `cli/` (not just `cli/archive/`), plus `pyproject.toml`, `uv.lock` and `README.md`, into the build context — so byte-identity between a merged-`develop` rebuild and the interim image is not a planning-time fact, only a condition to recompute: it holds iff nothing touching that build context (the sibling T0025 branch's `cli/snapshot/` changes, a dependabot `uv.lock` bump) lands on `develop` between the Task 6 build and this PR's merge, and the legs span days. Behavioural identity for the NAS holds regardless — `cli/archive/` is the only part of `cli/` on the pull path. The official re-pin is therefore **optional exactly when Task 9 Step 6's recomputation comes back empty**; take it anyway to preserve an "every fleet digest is develop-head-built" invariant, and record which you chose.
...
- [ ] **Step 4:** On the NAS, pull that digest and prove two things by RUNNING the image, never by reading a label or a tag: `runtime=compat` (import polars and print its version — an AVX build is a silent `Illegal instruction` on the Atom), and that the CLI is the new one (`zcrypto archive pull --help` contains `--hash-scope`). Confirm the rollback operand `9f814848fce1` is still resident (`docker image ls --digests`). Then run the fleet's replay-fingerprint pre-check (the one `infra/ansible/host_vars/nas/vars.yml` already documents, its wording) inside **both** the rollback operand `9f814848fce1` and the interim digest and compare: `docker run --rm --entrypoint python <image> -c 'from cli.engine.gate_cache import replay_fingerprint; print(replay_fingerprint())'`. Record equal-or-differing explicitly — it predicts FUTURE fingerprint-driven replays, not this converge's cost. Either way, size leg A's Step 3 wait for a cold gate-export replay, NOT one pull cycle: the converge recreates the container and the gate cache lives in its ephemeral `/tmp`, so a matching fingerprint buys nothing here. A cold replay takes the better part of an hour and grows with the journal — take the current figure from the standing constraint in `docs/reference/fleet-pins.md` and re-measure before sizing the window, never a remembered one. The NAS loop runs gate-export between the capture pulls and the liquidations/panel/reconciled pulls, so a cold replay would otherwise delay the last three channels past a one-cycle wait and read as a false FAIL on a partial channel set.
...
- [ ] **Step 3:** Wait for one full pull cycle (`ARCHIVE_PULL_INTERVAL`, 3600 s, plus the cycle's own work — the loop is `work; sleep`, so the period exceeds an hour); size this window for a cold gate-export replay regardless of what Task 6 Step 4 found about the fingerprints -- the converge recreates the container, which wipes the cache
...
- [ ] **Step 6 — post-merge, recompute, don't assume:** run `git diff <the Task 6 interim-build sha> develop -- cli/ pyproject.toml uv.lock README.md infra/docker/`. Empty → the image's build context is unchanged and the re-pin is optional exactly as written; take it only to preserve an "every fleet digest is develop-head-built" invariant. Non-empty → the third converge is **owed**, or is explicitly declined in the pins row naming the diff. Whichever you choose, record the choice in the pins row and keep or remove the exceptional-image constraint accordingly.
