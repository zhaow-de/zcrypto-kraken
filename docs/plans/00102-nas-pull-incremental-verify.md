...
- [ ] **Step 3:** Wait for one full pull cycle (`ARCHIVE_PULL_INTERVAL`, 3600 s, plus the cycle's own work — the loop is `work; sleep`, so the period exceeds an hour); if Task 6 Step 4 found the fingerprints differ, size this window for the cold gate-export replay instead. Then, by value:
