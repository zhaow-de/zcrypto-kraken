...
**Tech Stack:** Python 3.14, uv, pytest, nautilus-trader `2.0.0rc4.dev20260824+17843` (cp314 manylinux) from `https://packages.nautechsystems.io/simple`.

Spec: `docs/specs/00100-nautilus-v2-migration-design.md`. Every `D<N>` below refers to its decisions.

## Global Constraints

- **Pinned wheel:** `nautilus-trader==2.0.0rc4.dev20260824+17843`. Exact, never floating. Bumping it is a deliberate act that re-runs Task 1's pin.
...
with `nautilus-trader = { index = "nautechsystems" }` under `[tool.uv.sources]`, and the dependency pinned to `nautilus-trader==2.0.0rc4.dev20260824+17843`. `explicit = true` so this index serves only this package and never shadows PyPI.
