...
So the harness needs `node.run()` with the sequence driven from inside callbacks. The probe bodies need not become a state machine: a callback may not block but may RESUME a hand-driven coroutine, so the existing `async def` bodies survive with a small trampoline (`await wait_for(pred, t)` / `await sleep(n)` yielding to a 100 ms timer). That was built and measured working end to end.

**Independent of run mode, a safety defect must be fixed first**: the harness reads held `Order` objects at roughly thirty sites, and a held order in v2 never advances past `INITIALIZED`. Its `teardown()` classifies `INITIALIZED` as "never submitted" and returns a clean bill — while real orders rest at the venue on the account the engine trades. That is a real-money bug, exposed by the migration rather than caused by it.
