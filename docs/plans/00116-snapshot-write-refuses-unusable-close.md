...
**Goal:** `run_cycle` refuses to journal a snapshot holding a present close that is not a finite positive number, before the first snapshot file is written, naming the pair, the grid, the bar, the stamp and the value; the operator page names the host's real repair; `zcrypto engine seed` reads the canonical that repair needs; T0200 is resolved.

**Architecture:** One pre-pass inside `_journal_snapshots` in `cli/engine/cycle.py` over the union-aligned closes it is about to hash, collecting the offenders and raising `EngineError` once before the write loop; the raise propagates the way the forming-row guard's does, so the node logs it and the boundary stays journal-absent. No record field, no schema change, no change to the shared parquet writer. The cycle-stale runbook's bullet and its step 3 carry the reading and the host's re-delivery repair; `seed` in `cli/engine/command.py` resolves its canonical through `resolve_canonical_root`; the topic is resolved and archived, and T0199's file records what this decided for the shared helper.

**Tech Stack:** Python 3.14, pytest, polars (to spoil a store frame below the engine's own writer, which refuses a NaN); `infra/scripts/mutate-probe.sh` for the guard verdicts; `topic-ops` for the closeout; the universal test of `zcrypto-refine-rules` governs the runbook page Task 2 edits, through the `guidance-guard` commit-msg hook.

**Spec:** `docs/specs/00116-snapshot-write-refuses-unusable-close-design.md`

## Global Constraints

- The source change is confined to `_journal_snapshots` and the forming-row guard's comment in `cli/engine/cycle.py` (Task 1) and to `seed` in `cli/engine/command.py` (Task 3). `cli/engine/journal.py` (spec D3: no `SnapshotEntry` field, no `SCHEMA_VERSION` step), `cli/ohlc/dataset.py` (spec D4: `write_parquet` untouched) and `cli/engine/store.py` do not change. `soak-check`'s `--canonical-dir` default stays `CANONICAL_DIR` (spec D8).
- The refusal's message names the pair as `<symbol>@<interval>`, the bar as `close[<k>]`, the stamp in ISO 8601, the value as its `repr`, the count of offending closes and of series holding one, and what was not written: no snapshot, record, sidecar or orders; the venue record of step 0 is on disk before the refusal, so the message never says nothing was written. It carries no topic id, spec serial or decision number: `tests/test_internal_terms_not_operator_visible.py` reads every non-docstring string literal under `cli/` and `infra/scripts/`, and every page under `infra/runbooks/`. Provenance goes in the comment beside the code, which no walker reads, and in the commit message.
...
- `uv run pre-commit run -a` runs the pre-commit stage alone: `guidance-guard` and `message-citations` run at commit-msg, on `git commit`. The former refuses a universal word (every, never, always, only, any, cannot) in the prose of a list item on a page under `infra/runbooks/` with no count entry, so the runbook text Task 2 writes carries none outside code spans; the latter refuses a commit message whose `path:line`, `path::symbol` or `T<NNNN>` resolves nowhere.
- No step reaches a host, a venue or a mount: the spec's measured basis was read by the controller before this plan, and nothing here re-reads it; Task 3's tests monkeypatch the resolver.
...
- Modify `cli/engine/cycle.py` — `_journal_snapshots` gains the pre-pass and its raise; the forming-row guard's comment names the door one step earlier (Task 1).
...
- Modify `infra/runbooks/engine.md` — the `zcrypto-engine-cycle-stale` › What it means bullet on a raising `run_cycle` gains the reading and the repair; What to do step 3's artifact-state line gains the third cause (Task 2).
...
- Modify `docs/open-topics/T0199-store-nan-drops-a-tail-instead-of-refusing.md` — the one-door paragraph records what T0200 decided for the shared helper; the `write_parquet` arm leaves its trigger and its first next step (Task 2).
- Modify `docs/open-topics/README.md` — re-rendered by `infra/scripts/topics-index.py` (Task 2).
- Modify `cli/engine/command.py` — `seed` resolves its canonical through `resolve_canonical_root` (Task 3).
- Modify `tests/test_engine_command.py` — the seed summary case follows the resolver; one case for a resolver refusal (Task 3).
- Modify `docs/reference/data-catalog-full.md` — the `engine-store` bullet's account of the seed's canonical (Task 3).
...
**Files:**
- Modify: `cli/engine/cycle.py` (`_journal_snapshots`, the function whose first line is `rel_dir = Path(f"{cycle_ts:%Y-%m-%d}") / "snapshots" / f"cycle-{cycle_ts:%H}"`; and the comment block above `model_closes = {}` in `run_cycle`)
...
def test_an_unusable_present_close_refuses_the_cycle_before_any_snapshot_is_written(
    ...
    same. Both legs are written late in their grid's pass -- the daily grid goes first and `XRP/EUR` is its last
    leg -- so a check that moved into the write loop would leave files behind on both halves."""
...
- [ ] **Step 3: Add the pre-pass to `_journal_snapshots` and the line to the forming-row guard's comment**
...
- [ ] **Step 1: Extend the runbook bullet and its step 3**
...
Resolved by spec `00116` and the PR that carries it: the refusal lives at the engine's own write. `_journal_snapshots` in `cli/engine/cycle.py` walks every close of every grid before the first snapshot file is written and refuses a present close that is not a finite positive number as an `EngineError` naming the pair, the grid, the bar index, the stamp and the value, so a refused boundary leaves no snapshot directory, no record, no sidecar and no orders beside the venue record of step 0; `None` at a union absence is admitted. No `SnapshotEntry` field was added and the shared `write_parquet` is untouched, which answers the third next step, since the door is the engine's and reaches no other package, and records for [[T0199]] that its helper arm is not the place. The read-side refusal from [[T0193]] stays for records written before the door. The abort's shape is the raise, as the forming-row guard's; the operator reading is the cycle-stale runbook's bullet and its step 3, naming the host's repair as a re-delivery by the engine-tagged converge, since `zcrypto engine seed` is the workstation's command; and that seed now reads the newest whole frozen `ohlc-full` sibling, without which a 4h leg could not be re-seeded. Measured before the change over the NAS journal mirror on 2026-09-21: 433 records, 9,508 snapshot files, 0 unusable present closes among 156,359,450 close rows, so no record on disk is refused by the read for this reason. The owner's ruling of 2026-09-20 in chat set the placement and the shape.
```
...
The runbook's bullet on a raising \`run_cycle\` says what a traceback naming a pair, a grid, a bar and
a close that is not a finite positive number means and names the repair as it is on the host: a
re-delivery by the engine-tagged converge after the store dir is moved aside, since \`zcrypto engine
seed\` is the workstation's command and reaches nothing older than the REST window; step 3's
artifact-state line gains the cause the door adds, a boundary with no snapshots directory whose
store reads fine. T0200 is resolved and archived with the door's placement, the shape the spec
settled and the mirror's reading of 2026-09-21; T0199's one-door paragraph records that the shared
writer is not the door, and its trigger and first next step drop that arm.
...
### Task 3: The seed reads the canonical the repair needs

**Files:**
- Modify: `cli/engine/command.py` (`CANONICAL_DIR` at the module top, and `seed`, the command whose docstring begins `Seed/refresh the live price store`)
- Modify: `tests/test_engine_command.py` (`test_seed_prints_per_pair_overlap_summary`, and one case appended directly below `test_seed_engine_error_is_a_clean_exit_1`)
- Modify: `docs/reference/data-catalog-full.md` (the `- **`engine-store`**` bullet under the machine-state section)

**Interfaces:**
- Consumes: `resolve_canonical_root(data_root: Path) -> Path` from `cli/data/rebuild.py`, raising `DataSyncError` from `cli/data/errors.py` when no whole frozen set resolves; `seed_store(store_dir, canonical_dir)` from `cli/engine/store.py`; `_load_engine_config`, `_abort`, `runner`, `app`, `_patch_config`, `_output`, `SeedReport`, `SeedEntry`, `EngineError` from the command module and the test module's existing imports.
- Produces: `zcrypto engine seed` seeding from the root the resolver names and printing it; `CANONICAL_DIR` unchanged for `soak-check`.

- [ ] **Step 1: Change the seed summary case and add the refusal case in `tests/test_engine_command.py`**
...
```python
    ...
    roots = []
    monkeypatch.setattr(command, "seed_store", lambda store_dir, canonical_dir: calls.append((store_dir, canonical_dir)) or report)
    # The seed reads the newest whole frozen sibling, never the unstamped set by name: the resolver answers for it.
    monkeypatch.setattr(
        "cli.data.rebuild.resolve_canonical_root", lambda data_root: roots.append(data_root) or tmp_path / "ohlc-full-20260920"
    )
    ...
    assert calls == [(engine_cfg.store_dir, tmp_path / "ohlc-full-20260920")]
    out = _output(result)
...
def test_seed_refuses_when_no_whole_frozen_set_resolves(tmp_path, monkeypatch):
    """The resolver's refusal is the seed's: a missing or partial frozen set aborts the command with the resolver's
    own message, before any store file is touched."""
    ...
    assert "none found under data" in _output(result)
...
- [ ] **Step 2: Run the two cases to verify they fail**

Run: `uv run pytest -q tests/test_engine_command.py -k "seed_prints_per_pair or seed_refuses_when"`
Expected: `2 failed`: the summary case on `assert roots == [Path("data")]` (`[] == [PosixPath('data')]`), the refusal case on `assert result.exit_code == 1` (`assert 0 == 1`: the seed never calls the resolver, seeds from the stub and prints its summary).
...
# read this name -- it resolves the newest whole frozen sibling under DATA_ROOT, the one a REST gap-fill can seam with.
CANONICAL_DIR = Path("data/ohlc-full")
DATA_ROOT = Path("data")
```
...
```python
    """Seed/refresh the live price store from the newest whole frozen ohlc-full sibling plus a Kraken REST gap-fill
    (idempotent; also the documented repair for a poisoned store tail). A workstation command: the engine image
    carries no canonical dataset."""
    ...
    try:
        ...
        report = seed_store(config.store_dir, canonical_dir)
    except (DataSyncError, EngineError) as exc:
        raise _abort(str(exc)) from exc
    typer.echo(f"seeded {config.store_dir} from {canonical_dir} + REST gap-fill; seam QA per pair x grid:")
```
...
Run: `uv run python -c "import cli.engine.command as m; print(m.CANONICAL_DIR, m.DATA_ROOT)"`
Expected: `data/ohlc-full data`
...
```
`engine seed` reads the canonical only where a store file is absent, and its canonical is the newest whole frozen `ohlc-full` sibling (`resolve_canonical_root`, the root `ohlc-reach` joins), so a series moved aside is re-copied from a set whose tail the REST window overlaps. The two `/BTC` legs' **4h** grids were seeded **REST-only** at the twelve-leg widening — 720 bars from 2026-04-17, when the unstamped set ended 2026-03-31 and the REST window had receded past it — while every `/EUR` leg and both `/BTC` **daily** grids were canonical-backed from the start (REST reaches 2024-08-25 on the daily grid). Harmless to the model today because `select_model_inputs` contracts to the ten `/EUR` legs, so no `/BTC` datum reaches it — but a loop that starts consuming a `/BTC` series must not assume history before 2026-04-17 exists in the store as it stands; a re-seed of that series from the 2026-06-30 sibling carries the full tail.
```
...
git commit -m "fix(engine): the seed reads the newest whole frozen ohlc-full sibling
...
function \`data rebuild ohlc-reach\` joins with, the newest whole stamped sibling, aborts on the
resolver's refusal as it does on the store's, and prints the root it read; \`soak-check\`'s null
reference keeps the unstamped set, since its span is the null's instrument. The data catalog's
\`engine-store\` bullet says which canonical the seed reads and what a re-seed of a BTC-quoted 4h leg
now carries.

Cases: the seed summary follows the resolver's root and prints it; a resolver refusal is a clean
exit 1 with its message and no store call.
...
- Names: `_journal_snapshots`, `EngineError`, `read_parquet`, `_env`, `_tail_fetch`, `_clock`, `CYCLE_TS` are the test module's and the cycle module's existing names; `resolve_canonical_root`, `DataSyncError`, `seed_store`, `_abort`, `_load_engine_config`, `_patch_config`, `runner`, `app`, `_output` exist where Task 3 says; `DATA_ROOT` is Task 3's one new name and Task 3 is its only user.
