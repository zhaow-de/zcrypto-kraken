...
- Produces:
  ...
  - `INTERIOR_MAX_SECONDS = 300.0` — the absolute interior bound (measured; the real event sits 3.0× inside it)
...
Append to `tests/test_archive_settle.py`, and extend the existing `from cli.archive.settle import (...)` block with `CAPTURE_DIVERGENT`, `EpisodeVerdict`, `INTERIOR_MAX_SECONDS`, `UNDETERMINED`, `VENUE_SILENT`, `classify_dark_episode`.
...
def test_a_long_healthy_interior_is_two_incidents_not_one_episode():
    ...
    two = [DarkWindow(start=_at(0), end=_at(600), seconds=600.0),
           DarkWindow(start=_at(1600), end=_at(2200), seconds=600.0)]
...
def classify_dark_episode(
    ...
    if divergent:
        ...
        or not agreeing
        or not updates
        or interior_seconds >= dark_seconds
        or interior_seconds > INTERIOR_MAX_SECONDS
    ):
