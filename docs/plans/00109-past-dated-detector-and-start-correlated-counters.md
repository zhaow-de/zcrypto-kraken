...
Both land before the PR, so the merged tree is what Step 6 verifies; the split exists for the revert, not for review — and Step 6's last block is where the revert is exercised, because nothing else in the repo can tell two commits from one.
...
**Then exercise the hatch once, here, on the branch.** The c1/c2 split IS the hatch's mechanism and nothing else in the repo can see it: `staged-kind` polices claude-kind against everything else and no test, hook or grep tells two commits from one, so an implementer who lands both sets in one commit passes every check above. Take the revert and read what it leaves:

```bash
git revert --no-commit HEAD
git grep -n 'increase(zcrypto_capture_ts_past_dated_hour_total' -- infra/grafana/
git grep -ni 'hard.zero' -- infra/grafana/
git revert --abort
git status --porcelain
```

`git grep` with no tree-ish reads the working tree, which after `--no-commit` is the hatch's tree. Expected: **two** `increase(` hits — the rule's `expr` and panel 110's refId B, the two surfaces c2 owns — and **no** `hard.zero` output, because c1's prose survives a revert of c2. A `hard.zero` hit means the split collapsed and the hatch would re-ship the falsified baseline; fewer than two `increase(` hits means c2 moved only one of the two surfaces. Either is the collapse: re-split the commits and repeat. `git revert --abort` restores the tree on every path, and the empty `git status --porcelain` is what proves it did — the one failure mode this block introduces is leaving the worktree mid-revert, which Task 5's implementer would meet as an unexplained dirty tree.
...
Expected: the first returns **no lines**. The second returns only the three members Global Constraints names as already reading correctly, plus whatever survives the two `cli/` rewrites — read every surviving hit against the invariant Global Constraints states. The total is not the test; a single hit failing that invariant is, whatever the total.
...
**Commit-kind split.** Task 6 is the branch's only `.claude/**` change and commits alone; every other commit is code, tests, `infra/` or `docs/`. Task 4 lands two `infra/`-kind commits rather than one — the prose that holds under either expression form, then the form move — so Task 6's escape hatch is a `git revert` of a named subject instead of six strings re-edited by hand under time pressure — and Task 4 Step 6's last block reverts it once on the branch, because nothing in the repo can otherwise tell a correct split from a collapsed one.
