...
### Task 7: The adopt pass reconciles against venue truth (spec D7 -- added after the whole-branch review, and revised by its cold review)

**Files:** Modify `cli/engine/executor.py` (`_adopt_resting_orders` and `_publish_fill`'s docstring), `infra/runbooks/engine.md` (D7's operator surfaces), `tests/test_engine_executor.py` (new tests + the adopt fixtures' stub cache).

**Interfaces:** consumes `_OVERFILL_TOLERANCE`, `_ordered_qty`, `_trip_kill`, `_inc_order`, `update_submitted_row`, `execledger._ROW_STATES`. Produces no new public name. Do NOT reuse `_EXTERNAL_TERMINAL_STATES` -- it is keyed on event class names; D7 needs its own `OrderStatus` map.

- [ ] **Step 1: Read the installed source before writing anything.** Confirm from `.venv/lib/python3.14/site-packages/nautilus_trader/`: (a) `Cache.orders(venue=...)` -- the cold review read it at `cache/cache.pyx:4675` and reports it returns closed orders too; verify and quote the signature; (b) how a closed order's status is read from the object and the exact spelling of every closed `OrderStatus` member; (c) that `Order.filled_qty` is a `Quantity` and `float()` of it is exact. Record all three in the report.
- [ ] **Step 2: Write the failing tests FIRST**, against the existing adopt fixtures. The stub cache gains `orders(venue=...)`; `_open_order` gains a settable `filled_qty` (without it every existing adopt fixture raises inside the sweep); `_FlakyOrdersCache` will need its raise moved to whichever read the sweep does first, or its test stops proving the retry -- expect it to go red and fix it deliberately, not by weakening its assertion.
  - **sub-tolerance mismatch is SILENT**: ledgered and venue figures differing by less than `_OVERFILL_TOLERANCE` produce no event, no state write, no log. (Exact equality would not catch a missing dead-band -- a clean multi-fill restart differs by ulps.)
  - positive delta journals: row ordered 0.001, ledgered 0.0, cache order 0.0004 -> stored row reads 0.0004, carries a reconciliation event naming the delta, and `self._attached`'s row mirrors it.
  - delta COMPLETES: cache order `filled_qty` == ordered -> row state `filled` in the store AND mirrored in `_attached`, `_inc_order("filled")` exactly once.
  - delta OVERSHOOTS: past ordered + tolerance -> kill latched, and the repair still journaled first.
  - NEGATIVE delta -> kill latched, reason naming both figures.
  - closed-while-down, WITH a delta: `orders_open` EMPTY, `orders` returns one CLOSED FILLED order matching an open row -> row repaired, terminal state written, and it is in `_attached`.
  - closed-while-down, ZERO delta: a CLOSED CANCELED order whose quantity already matches -> **no repair event, but the row IS closed**. (This is the commonest shape and the one a naive `if delta == 0: continue` skips forever.)
  - the terminal map is TOTAL over the enum: derive the domain from `OrderStatus`'s closed members rather than hand-listing, and assert every mapped value is in `execledger._ROW_STATES`.
  - **D7 meets D1 (the highest-value test)**: sweep repairs 0.0004, then an external fill of 0.0006 arrives -> row 0.001, state `filled`, `_inc_order("filled")` exactly once, NO trip. Plus its mirror: after a repair completes a row, a later dust fill trips.
  - a row whose journal write RAISES still leaves the resting opener canceled (the classification pass survives the sweep).
  - idempotence: a second `on_timer` after `_adopted` appends nothing.
- [ ] **Step 3: Run them and read WHICH assertion fails** on each -- a test failing because the stub lacks a method has proven nothing yet.
- [ ] **Step 4: Implement.** Order of operations in `_adopt_resting_orders`: read the wide list `self._client.cache.orders(venue=_VENUE)` (failure -> the existing critical-log-and-return with `_adopted` NOT set); derive `resting` from that same list by the order's own open predicate rather than a second cache call, so one read failure cannot leave the two populations disagreeing; set `_adopted`; read rows; **reconcile sweep** (index the wide list by client order id, iterate the ROWS, per-row `try/except` logging CRITICAL and continuing, the whole sweep wrapped again); then `if not resting: return`; then the existing classification loop, byte-identical. Scope the ledger-read CRITICAL log to the case where something is actually resting -- moving the rows read ahead of the early return otherwise makes a corrupt exec record shout "every resting order will be canceled" on an idle boot where nothing is.
- [ ] **Step 5: Green, then the guard proofs** via `infra/scripts/mutate-probe.sh`, one at a time, reading WHICH assertion fires: (a) drop the dead-band from the repair arm -> the sub-tolerance test bites; (b) skip the terminal write when the delta is zero -> the closed-zero-delta test bites; (c) clamp the negative delta to zero -> the negative-delta test bites; (d) drop the `_attached` mirror -> the D7-meets-D1 test bites (its completion never fires), which is what proves the mirror moved the TRIP BASE and not merely a dict; (e) leave `if not resting: return` where it was -> both closed-while-down tests bite.
- [ ] **Step 6: The surfaces D6 now lists for D7, then commit.** `_adopt_resting_orders`' docstring gains the reconcile paragraph; `_publish_fill`'s docstring loses its two now-false sentences ("every path that WRITES A ROW" / "ONE fill is deliberately not published"); `infra/runbooks/engine.md` gains the new row event kind, the startup terminal write, the boot-time kill possibility, and the `editOrder` (cancel-replace, presents zero fills -- can latch the kill at the next boot) versus `amendOrder` (in-place, safe) distinction. Commit `feat(engine): the adopt pass reconciles each row against venue truth`.
...
1. **Spec coverage**: D1→Task 3, D2→Task 2, D3→Tasks 1+4, D4→Tasks 2-3, D5→Task 6/PR body, D6→Tasks 5+7, D7→Task 7. Verification section→Tasks 2-3 tests + mutation proofs.
