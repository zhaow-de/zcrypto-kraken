...
| Closeout: `docs/iterations-history-phase6.md`, `docs/research/14.phase6-decisions.md`, T0138 + T0140 (resolve+archive), T0018, T0121 + T0120 (partial updates), `docs/open-topics/README.md` | *modify* (Task 14 only) |
...
- `plan_refusals` returns every applicable reason (the gate's plural-reasons discipline, deterministic order): `"plan expired: created_at ... is over 60 minutes old"` when `now - created_at > PLAN_TTL`; `"created_at is in the future"` when `created_at > now`; `"plan_id already ledgered"` when `plan.plan_id in ledgered`; `"plan notional ... exceeds the cap ..."` when `sum(i.notional_eur or 0.0) > max_plan_notional_eur` (a `qty` intent has no EUR notional at validation time — no price exists yet — but it is NOT exempt from D8's cap, which has no exclusion: its notional exists at sizing time as `qty × the chosen limit price`, and the executor enforces the cumulative plan cap there — Task 6's first-submission check; stated in the docstring); `"margin floor: ..."` when `sum(i.notional_eur / i.leverage for margin intents) * 2.5 > free_zeur` (§10's 250 % floor at rung scale; a `qty` disposal intent stays EXCLUDED from the floor — a spot sell extends no margin — stated in the docstring).
...
- **First submission** (on the first quote in `awaiting_quote`): join the touch — buy at `bid`, sell at `ask` (never improving into the spread); `mode: "rest-cancel"` prices 5 % away on the passive side instead — buy at `bid * (1 - _REST_CANCEL_OFFSET)`, sell at `ask * (1 + _REST_CANCEL_OFFSET)` (never joining — a join can fill in the instant after acknowledgment); size through `size_probe_order`; **a `qty`-carrying disposal intent's EUR notional exists here — `qty × the chosen limit price` — and D8's cap has no exclusion: cumulated with the plan's other intents' `notional_eur`, a total over `exec_max_plan_notional_eur` refuses the intent (terminal `outcome="refused"`, reason naming the cap, no order); it stays EXCLUDED from the ×2.5 margin floor — a spot sell extends no margin**; post-only GTC; `phase = "resting"`.
...
**Discharges spec Verification:** the whole D10 bullet — opener at `reduce_only` refuses; over-|held| closer refuses; genuine closer passes with the flag; the spot disposal refuses on a venue record showing a positive balance smaller than `qty`, proceeds when the balance is zero-or-absent (the pre-restart record cannot see the settle; the venue rejection is the backstop), and post-restart at `reduce_only` takes the full `qty ≤ balance` bound both ways; the startup pass leaves exactly the ledger-attached reduce-only orders and cancels everything else including a reduce-only-flagged order with no ledger row; a post-restart fill on a re-attached order appends to its ledgered row.
...
- **Spot disposal** (`action == "close"`, `leverage is None`, explicit `qty`): side must be `"sell"`; the balance read is the newest schema-2 `venue-<HH>.json`'s `state.balances`, `validate_venue_record`-checked, through `_spot_balance(balances: dict, base: str) -> float` which tries the venue alias spellings in order (`("BTC", "XBT", "XXBT")` for BTC, else `(base, f"X{base}")`), returning `0.0` when none is present. **The record can REFUTE but not confirm the plan-carried `qty`** (D10): a **positive** balance **smaller** than `qty` refuses the intent (`"the venue record refutes the signed qty"`), while a zero-or-absent balance proves nothing — the record's balances come from the connect-time account read, so no pre-restart record can see the settle — and the intent **proceeds on the G2-signed ledger-export figure**, with the venue's insufficient-funds rejection (a Kraken error code, Task 6's rejected path) as the enforcing backstop, halting attended. **Post-restart — the verdict level reads `reduce_only`, which implies the hold, which implies a fresh startup account read — the record CAN confirm and the full `qty ≤ _spot_balance(...)` bound applies**: `qty` over the fresh record's balance (absent reading `0.0`) refuses. **NO venue-side `reduce_only` flag either way** (Kraken's reduce_only is a margin concept a spot order cannot carry — the executor-side bound plus the venue backstop is the whole guard); strictly shrinking, never flipping.
...
- [ ] **Step 1: Failing tests** (fixtures: `StubCache` grows `positions_open` returning constructed `SimpleNamespace(signed_qty=...)` lists and `orders_open` returning stub orders with `client_order_id` and `is_reduce_only` attributes; a `_venue_record(tmp_path, positions=..., balances=...)` helper writes a real schema-2 `venue-<HH>.json` via `write_venue_record`):
  ...
  - the disposal at gate `FULL` — the refutation construction: plan `qty=0.0006` against a venue record `balances={"XXBT": 0.0005}` (a positive balance smaller than `qty`) → refused `"the venue record refutes the signed qty"`, stub untouched; `qty=0.0004` against the same record → submits a plain spot sell (`reduce_only` **absent/False** in factory kwargs, no `leverage` param);
  - the disposal at gate `FULL` — the stale-record construction: a venue record whose `balances` carry **no** BTC alias at all (the pre-restart record cannot see the settle) → the intent **proceeds** on the signed figure and the stub records the sell;
  - the disposal post-restart at gate `reduce_only`: `qty=0.0004` against a fresh record `balances={"XXBT": 0.0005}` → submits; `qty=0.0006` → refused (the full `qty ≤ balance` bound applies); balance absent → refused;
...
```
  ...
  Outcome label values exactly `submitted|accepted|rejected|venue_canceled|canceled|filled|refused`; liquidity `maker|taker` (lower-cased from `str(event.liquidity_side)`). Fees: only a commission whose `currency.code` is `EUR`/`ZEUR` is added — anything else is logged and skipped, never summed cross-currency (a future `/BTC`-leg fee converts through the BTC/EUR close before summing — not built here). The executor updates position/realized after each fill from the Cache: `qty = sum(signed_qty over positions_open(instrument_id=...))`; realized = sum of `float(p.realized_pnl)` over `positions_open + positions_closed` for the instruments this process has traded — `Position.realized_pnl` is `Money | None` (the pinned fact), so a `None` is skipped (contributes zero), never `float()`-ed — EUR-denominated positions only (non-EUR logged and skipped). PnL is a **Gauge** — PnL falls, so never a counter.
...
def test_a_raising_ledger_writer_freezes_the_heartbeat_and_the_staleness_condition_goes_true(tmp_path, monkeypatch):
    ...
    assert (t1.timestamp() - frozen) > 17100                          # the deployed rule's evaluator: gt 17100 (4 h 45 m), as a number
```
...
- [ ] **Step 1: Append the section** after `refdata-sweep-due`, following the file's section protocol (explicit `<a name=…>` anchor, marker suffix, `### What you are seeing / What it means / What to do / Retire when`): anchor `engine-probe-window`, heading `## engine-probe-window — PROCEDURE`. *What you are seeing*: you are about to run (or are running) an attended live-order probe window on the engine — nothing has fired; this is the only sanctioned way to run one. *What it means*: the engine's order path submits only operator-authored probe plans, inside an attended window bounded by two arming keys; every step below exists because its omission has a named failure. *What to do* — the five phases from spec 00090 D13, verbatim in content, imperative in form:
  ...
  - **Arm**: converge `exec_armed = true` (full engine-converge discipline: inter-cycle window, secondary-bake canary, pins recorded — `fleet-deploys.md`); the restart latches the hold; verify reconciliation clean — `zcrypto engine exec-status` reasons read exactly `arm_file_absent,restart_hold`, positions/balances as expected in the newest venue record; the owner clears the hold; the owner creates the arm file.
...
- [ ] **Step 6: T0121 update in the same PR** (Task 10 completes its counter sub-item; the completing PR carries the topic's whole update — `open-topics.md`, mechanics per `topic-ops`): move the counter sub-item from `## Suggested next steps` to `## Done so far`, rewritten as its outcome — `_limits_bound` in `cli/engine/cycle.py` feeding `zcrypto_engine_limit_bound_total` in `_CycleGauges`, admitted end to end, with its tests in `tests/test_engine_cycle.py`/`tests/test_engine_metrics.py`; re-point `ripe_when` at the remaining decompose-table sub-item; `status` stays `partial`; sync its index bullet in `docs/open-topics/README.md`.
- [ ] **Step 7: T0120 re-defer, made durable where it is read**: its `ripe_when` ("the 6b executor iteration defines the node's REST call pattern") arguably fires at this spec, and the spec consciously re-defers to `00092` — 00090's reconciliation is post-terminal and startup-time, not a standing poll loop, so the intervals' budget is defined by a continuous call pattern only `00092` creates (the spec's bounded claim and Out-of-scope entry). Re-point `ripe_when` at `00092`'s continuous call pattern and echo the re-defer, with that reason, in its index bullet.
- [ ] **Step 8: Sweep `docs/open-topics/README.md`** so the pick-time view is true, and mention newly opened topics, if any, in the entry. The memo is **main-loop-only**: the implementer never edits `docs/memo.local.md` — any needed queue insertion (registration and queue insertion travel together) is **REPORTED to the orchestrator**, which performs it in the main loop.
- [ ] **Step 9: Gate, stage by explicit path (docs + topic files only — claude-kind vs docs-kind split does not arise here; all are docs), commit** `docs(engine): iter-140 closeout -- the rung-1 order path lands; T0138 + T0140 resolve`.

---

## The deploy and the probe window (after merge — attended, the owner runs each step; NOT an implementer task)

Per spec D14 and `fleet-deploys.md` — **the code lands disarmed; the `exec_armed=true` converge is deliberately NOT this deploy** (a code defect and a live-armed engine must never meet before a disarmed drill has run).
...
2. **One engine converge carries the code**, inside the 4-hourly inter-cycle gap, via `infra/ansible/scripts/converge.sh` with `--check --diff` read first — the proven `00094`/`00089` shape recorded in `docs/reference/fleet-pins.md`: `--limit zcrypto --tags capture,engine -e converge_primary=true -e engine_image_digest=sha256:<new> -e capture_image_digest=<currently-running capture digest> -e capture_alloy_digest=<currently-running alloy digest>`. `--tags engine` alone is refused twice over: the engine role hard-asserts `engine_image_digest` non-empty (`roles/engine/tasks/main.yml` — a `capture_image_digest`-only invocation fails the play), and the Alloy keep-list lives in the **capture** role, which `--tags engine` excludes — the new families would publish unadmitted. The capture digest is deliberately **HELD at its currently-running value** so live capture is neither re-pinned nor restarted (the unbackfillable path stays untouched; a held digest is not a re-pin, so no bake is owed); the alloy flag is owed because `config.alloy` changed (config-only, no bake owed; read both running digests from the containers, never the compose files).
3. **Verify the converge by outcome**: the next `cycle-HH.json` lands with `completed_at` inside `[B, B+30 min]`; `zcrypto engine exec-status` on the host reads `level=none` with reasons exactly `config_not_armed,arm_file_absent,restart_hold` — the config renders `exec_armed = false` and `execgate.py` appends `config_not_armed` first; the two-reason `arm_file_absent,restart_hold` read belongs exclusively to the arm phase, where `exec_armed = true` is rendered — (the new `exec_max_plan_notional_eur = 100.0` appears in the converge diff); the exec-status flip drill (kill file create/remove) still flips.
...
D1→T3/5/6/9; D2→no code (bounded claim; decisions-log entry in T14); D3→T3/5 (+T9 CLI); D4→T5/10/13; D5→T1/5/10; D6→T6; D7→T7 (the spot-disposal path) + T12/attended; D8→T3/4/5/6 (T6: the `qty` disposal's cap at sizing time); D9→T2 (+T14's T0140 resolution); D10→T7; D11→T8; D12→T10/11 (+T14's T0121 partial update); D13→T12; D14→attended section. Every Verification bullet's task is named in its owning task's "Discharges" line; the deploy-verification bullet is the attended section.
