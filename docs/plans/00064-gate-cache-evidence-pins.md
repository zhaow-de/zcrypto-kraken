...
- [ ] **Step 3 — schema gate, for the right reason (spec D6, test-list 8).** The existing v1 test passes via a `KeyError` fallback rather than the version check — fix it to fail for the right reason, then add a **higher** `schema_version` case. (Plan text corrected at review: it originally said *lower*, but the v1 case already existed and a lower case cannot catch the `<` mutation the same step requires — see spec D6's correction.) A test passing for the wrong reason reports coverage that does not exist.
