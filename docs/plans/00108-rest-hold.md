...
- **Assert on CLI output only through an ANSI-stripping read, and prove it in BOTH colour modes** — CI forces colour on, a plain shell leaves it off, and Rich styles an option's leading `--` as its own span, so a literal like `"--check"` is not a contiguous substring of a coloured `--help` at all (measured on this tree: contiguous `False` under `FORCE_COLOR=1`, `True` after stripping). Use the existing idiom — `_output()` in `tests/test_engine_command.py`, `_plain()` in `tests/test_engine_flatten.py`, both `re.sub(r"\x1b\[[0-9;]*m", "", …)` — strip before any whitespace normalisation, prefer asserting on a value styling cannot split, and run the file twice, `FORCE_COLOR=1 uv run pytest …` and bare, before committing.
...
Then, because the Global Constraints' colour rule is otherwise enforced by nothing that runs, re-run under forced colour the two files in that set whose assertions ANSI could split: `FORCE_COLOR=1 uv run pytest tests/test_engine_command.py tests/test_internal_terms_not_operator_visible.py -q`. Both runs must pass — CI forces colour on and the bare run leaves it off, so one green says nothing about the other environment. The second file is in the pair because its `_leaks` helper collapses whitespace but does **not** strip ANSI, while the `--help` output it sweeps does carry escapes under forced colour; there is no gap today, and this is the run that would show one.
