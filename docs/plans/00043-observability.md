...
- [ ] **Dashboard + alerts**: `scripts/grafana-push.sh` → the five rows populate (app rows stay empty until the exporters flip); each alert rule test-fired once (no-impact: rule test / `logger.error` probe); email arrives.
