...
| `deploy_nas_ed25519.pub` | **operator `~/.ssh` only** — the NAS is not ansible-managed, so no play ever connects to it and no vaulted private exists; the `.pub` is kept purely as the authorization record | nothing in ansible (the `nas` ssh alias uses the local private) |
