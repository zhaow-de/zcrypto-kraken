...
`verified-replay` GETs its healthchecks.io ping URL on a clean run (exit 0) only. `verify-replay` differs (spec 00077 D5): it pings whenever the sweep *ran* and produced a summary (`run_ok == 1`), not on exit code — once any bad hour exists, `rc` stays 1 forever, so gating on `rc` would withhold the ping (and page through healthchecks.io) forever; a sweep that completed and reported bad hours still ran. Both read role vars `ops_verify_replay_healthcheck_url` / `ops_verified_replay_healthcheck_url`, defaulting to empty, which **skips** the ping entirely (the CLI's optional `ping_healthcheck` semantics, shell edition). A run that never produces a summary (`verified-replay`: non-zero exit; `verify-replay`: `run_ok == 0`) pings nothing and alerts by silence.
