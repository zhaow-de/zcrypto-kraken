...
| `TRADE_BACKFILL_TEXTFILE` | Prometheus textfile the daily trade-backfill step (spec `00053`) writes its metrics to (`zcrypto_trade_backfill_*`), atomically (tmp + `mv`). Alloy's keep-regex must list that prefix too, same caveat as `RECONCILE_TEXTFILE` above — see [T0051](T0051-alloy-keep-regex-missing-trade-backfill.md), not yet wired in. | defaults to `/textfile/trade-backfill.prom` in `pull-entrypoint.sh` (no compose.yaml entry yet) |
