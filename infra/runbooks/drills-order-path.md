...
- **A drill whose instrument is not on the host yet is `blocked`, and one instrument is still missing.** No plan mode keeps an order resting — `rest-cancel` cancels the moment the venue acknowledges, and `cli/engine/probeplan.py` declares only `execute` and `rest-cancel` — so E, G, F2, A1 and A2 have nothing to act on until a `rest-hold` mode exists; it is a registered build-sequence item in `T0018`, its own spec and PR on the live trade path. **B's command IS built** (spec `00106`, `cli/engine/flatten.py`) and carries its own procedure at [`engine-procedures.md#engine-flatten`](engine-procedures.md#engine-flatten); what B waits on is the **engine converge** that puts `/usr/local/sbin/zcrypto-flatten` on the host. Each affected section states the gap in its own *Preconditions*; read it there rather than from any count here. Record **`blocked`** with that reason, never `fail`, and never run with a substitute plan mode: an order cancelled a second after it was placed exercises none of what these drills measure.
