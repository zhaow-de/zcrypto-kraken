...
- [`zcrypto-engine-cycle-stale`](engine.md#zcrypto-engine-cycle-stale) (critical, `metrics`) first, at ≈7 min. It is an instant read of a gauge the stopped container no longer publishes, so it pages by NoData from the first failed scrape: one 60 s scrape + `for: 5m` + 60 s.
...
- [`zcrypto-engine-exec-not-evaluated`](engine.md#zcrypto-engine-exec-not-evaluated) (warning, `metrics`) at ≈12 min: the gate's last-evaluation stamp goes stale with the container and that rule also pages by NoData from the first failed scrape, one scrape + `for: 10m` + 60 s.
...
- Past ≈7 min, [`zcrypto-engine-cycle-stale`](engine.md#zcrypto-engine-cycle-stale) (critical, `metrics`): the disconnect removes the published-port endpoint, so the scrape fails and the cycle gauge is stale from that scrape, NoData at one scrape + `for: 5m` + 60 s.
