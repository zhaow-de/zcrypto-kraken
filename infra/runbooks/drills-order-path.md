...
- **B cannot run today.** `zcrypto engine flatten` has not been built; it is a registered build-sequence item in `T0018`, its own spec and PR on the live trade path. Until it is built, deployed and carries its own procedure section, a run booked against B is **`blocked`** with that reason — never `fail`, which would assert a red button that did not work.
...
The flatten command, once it exists. **Its own procedure section lands with the command and is the authority on how to run it**; this section owns only the drill around it — the clock starts at the decision and stops when venue truth reads flat.

### Must fire

**Against a running engine with a plan in flight**, [`zcrypto-engine-exec-kill-tripped`](engine.md#zcrypto-engine-exec-kill-tripped) (warning, `metrics`) at **≈7 min** behind the decision, on the bound derived in drill E below. The kill file is the flatten's own first act; the page is that act arriving, not a fault.

**Against a running but idle engine, that page can be hours away and its absence proves nothing.** The gate publishes only when it is evaluated, and with no plan running the next evaluation is the 4-hourly boundary — so the gauge the rule reads has not moved yet. Read the kill file on the host instead of waiting for a page.
