...
**Nothing, and the absence of [`zcrypto-engine-exec-kill-tripped`](engine.md#zcrypto-engine-exec-kill-tripped) is not a finding.** The wrapper's order is kill file first, then `systemctl stop` on the unit, then a proof it is `inactive` (`zcrypto-flatten.sh.j2`). The exporter is therefore gone within a scrape or two of the kill file being written, so the rule never accumulates its `for:`, whether the engine was running a plan, idle, or already dark as D's response. Read the kill file on the host instead of waiting for a page.
