...
- the engine journal, **primary only** (no count command: `infra/ansible/inventory/hosts.yml`'s `engine_host` holds `zcrypto` alone) — `zcrypto-engine-journal-prune` at 01:23 UTC, day-dirs older than `engine_journal_retention_days` (60) with a keep-newest-`retention_days` floor;
