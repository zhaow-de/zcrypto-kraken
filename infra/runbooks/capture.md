...
1. **Read the state and its ladder.** The `system` value is on the page; the full transition sequence is in the capture log: `sudo docker logs zcrypto-capture 2>&1 | grep "venue status"` on either host (or the same line via Loki). Kraken's observed exit ladder — identical on all three known events — is `maintenance` → `cancel_only` → `post_only` → `online`. `effective_time` has been observed only as `None` — the 2026-08-06 event's five transitions and all sixteen non-`online` frames of 2026-09-03, both hosts each time — so expect no advance notice **in the frame**. The advance notice is on Kraken's published calendar instead, 41–49 h ahead on every event so far.
2. **A venue-side halt DOES book permanent loss. Expect the loss page, hours later.** All three known halts booked: **10588.382751 s** on 2026-08-06 (~15 min × 12 pairs), **6251.349974 s** on 2026-08-20 (~9 min × 12 pairs) and **2134.703242 s** on 2026-09-03 (~3.5 min × 12 pairs), each as `both_streams_silent` against hour 07, each written at the **09:12Z** tick. `zcrypto-reconcile-residual-gap` carries `for: 0s`, so it TRANSITIONS within a scrape of the booking — 09:12:40Z on 2026-09-03 — while the notification lands later, 09:24Z and 09:27Z on the two earlier events. Read the booking's own log line rather than waiting on the page: `archive reconcile: both_streams_silent hour=… verdict=… updates=… snapshots=…`, in the reconciler's journal on `zcrypto-ops` (needs `sudo`) or via Loki. The data is genuinely gone: the venue emitted nothing, so no host missed anything that existed, but "no component failed" is not "no loss". Do not tell yourself otherwise, and do not close the incident before the reconciler has spoken.
...
**Check the venue's published maintenance calendar before anything else.** Every firing in this rule's life has been one: 2026-08-06, 2026-08-20 and 2026-09-03, both hosts each time, each inside a "Kraken Website and API Maintenance" window announced 41–49 h ahead. That is 3 for 3 — no unexplained crossing has ever been observed.

It is nonetheless a **true positive, not noise**. The venue emitted nothing, so no host missed anything that existed, but L2 is unbackfillable and the hours are genuinely short. Quoting one convention throughout — **booked `both_streams_silent` seconds per stream**, which is what the reconciler pages on: 2026-08-06 cost **~882 s** per stream (10,588.382751 s over 12), 2026-08-20 **~521 s** (6,251.349974 s over 12) and 2026-09-03 **~178 s** (2,134.703242 s over 12). The venue-status section above quotes the same events as whole-window minutes; both are right and they are not the same number. Expect the reconciler's loss page about two hours later.
...
**If all 12 pairs on both hosts fire at once, this is not 24 stuck streams** — it is the fleet-wide event, and the critical rule above is the one to read. That is what both known firings were — 2026-08-06 and 2026-08-20. The fleet-wide event of 2026-09-03 never raised this rule at all: ~207 s per stream is under its 300 s bar, which is the length bound the venue-status section's step 4 describes.
