...
4. **Check whether CAPTURE degraded** — that is a different question from whether data was lost, and it is the one you can answer now: `zcrypto_capture_seconds_since_last_book_message` per pair, `RestartCount` on both daemons. **Which of the two capture silence rules fired measures the halt's LENGTH, not its severity**: the fleet-wide critical rule crosses at 120 s — guaranteed only by ~180 s of silence once a 60 s scrape is allowed for — while the per-pair rule needs over 300 s on a pair. So a halt of a few minutes fires the critical rule alone (2026-09-03, 206.66 s per stream, fired it on both hosts and left the per-pair rule Normal), a very short one can fire neither, and the per-pair rule sitting Normal BOUNDS the halt rather than exposing a blind spot. Whichever fired and auto-resolved around the window is that rule working, and each has its own section below if you need to triage one on its own. They report the same halt in a different vocabulary; they are not a second incident.
...
**If all 12 pairs on both hosts fire at once, this is not 24 stuck streams** — it is the fleet-wide event, and the critical rule above is the one to read. That is what all three known firings were.
