...
3. **Do NOT read `zcrypto_reconcile_residual_gap_seconds_total` during or just after the halt.** It cannot answer yet and it will answer reassuringly: hour H is bookable no earlier than H+2 h, at the next `:12`/`:42` tick, and `zcrypto_reconcile_last_success_timestamp_seconds` is run liveness, not coverage. Step 4 of the `zcrypto-capture-all-streams-silent` section below has the derivation.
