...
1. **Read the state and its ladder.** The `system` value is on the page; the full transition sequence is in the capture log: `sudo docker logs zcrypto-capture 2>&1 | grep "venue status"` on either host (or the same line via Loki). Kraken's exit ladder is `maintenance` → `cancel_only` → `post_only` → `online`. The frame's `effective_time` has only ever read `None` (no count command: the frames are venue data the tree does not keep), so expect no advance notice in the frame — read the value the line prints rather than assuming it; the advance notice is on Kraken's published calendar instead.
2. **A venue-side halt DOES book permanent loss. Expect the loss page, hours later.** The reconciler books it as `both_streams_silent` against the halt's hour. `zcrypto-reconcile-residual-gap` carries `for: 0s`, so it transitions within a scrape of the booking while the notification lands later. Read the booking's own log line rather than waiting on the page: `archive reconcile: both_streams_silent hour=… windows=… residual_s=… verdict=…`, in the reconciler's journal on `zcrypto-ops` (needs `sudo`) or via Loki. `residual_s` is the booked seconds summed across streams; divide by the stream count for the per-stream figure an incident record quotes. The data is genuinely gone: the venue emitted nothing, so no host missed anything that existed, but "no component failed" is not "no loss". Do not tell yourself otherwise, and do not close the incident before the reconciler has spoken.
3. **Do NOT read `zcrypto_reconcile_residual_gap_seconds_total` during or just after the halt.** It cannot answer yet (no count command: `cli/archive/settle.py`'s `SETTLE_HOURS`, pinned by `tests/test_archive_settle.py`) and it will answer reassuringly: hour H is bookable no earlier than H+2 h, at the next `:12`/`:42` tick, and `zcrypto_reconcile_last_success_timestamp_seconds` is run liveness, not coverage. Step 4 of the `zcrypto-capture-all-streams-silent` section below has the derivation.
4. **Check whether CAPTURE degraded.** That is a different question from whether data was lost, and it is the one you can answer now: `zcrypto_capture_seconds_since_last_book_message` per pair, `RestartCount` on both daemons. Whichever silence rule fired measures the halt's LENGTH, not its severity: the fleet-wide critical rule crosses at 120 s but its guaranteed detection floor is ~180 s of silence once a scrape interval is allowed for, and the per-pair rule needs over 300 s on a pair and gives notice at ~5–7 min once the series granularity and the group's evaluation interval are counted, so a short halt can fire the critical rule alone or neither. Both floors are derived in the rules' own comments. They report the same halt, not a second incident; each has its own section below.
5. **Do not converge or restart anything on this signal alone.** Nothing in the capture path reacts to venue status; a restart costs a resubscribe and buys nothing. A feed that heals itself has still cost unbackfillable L2 on every pair (no count command: a venue halt's reach is the venue's behaviour), which the loss page reports about two hours later.
6. **Silence `zcrypto-capture-venue-not-online` once triaged, time-boxed, and NEVER `zcrypto-capture-venue-state-recurrence`.** (no count command: a Grafana silence leaves no trace in the tree; `increase()` is Prometheus semantics; `venue_status` in `cli/capture/command.py` is an in-memory dict rebuilt at process start; `infra/grafana/alerts.yaml` has two rules on the venue counter, the latch the other) The latch is a counter-presence check, so it stays firing for the daemon's whole lifetime: the counter resets only when the capture daemon restarts, which is gated on a bake. Silence in Grafana on **that uid only**, for a bounded window, **with `system` matched to the states you have actually read**. Matchers are conjunctive, so a silence naming no `system` also suppresses the states you have not triaged, including a value new to this fleet; widen it as the ladder climbs, never ahead of it. **Delete it at each host's own next capture restart, per host, never fleet-wide**: the canary rule restarts the secondary days ahead of the primary, so one silence outlives the other host's reset. That deletion is the blind spot closing. A restart zeroes the counter, which makes the next event's states first sightings again, and on a first sighting the recurrence rule's `increase()` reads only what arrives after the first scrape, so a brand-new outage under a stale silence pages on a coin flip at best. Do not delete the rule. The recurrence rule is the one signal that still reports a repeat while the latch is stuck, and it self-resolves on its own; silencing it re-opens exactly the blind spot it was added to close.
...
2. **Treat it as current.** Unlike the latch, this one answers "is it recurring right now?", so a firing instance means a sighting inside the last 15 minutes rather than any time since the daemon started (no count command: the recurrence rule's expr in `infra/grafana/alerts.yaml` reads `[15m]`).
3. **Do NOT silence it.** It self-resolves roughly 15 minutes after the last sighting. Silencing it removes the only signal that still reports a repeat while the sibling latch sits stuck (no count command: `infra/grafana/alerts.yaml` has two rules on the venue counter, the latch the other).
...
Nothing. Do not "fix" this incidentally while working nearby without re-measuring the share first: the measured share is what makes it acceptable, and it is the number that would have to change to justify the I/O.
...
2. **Early closes firing, clock BEHIND** ⇒ benign. A clock lagging by less than five minutes closes every genuine boundary early on every writer whose first event past it lands inside the lag, up to 24 times a day (no count command: `tests/test_capture_segment_writer.py::test_t0037_lagging_clock_counts_early_by_design`). Fix the clock; nothing was lost. **The count itself is the tell**: order-100 over six hours means most boundaries on most writers, which is the clock; a handful means specific streams, which is a stamp.
...
Two causes, and this signal cannot tell them apart: a bogus exchange timestamp closed an hour early and that stream's remaining rows for it were refused as late (permanent, unbackfillable loss), or the machine's clock is running behind, which closes every genuine boundary early on every writer whose first event past it lands inside the lag, and costs nothing. The ambiguity is deliberate: the clock is not trusted with a veto over live data, so a lagging clock legitimately fires this.
...
It does not see the opposite fault: a clock running *ahead* truncates hours and hides the truncation from this measurement, so `zcrypto-capture-clock-skew` is the only detector for that case and this rule's silence is never coverage of the hour-rotation residuals. The `bogus-timestamp-hour-rotation` section above has the map.
...
3. **Name the streams.** Every early close is logged with its pair, kind, hour and how early it was (no count command: `cli/capture/segment_writer.py:_count_if_early` logs beside each increment): `sudo docker logs zcrypto-capture 2>&1 | grep "hour finalized early"` on the machine the page names, or the same line via Loki.
...
**There is no hard-zero baseline, and a step is not by itself a bad stamp.** The counter counts a first event that opened a past hour holding no captured parts. A fabricated stamp produces that shape, and so does a plain restart, which is why a step is evidence to work rather than a verdict. It steps **at most once per stream per capture process**, so it is a step detector rather than a rate and there is no threshold to tune. The four shapes below are what separate a real event from a benign restart.

The hour that opened held no confirmed capture on disk at the moment it opened. It is not a claim about the timestamp, and a re-open of an hour that already holds `.part` files does not count at all, so the commonest restart shape never reaches you. The bad hour reads as complete and certified, so nothing downstream will question it.
...
- This host's copy of the hour **matching** the peer's, with a restart on this host only (no count command: an unattended restart is container state the tree does not keep) ⇒ a crash replay, shape 2; the replay restored what the stop lost. A single-host crash leaves no gap on the peer, so "a gap on both" used as the sole test misfiles this one as a bad stamp.
- This host still short against a peer copy that is whole, with **no** stop, reboot or crash on this host accounting for the gap ⇒ shape 4. The rows never existed here (no count command: an inference from two hosts' copies the tree does not keep) and no replay is bringing them; the presence or absence of that accounting is the whole of what separates this from the two bullets above it.
...
1. **Anchor it to a process start**, because that is the only place it can happen (no count command: `cli/capture/segment_writer.py:_enter_hour` counts in its first-event branch alone): `sudo docker inspect --format '{{.State.StartedAt}}' zcrypto-capture` and `--format '{{.RestartCount}}'` on the machine the page names. A restart you did not expect is itself a finding.
2. **Name the hour and the stream.** The writer logs it: `sudo docker logs zcrypto-capture 2>&1 | grep "first stamp opened a past hour"` — the line carries pair, kind and the hour that was opened. **Read it as narrower than its wording**: the text says only "a past hour", but the writer emits it on exactly the counter's condition — a past hour that held no `.part` files when it opened. A restart re-opening an hour that already held parts logs nothing here, so every line you find is one of the four shapes above, never the wider condition the sentence reads like (no count command: `cli/capture/segment_writer.py:_enter_hour` logs and counts in one branch).
3. **Compare that hour against the peer machine's copy** — row counts and time coverage. This is not optional and it is not a tie-breaker: with step 1's restart record it is the only evidence separating a fabrication from a benign restart (no count command: an argument over evidence, which the four shapes above make), because shapes 1, 2 and 4 leave the same trace on this host. Take it before anything else touches the tree.
...
4. Until the exporter is back, treat any hour-rotation question on that host as unresolvable from metrics alone (no count command: an operator judgement nothing in the tree records).

### Retire when

`zcrypto-capture-clock-exporter-stale` is absent from `infra/grafana/alerts.yaml`, or `node_textfile_mtime_seconds{file=~".*/clock-offset.prom"}` is no longer what the rule reads — i.e. the offset's publisher carries its own liveness.
...
6. **Spills that persist across many hours are the design working**, not a stuck queue: that hour genuinely never had a second witness (no count command: `cli/capture/segment_writer.py:_open_hour` redeems a spill once quorum opens its hour), and keeping its rows out of a certified final is the safe side of an unanswerable question.
...
1. **Read the venue calendar first.** `curl -s https://status.kraken.com/api/v2/scheduled-maintenances.json`, filtered as `.claude/rules/fleet-deploys.md` states: `WebSocket` or `REST` (any case) in `components` **or in the entry's `name`**, covering the firing time; an empty `components` array is not an absent impact. **Use that endpoint, not `/scheduled-maintenances/upcoming.json`**, and match on the window rather than on the status: this feed keeps entries at `status: completed`, so it answers for a page you are triaging an hour or a day late, where the upcoming-only variant returns nothing and reads as "no window" (no count command: the venue's status API behaviour). A match explains the page completely — go to step 4. The recurring one is "Kraken Website and API Maintenance".
2. **No published window? Now it is a real incident.** Check both daemons: `sudo docker inspect --format '{{.RestartCount}}' zcrypto-capture` and whether parquet is advancing (`sudo find /var/lib/zcrypto-capture -name '*.parquet' -mmin -3 | head`). A silent-but-synced stream on a live connection never self-heals (no count command: `cli/capture/command.py` resubscribes on a desync alone; `_staleness_loop` books a gap), so this will not clear on its own if the daemon is wedged.
...
4. **Expect the loss page, and do not pre-empt it.** Booking waits on `SETTLE_HOURS = 2` measured from the hour's **start** (`settled_hours` takes `floor(now) − 2 h`), and the reconcile timer runs at `*:12,42`, so hour H is bookable no earlier than H+2 h, at the next `*:12`/`*:42` tick. Reading `zcrypto_reconcile_residual_gap_seconds_total` before then answers reassuringly and wrongly, and `zcrypto_reconcile_last_success_timestamp_seconds` does not protect you: it is run liveness, not coverage, and reads maximally fresh while a lost hour is still unbooked.
...
A Grafana query-execution failure does **not** raise this page: `execErrState: OK`, deliberately — see the `capture-silence-rules-and-datasource-errors` section at the end of this file.
...
3. **Restarting the capture daemon re-establishes every stream** (no count command: `cli/capture/ws_client.py:_subscribe_all` subscribes the configured pairs on connect), and is the remedy here precisely because the daemon will not self-heal a synced-but-silent stream on a live connection. Weigh it against the resubscribe cost and the canary rule if a converge is in flight.
...
Grafana Cloud intermittently fails to reach its own Prometheus. `Alerting` here would not *report* a blackout, it would assert one — a named host, every stream silent for minutes — that the query never ran to observe. Both rules carry `for: 0s`, load-bearing for their detection arithmetic, so a one-minute hiccup fired instantly. A correlated datasource outage is still reported loudly: four other `for: 0s` rules in the `zcrypto-capture` group keep `execErrState: Alerting`. A rule-scoped error on these two — an expression broken by a later edit, a permission or cardinality failure on this query alone — pages nothing, and is read instead by the daily pass's `## Rules not evaluating` report (`infra/scripts/ops_daily.py`).

### What to do

Several `zcrypto-capture-*` rules firing and auto-resolving within about a minute, with no host-side symptom, is a datasource error rather than a fleet event, and these two are deliberately absent from that storm. If you suspect the alerting pipeline is blind rather than the fleet quiet, check the datasource directly: `uv run python infra/scripts/grafana-query.py 'up{job="capture_app"}'` (a workstation step, it needs the repo and the vaulted token), and the healthchecks.io dead-man switches, an independent failure domain unaffected by Grafana.

### Retire when

Either rule's `execErrState` is no longer `OK` in `infra/grafana/alerts.yaml`.
