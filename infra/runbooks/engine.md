...
- **Order placeability moves with it, and not necessarily upward.** At one-sleeve gross the intended orders sit under the venue's minimum order sizes, which is why so few of them would clear at small live size. More gross moves orders back above those minimums; but more *sleeves* at unchanged gross spreads the same exposure across more instruments, which pushes individual notionals DOWN. Which of the two happened is a measurement, per leg, not an inference from the count.
