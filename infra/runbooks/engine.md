...
A book with one sleeve live is structurally a **one-sleeve book at a third of that sleeve's own gross** — the fixed weights — and lower again only while the exposure governor sits below its top rung, which happens under drawdown: `cli/risk/governor.py`'s ladder cuts to 0.5, then 0.25, then flat. Two consequences follow, and both are why this alert exists rather than a dashboard panel:
