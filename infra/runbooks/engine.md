...
A book with one sleeve live is structurally a **one-sleeve book at a third of that sleeve's own gross** — the fixed weights — and lower again while the exposure governor sits below its top rung. `cli/risk/governor.py` puts it there two independent ways: five bars at half after any bar losing more than the daily limit, and a drawdown ladder cutting to 0.5, then 0.25, then flat. A halved book is as likely to be the daily-loss arm as a deep drawdown. Two consequences follow, and both are why this alert exists rather than a dashboard panel:
