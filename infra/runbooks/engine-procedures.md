...
**What the flat verdict reads.** The command caches the venue's whole listing first, and its open-order and position reads are unscoped: the adapter resolves each row under either of Kraken's spellings of its pair, the `AssetPairs` key (`XXBTZEUR`) or the altname an order carries (`XBTEUR`). A row it cannot resolve — one on a pair the listing does not carry — fails the whole read rather than dropping out of it, and the command names that failure instead of counting past it. Before the cancel, a failed open-order read is recorded `orders_unread`, and the account-wide cancel, which names no pair, still goes out and reaches the orders nobody read. A failed whole-account position read is recorded `positions_unreadable` and retried one basket pair at a time, each pair still unread recorded `position_unread`; a per-pair read matches a position only under the pair's `AssetPairs` key, so a position outside the basket, or one Kraken spells by its altname, is outside those reads. Before the cancel, a first per-pair read that fails as well stops the run at exit 3 with nothing sent. The closers are sized from a fresh position read taken after the cancel, which retries the same way, and a position that read misses gets no closer. A press that read around a failure before the cancel ends at exit 2, never 0; so does one whose final read failed or could not see every position, and one whose closer-sizing or balance read after the cancel failed outright, while the two reads taken after the cancel only for the record step over a failure. So exit 0 means the reads before the cancel and the final read each came back whole, with no row on a pair the listing does not carry. It does not mean the order count held every resting order: an open-order row the adapter cannot parse is dropped from the read, with a `Failed to parse order` warning of the adapter's own, rather than failing it, and the account-wide cancel still reaches it. **Confirm it on Kraken's own pages all the same — step 4** — because the final read is taken the moment the writes finish, an order row the adapter dropped is in no count, and a page at Kraken is a reading from outside this command; if an order is still resting there, press the button again, which sends the account-wide cancel again.
...
4. **Confirm it by eye on Kraken.** Open orders and open positions on Kraken's own pages: exit 0 rests on the command's own reads, which leave out an order row the adapter could not parse ([what the flat verdict reads](#flat-verdict-reads)). The engine stays stopped and the kill file stays in place until you decide otherwise — that is what stops anything re-opening.
...
The wrapper runs the digest it was rendered with, the one the engine runs now. To read the account through an image a converge is about to deploy, before the engine depends on it, run the wrapper's own `docker run` line by hand with only the digest changed. It sends nothing and touches neither the engine nor its kill file. It does not replace the dry run above, which runs through the wrapper the converge renders and is what the second limit waits on. The commands below are typed in a shell on the engine host, opened with ssh's `-t`, because the `docker run` line takes `-it`.
