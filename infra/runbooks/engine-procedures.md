...
- **An engine converge that changes anything the restart handler watches re-owes the proving cycle** (no count command: the proof is a journal artifact on the engine host and reading it is an operator act nothing in the tree records). What it watches is whatever notifies `restart engine service`, and a `docker` converge on this same host bounces dockerd under the engine as well, so what settles the re-owe is whether the run restarted the engine, whatever the run was tagged — the disarm step below spells out what that cycle has to look like, and its own re-owe is one instance rather than the rule.
