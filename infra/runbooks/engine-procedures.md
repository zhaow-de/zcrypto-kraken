...
8. **Read the account through the second producer, and compare before arming.** The engine reconciles the account through its own Nautilus client at every restart, and on the pinned wheel that client is blind by spelling on five legs — BTC/EUR, ETH/EUR, XRP/EUR, LTC/EUR, ETH/BTC — and reports venue-held EUR as free (\[[T0160]\]). The Kraken CLI reaches the same account by a separate implementation, so a disagreement between the two is the finding and never a tie-breaker. From the workstation only (`kraken-cli` never runs on a remote host — `.claude/rules/fleet-deploys.md`): `kraken open-orders -o json`, `kraken extended-balance -o json`, `kraken positions -o json`, `kraken trades-history -o json`, read beside the boot's line `Reconciliation complete for KRAKEN: … external=N, open=N, fills=N, positions=N` (`sudo docker logs zcrypto-engine --since <the container's StartedAt>`). An order the CLI lists that the boot line did not count is looked for in the same log, where the observer logs each external order event it receives (`external order event ignored: …`); an order the log never names is outside the engine's view, and the engine's count alone never arms. `hold_trade` in `extended-balance` is the EUR the venue holds against resting orders. Record the four CLI counts and the held amount beside the boot line in the window's arming record.
