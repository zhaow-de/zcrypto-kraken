...
**The `BTC/EUR` above is the plan's SHAPE, and for three of the drills it is the wrong pair.** A1, F2 and G all turn on what the startup pass does with the order across a restart, and BTC/EUR is one of the five legs where that pass never sees it ([the restart's blind legs](#adopt-pass-blind-legs)) — the drill would produce no adopt line, cancel nothing, and read as a defect. Author those three on a leg Kraken spells one way: SOL/EUR, ADA/EUR, DOT/EUR, LINK/EUR, DOGE/EUR, AVAX/EUR or SOL/BTC.
