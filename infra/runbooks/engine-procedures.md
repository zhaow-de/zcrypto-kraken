...
- **An engine converge that changes anything the restart handler watches re-owes the proving cycle** (no count command: the proof is a journal artifact on the engine host and reading it is an operator act nothing in the tree records). Four tasks notify it — the secrets env file, `zcrypto.toml`, the compose file and the systemd unit — so a converge that touches none of them restarts nothing and owes nothing, and one that touches any of them owes one clean boundary cycle again, whatever the converge was for. The disarm step below spells out what that cycle has to look like, and its own re-owe is one instance rather than the rule.
