...
**Three limits it carries today.** From `2.0.0rc6.dev20260915` a fourth rides with them until the dry run below is taken: the command's FIRST read, `request_instruments`, is an authenticated call on this version — the adapter resolves fees from the account's own tier through `POST /0/private/TradeVolume` whenever the client carries credentials, and this client always does — so it now depends on the key's scope covering that endpoint and on every pair the listing returns carrying a fee entry. Nothing has exercised it: the engine's and the probe harness's instrument loads are both uncredentialed. A failure there is `FlattenUnreachable` and exit 3 before anything is sent, so the failure mode is a button that refuses rather than one that half-presses — but it refuses in the moment you need it. The reduce-only market close on a margin position has never been sent live from this repository on any order type; the first real press through this wrapper — on a position minted for the purpose against the current pin, with a margin leg wherever `ordermin` allows — is where that is first proven, and until its `docs/reference/adapter-verification/` row records it, treat a margin close as unverified and read Kraken's own positions page afterwards by eye. The drill program's decision-to-flat drill is a later, separate measurement and does not lift this caveat. And the five account reads have been proven against the live venue only once the [read-only dry run](#flatten-read-only-dry-run) recorded in `docs/reference/adapter-verification/` has been run through this wrapper; until that row exists, a live `--execute` is being run against read shapes nothing has confirmed.
