...
**Two limits it carried until 2026-09-24, both lifted by the attended window recorded in `docs/reference/adapter-verification/2.0.0rc6.dev20260921.md` (*The window as run*).** The reduce-only market close on a margin position was first sent live through this wrapper on 2026-09-24 against a position minted for the purpose on the pin of that day, `2.0.0rc6.dev20260921`: the closer went out `reduce_only` at leverage 2 on the MARGIN account and Kraken filled it, and the record's step 7 row holds it. The five account reads were proven against the live venue through this wrapper the same day, by the [read-only dry run](#flatten-read-only-dry-run) over that fixture — a resting order, a margin position and a spot balance each found — the record's step 5 row. A later pin re-earns neither by itself: the pre-probe sweep reads the version's own record. The drill program's decision-to-flat drill is a separate measurement.
...
Not part of a press — nothing is sent. This is what produced the row the second of the two limits above waited on, on 2026-09-24; the first waited on the row the first real press wrote the same day.
