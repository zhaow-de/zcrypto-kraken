...
**This rule is the only witness that the journal's parquet bytes are still being re-read at all.** The scoring cache skips `replay_cycle` on a fingerprint hit, and that replay is the single place the bytes are re-hashed — the journal pull runs `--no-verify` and delegates verification to it. So each run additionally **force-replays a rotating ~1/24 slice** regardless of a cache hit: a cycle's slice is `sha256(cycle_ts) % 24`, a permanent property of the cycle, and a run re-verifies the slice matching the index the pull loop hands it as `--slice` — **its own run counter modulo 24**, not the clock. So the whole journal is re-read within **24 runs**, about 26 h at the loop's measured 64-minute period, whatever that period drifts to. The bound is in runs, so it stretches with the period: past a ~3 h period one sweep exceeds this alert's 3-day bar and a healthy rotation would page.
