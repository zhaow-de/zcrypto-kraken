...
- **A repair can re-page you for the thing you just fixed.** The value can fall — re-fetch a corrupted parquet and the count drops — and PromQL treats any decrease in an `increase()` argument as a counter reset, adding the pre-reset value back. So a successful repair can synthesize a positive increase inside the window. **Confirm state by VALUE (`zcrypto_gate_mismatch_total`), never by whether the alert is firing** (no count command: PromQL's counter-reset rule is upstream; an operator's reading leaves no record in the tree).
- **A rise is not necessarily today's cycle.** The count spans every day the mirror retains; the journal pull is `rsync -a` with no `--delete`, so the NAS mirror never drops a day even though the engine host prunes at 60 days (no count command: `_evaluate_journal` globs every day-dir; `cli/archive/command.py`'s rsync carries no `--delete`).
...
2. **Is the export failing?** `sudo /usr/local/bin/docker logs --since 6h zcrypto-archive-pull | grep -E 'gate-export failed|could not write gate textfile|gate-export cache'`. Every `gate-export` failure reports through at least the loop's own ERROR line — the loop logs `gate-export failed (dest=...), continuing` and never exits (no count command: `infra/nas/pull-entrypoint.sh` runs `gate-export` under `if !` and logs that line). Print the log's line count before trusting an empty grep; `--since` needs a duration or a full timestamp, and a mis-parsed one yields empty output that reads like a clean bill.
...
**This rule is the only witness that the journal's parquet bytes are still being re-read at all.** The scoring cache skips `replay_cycle` on a fingerprint hit, and that replay is the single place the bytes are re-hashed — the journal pull runs `--no-verify` and delegates verification to it. So each run additionally **force-replays a rotating ~1/24 slice** regardless of a cache hit: a cycle's slice is `sha256(cycle_ts) % 24`, a permanent property of the cycle, and a run re-verifies the slice matching the UTC hour the run **started**. **That key is the clock, and the loop's period is 3600 s plus work, so the hour sampled drifts from run to run and coverage is not uniform — at some periods the same slices are never sampled at all, and the cycles in them are never re-verified.** Read the bar as "some part of the journal has gone three days unread", not as the three sweeps of slack over a daily sweep that the rule's own comment assumes. Because the fingerprint digests the `content_hash` the record *claims*, not a fresh hash of the file, a cache hit on an altered file would be served as a PASS forever without this rotation.
...
6. **Never point `--cache` at `/archive` or any path both hosts reach**, whatever the disk pressure (no count command: `infra/nas/pull-entrypoint.sh` holds the tree's one deployed `--cache`, at `/tmp`) — that is the poisoning the ephemeral siting exists to prevent.
