...
Both families break the clean-day streak, so `zcrypto-gate-streak-reset` follows at the next day-close if it has not already fired, and the hc.io `zcrypto-gate-verify` check goes red on the same run.
...
Causes, in the order they are worth checking: the archive-pull loop is dead or hung; `gate-export` runs and fails every time; `gate-export` runs but takes longer than 2 h; the textfile is fresh but Alloy is not shipping it.
...
4. **Is it just slow?** Read `zcrypto_gate_export_duration_seconds` on panel 503. A **cold** run after a container recreate replays the whole journal: measured **3505 s over 292 cycles on 2026-08-28**, and it grows with the journal — two such runs in a row can hold this alert up on their own. A warm run is a fraction of that. If a converge or re-pin just recreated the container, this is expected and self-clears; re-read the newest duration rather than trusting that figure.
5. **Is the file fresh but unshipped?** `ls -l --time-style=full-iso /volume1/docker/zcrypto-archive/textfile/gate.prom` and `cat` it — it holds only gate metrics, no secrets, and it is the ground truth when Grafana is dark. A recent mtime means the fault is Alloy: check whether `zcrypto-alloy-dark-nas` is firing, `sudo /usr/local/bin/docker logs --since 1h grafana-alloy | tail`, and `uv run python infra/scripts/grafana-query.py 'node_textfile_scrape_error{host="nas"}'` — a 1 means the collector rejected a file in that directory, which five other `.prom` files also share. Restarting Alloy is `cd /volume1/docker/zcrypto-archive && sudo /usr/local/bin/docker compose restart alloy`.
...
4. **Know what that costs before you run it.** The next run is a **cold replay of the whole journal — measured 3505 s (~58 min) over 292 cycles on 2026-08-28**, and the figure grows with the journal, so read `zcrypto_gate_export_duration_seconds` for the current one rather than trusting that number. The hc.io `zcrypto-gate-verify` dead-man was measured green throughout a post-recreate cold replay (2026-08-29), so the delayed ping is not itself a paging event — re-check that if the journal has grown much further. The metric disappears while the cache is empty (the rule reads `OK`, which is why `noDataState` is not `Alerting`) and returns on the following run.
