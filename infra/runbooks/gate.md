...
6. **Expect the healthchecks.io `zcrypto-gate-verify` check to go red too when the streak dropped to 0, and to stay red for at least a day.** The ping is clean only when the gate is healthy — `streak > 0`, or no complete day has ever been evaluable — **and** the journal lag is at or under the 21600 s bar; either alone fails it. A streak still above 0 keeps the ping clean; from 0 it pings `/fail` on every run until a fresh clean day accrues. The two domains are independent; once the streak is at 0 they agree here, and a disagreement is itself information.
