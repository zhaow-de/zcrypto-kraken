...
- **Predict an induction's collateral from its own *Must fire* clause, never from a fresh sweep of `alerts.yaml`.** Each section below already enumerates what else pages, with timings, because someone did that work under induction. A sweep for rules NAMING the thing you stop misses the ones watching what it PRODUCES — on 2026-09-02 that method predicted two of drill N's four, having skipped the section listing all four. **Treat the clause as a floor and add to it**: it has grown before (08-31 added a fourth to N's three). An unlisted page during an induction is *unclassified*, not *unexpected* — a prediction used as a triage filter turns one into the other.
