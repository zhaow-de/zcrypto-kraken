...
- **Never induce inside a published Kraken maintenance window.** Read `curl -fsS https://status.kraken.com/api/v2/scheduled-maintenances.json` at planning time **and again immediately before each induction** — the entries that matter carry `WebSocket` or `REST` in `components` **or in the entry's `name` where that name refers to an API** — an empty `components` array is not an absent impact (measured 2026-09-02: *"Scheduled maintenance for Kraken Prime REST, WebSocket, and FIX API"* ships `components: []`), and they appear only 2–6 days ahead.
...
- **A collateral list is NON-EXHAUSTIVE by construction, and an unlisted page during an induction is *unclassified*, not *unexpected*.** Predicting what an induction will trip is worth doing, but a prediction handed to anyone as a triage filter turns *unpredicted* into *unexplained* — so an incomplete list is worse than none for the signal it omits. **Sweeping `alerts.yaml` for rules that NAME what you stop finds only the first ring**; the second ring is the rules that watch what that thing PRODUCES, and they name neither it nor its host. Measured 2026-09-02: stopping `zcrypto-archive-pull` on the NAS tripped `zcrypto-reconcile-source-lag` — published by the **ops** node, watching the freshness of the pulled mirror, matching no container, host or metric that was touched — and it fired **four minutes BEFORE** the page that had been predicted. No search over names or label matchers could have found it; the only method that does is asking what stops being produced, then what watches that.
...
- **A Loki-sourced rule never sends a resolved notice, and its labels are not why.** Every Loki rule pins the `logs` receiver, minted `disableResolveMessage: true` deliberately since T0047. **So read a clear from rule state — `/api/prometheus/grafana/api/v1/rules` — never from channel silence**, which cannot distinguish resolved from still-firing. Do not try to fix it by re-grouping the expression: that was tried, on the theory that an unlabelled fire arm made recovery a `MissingSeries` deletion, and drill N's 2026-09-02 re-induction disproved it (see the entry). A fix is a routing change, not an expression edit.
...
| N | NAS archive-pull stall | re-drilled 2026-08-31 on the post-`T0048` path — the dead-man catches it at ~2 h 31 m; four collateral pages OBSERVED (a count is a floor, never a list — see the standing rule) |
