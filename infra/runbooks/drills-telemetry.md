...
- **A Loki-sourced rule never sends a resolved notice, and its labels are not why.** Every Loki rule pins the `logs` receiver, minted `disableResolveMessage: true` deliberately since T0047. **So read a clear from rule state — `/api/prometheus/grafana/api/v1/rules` — never from channel silence**, which cannot distinguish resolved from still-firing. Do not try to fix it by re-grouping the expression: that was tried, on the theory that an unlabelled fire arm made recovery a `MissingSeries` deletion, and drill N's 2026-09-02 re-induction disproved it for THIS rule (see the entry). `MissingSeries` self-resolution is still real on `metrics` rules — `alerts.yaml` records it on `zcrypto-reconcile-residual-gap` after a real loss — it is simply not what silences a Loki rule's clear. A fix here is a routing change, not an expression edit — registered as \[[T0162]\].
