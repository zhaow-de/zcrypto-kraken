...
**Secondary half**: `zcrypto-alloy-dark-capture-secondary` (critical, `metrics`) at the same ≈16 min. `zcrypto-hcio-watchdog` must stay **quiet** here — no check is fed by that host's Alloy.
...
**Secondary half run 2026-08-31 — `pass`, and it BOUNDS the ops half's finding.** Held 2 h 01 m 01 s; two pages, not six — `zcrypto-alloy-dark-capture-secondary` at 14 m 54 s and `zcrypto-capture-textfile-missing` at 25 m 14 s, the latter a **value** page (`count(...) lt 2`, both capture hosts must publish, `for: 20m`) that doubles as an Alloy-dark detector for the pair. The capture daemon ran throughout, confirmed by its own dead-man pinging while Alloy was 52 min down. **Neither plane recovered anything here.** A naive count shows 76 log lines inside the window and none of them are replay: 28 are Alloy's own shutdown burst stamped at the stop second with zero in the middle, and 48 are the capture daemon's direct-shipped JSON, which never traverses Alloy. So the ops half's rule — logs recover, metrics do not — **holds only where something writes to the journal during the outage**. Ops has timers doing that and replayed 8 of 8; this host has none and replayed nothing. Recovery is a property of the host's workload, not of Alloy. **And `msg="Done replaying WAL"` on restart is not evidence of backfill**: the remote_write WAL buffers scraped-but-unsent samples, and a stopped Alloy scraped none — that message attests to a dark destination, never to a stopped shipper.

**Ops half run 2026-08-31 — `pass`.** Held 2 h 00 m 55 s (12:32:25Z → 14:33:20Z); all six of drill K's pages fired and nothing outside that set did. **The planes recover asymmetrically, which is this drill's finding**: logs replayed the window IN FULL — 8 owed and 8 recovered, **derived from the timers' own `OnCalendar` schedules** rather than from a neighbouring window, under `max_age = "48h"` — while metrics recovered NOTHING, 5 samples in 120 min and all inside the first four, which is the range query's 5 min lookback carrying the last real sample forward rather than late ingestion. Nothing scraped leaves no buffer to replay, so a metrics dashboard across an Alloy outage shows a hole rather than a flat line. **Count a neighbouring window only if that neighbour was undisturbed** — the 2 h before this stop was not, since drill O held the panel timer down through it. **No rule misfired on return among the candidates whose `for` the query window outlives** (nine, up to `for: 15m`, with a control proving the window visible); the `for: 30m` and `for: 27h` candidates are unverified and named as such rather than counted clean. `zcrypto-fleet-daemon-restarted` stays quiet because a 2 h gap leaves only one sample in its `[15m]` window — a SHORT hold is what would trip it, derived from the expression and not yet observed. Restored with `docker start`, this drill's own recipe rather than K's `restart`; clear by value 1 min 36 s later.
...
**Run 2026-08-31 — `pass`.** Native route 2 s to Slack. The watchdog: induction→`Alerting` **367 s** against the 420 s derived bound, plus 33 s of Slack delivery the bound has no term for — 400 s to the message. `Engine · cycles have stopped` stayed inactive, recorded but **not** evidence of anything: it reads an engine-side gauge on a 4 h 35 m bar that no healthchecks.io ping can move. Device leg not taken here by design — same route as drill I, where Q's path 3 measures it. Two measurement traps this run uncovered are in this page's standing rules above — the rule-state endpoint keys on title, and a firing `activeAt` is firing-onset.
...
**Run 2026-08-31 — `pass`.** All six pages fired inside their derived bounds, measured induction → `Alerting`: the watchdog at 9 min 51 s and the four NoData rules between 10 min 01 s and 10 min 51 s against ≈11 min, `zcrypto-alloy-dark-ops` at 15 min 51 s against ≈16 min. Slack delivery ran a further 31–39 s and is not part of those bounds. Every rule went `Pending` between +4 min 51 s and +5 min 51 s and `Alerting` exactly its own `for` later — **consistent with** the ~5 min staleness term, not a confirmation of it: staleness runs from each series' last sample rather than from the induction. **Three of the six cleared their bound by only 9 s** (archive-pull, verified-replay, Alloy-dark) — under one evaluation tick, so treat those three bounds as tight rather than proven roomy. The `docker restart` recipe worked on a stopped container; ops read fully recovered by value 1 min 42 s later. The watchdog measured **999**, not 1 — its hc.io-dark fallback, since the ops Alloy is that scrape — so this drill proved the half of that rule drill J′ did not.
...
**N is due now, and it is the only row that is.** Its proof was the 2026-07-15/16 incident, in which the NAS archive-pull's log shipping went dark and `zcrypto-nas-archive-pull-stalled` was the rule that noticed. The path it fired through has been rebuilt since: `discovery.docker` and `loki.source.docker` are gone from every Alloy config, and that container's lines now reach Loki over the journald driver through `loki.source.journal` and its `journal_units` relabel (`infra/nas/config.alloy`). The dead-man is unchanged; **the transport underneath it is not the one the incident proved**, so the proof no longer covers what runs today.
