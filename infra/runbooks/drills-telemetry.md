...
**Run 2026-08-31 — `pass`.** Native route 2 s to Slack; the watchdog 6 min 40 s, inside its ~7 min derived bound; `Engine · cycles have stopped` correctly stayed inactive; device timestamp owed. Two measurement traps this run uncovered are in this page's standing rules above — the rule-state endpoint keys on title, and a firing `activeAt` is firing-onset.
