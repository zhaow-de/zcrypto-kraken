...
That an explicit `/fail` ping actually reaches a phone. The engine's own call is unit-tested; what has never been observed end to end is the **routing** — the ping leaving a process, healthchecks.io transitioning the check, and Slack showing it. This drill proves the route, not the caller, and the entry says so.
