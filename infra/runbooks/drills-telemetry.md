...
- **The primary's capture daemon and the primary's Alloy are never a drill's subject** (set: `docs/reference/drill-log.md` entries; count: `infra/scripts/count-list.sh drills-on-the-primary`). The rule is `.claude/rules/fleet-deploys.md`; the subjects here are ops, the secondary and the NAS. The order-path tier's entries run on the engine host and open their *host* clause `` `zcrypto`, the engine ``, which the count reads as outside this set.
