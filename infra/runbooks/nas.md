...
| -- | -- | -- |
| `archive pull: rsync failed source=… dest=… returncode=N` | `pull` in `cli/archive/command.py` | transport to that source — 255 = ssh itself; 23 = partial transfer (an unreadable file at the source); 12 = protocol/disk; 24 = files vanished between the file list and the transfer: on a capture or liquidations channel at an hour boundary, the segment writer merging an hour's parts, benign when its next pass reads `failed=0` |
