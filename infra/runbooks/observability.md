...
4. **Value 999** ⇒ `ssh hp`, then `sudo docker ps --format '{{.Names}} {{.Status}}' | grep alloy` and `sudo docker logs grafana-alloy --since 15m 2>&1 | grep -i healthchecks`. A 401/403 means the read-only metrics key changed — it is vaulted and reaches the container through `alloy-secrets.env`, so fixing it takes a converge of the ops role plus an Alloy recreate, which is attended. A timeout or DNS error is egress from ops: `curl -fsS -m 10 -o /dev/null -w '%{http_code}\n' https://healthchecks.io/` from the ops node separates hc.io being down from ops being unable to reach it. **Never print the container environment to find the key** — no `'{{json .Config.Env}}'`, no `docker exec … env`, no `docker compose config`.
