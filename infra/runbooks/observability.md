...
**On ops the tighter clock is `zcrypto-hcio-watchdog`, not this rule**: once `hc_checks_down_total` goes stale, its `vector(999)` fallback fires after `for: 5m`, while this rule waits `for: 10m` on the same staleness.

**This family sees "not shipping" and nothing else.** The alive-but-wedged state — Alloy up, its `up` series flowing, a whole class of target silently unscraped — is read by nothing. So when a specific family is missing while this rule reads Normal, do not conclude the transport is fine: prove the family by value with `grafana-query.py`, because `(no series)` is a FAIL and never a zero.
...
2. **Is the container running?** `sudo docker ps --filter name=grafana-alloy` — **on the NAS, `docker` is off the non-interactive ssh PATH, so always `sudo /usr/local/bin/docker …` for every docker command in this section.** (no count command: an operator's shell on the NAS leaves no record)
3. **Read the container's state with SCOPED fields only** — this container's environment holds the Grafana Cloud push credentials, so never `docker inspect … '{{json .Config}}'`, never `{{json .Config.Env}}`, never `docker exec … env`, never `docker compose config` (set: the non-comment lines of the non-Markdown files under `infra/`, `.claude/`, `cli/` — the roles, templates, units, hooks and scripts a command runs from — with the counter itself excluded; the Markdown runbooks and skills, where the form appears as the prohibition's own text, are outside it; count: `infra/scripts/count-list.sh engine-env-forms-invoked`):
   ...
   `oom=true` ⇒ it hit its container memory cap; `fleet.md#zcrypto-fleet-alloy-memory-headroom` is the warning that precedes it and carries the caps.
...
6. **A recreate is needed instead when the container is absent, or when its env / memory cap changed** — a config reload is an HTTP `POST /-/reload` that never re-reads container env (no count command: a property of the upstream binary; a container's env is fixed at create): `cd /etc/zcrypto-capture/alloy && sudo docker compose up -d` on a capture host, `/etc/zcrypto-ops/alloy` on ops. **`sudo` is required** — `alloy-secrets.env` is mode 0600 owned by the `zcrypto-alloy` user, and an unprivileged `up -d` dies reading it *before* touching the container, so the old one keeps running and no new dark window opens. On the NAS, Alloy shares one compose project with `archive-pull` (`/volume1/docker/zcrypto-archive`), so a `compose up -d` there bounces the pull loop too — prefer `sudo /usr/local/bin/docker restart grafana-alloy`.
7. **A config fault is an attended converge, not a host edit.** Compare the deployed file to the repo (`sha256sum /etc/zcrypto-capture/alloy/conf/config.alloy` against `sha256sum infra/ansible/roles/capture/files/config.alloy`). Re-converging is an attended action with the user's word — load `.claude/skills/zcrypto-bump-alloy/SKILL.md` first; it carries the digest-drift assert (pass the **currently running** Alloy digest, and never an empty `-e …_digest=`, which counts as defined and renders a broken image ref (set: the `extra_vars` of every row in `docs/reference/deploy-log.jsonl`; count: `infra/scripts/count-list.sh deploy-rows-with-an-empty-digest-var`)), the primary's mandatory `--skip-tags engine -e converge_primary=true`, and the NAS's `-e nas_apply_compose=true`.
...
2. **Write down which rules are disarmed, and which will page instead,** from the table above, before you start fixing — that list is what you must re-verify at the end, and it is what you are flying without in the meantime.
...
4. **Go to the collector's own cause:**
   ...
   - `filesystem` — a mount the exporter cannot stat (no count command: the host's mount state, which nothing in the tree records). A hung NFS mount is the classic one on ops and the NAS; the tell is `df` itself hanging. `mount | grep nfs`, then `df -h` in a separate shell you are willing to lose.
   - `cpu` / `loadavg` / `meminfo` / `netdev` — the `/host/proc`, `/host/sys` or rootfs mounts are missing after a compose edit. Check them with a **scoped** inspect: `sudo docker inspect grafana-alloy --format '{{json .Mounts}}'` — never `.Config`, never `.Config.Env` (set: the non-comment lines of the non-Markdown files under `infra/`, `.claude/`, `cli/` — the roles, templates, units, hooks and scripts a command runs from — with the counter itself excluded; the Markdown runbooks and skills, where the form appears as the prohibition's own text, are outside it; count: `infra/scripts/count-list.sh engine-env-forms-invoked`).
5. **Fix the underlying mount or file.** Alloy needs no restart for a transient error — the next scrape recovers on its own. `sudo docker restart grafana-alloy` only if the exporter itself is wedged (no count command: a restart is an operator action nothing records); it is telemetry-only and the WAL survives.
6. **Verify by value, twice:** `grafana-query.py 'min by (host) (node_scrape_collector_success{host="<host>"})'` reads 1, **and** the dependent family is actually back — e.g. `node_filesystem_avail_bytes{host="<host>"}` returns rows. `(no series)` on the second read means the collector reports success while shipping nothing, which is the failure this rule cannot see (no count command: its expr in `infra/grafana/alerts.yaml` reads `node_scrape_collector_success` alone).
...
- **`Logs · lines dropped before reaching Loki`** (uid `zcrypto-logship-lines-dropped`) — `increase(zcrypto_logship_dropped_lines_total[6h]) > 0`, held 15 min. Log lines were **discarded**. Baseline is zero.
...
*Dropped* means lines were rejected or evicted. `cli/logging/ship.py` increments `dropped_total` on exactly two paths: the in-memory ring overflowed (`RING_CAPACITY = 4096`, the deque evicts the oldest), or the push came back as a permanent rejection — any HTTP 4xx **other than 429**, which in practice is a revoked token, a wrong URL path, or entries aged past Loki's out-of-order window after a long outage.
...
3. **Never print the environment to find the credential** (CLAUDE.md § Secrets) (set: the non-comment lines of the non-Markdown files under `infra/`, `.claude/`, `cli/` — the roles, templates, units, hooks and scripts a command runs from — with the counter itself excluded; the Markdown runbooks and skills, where the form appears as the prohibition's own text, are outside it; count: `infra/scripts/count-list.sh engine-env-forms-invoked`). Its identity is knowable from the repo: `logship_loki_token`, vaulted, rendered into `logship-secrets.env` — mode 0600, `root:root` at `/opt/zcrypto-capture/logship-secrets.env` on the capture hosts, `zcrypto-deploy:zcrypto-deploy` at `/etc/zcrypto-ops/logship-secrets.env` on ops.
...
3. **Separate "never started shipping" from "shipping is failing":** (no count command: it names one daemon's state, which the query below reads)
...
4. **Never print the env to inspect the credential** (CLAUDE.md § Secrets) (set: the non-comment lines of the non-Markdown files under `infra/`, `.claude/`, `cli/` — the roles, templates, units, hooks and scripts a command runs from — with the counter itself excluded; the Markdown runbooks and skills, where the form appears as the prohibition's own text, are outside it; count: `infra/scripts/count-list.sh engine-env-forms-invoked`) — on the primary that host also carries the live trade key.
5. **Fixing it means recreating the capture container, which is an ATTENDED action.** Re-rendering `logship-secrets.env` takes a converge, and the new env only reaches the process on a container recreate — a restart of the unbackfillable capture daemon. Do not do it on your own initiative: get the user's word, read `.claude/rules/fleet-deploys.md`, do the **secondary first**, and on the primary pass `-e converge_primary=true --skip-tags engine` so the engine role never runs (no count command: Docker reads `env_file` at create, and `site.yml` tags the engine role `engine`).
6. **Verify by positive trace, never by the rule going quiet:** a Loki query `{host="<host>", container="capture", level=~".+"}` showing lines inside the last 5 minutes (no count command: a verification the operator performs, which nothing records). An empty result is a FAIL. The rule returns to Normal within one evaluation after real lines land.
...
- **Through Alloy**: the `zcrypto-*.service` units the keep-regex above names and Alloy's own container stream, both read by one `loki.source.journal "zcrypto_units"` component off `/var/log/journal`, then labelled and level-parsed by two separate `stage.match` blocks (`{container=~"zcrypto-.*"}` for the units' Python logging shape, `{container="alloy"}` for Alloy's logfmt).
...
- **`log-pipeline-dead` too** ⇒ **Alloy itself** is dead or cannot reach Loki (no count command: a reading of the live host, which the tree does not hold). Check whether metrics still flow (`node_load1{host="ops"}` freshness); if they are gone as well, `zcrypto-alloy-dark-ops` owns it.
- **`poller-log-dead` alone** ⇒ the poller's own ship path. The Alloy-side rules say nothing about it and never could (no count command: the three Alloy-side exprs in `infra/grafana/alerts.yaml` select no `liquidations` stream).
- **All four** ⇒ the host or its egress, not any one component (no count command: a reading of the live host, which the tree does not hold).
...
**The margin on `log-pipeline-dead` is thin by design.** Read a low count on that stream as a reason to look, not as headroom.
...
1. `sudo docker logs grafana-alloy --since 6h 2>&1 | tail -50` — Alloy is quiet but never silent while alive (no count command: upstream Alloy's behaviour, measured in `zcrypto-ops-log-pipeline-dead`'s summary), so an **empty** tail is itself the finding.
2. `sudo journalctl CONTAINER_NAME=grafana-alloy --since -6h --no-pager | wc -l` — print the count. A non-zero count here with nothing in Loki isolates the fault to `loki.write`, not to the journal. **Run it with `sudo`**: unprivileged `journalctl` prints a you-cannot-see-system-messages hint on stderr and `-- No entries --` below it on stdout (no count command: the hint is the upstream `journalctl` binary's own output), and that empty result is a permissions artifact, not an idle unit.
3. `sudo docker logs grafana-alloy --since 1h 2>&1 | grep -i 'loki\|remote'` for the push error.

**For `journal-transport-dead` (the reader):**

1. **Read the journal, and prove you read something**: `sudo journalctl -u zcrypto-archive-pull.service --since -2h --no-pager | wc -l` first, then the same without `wc`. `-- No entries --` **with** the you-cannot-see-system-messages hint (no count command: the hint is the upstream `journalctl` binary's own output) means the `sudo` did not take — a permissions artifact, not an idle unit. `-- No entries --` **without** it is an empty window, and that has two causes: with the staleness rules firing alongside the unit is genuinely not running and they own it, not this one; without them the unit may be running under a name this command is not asking for, so continue to step 4 rather than stopping here. Do not add `-q` — it silences the hint and with it the discriminator. No output at all is the command itself failing: `journalctl` exits non-zero and says why on stderr, while `wc` still prints 0 and the pipeline's status is `wc`'s.
...
4. Did a unit get renamed? `systemctl list-units 'zcrypto-*.service' --all` against the keep-regex in `infra/ansible/roles/ops/files/config.alloy`, read from the file itself (`grep -n 'zcrypto-(' infra/ansible/roles/ops/files/config.alloy`) — the glob matches more units than the regex keeps, so a unit missing from the regex is not by itself a rename. A rename must touch that regex in the same change or its lines stop being kept.
...
4. Unparsed lines are **not** lost: `action_on_failure = "skip"` keeps their journal capture time, so the scripts' own echoes still arrive — they simply carry no `level`.
...
1. `sudo docker ps -a --filter name=zcrypto-ops-liquidations` and
   ...
   — the entrypoint must include `--ship-logs`. It is omitted at render time when the vaulted token is undefined, and the container then starts normally on stdout only. **Inspect the entrypoint, never `.Config.Env`.** (set: the non-comment lines of the non-Markdown files under `infra/`, `.claude/`, `cli/` — the roles, templates, units, hooks and scripts a command runs from — with the counter itself excluded; the Markdown runbooks and skills, where the form appears as the prohibition's own text, are outside it; count: `infra/scripts/count-list.sh engine-env-forms-invoked`; for the stdout-only shape, no count command: `infra/ansible/roles/ops/templates/compose.yaml.j2` renders `--ship-logs` only under the vaulted token)
2. Separate "poller dead" from "ship path dead" **before** touching anything: `grafana-query.py 'zcrypto_liquidations_last_success_timestamp_seconds{host="ops"}'` — a fresh timestamp means the recorder is recording and only its logs are missing (no count command: a reading of the live gauge, which the tree does not hold).
...
4. **Value 999** ⇒ `ssh hp`, then `sudo docker ps --format '{{.Names}} {{.Status}}' | grep alloy` and `sudo docker logs grafana-alloy --since 15m 2>&1 | grep -i healthchecks`. A 401/403 means the read-only metrics key changed — and it lives in TWO vault variables that must match: `hc_prometheus_metrics_path` (`group_vars/observed/`), which Alloy scrapes with, and `healthchecks_readonly_api_key` (`group_vars/all/`), which the daily pass reads. Only the metrics-path copy reaches this host, through `alloy-secrets.env`: fixing it takes a converge of the ops role plus an Alloy recreate, which is attended. The `group_vars/all/` copy is read from the workstation by file path — rewrite it and `infra/scripts/ops-daily.py` picks it up on its next run. A timeout or DNS error is egress from ops: `curl -fsS -m 10 -o /dev/null -w '%{http_code}\n' https://healthchecks.io/` from the ops node separates hc.io being down from ops being unable to reach it. Never print the container environment to find the key (CLAUDE.md § Secrets) (set: the non-comment lines of the non-Markdown files under `infra/`, `.claude/`, `cli/` — the roles, templates, units, hooks and scripts a command runs from — with the counter itself excluded; the Markdown runbooks and skills, where the form appears as the prohibition's own text, are outside it; count: `infra/scripts/count-list.sh engine-env-forms-invoked`; for the key's copies on a host, set: the non-comment lines of `infra/ansible/roles/`; count: `infra/scripts/count-list.sh hc-readonly-key-in-a-role`).
5. **Never silence this rule to stop the double-page.** (no count command: a Grafana silence leaves no trace in the tree) The double is the design: healthchecks.io's own notification may be the broken half. Silence the check's own cause instead.
6. **Confirm by value:** `hc_checks_down_total` reads 0 **and** every `hc_check_up` reads 1 (no count command: a reading of the live gauges, which the tree does not hold). An empty result is a FAIL — it is the 999 state again, not a green fleet.
...
**Time from the outage to the phone differs by route.** The readings live in [`docs/reference/drill-log.md`](../../docs/reference/drill-log.md)'s `C′` and `C′-fail` entries, and the procedure is [`drills-telemetry.md#drill-c-prime`](drills-telemetry.md#drill-c-prime). Staleness route (the prober dies): the `zcrypto-grafana-watchdog` check pages natively at its own `timeout 600 + grace 600`, measured from its LAST CLEAN PING and not from the outage — the two anchors differ, so quote the anchor with the number. That notice travels healthchecks.io's own Slack integration, the domain that survives Grafana being dark; the Grafana rule `zcrypto-hcio-watchdog` follows separately and later. `/fail` route (Grafana unreachable from ops): a `/fail` is an immediate down transition, so no `timeout` + `grace` term applies and the number is notification latency alone, but the probe runs `OnCalendar=*:0/5:41`, so add up to one 5 min period from the outage itself. On that route `hc_check_up` moves only when the ops Alloy next scrapes healthchecks.io (`prometheus.scrape "healthchecks"`, `scrape_interval = "60s"`), so a responder watching Grafana can still read green after the page has fired: time it from the device, never from the metric. Neither route bounds what a Cloud outage LOSES — that is the DERIVED paragraph below.
...
**What a Cloud outage loses permanently, per plane** — a stopped shipper and a dark destination lose opposite planes, so this statement carries a measured half and a derived half, each labelled.

**MEASURED — a stopped shipper** (drill C, both halves; entries in [`docs/reference/drill-log.md`](../../docs/reference/drill-log.md)). **The metrics plane is lost outright and nothing recovers it**: nothing was scraped, so there is no buffer to replay from. **A metrics dashboard read across an Alloy outage shows a hole, not a healthy flat line** — treat a flat trace over a known outage as absent, never as steady. **The logs plane replays from the journal positions file under `max_age`**, but only what was written to the shipped journal while the shipper was down, which is a property of the host's workload rather than of Alloy: on ops, whose timer units log through the hold, the replay was complete; on the capture secondary it recovered nothing, so do not generalise the ops result to a host that writes nothing to the journal. On return, split in-window lines by WHERE IN THE WINDOW they fall: a bulk count can exceed the pre-outage baseline and contain no replay at all, a shutdown burst at the stop second is not recovery, and direct-shipped lines were never lost. Alloy's own `msg="Done replaying WAL"` attests to the remote_write WAL replaying **scraped-but-unsent** samples, so a stopped shipper has none — that line speaks to the case below, never to this one.

**DERIVED — a dark destination**, from `config.alloy` rather than from a drill; no induction here has taken Grafana Cloud away. The planes swap. `prometheus.remote_write "grafana"` keeps a WAL, so metrics scraped while the endpoint is unreachable are buffered and re-sent on recovery, bounded by that WAL's retention. `loki.write "grafana"` declares **no `wal` block at all**, so logs are bounded by whatever the running Alloy's default is — read that off the version actually deployed rather than trusting a number written here, and treat the direct-shipped daemon logs separately per the paragraph below. **Neither half of this bound has been drilled.**
...
1. **Confirm the stack is dark rather than unreachable from one place.** Run the watchdog's own probe twice — once on ops (`ssh hp`) and once from the workstation — with the watchdog's own flags: `curl -fsS -m 20 --retry 2 --retry-delay 5 -o /dev/null -w '%{http_code}\n' https://zcrypto2026.grafana.net/api/health`. **Do not shorten the timeout or drop the retries.** The check is pinged green on anything this command accepts (`grafana-watchdog.sh.j2`), so a less patient probe can fail against a Grafana the watchdog is still calling healthy, and you would read a degraded stack as a dark one. Failing from both is the outage. Failing from ops alone is ops egress with Grafana healthy — a fault in the watchdog's route, and this is the wrong page for it.

2. **Read the dead-man domain directly — a read that never passes through Grafana.** `uv run python infra/scripts/ops-daily.py report --since 24h`: its dead-man half fetches `https://healthchecks.io/api/v3/checks/` with the vaulted read-only key (`healthchecks_readonly_api_key`, `group_vars/all/vault.yml`) and is unaffected. **Expect exit 2 and a `## Sources that could not be read` block naming the Grafana ones** — that is the outage being reported, not a fleet finding, and every part of the report not read through Grafana still stands. **Its `## Dead-men` line prints how many checks were read, never which are down** (no count command: all three describe `infra/scripts/ops_daily.py`'s `read_deadmen` and `Report.markdown`): for the names, statuses and last ping times, open `https://healthchecks.io` itself.

3. **Then read each host by hand.** Every command here is host-local and needs no telemetry stack — except `exec-status`, whose gate re-reads Kraken's public status endpoint on every evaluate, so a `venue_not_online` reason carrying `venue_status=unreachable` is a transport failure on that path — this host's egress or Kraken's endpoint, not a state Kraken published (no count command: `cli/engine/venue.py`'s `read_system_status` is this step's one outbound call):
   ...
   - **Engine**, on the primary — **the gate**: `sudo docker exec zcrypto-engine zcrypto engine exec-status`, the only place, besides `probe-plan --check`, the gate's `reasons` and the two arming keys are visible separately, re-evaluated on the spot (no count command: `cli/engine/command.py`'s `_echo_gate_verdict` has those two callers). It prints `level`, `reasons` and the gate's inputs and **nothing else — no position, no resting order**, so it answers half of the dark position alert and must not be read as answering the other.
   - **Engine**, on the primary — **the exposure** as the engine last journaled it, which is on disk and not in any command's output. [`engine-procedures.md#engine-probe-window`](engine-procedures.md#engine-probe-window) carries both reads whole — use them rather than retyping one. Two things to carry into the read: the position it reads is the same quantity `zcrypto_exec_position` is seeded from, so it is the dark alert's own number rather than a proxy, and `<HH>` is the 4-hourly cycle boundary (00/04/08/12/16/20 UTC), never the wall-clock hour (no count command: no entry scans the engine's echoes; `_boundary` in `cli/engine/executor.py` floors the hour each record is filed under). Then `sudo docker logs zcrypto-engine --since 1h` for the cycle.
   - **Ops** — `ssh hp`, then `sudo docker ps --format '{{.Names}} {{.Status}}'` and `sudo systemctl list-timers 'zcrypto-*' --all --no-pager` for each unit's last and next run. **Not `ops-postverify.sh`**: every check in it will read FAIL because the query behind it cannot run (no count command: `infra/scripts/ops-postverify.sh`'s `check()` sends each one through `$QUERY`), and a FAIL there would be read as a fleet fault.
...
4. **Change nothing you cannot verify.** (no count command: nothing in the tree records a Grafana Cloud outage's span) A converge, a re-pin, a restart or a prune each end in a verify-by-value that this outage has taken away, and `.claude/rules/fleet-deploys.md` requires that verification. Unless a read above found a real fault, the action is to wait — and to write what you read into the incident record, because none of it is recoverable from Grafana afterwards.

5. **On return, re-verify by VALUE what nothing evaluated.** No rule ran while the stack was dark, so a rule reading Normal now is reporting on the evaluation it has just done and on nothing that happened during the outage. **Do not read the metrics plane's replay as cover**: `prometheus.remote_write` does refill its WAL's worth of samples, so a window-reading rule can still see an event that fits inside both the WAL's retention and the rule's own window — but a longer outage overruns the first, `loki.write` has no WAL to replay at all, and a condition that both arose and cleared inside the gap was evaluated by nothing either way. The rules reading a *change* across a window are where such an event leaves no trace afterwards: `grep -nE 'delta\(|increase\(|resets\(' infra/grafana/alerts.yaml` names every rule of that shape except those built on `changes()` (no count command: the grep is the enumeration; no entry counts the `changes()` rules it misses). Read each subject's level rather than the rule's state, starting with the ones whose miss is permanent:

   - `zcrypto-reconcile-residual-gap` — permanent, unrecoverable L2 loss, the highest-severity rule in the system. Its authority is the ops ledger, which never went dark (no count command: the ledger is a file on the ops host, which a Cloud outage does not reach): `ssh hp "sudo cat /var/lib/zcrypto-ops/capture-reconciled/reconcile-ledger.jsonl"`.
   ...
   - `zcrypto-ops-verify-replay-new-breakage` and `zcrypto-ops-verify-replay-backlog-stuck` — read `ops_verify_replay_failed_hours` and `ops_verify_replay_pending_hours` as levels; a `delta()` over a window wider than a day says nothing about where inside it the step happened.

6. **A page arriving on the way back is not proof of an event during the gap.** A first-sample rule can fire spuriously on return; which ones do is what [`drills-telemetry.md#drill-c`](drills-telemetry.md#drill-c) measures on the shipper side. Read the subject by value before acting on any of them (no count command: an operator's read before acting leaves no record).
