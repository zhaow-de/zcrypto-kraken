...
The ops row's 21 rules are the query lines this prints: `grep -nE 'expr: .*(ops_(archive_pull|panel|verify_replay|verified_replay)|zcrypto_(reconcile|trade_backfill|tapebars|grafana_keepalive))_' infra/grafana/alerts.yaml`.
...
4. **Never print the env to inspect the credential** (CLAUDE.md § Secrets) — on the primary that host also carries the live trade key.
...
3. **Then read each host by hand.** Every command here is host-local and needs no telemetry stack:
   ...
   - **Engine**, on the primary — **the gate**: `sudo docker exec zcrypto-engine zcrypto engine exec-status`, the only place the gate's `reasons` and the two arming keys are visible separately, re-evaluated on the spot. It prints `level`, `reasons` and the gate's inputs and **nothing else — no position, no resting order**, so it answers half of the dark position alert and must not be read as answering the other.
   - **Engine**, on the primary — **the exposure**, which is on disk and not in any command's output. [`engine-procedures.md#engine-probe-window`](engine-procedures.md#engine-probe-window) carries both reads whole — use them rather than retyping one. Two things that page cannot tell you from here: the position it reads is the same quantity `zcrypto_exec_position` is seeded from, so it is the dark alert's own number rather than a proxy, and `<HH>` is the 4-hourly cycle boundary (00/04/08/12/16/20 UTC), never the wall-clock hour. Then `sudo docker logs zcrypto-engine --since 1h` for the cycle.
