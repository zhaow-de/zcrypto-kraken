...
1. **First separate a hibernation from an outage** — identical from a browser, not the same event. The instance sleeps after days without a browser visit; ingest keeps running, so nothing is lost. On ops, `grep zcrypto_grafana_keepalive /var/lib/zcrypto-ops/textfile/grafana-keepalive.prom` — read on the host, because reading it through Grafana is the thing you cannot do. Status 503 is hibernating and needs only a dashboard opened and the loading waited out; 0 is no answer at all, which is this section's subject; several seconds of `_duration_seconds` beside a 200 is a wake-up already in progress. **An absent file, or an empty grep, means the keep-alive is not running on ops** — not converged there yet, or its timer has stopped — which says nothing about Grafana either way: carry on to step 2 and raise the stopped timer afterwards.
