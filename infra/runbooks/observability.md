...
**The bridgehead's Alloy takes no digest operand and owes no bake** — none of step 7 applies to `zaccess`. It is a native deb whose version is **followed from apt**: the `access` role installs it `state: present` with no version and clears any `dpkg` hold, because a hold makes `apt upgrade` skip the package silently and a forced version turns an upstream bump into a failed task that drops the host from the play. Its `config.alloy` is an **ungated `copy`** — every converge ships it, so a hand edit cannot outlive the next run and there is no drift assert to satisfy. **That missing gate is deliberate, not an oversight**: there is no `zaccess_alloy_digest` and no pins row, so do not add one — read the installed version off the host with `dpkg-query -W alloy`. A keep-regex or `config.alloy` change under `infra/ansible/roles/access/files/` therefore converges with a plain `site.yml --limit zaccess --tags access`, whose SSH-agent constraint is [`zaccess-converge`](zaccess.md#zaccess-converge).
