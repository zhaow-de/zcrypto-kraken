...
**Ten checks exist**, and `tests/fixtures/healthchecks_descriptions.json` records each one by name and tags (`T0083`'s scheme) — a snapshot fetched through the read-only key, so a rename does not reach it until it is re-fetched. Use the tags to identify a row, then the runbook column for the daemon that owns it. Its `description` field is healthchecks.io's own text and is not authoritative here: `zcrypto-archive-pull`'s says the ping is withheld on a gate skip, which the row below corrects.
...
**MEASURED — a stopped shipper** (drill C, both halves, 2026-08-31; entries in [`docs/reference/drill-log.md`](../../docs/reference/drill-log.md)). **The metrics plane is lost outright and nothing recovers it.** Nothing was scraped, so there is no buffer to replay from: across a 2 h hold the window carried five samples, all inside the first four minutes, which is the range query's own lookback carrying the last real sample forward rather than data. **A metrics dashboard read across an Alloy outage shows a hole, not a healthy flat line** — treat a flat trace over a known outage as absent, never as steady. **The logs plane replays from the journal positions file under `max_age`** — but only what was *written to the shipped journal while the shipper was down*, and that is a property of the host's workload rather than of Alloy. On ops, whose timer units log through the hold, the replay was complete: every run the units' own `OnCalendar` owed came back. On the capture secondary it recovered nothing, because its capture daemon direct-ships and the only other kept units were silent — **so do not generalise the ops result to a host that writes nothing to the journal.** Two readings that mislead on return: a bulk count of in-window lines can exceed the pre-outage baseline and still contain no replay at all (split them by *where in the window they fall* — a shutdown burst at the stop second is not recovery, and direct-shipped lines were never lost), and Alloy's own `msg="Done replaying WAL"` attests to the remote_write WAL replaying **scraped-but-unsent** samples, so a stopped shipper has none — that line speaks to the case below, never to this one.

**DERIVED — a dark destination**, from `config.alloy` rather than from a drill; no induction here has taken Grafana Cloud away. The planes swap. `prometheus.remote_write "grafana"` keeps a WAL, so metrics scraped while the endpoint is unreachable are buffered and re-sent on recovery, bounded by that WAL's retention. `loki.write "grafana"` declares **no `wal` block at all**, so logs are bounded by whatever the running Alloy's default is — read that off the version actually deployed rather than trusting a number written here, and treat the direct-shipped daemon logs separately per the paragraph below. **Neither half of this bound has been drilled**, and C′ did not supply it: C′ stopped the ops-side prober, so it proved the route that fires when the *prober* dies, not the one that fires when Grafana is unreachable.
...
1. **Read what the last call actually got, before working this as an outage** — a hibernation and an outage are identical from a browser and this step does not tell them apart; it tells you whether they are still both open. The instance sleeps after days without a browser visit; ingest keeps running, so nothing is lost. On ops, `grep zcrypto_grafana_keepalive /var/lib/zcrypto-ops/textfile/grafana-keepalive.prom` — read on the host, because reading it through Grafana is the thing you cannot do. The metric's own `# HELP` text, printed in that same file beside each value, is what says how to read them — read it there rather than carrying a copy in your head. What this step needs from it: a 503 does NOT settle which event this is, and a 0 with a duration at the 120 s cap does not either, so neither ends this section — continue to step 2 in both cases. **A `200` does not end it either**: it says the call succeeded at the last tick, and that tick is `OnCalendar=*:37:00`, so the reading can be an hour old. The watchdog's own probe runs `*:0/5:41` and is the fresher read, so a `200` here is not evidence that Grafana is up now. Opening any dashboard and waiting out the loading costs nothing to do now while you continue — it is not a reason to stop, because this step cannot tell you that a hibernation is what you have. **An absent file, or an empty grep, means no call has been recorded on ops** — not converged there yet, the timer stopped, or the token never rendered, in which case the timer fires and the runner exits without writing — which says nothing about Grafana either way: carry on to step 2 and raise it afterwards.

2. **Confirm the stack is dark rather than unreachable from one place.** Run the watchdog's own probe twice — once on ops (`ssh hp`) and once from the workstation — with the watchdog's own flags: `curl -fsS -m 20 --retry 2 --retry-delay 5 -o /dev/null -w '%{http_code}\n' https://zcrypto2026.grafana.net/api/health`. **Do not shorten the timeout or drop the retries.** The check is pinged green on anything this command accepts (`grafana-watchdog.sh.j2`), so a less patient probe can fail against a Grafana the watchdog is still calling healthy, and you would read a degraded stack as a dark one. Failing from both is the outage. Failing from ops alone is ops egress with Grafana healthy — a fault in the watchdog's route, and this is the wrong page for it.

3. **Read the dead-man domain directly — the one read that never passes through Grafana.** `uv run python infra/scripts/ops-daily.py report --since 24h`: its dead-man half fetches `https://healthchecks.io/api/v3/checks/` with the vaulted read-only key (`healthchecks_readonly_api_key`, `group_vars/all/vault.yml`) and is unaffected. **Expect exit 2 and a `## Sources that could not be read` block naming the Grafana ones** — that is the outage being reported, not a fleet finding, and every other part of the report still stands. **Its `## Dead-men` line prints how many checks were read, never which are down**: for the names, statuses and last ping times, open `https://healthchecks.io` itself.

4. **Then read each host by hand.** Every command here is host-local and needs no telemetry stack:
   ...
   - **Engine**, on the primary — **the exposure**, which is on disk and not in any command's output: `state.positions` in the newest `"ok"` `venue-<HH>.json` under `/var/lib/zcrypto-engine/journal/<YYYY-MM-DD>/` is the same reading `zcrypto_exec_position` is seeded from, so it is the dark alert's own quantity rather than a proxy for it; resting orders and their fills are the sibling `exec-<HH>.json`. [`engine-procedures.md#engine-probe-window`](engine-procedures.md#engine-probe-window) carries both reads whole — use them rather than retyping one. `<HH>` is the 4-hourly cycle boundary (00/04/08/12/16/20 UTC), never the wall-clock hour. Then `sudo docker logs zcrypto-engine --since 1h` for the cycle.
...
7. **A page arriving on the way back is not proof of an event during the gap.** A first-sample rule can fire spuriously on return; which ones do is what [`drills-telemetry.md#drill-c`](drills-telemetry.md#drill-c) measures on the shipper side. Read the subject by value before acting on any of them.
