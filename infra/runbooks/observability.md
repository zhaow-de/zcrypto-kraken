...
1. **Read the journal, and prove you read something**: `sudo journalctl -u zcrypto-archive-pull.service --since -2h --no-pager | wc -l` first, then the same without `wc` — an unprivileged `journalctl -u` prints `-- No entries --` under a hint to rerun with `sudo`, which is a permissions artifact and not an idle unit. The staleness rule owns an idle unit, not this one.
