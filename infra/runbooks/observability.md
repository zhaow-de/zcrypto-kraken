...
1. **First separate a hibernation from an outage** — identical from a browser, not the same event. The instance sleeps after days without a browser visit; ingest keeps running, so nothing is lost. On ops, `grep zcrypto_grafana_keepalive /var/lib/zcrypto-ops/textfile/grafana-keepalive.prom` — read on the host, because reading it through Grafana is the thing you cannot do. Status 503 is what BOTH a hibernation and a Grafana Cloud outage look like from here — the call saw a 503, not a reason — so treat it as *either* until the vendor's status page or step 2 says which; if it is a hibernation, opening a dashboard and waiting out the loading is the whole of it. 0 is no answer at all, which is this section's subject. Several seconds of `_duration_seconds` beside a 200 is a wake-up already in progress. **An absent file, or an empty grep, means the keep-alive is not running on ops** — not converged there yet, its timer stopped, or the token never rendered, since the runner exits without writing when it has none — which says nothing about Grafana either way: carry on to step 2 and raise it afterwards.
