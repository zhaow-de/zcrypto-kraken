...
1. **Read the journal, and prove you read something**: `sudo journalctl -u zcrypto-archive-pull.service --since -2h --no-pager | wc -l` first, then the same without `wc`. Three results, and the hint on stderr is what separates them. `-- No entries --` **with** the you-cannot-see-system-messages hint means the `sudo` did not take — a permissions artifact, not an idle unit. `-- No entries --` **without** it is a genuinely empty window, which the staleness rule owns, not this one. No output at all is the command itself failing: `journalctl` exits non-zero and says why on stderr, while `wc` still prints 0 and the pipeline's status is `wc`'s, so neither the count nor `$?` will tell you.
