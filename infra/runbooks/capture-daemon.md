...
2. **Free space from the safe pools, in this order.**
   ...
   - **On the primary only**, the engine's own journal ring (set: the members of `infra/ansible/inventory/hosts.yml`'s `engine_host` group other than `zcrypto`; count: `infra/scripts/count-list.sh engine-hosts-besides-the-primary`): `sudo systemctl start zcrypto-engine-journal-prune.service` (idempotent; its daily timer exists on the engine host alone).
