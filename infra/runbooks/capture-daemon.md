...
**The summary's "that pair's L2 is not being captured" overstates it, and the correction changes what you check.** In `_handle_book_message` only a watermark breach skips the writer — a desynced pair keeps appending every update row. What the desync costs is *certification*, not rows: the transition opens a `checksum_resync` gap in `GapMonitor`, so the window is booked into `zcrypto_capture_gap_seconds_total{pair=…}`, and the book replayed across it no longer reconciles against Kraken's checksum. Treat the window as **unverified** L2 on that pair, not as an empty one.
