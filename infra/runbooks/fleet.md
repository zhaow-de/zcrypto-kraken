...
**If a converge or an Alloy bump just ran on that host, this is that action.** `job` names the daemon — a capture or engine daemon, the ops poller, or Alloy itself (`integrations/self`). It fires ≈2–3 min after the restart and clears on its own ≈15 min after it, once `changes(…[15m])`'s window holds no sample from before the restart: one firing that clears is the action's own record and needs nothing. It is blind to a daemon that comes back after an absence longer than the window: `changes()` needs a pre-restart sample inside the 15 minutes, so a daemon stopped for longer than that restarts unpaged here — the stop itself pages through the staleness rules (measured 2026-09-26: the engine stopped 13:43Z, restored 13:59Z, no firing; the reboot's 76 s absence fired). A crash loop does not look like a second firing: `changes(…[15m])` stays above zero while the restarts continue, so the tell is a firing that does not clear after the window, or clears and returns with no action from you. **If nothing was converged, the daemon was OOM-killed or crashed and came back on its own** — and this is the only signal for that: the container is back in seconds, the dead-man keeps pinging, and the log-dead rules never see a gap that short. There is no container-restart metric on these hosts, so this rule reads the same fact from inside the process.
