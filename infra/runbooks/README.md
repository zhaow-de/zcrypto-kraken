...
- [`reconcile-ledger-scan-cost`](ops.md#reconcile-ledger-scan-cost) — ALERT: the append-only reconcile ledger has grown into the cost driver of every cycle; warning at 30 s says re-measure, critical at 60 s says the constraint is memory rather than time and the cadence alert structurally cannot see it.
