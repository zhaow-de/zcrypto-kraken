...
- [`zcrypto-fleet-alloy-memory-headroom`](fleet.md#zcrypto-fleet-alloy-memory-headroom) — ALERT: Alloy is above 90 % of its 512 MiB limit — it runs nearer its ceiling than the app daemons by design, so it has its own bar.
