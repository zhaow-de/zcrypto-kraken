...
4. **`undetermined`** — including a record with no `verdict` key: no evidence either way from the cross-host comparison, which is where a hard halt lands, since a venue emitting *nothing* leaves no interior messages for the two hosts to agree on. Treat it as loss, then ask whether the venue **announced** it. Fastest: look **back** in the alert list for the two venue rules — `Capture · Kraken reports the venue is not online` and `Capture · Kraken reported a degraded venue state again`. Both read the venue's own WebSocket `status` frames, and an `activeAt` **inside hour H** is the announcement time. An `activeAt` outside hour H answers nothing, and neither does the absence of one: the first rule LATCHES, so an instance already firing before hour H keeps the earlier event's `activeAt` for the daemon's whole lifetime, and the second is an `increase()` form that stays Normal whenever a whole burst lands between two scrapes — on 2026-09-03 it never fired on `zcrypto-red`, which logged frames identical to the primary's within 30 ms. With no `activeAt` inside hour H, read the frames. Otherwise read the frames directly: `sudo docker logs zcrypto-capture 2>&1 | grep "venue status"` on either capture host, or the same line via Loki — each line carries its own UTC timestamp, so scope by reading the line, never by `--since`. **Reading `zcrypto_capture_venue_status_total` instead needs care, and neither obvious form works**: it *latches* until the capture daemon restarts, so bare presence at H+2 h can be a different day's event; and `increase()` is blind to a first sighting, so a clean one-frame transition reads 0 — which is exactly the shape of a host that restarted shortly before the event. Take a series **born** inside hour H, or one that **steps** inside it, and nothing else; a series flat across H is not evidence either way, because Kraken pushes `status` on connect and on change only. **If the venue announced it, write the event's row in `docs/reference/capture-era-data-hygiene-map.md`** — that map, not this counter, is the durable answer to "was this hour ours?", because the counter and the log both age out with retention.
