...
3. **`duration_s` under 7200 s beside a non-zero `pending`** cannot come from one completed run: `pending` is set only when the drain spends its whole budget, and the nightly runner passes no budget, so the budget is the CLI default of 7200 s (`--drain-budget-seconds`, `cli/archive/command.py`). A failed run carries the previous `pending` and `duration_s` forward together, so the pair may be an earlier run's — but that run faced the same arithmetic, so carrying it forward does not explain it either. Grep `infra/ansible/roles/ops/templates/verify-replay.sh.j2` for `--drain-budget-seconds`: present, someone raised the budget for step 5's reason and the pair is ordinary; absent, distrust the reading before you act on it.
