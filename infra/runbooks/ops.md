...
3. **`duration_s` under 7200 s beside a non-zero `pending`** cannot come from one completed run: `pending` is set only when the drain spends its whole budget, and the nightly runner passes no budget, so the budget is the CLI default of 7200 s (`--drain-budget-seconds`, `cli/archive/command.py`) unless someone has added the flag to the runner for step 5's reason — check the template before ruling that out. A failed run carries the previous `pending` and `duration_s` forward together, so the pair you are reading may be an earlier run's — but that run faced the same arithmetic, so carrying it forward does not explain the pair either. Read the runner's template: if someone has added `--drain-budget-seconds` for step 5's reason, the pair is ordinary; if they have not, distrust the reading before you act on it.
