...
3. **`duration_s` under 7200 s beside a non-zero `pending`** cannot come from one completed run: `pending` is set only when the drain spends its whole budget, and the nightly runner passes no budget, so the budget is always the CLI default of 7200 s (`--drain-budget-seconds`, `cli/archive/command.py`), never a misconfiguration. That census line is carried forward from an earlier run, which is step 1's case.
...
1. **Count the qualifying days from the LEDGER, never from Grafana Cloud.** Cloud's retention is short (~14 days, measured in \[[T0103]\]), so a long selector there reads zero over a series it has never seen move.
...
2. **Count only days after the counter fix.** Earlier values are not fittable; \[[T0103]\] records which days and why.
...
3. **If it is skipping, read the gate's input rather than guessing**: `cat /mnt/zhao-crypto/.pull-status` and `date -u +%s`, and compare `ts_epoch` against the gate's two bounds: at most 14400 s (4 h) old, at most 600 s in the future. **Do not hand-edit `.pull-status`** — it is the reader's ground truth, and the gate exists precisely to refuse a frozen view. Take the incident to the NAS: its `zcrypto-nas-archive-pull-stalled` / `-errors` rules and `hc_checks_down_total`.
