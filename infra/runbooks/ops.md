...
3. **`duration_s` under 7200 s beside a non-zero `pending`** cannot come from one completed run: `pending` is set only when the drain spends its whole budget, and the nightly runner passes no budget, so the budget is the CLI default of 7200 s (`--drain-budget-seconds`, `cli/archive/command.py`) unless someone has added the flag to the runner for step 5's reason — check the template before ruling that out. That census line is carried forward from an earlier run, which is step 1's case.
