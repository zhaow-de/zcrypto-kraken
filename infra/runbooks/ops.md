...
5. **A state-write failure part-way through a long run costs replay work, not verdicts.** The state file is written whole and swapped into place, so a failed write leaves the previous one intact and readable, and the run is killed deliberately rather than allowed to keep reporting green against state it can no longer record. Progress is saved every 250 replayed hours, so the next run resumes from the last save and loses at most ~250 hours of replaying. Fix the mount or the permissions and re-run; do not delete the state directory for this one.
...
5. **`pending` rising while `replayed` merely keeps pace with the archive's daily growth** means hours are going stale faster than the drain clears them — a re-verification larger than one night's budget. Confirm it was deliberate; if it was, either wait it out (the count must still fall every night) or raise the budget for a few nights by passing `--drain-budget-seconds` in the runner's invocation (`infra/ansible/roles/ops/templates/verify-replay.sh.j2`).
6. **Do not delete the state directory to clear this.** A rebuild replays the entire archive, which is the same work the backlog already represents. Deleting it is the right move only for a refused eviction after a deliberate archive shrink — the run-broken section above.
...
1. **Bound the event before acting on it.** Query the same counter across widening windows — `increase(<metric>[1h])`, `[6h]`, `[24h]`, `[7d]`. Equal values at 6 h and 7 d mean one event, not a trend; a zero at 1 h means it has stopped. Two queries separate a degrading host from one bad patch, and the alert's own wording cannot.
2. **Read the ledger for shape, not just totals.** Per-pair and per-hour records distinguish "every pair briefly" from "one pair for a long time" — different faults with the same total.
...
1. **Count the qualifying days from the LEDGER, never from Grafana Cloud.** Cloud's retention is short (~14 days), so a long selector there reads zero over a series it has never seen move.
...
2. **Count only days after the 2026-07-28 counter fix.** Earlier values are not fittable — the fix corrected an overstatement, and the days before it carry it.

3. **Fewer than three qualifying days ⇒ do nothing except re-arm.** Schedule the next reminder in `#zcrypto` — the following month, matching `refdata-sweep-due`'s cadence — and stop. The re-arm is a convenience — the pass re-evaluates the counter every day whether or not the message lands, so a message that never arrives is not replaced out of band: the next one is scheduled here, when this section next runs.
...
4. **A manual mutation of the reconcile ledger or the overlay must delete `<reconciled_root>/scan-cache.json` in the same act.** The fingerprint cannot see a ledger edit, so the next cycle would skip exactly the hours the correction meant to re-examine, and nothing pages meanwhile. Deleting it makes the next cycle deliberately full instead. The procedure, and how long the sampled audit would take to catch a stale cache, are in `infra/nas/README.md`.
...
- **Warning (10 s)** — the ledger has become the cost driver. Nothing is at risk, but append-only-forever stopped being free and is now worth revisiting on evidence.
- **Critical (30 s)** — the constraint is **memory, not time**. This is the failure `zcrypto-reconcile-cycle-duration` cannot warn you about: a cycle killed for memory publishes nothing, so that gauge goes **stale rather than high**, and the only other cover is the 3-hour exporter-stale page. The arithmetic behind both bars is at the rules in `infra/grafana/alerts.yaml`.
...
2. **Read the host's MemAvailable — never MemFree**, because that is what the critical bar encodes and it moves independently of this repo. The two differ about tenfold on a busy box, so reading the wrong one turns a healthy host into an apparent three-times over-commit. Query `node_memory_MemAvailable_bytes{host="ops"}` via `uv run python infra/scripts/grafana-query.py`, or read `MemAvailable` in `/proc/meminfo` on `ssh hp`. The reconciler runs uncapped, so its ceiling is whatever the host has available.
...
5. **Scope every `docker inspect` to the field you need** while triaging on any host — `docker inspect --format '{{.State.Status}} {{.RestartCount}}' <name>`; the container's environment is never printed (`CLAUDE.md`, Secrets).
...
1. **Confirm the reading is live.** `zcrypto-reconcile-exporter-stale` must not be firing, and `bash infra/scripts/ops-postverify.sh`'s `reconcile freshness (s)` check must pass — `(no series)` is a FAIL there, never a zero.
