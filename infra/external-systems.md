...
- At the terminal:
  ...
  - `vim /etc/passwd` to:
    ...
    - change the shell of user `zcrypto-data` to `/volume1/docker/zcrypto-archive/rrsync-shell` — the rrsync-only login shell the `nas` role deploys (spec 00057). `zcrypto-data` **serves the inbound hot-push**, which runs an rrsync forced command, and sshd runs a forced command via the account's login shell; `/usr/bin/nologin` would swallow it, while a general shell grants more than needed, so the wrapper permits **only** rrsync and refuses everything else. Deploy the wrapper first (`./scripts/run.sh site.yml --limit nas --tags nas` from `infra/ansible/`) so the shell resolves.
