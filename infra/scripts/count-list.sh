...
# The skip gates under `tests/` the guard finds: one opt-in name over those; every guard one of six forms -- a path
# presence check, a uid test, a `shutil.which`, a read of the one opt-in, a membership test, or a call into
# tests/skip_gates.py -- and every other shape refused. The guard is the count.
