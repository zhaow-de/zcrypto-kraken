...
# Inside a LIVE prefix but a record of what was cited when written, not a surface anyone acts on.
RECORDS = (
    "docs/open-topics/archive/",
    "docs/reference/ops-journal/",
    "docs/reference/drill-log.md",
    "docs/reference/change-index.md",
    "docs/reference/adapter-verification/",
)
...
def _walked(listed: list[str]) -> list[str]:
    return [r for r in listed if r.startswith(LIVE) and not r.startswith(RECORDS)]
...
def _fake(tmp_path: Path) -> list[str]:
    ...
    for rel in (_LIVE_RULE, _KEEP_SKILL, _A_FLOW, _A_HOOK, _A_REF, _A_SCRIPT):
        (tmp_path / rel).parent.mkdir(parents=True, exist_ok=True)
        (tmp_path / rel).write_text("a guidance file\n")
    (tmp_path / "docs" / "plans").mkdir(parents=True)
    (tmp_path / "CLAUDE.md").write_text(f"reads `{_LIVE_RULE}`, `{_KEEP_SKILL}`, `{_A_FLOW}` and `{_A_HOOK}`\n")
    ...
    return ["CLAUDE.md", _LIVE_RULE, _KEEP_SKILL, _A_FLOW, _A_HOOK, _A_REF, _A_SCRIPT, "docs/plans/00001-a.md"]
...
def test_the_checker_reads_every_citation_shape(tmp_path):
    listed = _fake(tmp_path)
    assert set(citations(tmp_path, listed)) == {_LIVE_RULE, _KEEP_SKILL, _A_FLOW, _A_HOOK, _A_REF, _A_SCRIPT}, (
