...
def test_control_mutation_must_fail_first(tmp_path):
    make_repo(tmp_path)
    # a control that matches but does not break the probe: a non-matching sed would trip the no-op
    # guard (rc 6) instead of the control abort this test pins.
...
def mutated_script(tmp_path, sed_expr):
    """A copy of the script with one guard mutated, OUTSIDE the scratch repo: a copy inside it would
    dirty the worktree and trip the refusal before the guard under test is reached."""
...
def test_sandbox_seeds_from_committed_head_not_the_worktree(tmp_path):
    ...
    # cp -a would seed the DIRTY `VALUE = 9`, so the probe cannot pass on the seeded tree and the
    # baseline gate refuses. Swapping the seeding command inside its own rc-8 guard keeps the mutant
    # valid bash, differing from the real script only in where the sandbox's contents come from.
...
def test_purge_removes_a_pre_existing_pycache(tmp_path):
    ...
    # restore the stale dir byte-identically, so the worktree is clean again
    cache.mkdir()
    (cache / "mod.cpython-314.pyc").write_bytes(b"stale")
...
def _signal_mid_probe(script, repo, target, tmpdir, args, before, marker, sig=signal.SIGTERM):
    ...
    Wait on a marker the PROBE writes, not on the mutated content: the mutation lands while `apply`
    is still finishing, before the probe is spawned. Re-send the signal until the process exits --
    bash defers a trapped signal until the foreground command returns, so one landing just before
    `sleep` spawns reaches nothing; `cleanup` is idempotent, so re-signalling is safe."""
...
def test_signal_during_probe_restores_the_target_before_cleaning(tmp_path):
    ...
    # unmutated baseline run and signal a cycle with nothing applied.
