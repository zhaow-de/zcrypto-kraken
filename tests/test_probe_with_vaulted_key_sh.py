"""`infra/scripts/probe-with-vaulted-key.sh` runs the order-semantics harness with the vaulted trade credential in its
environment and nothing else. Driven from a copy in a scratch tree, so every path it derives from its own location
lands there: it refuses outside a repo root, without the harness, and without the venv interpreter; its in-process
stage refuses a vault password that cannot be read or comes back empty, and a credential absent or empty under its
names; and the harness receives `-I`, the arguments verbatim, the two variables from the vault and the repo root as
cwd. The vault helper, the vault file and the harness are stubs -- no key, no venue -- and nothing but comments sits
above the guard line, where a function named `exec` would shadow the builtin."""
...
_SCRIPT = _REPO / "infra" / "scripts" / "probe-with-vaulted-key.sh"
_HARNESS = "kraken-order-semantics-probe.py"
...
def test_nothing_but_comments_sits_above_the_guard_line():
    head, sep, _ = _SCRIPT.read_text(encoding="utf-8").partition(_GUARD)
    assert sep == _GUARD
    assert all(line.startswith("#") for line in head.splitlines() if line.strip())
...
def test_a_missing_harness_is_refused(tmp_path):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("echo pw", "kraken_trade_api_key: ''\nkraken_trade_api_secret: s\n", "refusing: the vaulted trade credential is empty"),
    ],
)
def test_the_in_process_stage_refuses_before_the_harness_runs(tmp_path, helper, vault, message):
    done = _run(_tree(tmp_path, helper=helper, vault=vault), "--probes", "1")
    assert done.returncode == 1 and message in done.stderr
    assert "argv" not in done.stdout


def test_the_harness_gets_the_arguments_the_credential_isolated_mode_and_the_repo_root(tmp_path):
    script = _tree(tmp_path)
    done = _run(script, "--probes", "1,2", "--no-exec", "--pair", "ETH/EUR")
    assert done.returncode == 0, done.stderr
    assert json.loads(done.stdout) == {
        "argv": ["--probes", "1,2", "--no-exec", "--pair", "ETH/EUR"],
        "key": "k-fixture",
        "secret": "s-fixture",
        "isolated": 1,
