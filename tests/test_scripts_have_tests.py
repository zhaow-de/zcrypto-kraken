...
so one test cannot stand for two scripts whose stems differ only by that normalisation. The rule keys on the
`.py`/`.sh` suffix: an extensionless file is not walked at all, and the two `.zsh` files -- the operator's own
terminal tooling, a tmux cockpit and a workstation-to-ops transport -- are out on the suffix alone. No
allowlist -- a script without a test is named here until it has one."""
...
def test_the_walk_reaches_a_subdirectory_of_either_tree(tmp_path):
    _tree(
        tmp_path,
        {
            ...
            "tests/test_tool.py": "drives tool.py",
        },
    )
    assert _missing(tmp_path) == ["infra/scripts/helpers/nested-uncovered.py"]
