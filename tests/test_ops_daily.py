...
# --- the soak stubs: the payload builders and the refusing runner, shared with `test_ops_daily_soak.py` --------
...
def live_soak_run(monkeypatch):
    ...
    of starting a real `soak-check` against the production journal mount from inside the suite. The cases
    that drive the runner itself take this fixture's value, which is the real one -- `monkeypatch` has
    already replaced the module attribute by the time they run."""
...
def test_a_resolver_answering_a_strict_subset_of_the_operands_is_prepared(cmd):
    """Two glob-free operands answered with one path is below the floor, so it is refused rather than
    read as clean over the one it answered; both spelled operands are safe either way."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # `--since` is a real `decompose` option the sub cannot parse: `strptime(raw, "%Y-%m-%d")`, hence `_DAY`.
