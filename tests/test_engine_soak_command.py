...
def test_soak_check_json_carries_the_fields_the_daily_pass_reduces(tmp_path, monkeypatch):
    ...
    # The whole SET, read off the instrument rather than guessed here: the row's panel counts and its `hhi`
    # clause are the only places a metric reaches the operator, so an eighth one added to `gating_verdicts`
    # and never taught to the reader would go unread every morning. This is what goes red instead.
