...
def test_verify_tree_walks_past_a_year_directory_too_large_for_a_c_int(tmp_path):
    """A `2147483648` year directory is walked and hashed like any other final; only its hour is lost."""
    ...
    _seg(tmp_path, "BTC/EUR", "book", "10")
    d = tmp_path / "BTC" / "EUR" / "book" / str(2**31) / "07" / "12"
    d.mkdir(parents=True)
    p = d / "11.parquet"
    pl.DataFrame({"x": [1, 2, 3]}).write_parquet(p)
    ...
    assert r.checked == 2 and r.ok == 2 and r.failed == ()
    # the true positive keeps its stamp: the undatable path contributes no hour, it does not blank one
