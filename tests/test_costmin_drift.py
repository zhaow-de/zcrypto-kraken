...
def test_the_committed_costmin_matches_the_newest_refdata_snapshot():
    ...
    snaps = scan(Path("data/snapshots"), "kraken-refdata-*.json")  # the name carries the stamp; mtime does not survive a copy
    if nothing_found_under(Path("data/snapshots"), "kraken-refdata-*.json"):
