...
| -- | -- |
| `seed` | Seed/refresh the live price store (`store_dir`) from the canonical dataset (`data/ohlc-full`) plus a REST gap-fill; idempotent, prints the per-pair × grid seam-QA summary. Also the repair for a poisoned store tail. |
...
`ohlc-reach` is the exception to "from the OHLCVT dumps": it carries the newest frozen `ohlc-full` forward — the newest stamped `ohlc-full-<stamp>` sibling a dump ingest minted, else `ohlc-full` itself; a newest sibling missing a basket leg or its manifest, the leavings of a mint killed mid-build, is refused by name rather than joined — using Kraken's public REST OHLC window, which serves the most recent ~720 bars per interval. That window recedes at a rate set by the interval — roughly 720 days at the daily grid, 120 days at 4h, but only 30 days at 1h — so a reach set is routinely **mixed**, and each series lands one of two ways:
