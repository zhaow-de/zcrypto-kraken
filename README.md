...
| -- | -- |
| `seed` | Seed/refresh the live price store (`store_dir`): a REST gap-fill over every series, and for an absent store file a copy of the newest whole frozen `ohlc-full` sibling under the configured data root (the unstamped set when there is none); workstation-only, the engine image carrying no canonical dataset; idempotent, prints the per-pair × grid seam-QA summary. Also the repair for a poisoned store tail. |
