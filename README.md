...
| `--ship-logs` | Also ship logs to Grafana Cloud Loki. Requires `ZCRYPTO_LOKI_URL`, `ZCRYPTO_LOKI_USERNAME`, `ZCRYPTO_LOKI_PASSWORD`, `ZCRYPTO_LOG_HOST`, `ZCRYPTO_LOG_SERVICE`; any one unset or empty exits with an error naming it. |
...
| -- | -- |
| `seed` | Seed/refresh the live price store (`store_dir`): a REST gap-fill over every series, and for an absent store file a copy of the newest whole frozen `ohlc-full` sibling under the configured data root (the unstamped set when no stamped sibling exists); workstation-only, the engine image carrying no canonical dataset; idempotent, prints the per-pair × grid seam-QA summary. Also the repair for a poisoned store tail. |
| `run` | Run the shadow node in the foreground — one journaled cycle per 4h boundary. Fails fast (exit 1) when the store is missing any basket series, or when `ZCRYPTO_REQUIRE_CONFIG` is set and no `zcrypto.toml` exists; a start the node cannot complete aborts and exits non-zero rather than resting as a live-looking node, and the supervisor's restart is the recovery. Set `HEALTHCHECK_URL` to enable the per-cycle dead-man's-switch ping: a success record pings the URL, a failed-cycle sidecar `<url>/fail`, and a propagating exception pings nothing and alerts by silence. |
| `cycle [--at ISO_TS] [--replace]` | Run one cycle manually, defaulting to the most recent elapsed boundary; `--at` must be an aware ISO-8601 timestamp exactly on the 4h UTC grid. An already-journaled boundary is refused unless `--replace`, which first deletes its record/sidecar and its snapshots. Exits non-zero when the cycle fails. |
| `replay [--date YYYY-MM-DD] [--path fast\|verified] [--journal-dir <PATH>]` | Replay journaled success cycles through the builder and compare recomputed targets against the journaled ones. Hash mismatches and validation failures are classified per cycle rather than crashing the sweep, sidecars are listed as failed cycles, and either exits non-zero. `--journal-dir` reads a journal other than the configured one. |
| `report [--journal-dir <PATH>]` | Rebuild every journaled cycle outcome by replay-on-demand (fast path) and evaluate the ratified ≥ 14-clean-day gate: streak length, gate status, most recent failure. An absent boundary scores missing, never fabricated. `--journal-dir` reads a journal other than the configured one. |
| `gate-export --textfile <PATH> [--journal-dir <PATH>] [--healthcheck-url <URL>] [--lag-fail-seconds <SECS>] [--cache <PATH> --slice <0-23>]` | Evaluate the gate (the same replay-on-demand pass as `report`) and atomically write it as a Prometheus node-exporter textfile, then ping an independent dead-man's-switch healthcheck. Emits `zcrypto_gate_status` (1/0), `zcrypto_gate_streak_days`, `zcrypto_gate_journal_pull_lag_seconds` (omitted when the journal is empty), `zcrypto_gate_mismatch_total` (every not-clean cycle: replay mismatches + validation failures + failed-cycle sidecars), `zcrypto_gate_cache_replayed`, `zcrypto_gate_cache_hits` (per-run gauges, no `_total` suffix), `zcrypto_gate_cache_invalidated` (1/0), `zcrypto_gate_cache_oldest_verification_age_seconds` (omitted when the cache is inactive or empty), `zcrypto_gate_export_duration_seconds` (wall time to evaluate the journal; excludes writing the textfile and the healthcheck ping), and `zcrypto_gate_export_timestamp_seconds`. `--healthcheck-url` pings clean (GET the URL) iff the gate is healthy — a streak above zero, or no failure recorded yet — **and** the journal-pull lag is within `--lag-fail-seconds` (default `21600`, 6h); otherwise pings `<url>/fail`. `mismatch_total` is **not** an input: the ping is a liveness signal, so a gate with no evaluable complete day yet pings clean, and an empty journal (no lag to measure) pings `/fail`. Omit `--healthcheck-url` to skip the ping. `--cache <PATH>` — with `--slice <0-23>`, which it requires and which is refused without it — reuses a prior run's replay outcome for any cycle whose journaled evidence is unchanged, so only new cycles are replayed (byte-identical outcome either way — the cache metrics still emit, zeroed); each run still force-replays the ~1/24 slice `--slice` names — the caller's run counter modulo 24 — so the whole journal's parquet bytes are re-verified within 24 runs with the cache warm (up to 47 across a counter reset that keeps the cache), and `infra/runbooks/gate.md` carries what that costs and the cache deletion a repaired journal needs. Exits 0 even when the gate has a mismatch or is stale (those are findings, surfaced via the metrics/ping); non-zero only on an operational failure (unreadable journal, unwritable textfile). |
| `soak-check [--journal-dir <PATH>] [--store-dir <PATH>] [--canonical-dir <PATH>] [--registry <PATH>] [--fee-per-side <RATE>] [--band <FLOAT>] [--floor <N>] [--null windows\|block-bootstrap\|both] [--path fast\|verified] [--json <PATH>]` | Compare the realized shadow-engine journal against a backtest null rebuilt from the frozen canonical dataset and judged over that dataset's complete-basket era, from the first bar at which every basket leg carries a price: it judges the structural metrics gross/net/active_frac/turnover/hhi/governor_engagement/cap_breach — the last two recovered by a one-time internals rebuild off the newest journaled cycle's snapshots — the governor-bias (governed-vs-live) gap, and non-gating P&L. `--canonical-dir` defaults to `data/ohlc-full`; when it is absent, or the null cannot be built over it, or the scored window — the newest boundary-contiguous run of journaled cycles long enough to score `--floor` (default `30`) bars, the longest run when none is — is shorter than `--floor`, or the null retains too few bars for any gating metric to discriminate, or a self-test fails, or the internals rebuild succeeded but its identity/cap-consistency proof failed, the report prints "NO VERDICT" and suppresses the per-metric table rather than a false conclusion — a self-test that ran and failed also exits non-zero, a short or void window still exits 0. An internals rebuild that could not run at all (missing/corrupt snapshots, a builder error) instead degrades governor_engagement/cap_breach to "n/a" without voiding the run. `window_bound`, in the realized-series block and in the JSON provenance, states **what bounded the scored window**: `journal` (it ran to the end of the journal's contiguous cycle segment — the benign case, since the last cycle never scores), `store` (the price store's last usable bar predates the journal and truncated the window), or `clock` (the trailing cycles' successors have not happened yet). A store-bound run still scores what it can, but prints a warning above the verdict table naming the store's last usable bar, the journal's last cycle and how many cycles the gap cost — its numbers read a stale window, so point `--store-dir` at a store that covers the journal to widen it. `--null` (default `both`) picks which backtest null construction(s) judge each metric: `windows` (the overlapping-window reference), `block-bootstrap` (an independently resampled reference), or `both`, which judges under each and reconciles — agreement keeps the shared label, a mild disagreement takes the milder one, and an outright `consistent`-vs-`inconsistent` split degrades to `indeterminate (instrument-fragile)`; the `verdict`/`primary`/`secondary` columns carry the reconciled label and each null's own raw one. `--path` (default `fast`) selects the builder for the null rebuild and the identity self-check: `fast`, or the much slower `verified` oracle rebuild for re-reading a suspicious result without editing code. `--registry` is the trial-registry JSON-lines file the instrument self-test reproduces the ratified record against. A `NULL REFERENCE SPAN` block states that span and the no-book bar counts for the null and the realized window; `--json` writes the full payload atomically, carrying the null's half of that block as `null_reference` and the realized counts at the top level, so two runs judged against different references are not silently compared. Read-only decision-support — never a validation exercise, never the concordance gate — and it consumes no holdout budget. |
| `decompose [--journal-dir <PATH>] [--since YYYY-MM-DD] [--until YYYY-MM-DD] [--json]` | Attribute each journaled cycle's gross across the pipeline stages — per-sleeve gross → combined → capped → governed final, plus the ratio between each consecutive pair, so a shrinking book has a named cause rather than an observed drop. Every intermediate stage is recomputed from public builder parts and then proven twice per cycle: against the in-process build (`multiplier × capped == final_targets`, exactly) and against the journaled `final_targets`, so a builder change or a rebuild that diverges from what the engine actually traded surfaces as a named failure. Each summary ratio is the median of the per-cycle ratios, never a ratio of medians. `--since`/`--until` bound the UTC day window (both inclusive). A cycle record that cannot be parsed aborts the run; one that parses but fails to replay is named, counted in the output and exits non-zero — a silently dropped cycle would bias every aggregate. `--json` emits the full payload on stdout instead of the table, with non-finite values (a flat cycle's 0/0 cancellation ratio) as `null`. Read-only. |
| `accum-replay [--journal-dir <PATH>] [--since YYYY-MM-DD] [--until YYYY-MM-DD] [--minimums <PATH>] [--nav <EUR>]… [--json]` | Measure the position drift the venue's order minimums impose at each portfolio size: replays an accumulate-until-placeable policy over the journaled targets, carrying held state in base units priced at each cycle's journaled close, and placing only when the delta clears both `ordermin` (a quantity) and `costmin` (euros) — independent gates, never a max over mixed units. Drift is measured after the placement decision, in bps of NAV: per-cycle median and p95 over the whole window, plus a per-ISO-week mean carrying each week's cycle count and a partial-week flag. There is deliberately no weekly p95, and NAV is held constant across the window, so the number stays a pure placement floor rather than folding in P&L. `--nav` is repeatable and defaults to 500/1,000/2,500/5,000/10,000 EUR. `--minimums` defaults to the newest `kraken-refdata-*.json` under `<data_dir>/snapshots`; that snapshot's `fetched_at` stamp is printed beside the drift table — these floors move, so a band quoted from an older table is stale. Unparseable and unreplayable records behave exactly as in `decompose`. `--json` emits the payload with non-finite values as `null` and the per-NAV table keyed by each NAV's string form (every row also carries its numeric `nav`). Read-only. |
| `tracking-report [--journal-dir <PATH>] [--since YYYY-MM-DD] [--until YYYY-MM-DD] [--nav <EUR>] [--gate-from YYYY-Www] [--minimums <PATH>] [--simulated-fills] [--ledger-export <PATH>] [--json]` | Compare each ISO week's **realized** drift — the position the journaled fills actually built — against the venue-minimum drift floor `accum-replay` measures over the same cycles, and reprice the traded cost from the same fills. Per ISO week: the floor's p95 over that week's cycles, the realized mean, and whether the realized mean sits within the floor. A week is **decided** only when it is complete, sits on or after the gate boundary, and does not straddle the first fill; the reason a week is not decided is named in its own column, and the overall verdict stays `insufficient-data` until at least 3 weeks are decided. **`--gate-from YYYY-Www` names the first ISO week that counts** — absent it, **no** week is decided whatever the numbers say, because a week nobody has declared in-scope must not be able to produce a pass; the report says so and names the flag, and every week's floor p95 and realized mean print either way. `--nav` is **single-valued** (unlike `accum-replay`'s repeatable sweep) and defaults to the configured `engine.shadow_nav_eur`, the size the engine itself trips at; the compared NAV is printed on the `Verdict:` line. The cost block reports the notional-weighted maker/taker split of the **priceable** book (a fill the venue gave no liquidity side for is counted but has no side to split), the realized fee per side against the currently assumed one, and the per-fill min/median/max spread rather than a standard deviation. `--minimums` behaves exactly as in `accum-replay`, and its `fetched_at` stamp is printed above the table. `--simulated-fills` replaces the journaled fills with the drift floor's own modelled placements at each journaled close, so the comparison can be exercised before a single real fill exists; the report is labelled, and its fees come from the assumed maker rate, so that run's cost blend recalibrates nothing. **`--ledger-export <PATH>` reconciles the window against a Kraken ledger export** (History → Export → Ledgers), the hand-made artifact carrying the one cost no fill can: the venue charges a margin position's **rollover** against the POSITION. The reader is header-driven and **refuses an export whose header lacks a column it needs, naming the column**. A `trade` row whose venue trade id matches no journaled fill **fails the reconciliation, not the run**: every unmatched id is named, the drift half still prints, and the exit code is non-zero. **An export with no `trade` row reports `insufficient-data`, never `ok`** — nothing was compared, and a clean bill there would read exactly like an export whose every trade row matched. **A failed reconciliation also withdraws `cost.proposed_fee_per_side`** — it reads `null` and `cost.basis` names the unmatched count, so a `--json` consumer cannot read a rate off a book the ledger could not reconcile; the measured blend stays. Rollover rows are summed as a euro cost only when the row's asset is a euro (both venue spellings). Only `trade` rows are matched today, so any row type the reader places nowhere — `margin` above all, which a margin position writes carrying its trade's id — is **counted by type and printed**, and the block reports how many rows were read at all. Combined with `--simulated-fills` the block is a guaranteed failure (a modelled fill carries no venue trade id) and says so; absent the flag the report omits it. Unparseable and unreplayable records behave exactly as in `decompose`; an execution record that cannot be read aborts, because its fills move the position every later cycle is measured against. `--json` emits the payload with non-finite values as `null`. Read-only. |
| `exec-status [--state-dir <PATH>]` | Report whether the engine may submit orders right now, and every input that decided it: the arming keys (config flag + arm file), the kill switch, the restart hold, and the last venue reading — each printed separately, because the published `zcrypto_exec_gate_level`/`zcrypto_exec_armed` gauges carry only a number and conflate the two arming keys. Prints `level=<none\|reduce_only\|full>`, `reasons=<comma-separated, or "-">`, then every gate input on its own line. `--state-dir` reads the control files from a directory other than the configured `journal_dir`'s parent. Read-only. |
| `flatten --state-dir <PATH> [--execute]` | Close every open position and sell every non-EUR balance at **market**, account-wide — the emergency halt, run on the engine host through `sudo zcrypto-flatten` and never by hand. Without `--execute` it reads the account, prints every resting order with its pair, side, volume and venue txid, every leg with its side, quantity, pair and estimate at the taker rate, lists every balance below the venue minimum and every balance no EUR or BTC pair can carry, and **sends nothing**. With `--execute` it refuses unless the engine's kill file is already present, refuses without a controlling terminal, prints the same plan, and reads a typed `FLATTEN` from the terminal (never from stdin; there is deliberately no flag that skips it) before it cancels every resting order account-wide, closes each margin position with a reduce-only market order sized from a fresh post-cancel read, and sells each non-EUR balance at market in two passes so a BTC-quoted leg's proceeds are sold too. Dust — a balance below the venue's quantity or notional minimum — is listed and not sent, and does not make the account not-flat; a margin remainder is never dust and is sent regardless. A spot sale goes out one eight-decimal unit short of the balance (one lot step on a pair whose lot is coarser) whenever that still clears the venue minimums: the adapter rounds balances to eight decimals, and one it rounded up would otherwise be refused on every run. Every request and every venue answer is written to `<state-dir>/exec/flatten-<timestamp>.json`. Exit **0** the final read shows no resting order, no open position and nothing sellable left, on order, position and balance reads before the cancel and final reads that each came back whole (an open-order or balance row the adapter could not parse drops out of its read without failing it, so confirm on Kraken's own pages) — the account-wide order and position reads resolve both of Kraken's spellings of a pair (the per-pair position reads that follow a failed one cover the basket only and match the AssetPairs key only), and an order or position a read cannot resolve never reads as flat: before the cancel the plan names the failed read and the cancel still goes out, and the run ends at 2; **1** refused with nothing sent, the reason printed; **2** something is still open, or the cancel failed, or a read after the cancel failed; **3** the venue could not be reached or read before anything was sent. Re-runnable: a second run finds less to do and does it. |
...
With `--textfile`, `pull` publishes this run's verify cost as three gauges labelled `{channel="<name>"}` — `zcrypto_archive_pull_verify_seconds`, `zcrypto_archive_pull_files_hashed`, and `zcrypto_archive_pull_files_walked` — written atomically (tmp + rename) before the exit-1 path, so a hash failure still publishes its cost. Omitting both options publishes nothing.
...
**`--detect-only` is the safe default; the deployed reconciler runs `--mint`.** Detect-only remains the default for ad-hoc runs: it ledgers every `would_mint` and writes no parquet. The `--min-gap-seconds` default is validated cross-host against the worst observed coalescing artifact and the smallest real outage on record — `zcrypto archive reconcile --help` carries both ratios.
...
| `--drain-budget-seconds` | Wall-clock budget for re-verifying cached hours whose bytes changed (default `7200`). Whatever does not fit is reported as `pending` and picked up by the next run. Applies only with `--state-dir`. |
| `--audit-sample` | How many hours served from the cache to replay again and compare against it each run (default `25`). A disagreement fails the run. Applies only with `--state-dir`. |
...
| `--settle-hours` | Defer hours newer than this (default `7`): the heal-settle margin so an hour is only materialized once the reconciler (H+6h max mint) has healed it. |
...
`ohlc-reach` is the exception to "from the OHLCVT dumps": it carries the newest frozen `ohlc-full` forward — the newest stamped `ohlc-full-<stamp>` sibling a dump ingest minted, else `ohlc-full` itself — using Kraken's public REST OHLC window, which serves the most recent ~720 bars per interval. That window recedes at a rate set by the interval — roughly 720 days at the daily grid, 120 days at 4h, but only 30 days at 1h — so a reach set is routinely **mixed**, and each series lands one of two ways:
...
| `exec_armed` | `false` | The config half of the two arming keys: the engine may submit only when this is true AND the arm file is present. `engine exec-status` names the missing one. |
